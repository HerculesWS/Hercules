...
HPExport bool HPM_Plugin_AddHook(enum HPluginHookType type, const char *target, void *hook, unsigned int pID)
{
	...
	if (hp_db && (hpd = (struct HookingPointData *)strdb_get(hp_db, target)) != NULL) {
		...
		if (type == HOOK_TYPE_PRE) {
			hp    = (struct HPMHookPoint **)((char *)&HPMHooks.list
			                                 + (sizeof(struct HPMHookPoint *) * hpd->idx));
			count = (int *)((char *)&HPMHooks.count + (sizeof(int) * hpd->idx));
		} else {
			hp    = (struct HPMHookPoint **)((char *)&HPMHooks.list
			                                 + (sizeof(struct HPMHookPoint *) * (hpd->idx + 1)));
...
void HPM_HP_final(void)
{
	...
	for (i = 0; i < len; i++) {
		...
		if (count && *count) {
			struct HPMHookPoint **hp
			    = (struct HPMHookPoint **)((char *)&HPMHooks.list + (sizeof(struct HPMHookPoint *) * (i)));
