...
typedef enum _BasicType { btNoType = 0, btVoid = 1, btChar = 2, btWChar = 3, btInt = 6, btUInt = 7, btFloat = 8, btBCD = 9, btBool = 10, btLong = 13, btULong = 14, btCurrency = 25, btDate = 26, btVariant = 27, btComplex = 28, btBit = 29, btBSTR = 30, btHresult = 31 } BasicType;

typedef enum _UdtKind { UdtStruct, UdtClass, UdtUnion } UdtKind;
/*
...
typedef BOOL(WINAPI *SYMGETTYPEINFO)(HANDLE hProcess, DWORD64 ModBase, ULONG TypeId, IMAGEHLP_SYMBOL_TYPE_INFO GetType, PVOID pInfo);
typedef BOOL(WINAPI *SYMGETLINEFROMADDR)(HANDLE hProcess, DWORD dwAddr, PDWORD pdwDisplacement, PIMAGEHLP_LINE Line);
typedef BOOL(WINAPI *SYMENUMSYMBOLS)(HANDLE hProcess, ULONG64 BaseOfDll, PCSTR Mask, PSYM_ENUMERATESYMBOLS_CALLBACK EnumSymbolsCallback, PVOID UserContext);
typedef BOOL(WINAPI *SYMSETCONTEXT)(HANDLE hProcess, PIMAGEHLP_STACK_FRAME StackFrame, PIMAGEHLP_CONTEXT Context);
typedef BOOL(WINAPI *SYMFROMADDR)(HANDLE hProcess, DWORD64 Address, PDWORD64 Displacement, PSYMBOL_INFO Symbol);
typedef BOOL(WINAPI *STACKWALK)(DWORD MachineType, HANDLE hProcess, HANDLE hThread, LPSTACKFRAME StackFrame, PVOID ContextRecord, PREAD_PROCESS_MEMORY_ROUTINE ReadMemoryRoutine, PFUNCTION_TABLE_ACCESS_ROUTINE FunctionTableAccessRoutine, PGET_MODULE_BASE_ROUTINE GetModuleBaseRoutine, PTRANSLATE_ADDRESS_ROUTINE TranslateAddress);
...
static VOID Dhp__WriteMinidumpFile(const char *file, PEXCEPTION_POINTERS ptrs, MINIDUMP_CALLBACK_ROUTINE module_callback, FILE *log_file)
{
	// open minidump file
	HANDLE minidump_file = CreateFileA(file, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
	...
	if (minidump_file != INVALID_HANDLE_VALUE) {
		...
		MiniDumpWriteDump_(GetCurrentProcess(), GetCurrentProcessId(), minidump_file, MiniDumpNormal, ptrs ? &expt_info : NULL, NULL, &dump_cb_info);
...
/// @return
static BOOL CALLBACK Dhp__PrintModuleInfoCallback(void *data, CONST PMINIDUMP_CALLBACK_INPUT callback_input, PMINIDUMP_CALLBACK_OUTPUT callback_output)
{
	if (data != NULL && callback_input != NULL && callback_input->CallbackType == ModuleCallback) {
		...
		fprintf(log_file, "  (%d.%d.%d.%d, %d bytes)\n", HIWORD(module.VersionInfo.dwFileVersionMS), LOWORD(module.VersionInfo.dwFileVersionMS), HIWORD(module.VersionInfo.dwFileVersionLS), LOWORD(module.VersionInfo.dwFileVersionLS), module.SizeOfImage);
	}
...
/// @param log_file Log file
static VOID Dhp__PrintProcessInfo(EXCEPTION_RECORD *exception, CONTEXT *context, FILE *log_file)
{
	LPSTR cmd_line;
	...
	if (sysinfo) {
		fprintf(log_file, "Platform: %s\n CPU: %s\nApplication architecture: %s\nCompiler: %s\n%s: %s\n", sysinfo->osversion(), sysinfo->cpu(), (sysinfo->is64bit()) ? "x64" : "x86", sysinfo->compiler(), sysinfo->vcstype(), sysinfo->vcsrevision_src());
	}
...
#undef PRINT
	...
	if (context) {
		...
		if (context->ContextFlags & CONTEXT_INTEGER) {
			fprintf(log_file, "eax=%08x ebx=%08x ecx=%08x edx=%08x esi=%08x edi=%08x\n", context->Eax, context->Ebx, context->Ecx, context->Edx, context->Esi, context->Edi);
		}
		if (context->ContextFlags & CONTEXT_CONTROL) {
			fprintf(log_file, "eip=%08x esp=%08x ebp=%08x iopl=%1x %s %s %s %s %s %s %s %s %s %s\n", context->Eip, context->Esp, context->Ebp,
		...
		if (context->ContextFlags & CONTEXT_SEGMENTS) {
			fprintf(log_file, "cs=%04x  ss=%04x  ds=%04x  es=%04x  fs=%04x  gs=%04x", context->SegCs, context->SegSs, context->SegDs, context->SegEs, context->SegFs, context->SegGs);
			if (context->ContextFlags & CONTEXT_CONTROL)
				fprintf(log_file, "             efl=%08x", context->EFlags);
			fprintf(log_file, "\n");
		} else if (context->ContextFlags & CONTEXT_CONTROL)
			fprintf(log_file, "                                                                       efl=%08x\n", context->EFlags);
...
static VOID Dhp__PrintTypeName(DWORD typeIndex, DWORD symtag, BOOL withParens, InterData *interData)
{
	// inter data
	FILE *log_file;
	HANDLE hProcess;
	...
	switch (symtag) {
		...
		case SymTagBaseType: {
			...
			switch (basetype) {
				case btVoid:
					...
					break;
				case btChar:
					fprintf(log_file, "char");
					break;
				case btWChar:
					fprintf(log_file, "wchar");
					break;
				case btULong:
					fprintf(log_file, "unsigned "); // next
				case btLong:
					fprintf(log_file, "long");
					break;
				case btUInt:
					fprintf(log_file, "unsigned "); // next
				case btInt:
					if (length == sizeof(char))
						...
						fprintf(log_file, "short");
					else if (length == sizeof(int))
						fprintf(log_file, "int");
					else if (length == sizeof(long long))
						fprintf(log_file, "long long");
					else
						fprintf(log_file, "<int%d>", length * 8);
					break;
				case btFloat:
					if (length == sizeof(float))
						fprintf(log_file, "float");
					else if (length == sizeof(double))
						fprintf(log_file, "double");
					else if (length == sizeof(long double))
						fprintf(log_file, "long double");
					else
						fprintf(log_file, "<float%d>", length * 8);
					break;
				default:
					fprintf(log_file, "<TODO name of basetype %d %d>", basetype, length);
					break;
			}
		} break;
		...
		default: {
			...
			if (SymGetTypeInfo_(hProcess, modBase, typeIndex, TI_GET_UDTKIND, &udtkind)) {
				switch ((UdtKind)udtkind) {
					case UdtStruct:
						...
						break;
					case UdtClass:
						fprintf(log_file, "class ");
						break;
					case UdtUnion:
						fprintf(log_file, "union ");
						break;
					default:
						fprintf(log_file, "<unknown udtkind %d> ", udtkind);
						break;
				}
			}
			if (SymGetTypeInfo_(hProcess, modBase, typeIndex, TI_GET_SYMNAME, &pSymname)) {
				...
				fprintf(log_file, "<TODO typename of symtag %d>", symtag);
			break;
		} break;
	}
...
static VOID Dhp__PrintValueBytes(FILE *log_file, BYTE *p, ULONG64 length)
{
	ULONG64 i;
...
static VOID Dhp__PrintValueWideChars(FILE *log_file, WCHAR *wstr, ULONG64 length, BOOL isString)
{
	ULONG64 i;
	char *buf;
	char delim;
...
/// @param p Pointer to the value
/// @param length Length of the value in bytes
static VOID Dhp__PrintValueChars(FILE *log_file, char *str, ULONG64 length, BOOL isString)
{
	ULONG64 i;
	char delim;
		...
		switch (str[i]) {
			...
			default:
				if (isprint((unsigned char)str[i]))
					...
					fprintf(log_file, "\\x%02X", (unsigned char)str[i]);
				break;
		}
	}
	fprintf(log_file, "%c", delim);
...
/// @param p Pointer to the value
/// @param length Length of the value in bytes
static VOID Dhp__PrintValueFloat(FILE *log_file, VOID *p, ULONG64 length)
{
	if (length == sizeof(float))
		fprintf(log_file, "%f", *(float *)p);
	else if (length == sizeof(double))
		fprintf(log_file, "%lf", *(double *)p);
	else if (length == sizeof(long double))
		fprintf(log_file, "%Lf", *(long double *)p);
	else {
...
/// @param p Pointer to the value
/// @param length Length of the value in bytes
static VOID Dhp__PrintValueHex(FILE *log_file, VOID *p, ULONG64 length)
{
	if (length == sizeof(UINT32))
		fprintf(log_file, "0x%I32X", *(UINT32 *)p);
	else if (length == sizeof(UINT64))
		fprintf(log_file, "0x%I64X", *(UINT64 *)p);
	else if (length == sizeof(char))
		fprintf(log_file, "0x%X", *(unsigned char *)p);
	else if (length == sizeof(short))
		fprintf(log_file, "0x%X", *(unsigned short *)p);
	else if (length == sizeof(int))
		fprintf(log_file, "0x%x", *(unsigned int *)p);
	else if (length == sizeof(long))
		fprintf(log_file, "0x%lX", *(unsigned long *)p);
	else if (length == sizeof(long long))
		fprintf(log_file, "0x%llX", *(unsigned long long *)p);
	else {
		fprintf(log_file, "<unexpected length %I64u>", length);
		Dhp__PrintValueBytes(log_file, (BYTE *)p, length);
	}
...
/// @param p Pointer to the value
/// @param length Length of the value in bytes
static VOID Dhp__PrintValueUnsigned(FILE *log_file, VOID *p, ULONG64 length)
{
	if (length == sizeof(INT32))
		fprintf(log_file, "%I32u", *(INT32 *)p);
	else if (length == sizeof(INT64))
		fprintf(log_file, "%I64u", *(INT64 *)p);
	else if (length == sizeof(char))
		fprintf(log_file, "%u", *(unsigned char *)p);
	else if (length == sizeof(short))
		fprintf(log_file, "%u", *(unsigned short *)p);
	else if (length == sizeof(int))
		fprintf(log_file, "%u", *(unsigned int *)p);
	else if (length == sizeof(long))
		fprintf(log_file, "%lu", *(unsigned long *)p);
	else if (length == sizeof(long long))
		fprintf(log_file, "%llu", *(unsigned long long *)p);
	else {
		fprintf(log_file, "<unexpected length %I64u>", length);
		Dhp__PrintValueBytes(log_file, (BYTE *)p, length);
	}
...
/// @param p Pointer to the value
/// @param length Length of the value in bytes
static VOID Dhp__PrintValueSigned(FILE *log_file, VOID *p, ULONG64 length)
{
	if (length == sizeof(INT32))
		fprintf(log_file, "%I32d", *(INT32 *)p);
	else if (length == sizeof(INT64))
		fprintf(log_file, "%I64d", *(INT64 *)p);
	else if (length == sizeof(char))
		fprintf(log_file, "%d", *(signed char *)p);
	else if (length == sizeof(short))
		fprintf(log_file, "%d", *(signed short *)p);
	else if (length == sizeof(int))
		fprintf(log_file, "%d", *(signed int *)p);
	else if (length == sizeof(long))
		fprintf(log_file, "%ld", *(signed long *)p);
	else if (length == sizeof(long long))
		fprintf(log_file, "%lld", *(signed long long *)p);
	else {
		fprintf(log_file, "<unexpected length %I64u>", length);
		Dhp__PrintValueBytes(log_file, (BYTE *)p, length);
	}
...
static VOID Dhp__PrintValueCWideString(FILE *log_file, WCHAR *str)
{
	ULONG64 length = 0;
	...
	__try {
		...
		if (length)
			Dhp__PrintValueWideChars(log_file, str, length * sizeof(WCHAR), TRUE); // print readable part
...
/// @param log_file Log file
/// @param str Target string
static VOID Dhp__PrintValueCString(FILE *log_file, char *str)
{
	ULONG64 length = 0;
	...
	__try {
		...
		if (length)
			Dhp__PrintValueChars(log_file, str, length * sizeof(char), TRUE); // print readable part
		fprintf(log_file, "<invalid memory>");
		return;
	}
...
static VOID Dhp__PrintDataValue(DWORD typeIndex, DWORD symtag, PVOID pVariable, InterData *pInterData)
{
	// inter data
	FILE *log_file;
	DWORD64 modBase;
	...
	switch (symtag) {
		case SymTagBaseType: {
			...
			switch (basetype) {
				...
				case btChar: {
					if (length == sizeof(char))
						fprintf(log_file, "%u ", *(unsigned char *)pVariable);
					Dhp__PrintValueChars(log_file, (char *)pVariable, length, FALSE);
				} break;
				case btWChar: {
					if (length == sizeof(WCHAR))
						fprintf(log_file, "%u ", *(WCHAR *)pVariable);
		...
		case SymTagPointerType: {
			...
			if (SymGetTypeInfo_(hProcess, modBase, typeIndex, TI_GET_TYPE, &childTypeIndex) && SymGetTypeInfo_(hProcess, modBase, childTypeIndex, TI_GET_SYMTAG, &childSymtag) && childSymtag != SymTagPointerType) {
				DWORD childBasetype;

				// child isn't a pointer, print the contents
				fprintf(log_file, " ");
				if (childSymtag == SymTagBaseType && SymGetTypeInfo_(hProcess, modBase, childTypeIndex, TI_GET_BASETYPE, &childBasetype) && (childBasetype == btChar || childBasetype == btWChar)) {
					// string or wide string
					if (childBasetype == btChar)
						Dhp__PrintValueCString(log_file, *(char **)pVariable);
					else if (childBasetype == btWChar)
						Dhp__PrintValueCWideString(log_file, *(WCHAR **)pVariable);
					else
						fprintf(log_file, "<unexpected child basetype %d>", childBasetype);
					break;
				}
...
#ifdef DBG_PRINT_CHILDREN
		{
			...
			// count children
			if (!SymGetTypeInfo_(hProcess, modBase, typeIndex, TI_GET_CHILDRENCOUNT, &childCount) || !childCount) {
...
/// @param pInterData Inter data
static VOID Dhp__PrintDataContents(DWORD typeIndex, PVOID pVariable, InterData *pInterData)
{
	// inter data
	FILE *log_file;
	HANDLE hProcess;
...
/// @param pInterData Inter data
static VOID Dhp__PrintDataInfo(PSYMBOL_INFO pSymInfo, InterData *pInterData)
{
	// inter data
	FILE *log_file;
	STACKFRAME *pStackframe;
	...
	if (pSymInfo->Flags & SYMFLAG_REGREL) {
		...
		if (pSymInfo->Flags & SYMFLAG_PARAMETER)
			...
			scope = LOCAL; // local
#if defined(_M_IX86)
			if ((LONG64)pSymInfo->Address > 0)
				scope = PARAM; // parameter as local (bug in DBGHELP 5.1)
#endif
		}
	...
	if ((scope == PARAM && log_params) || (scope == LOCAL && log_locals) || (scope == GLOBAL && log_globals)) {
...
/// @param pSymInfo Symbol info
/// @param pInterData Inter data
static VOID Dhp__PrintSymbolInfo(PSYMBOL_INFO pSymInfo, InterData *pInterData)
{
	assert(pSymInfo != NULL);
	assert(pInterData != NULL);
	...
	switch (pSymInfo->Tag) {
		case SymTagData:
			Dhp__PrintDataInfo(pSymInfo, pInterData);
			break;
		default: /*fprintf(pInterData->log_file, "<unsupported symtag %d>", pSymInfo->Tag);*/
			break;
	}
}
...
static BOOL WINAPI Dhp__EnumSymbolsCallback(PSYMBOL_INFO pSymInfo, ULONG symSize, PVOID pData)
{
	if (pSymInfo == NULL)
...
/// @param log_file Log file
static VOID Dhp__PrintSourceLine(FILE *log_file, char *filename, DWORD line)
{
	...
	if (SearchPathA(path, filename, NULL, MAX_PATH, pathBuffer, NULL)) {
		...
		while (fgets(code, 1024, fp)) {
			if (i == line) { // found line
				char *term = strchr(code, '\n');
				if (term && term != code && term[-1] == '\r')
					--term;
				if (term)
					*term = '\0';
...
///
/// @param interData Inter data
static VOID Dhp__PrintFunctionDetails(InterData *pInterData)
{
	// inter data
	HANDLE hProcess;
	STACKFRAME *pStackframe;
	...
	// frame info
	fprintf(log_file, "#%d  0x%p", nr_of_frame, (void *)(DWORD_PTR)pStackframe->AddrPC.Offset);
	...
	if (SymFromAddr_(hProcess, pStackframe->AddrPC.Offset, &funcDisplacement, pSymbolInfo) == TRUE) {
		fprintf(log_file, " in %.1024s+0x%I64X (", pSymbolInfo->Name, funcDisplacement);
		...
		fprintf(log_file, "in <unknown function>");
...
/// @param context Exception context
/// @param log_file Log file
static VOID Dhp__PrintStacktrace(CONTEXT *context, FILE *log_file)
{
	...
	for (i = 0;; ++i) {
		if (!StackWalk_(machine, hProcess, GetCurrentThread(), &stackframe, context, NULL, SymFunctionTableAccess_, SymGetModuleBase_, NULL)) {
			break;
		}
...
static BOOL Dhp__IsDebuggerPresent(void)
{
	HANDLE kernel32_dll;
...
static BOOL Dhp__LoadDbghelpDll(void)
{
	...
	if (dbghelp_dll != INVALID_HANDLE_VALUE) {
		...
		if (MiniDumpWriteDump_ && SymInitialize_ && SymSetOptions_ && SymGetOptions_ && SymCleanup_ && SymGetTypeInfo_ && SymGetLineFromAddr_ && SymEnumSymbols_ && SymSetContext_ && SymFromAddr_ && StackWalk_ && SymFunctionTableAccess_ && SymGetModuleBase_) {
...
static VOID Dhp__UnloadDbghlpDll(void)
{
...
static BOOL Dhp__CreateFiles(char *out_logFileName, char *out_dmpFileName)
{
...
#define LEN_EXT 4        // ".rpt" or ".dmp"
	...
	if (GetModuleFileName(NULL, baseFileName, MAX_PATH - LEN_TIMESTAMP - LEN_EXT)) {
		...
		pTerm = strrchr(pTerm, '.');
		if (pTerm)
			*pTerm = '\0';                                                           // remove extension
	} else if (GetTempPathA(MAX_PATH - 6 - LEN_TIMESTAMP - LEN_EXT, baseFileName)) { // in temp folder
...
LONG WINAPI Dhp__UnhandledExceptionFilter(PEXCEPTION_POINTERS ptrs)
{
	...
	Dhp__PrintProcessInfo(ptrs ? ptrs->ExceptionRecord : NULL, ptrs ? ptrs->ContextRecord : NULL, log_file);

	// print the stacktrace
	Dhp__PrintStacktrace(ptrs ? ptrs->ContextRecord : NULL, log_file);

	// write the minidump file and use the callback to print the list of modules to the log file
	Dhp__WriteMinidumpFile(szDmpFileName, ptrs, Dhp__PrintModuleInfoCallback, log_file);
