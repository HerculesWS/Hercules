...
void sample_packet0f3(int fd)
{
	...
	if (!(data = (struct sample_data_struct *)getFromSession(sockt->session[fd], 0))) {
		...
		ShowInfo(
		    "Created Appended sockt->session[] data, %d %d %d %u\n", data->lastMSGPosition.map,
		    data->lastMSGPosition.x, data->lastMSGPosition.y, data->someNumber
		);
		addToSession(sockt->session[fd], data, 0, true);
	} else {
		ShowInfo(
		    "Existent Appended sockt->session[] data, %d %d %d %u\n", data->lastMSGPosition.map,
		    data->lastMSGPosition.x, data->lastMSGPosition.y, data->someNumber
		);
		if (rnd() % 4 == 2) {
	...
	if (!(data = (struct sample_data_struct *)getFromMSD(sd, 0))) {
		...
		ShowInfo(
		    "Created Appended map_session_data data, %d %d %d %u\n", data->lastMSGPosition.map,
		    data->lastMSGPosition.x, data->lastMSGPosition.y, data->someNumber
		);
		addToMSD(sd, data, 0, true);
	} else {
		ShowInfo(
		    "Existent Appended map_session_data data, %d %d %d %u\n", data->lastMSGPosition.map,
		    data->lastMSGPosition.x, data->lastMSGPosition.y, data->someNumber
		);
		if (rnd() % 4 == 2) {
...
/* my custom prehook for pc_dropitem, checks if amount of item being dropped is higher than 1 and if so cap it to 1 and
 * store the value of how much it was */
...
/* postHook receive retVal as the first param, allows posthook to act accordingly to whatever the original was going to
 * return */
int my_pc_dropitem_post(int retVal, struct map_session_data *sd, int n, int amount)
{
	...
	if (my_pc_dropitem_storage) { /* signs whether pre-hook did this */
		char output[99];
		snprintf(
		    output, 99, "[ Warning ] you can only drop 1 item at a time, capped from %d to 1",
		    my_pc_dropitem_storage
		);
 ...
 * needed the pre-hook takes this currently unused packet and show a notice whenver a player sends it
 **/
...
HPExport void plugin_init(void)
{
	...
	if (SERVER_TYPE == SERVER_TYPE_MAP) {
		/* addAtcommand("command-key",command-function) tells map server to call ACMD(sample) when "sample"
		 * command is used */
	...
	if (SERVER_TYPE == SERVER_TYPE_MAP) {
		/* addScriptCommand("script-command-name","script-command-params-info",script-function) tells map server
		 * to call BUILDIN(sample) for the "sample(i)" command */
		/* - it will print a warning when used on a non-map-server plugin */
	...
	/* in "console-command-name" usage of ':' indicates a category, for example 'this:is:a:sample' translates to
	 * 'this is a sample', therefore 'this -> is -> a -> sample', it can be used to aggregate multiple commands
	 * under the same category or to append commands to existing categories categories inherit the special keyword
	 * 'help' which prints the subsequent commands, e.g. 'server help' prints all categories and commands under
	 * 'server' therefore 'this help' would inform about 'is (category) -> a (category) -> sample (command)'*/
	addCPCommand("this:is:a:sample", sample);

	/* addPacket(packetID,packetLength,packetFunction,packetIncomingPoint) */
	/* adds packetID of packetLength (-1 for dynamic length where length is defined in the packet { packetID (2
	 * Byte) , packetLength (2 Byte) , ... }) to trigger packetFunction in the packetIncomingPoint section (
	 * available points listed in enum HPluginPacketHookingPoints within src/common/HPMi.h ) */
	...
	if (SERVER_TYPE == SERVER_TYPE_MAP) {
		...
		/* if the original pc->dropitem was successful and the amount stored on my_pc_dropitem_storage is higher
		 * than 1, */
		/* our posthook will display a message to the user about the cap */
		/* - by checking whether it was successful (retVal value) it allows for the originals conditions to take
		 * place */
...
HPExport void server_preinit(void)
{
	 ...
	 * however for battle config to be returned to our script engine we need it to be number (int) so keep use it as
	 * int only */
