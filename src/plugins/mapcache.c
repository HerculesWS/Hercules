...
bool convert_old_mapcache(void)
{
	...
	for (i = 0; i < header.map_count; ++i) {
		...
		ShowStatus("Creating mapcache: %s"CL_CLL"\r", info->name);
...
bool mapcache_rebuild(void)
{
	...
	for (i = 0; i < VECTOR_LENGTH(maplist); ++i) {
		snprintf(file_path, sizeof(file_path), "%s%s%s.%s", "mapcache/", DBPATH, VECTOR_INDEX(maplist, i), "mcache");
...
{
	addArg("--convert-old-mapcache", false, convertmapcache, NULL);
	addArg("--rebuild-mapcache", false, rebuild, NULL);
