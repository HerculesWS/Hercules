...
struct clan_interface {
	...
	void (*read_db) (struct config_setting_t *settings, const char *source);
	bool (*read_db_sub) (struct config_setting_t *clans, int n, const char *source);
	...
	int (*ally_count) (const struct clan *c);
	int (*antagonist_count) (const struct clan *c);
