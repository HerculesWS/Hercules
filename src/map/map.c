		...
		if (iMap->getcell(m,*x,*y,CELL_CHKREACH))
		{
			...
			if(flag&4) {
				...
				if (spawn++ < battle_config.no_spawn_on_player &&
					iMap->foreachinarea(map_count_sub, m,
...
int map_removemobs_timer(int tid, unsigned int tick, int id, intptr_t data)
{
	...
	count = iMap->foreachinmap(map_removemobs_sub, m, BL_MOB);
...
{
	return (m < 0 || m >= MAX_MAP_PER_SERVER) ? 0 : iMap->getcellp(&map[m],x,y,cellchk);
}
...
void map_setcell(int16 m, int16 x, int16 y, cell_t cell, bool flag)
{
	...
	default:
		ShowWarning("iMap->setcell: invalid cell type '%d'\n", (int)cell);
		break;
	}
}
...
bool map_iwall_set(int16 m, int16 x, int16 y, int size, int8 dir, bool shootable, const char* wall_name)
{
	...
	for( i = 0; i < size; i++ )
	{
		...
		iMap->setcell(m, x1, y1, CELL_WALKABLE, false);
		iMap->setcell(m, x1, y1, CELL_SHOOTABLE, shootable);
...
void map_iwall_remove(const char *wall_name)
{
	...
	for( i = 0; i < iwall->size; i++ ) {
		...
		iMap->setcell(iwall->m, x1, y1, CELL_SHOOTABLE, true);
		iMap->setcell(iwall->m, x1, y1, CELL_WALKABLE, true);
...
int map_addmap(char* mapname)
{
	...
	if( iMap->map_num >= MAX_MAP_PER_SERVER - 1 )
	{
...
int map_readallmaps (void) {
	...
	instance_start = iMap->map_num; // Next Map Index will be instances
...
{
	// main db connection
	iMap->mmysql_handle = SQL->Malloc();

	ShowInfo("Connecting to the Map DB Server....\n");
	if( SQL_ERROR == SQL->Connect(iMap->mmysql_handle, map_server_id, map_server_pw, map_server_ip, map_server_port, map_server_db) )
		exit(EXIT_FAILURE);
	ShowStatus("connect success! (Map Server Connection)\n");

	if( strlen(default_codepage) > 0 )
		if ( SQL_ERROR == SQL->SetEncoding(iMap->mmysql_handle, default_codepage) )
			Sql_ShowDebug(iMap->mmysql_handle);
...
int map_sql_close(void)
{
	...
	SQL->Free(iMap->mmysql_handle);
	iMap->mmysql_handle = NULL;
	if (logs->config.sql_logs) {
		ShowStatus("Close Log DB Connection....\n");
		SQL->Free(iMap->logmysql_handle);
		iMap->logmysql_handle = NULL;
	}
	return 0;
}
...
{
	// log db connection
	iMap->logmysql_handle = SQL->Malloc();

	ShowInfo(""CL_WHITE"[SQL]"CL_RESET": Connecting to the Log Database "CL_WHITE"%s"CL_RESET" At "CL_WHITE"%s"CL_RESET"...\n",log_db_db,log_db_ip);
	if ( SQL_ERROR == SQL->Connect(iMap->logmysql_handle, log_db_id, log_db_pw, log_db_ip, log_db_port, log_db_db) )
		exit(EXIT_FAILURE);
	ShowStatus(""CL_WHITE"[SQL]"CL_RESET": Successfully '"CL_GREEN"connected"CL_RESET"' to Database '"CL_WHITE"%s"CL_RESET"'.\n", log_db_db);

	if( strlen(default_codepage) > 0 )
		if ( SQL_ERROR == SQL->SetEncoding(iMap->logmysql_handle, default_codepage) )
...
void do_final(void)
{
	...
	for (i = 0; i < iMap->map_num; i++) {
		...
		if (map[i].m >= 0)
			iMap->foreachinmap(iMap->cleanup_sub, i, BL_ALL);
	}
...
void map_hp_symbols(void) {
	...
	/* sql link */
	/*HPM->share(iMap->mmysql_handle,"sql_handle");*/
...
int do_init(int argc, char *argv[])
{
	...
	Sql_HerculesUpdateCheck(iMap->mmysql_handle);
...
void iMap_defaults(void) {
	...
	iMap->getcellp = map_getcellp;
	iMap->setcell = map_setcell;
