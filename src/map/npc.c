...
{
	if (battle_config.min_npc_vendchat_distance > 0 && map->foreachinrange(npc->isnear_sub, bl, battle_config.min_npc_vendchat_distance, BL_NPC))
		return true;
...
static int npc_enable_sub(struct block_list *bl, va_list ap)
{
	...
	if (bl->type == BL_PC) {
		...
		if (npc->ontouch_event(sd, nd) > 0 && npc->ontouch2_event(sd, nd) > 0) { // failed to run OnTouch event, so just click the npc
			if (sd->npc_id != 0)
				return 0;
...
static int npc_event_export(struct npc_data *nd, int i)
{
	...
	if ((nd->u.scr.label_list[i].flags & LABEL_IS_EXTERN) != 0 && ((nd->u.scr.label_list[i].flags & LABEL_IS_USERFUNC) == 0 || script->config.functions_as_events)) {
...
static int npc_touchnext_areanpc(struct map_session_data *sd, bool leavemap)
{
	...
	if (sd->bl.m != nd->bl.m || sd->bl.x < nd->bl.x - xs || sd->bl.x > nd->bl.x + xs || sd->bl.y < nd->bl.y - ys || sd->bl.y > nd->bl.y + ys || pc_ishiding(sd) || leavemap) {
		char name[EVENT_NAME_LENGTH];
	...
	switch (map->list[m].npc[i]->subtype) {
		...
		case SCRIPT:
			for (j = i; j < map->list[m].npc_num; j++) {
				...
				if ((sd->bl.x >= (map->list[m].npc[j]->bl.x - map->list[m].npc[j]->u.warp.xs) && sd->bl.x <= (map->list[m].npc[j]->bl.x + map->list[m].npc[j]->u.warp.xs)) && (sd->bl.y >= (map->list[m].npc[j]->bl.y - map->list[m].npc[j]->u.warp.ys) && sd->bl.y <= (map->list[m].npc[j]->bl.y + map->list[m].npc[j]->u.warp.ys))) {
					if (pc_ishiding(sd) || (sd->sc.count && sd->sc.data[SC_CAMOUFLAGE]))
						break; // hidden chars cannot use warps
			...
			if (npc->ontouch_event(sd, map->list[m].npc[i]) > 0 && npc->ontouch2_event(sd, map->list[m].npc[i]) > 0) { // failed to run OnTouch event, so just click the npc
				struct unit_data *ud = unit->bl2ud(&sd->bl);
				if (ud && ud->walkpath.path_pos < ud->walkpath.path_len) { // Since walktimer always == INVALID_TIMER at this time, we stop walking manually. [Inkfish]
					clif->fixpos(&sd->bl);
...
static int npc_check_areanpc(int flag, int16 m, int16 x, int16 y, int16 range)
{
	...
	for (i = 0; i < map->list[m].npc_num; i++) {
		...
		if (x1 >= map->list[m].npc[i]->bl.x - xs && x0 <= map->list[m].npc[i]->bl.x + xs && y1 >= map->list[m].npc[i]->bl.y - ys && y0 <= map->list[m].npc[i]->bl.y + ys)
			break; // found a npc
	}
	if (i == map->list[m].npc_num)
		return 0;
...
static struct npc_data *npc_checknear(struct map_session_data *sd, struct block_list *bl)
{
	...
	nullpo_retr(NULL, bl);
	if (bl->m != sd->bl.m || bl->x < sd->bl.x - distance || bl->x > sd->bl.x + distance || bl->y < sd->bl.y - distance || bl->y > sd->bl.y + distance) {
		return NULL;
	}
...
static int npc_scriptcont(struct map_session_data *sd, int id, bool closing)
{
	...
	if (sd->npc_idle_timer != INVALID_TIMER) { /// Not yet timed out.
		...
		if (id != sd->npc_id) {
			...
			ShowDebug("npc_scriptcont: %s (sd->npc_id=%d) is not %s (id=%d).\n", (nd_sd != NULL) ? nd_sd->name : "'Unknown NPC'", sd->npc_id, (nd != NULL) ? nd->name : "'Unknown NPC'", id);
...
static int npc_cashshop_buylist(struct map_session_data *sd, int points, struct itemlist *item_list)
{
	...
	if (nd->subtype != CASHSHOP) {
		if (nd->subtype == SCRIPT && nd->u.scr.shop && nd->u.scr.shop->type != NST_ZENY && nd->u.scr.shop->type != NST_MARKET && nd->u.scr.shop->type != NST_BARTER && nd->u.scr.shop->type != NST_EXPANDED_BARTER) {
...
static void npc_market_fromsql(void)
{
	...
	if (SQL_ERROR == SQL->StmtPrepare(stmt, "SELECT `name`, `itemid`, `amount` FROM `%s`", map->npc_market_data_db) || SQL_ERROR == SQL->StmtExecute(stmt)) {
...
static void npc_market_tosql(struct npc_data *nd, int index)
{
	...
	if (SQL_ERROR == SQL->Query(map->mysql_handle, "REPLACE INTO `%s` VALUES ('%s','%d','%d')", map->npc_market_data_db, nd->exname, nd->u.scr.shop->item[index].nameid, nd->u.scr.shop->item[index].qty))
...
static void npc_market_delfromsql_sub(const char *npcname, int index)
{
	if (index == INT_MAX) {
		...
		if (SQL_ERROR == SQL->Query(map->mysql_handle, "DELETE FROM `%s` WHERE `name`='%s' AND `itemid`='%d' LIMIT 1", map->npc_market_data_db, npcname, index))
			Sql_ShowDebug(map->mysql_handle);
	}
}
/**
...
static void npc_barter_fromsql(void)
{
	...
	if (SQL_ERROR == SQL->StmtPrepare(stmt, "SELECT `name`, `itemId`, `amount`, `priceId`, `priceAmount` FROM `%s`", map->npc_barter_data_db) || SQL_ERROR == SQL->StmtExecute(stmt)) {
		SqlStmt_ShowDebug(stmt);
		SQL->StmtFree(stmt);
		return;
	}
...
static void npc_barter_tosql(struct npc_data *nd, int index)
{
	...
	if (item->qty == -1)
		return;
	if (SQL_ERROR == SQL->Query(map->mysql_handle, "REPLACE INTO `%s` VALUES ('%s', '%d', '%d', '%u', '%d')", map->npc_barter_data_db, nd->exname, item->nameid, item->qty, item->value, item->value2)) {
		Sql_ShowDebug(map->mysql_handle);
	}
}
...
static void npc_barter_delfromsql_sub(const char *npcname, int itemId, int itemId2, int amount2)
{
	...
	if (itemId == INT_MAX) {
		...
		if (SQL_ERROR == SQL->Query(map->mysql_handle, "DELETE FROM `%s` WHERE `name`='%s' AND `itemId`='%d' AND `priceId`='%d' AND `priceAmount`='%d' LIMIT 1", map->npc_barter_data_db, npcname, itemId, itemId2, amount2)) {
			Sql_ShowDebug(map->mysql_handle);
		}
	}
}
		...
		for (i = 0; i < nd->u.scr.shop->items; i++) {
			...
			if (item->nameid == itemid && item->value == zeny) {
				...
				for (curIndex = 0; curIndex < count; curIndex++) {
					...
					if (currency->nameid != currency2->nameid || currency->amount != currency2->amount || currency->refine != currency2->refine) {
						break;
					}
				}
				if (curIndex == count) {
...
static void npc_expanded_barter_delfromsql_sub(const char *npcname, int itemId, int zeny, int currencyCount, struct npc_barter_currency *currency)
{
	...
	if (itemId == INT_MAX) {
		...
		StrBuf->Init(&buf);
		StrBuf->Printf(&buf, "DELETE FROM `%s` WHERE `name`='%s' AND `itemId`='%d' AND `zeny`='%d'", map->npc_expanded_barter_data_db, npcname, itemId, zeny);
		int k = 0;
		if (currencyCount > 10)
			currencyCount = 10;
		for (k = 0; k < currencyCount; k++) {
			struct npc_barter_currency *currency1 = &currency[k];
			StrBuf->Printf(&buf, " AND currencyId%d='%d' and currencyAmount%d='%d' and currencyRefine%d='%d'", k + 1, currency1->nameid, k + 1, currency1->amount, k + 1, currency1->refine);
		}
		for (; k < 10; k++) {
			StrBuf->Printf(&buf, " AND currencyId%d='0' and currencyAmount%d='0' and currencyRefine%d='0'", k + 1, k + 1, k + 1);
		}
...
static int npc_cashshop_buy(struct map_session_data *sd, int nameid, int amount, int points)
{
	...
	if ((int64)shop[i].value * amount > INT_MAX) {
		...
		ShowDebug("(NPC:'%s' (%s,%d,%d), player:'%s' (%d/%d), value:%u, amount:%d)\n", nd->exname, map->list[nd->bl.m].name, nd->bl.x, nd->bl.y, sd->status.name, sd->status.account_id, sd->status.char_id, shop[i].value, amount);
		return ERROR_TYPE_ITEM_ID;
	}
...
static int npc_expanded_barter_buylist(struct map_session_data *sd, struct barteritemlist *item_list)
{
	...
	if (nd == NULL || nd->subtype != SCRIPT || VECTOR_LENGTH(*item_list) == 0 || !nd->u.scr.shop || nd->u.scr.shop->type != NST_EXPANDED_BARTER) {
		return 11;
	}
...
static const char *npc_parse_warp(const char *w1, const char *w2, const char *w3, const char *w4, const char *start, const char *buffer, const char *filepath, int *retval)
{
	...
	if (sscanf(w1, "%31[^,],%d,%d", mapname, &x, &y) != 3 || sscanf(w4, "%d,%d,%31[^,],%d,%d", &xs, &ys, to_mapname, &to_x, &to_y) != 5) {
...
static const char *npc_parse_mob(const char *w1, const char *w2, const char *w3, const char *w4, const char *start, const char *buffer, const char *filepath, int *retval)
{
	...
	if (sscanf(w1, "%31[^,],%d,%d,%d,%d", mapname, &x, &y, &xs, &ys) < 5 || sscanf(w3, "%23[^,],%d", mobname, &mob_lv) < 1 || sscanf(w4, "%d,%d,%u,%u,%50[^,],%d,%d[^\t\r\n]", &class_, &num, &mobspawn.delay1, &mobspawn.delay2, mobspawn.eventname, &size, &ai) < 4) {
...
static void npc_debug_warps_sub(struct npc_data *nd)
{
	...
	if (map->getcell(m, &nd->bl, nd->u.warp.x, nd->u.warp.y, CELL_CHKNPC)) {
		ShowWarning("Warp %s at %s(%d,%d) warps directly on top of an area npc at %s(%d,%d)\n", nd->name, map->list[nd->bl.m].name, nd->bl.x, nd->bl.y, map->list[m].name, nd->u.warp.x, nd->u.warp.y);
	}
