...
int npc_enable(const char* name, int flag)
{
	...
	if ( nd == NULL ) {
		ShowError("%s: Attempted to %s a non-existing NPC '%s' (flag=%d).\n", __func__, (flag&3) ? "show" : "hide", name, flag);
		return 0;
	}
...
int npc_event_dequeue(struct map_session_data* sd)
{
	...
	if (!pc->addeventtimer(sd,100,sd->eventqueue[0])) { //Failed to dequeue, couldn't set a timer.
		ShowWarning("%s: event timer is full !\n", __func__);
		return 0;
	}
...
int npc_timerevent(int tid, int64 tick, int id, intptr_t data) {
	...
	if( nd == NULL ) {
		ShowError("%s: NPC not found??\n", __func__);
		return 0;
	}

	if( ted->rid && !(sd = map->id2sd(ted->rid)) ) {
		ShowError("%s: Attached player not found.\n", __func__);
...
void npc_timerevent_quit(struct map_session_data* sd)
{
	...
	if( nd && nd->bl.type == BL_NPC )
	{
		...
		if( ev && ev->nd != nd )
		{
			ShowWarning("%s: Unable to execute \"OnTimerQuit\", two NPCs have the same event name [%s]!\n", __func__, buf);
			ev = NULL;
		}
		if( ev )
		{
...
int npc_touch_areanpc(struct map_session_data* sd, int16 m, int16 x, int16 y)
{
	...
	if( i == map->list[m].npc_num ) {
		if( f == 1 ) // no npc found
			ShowError("%s : stray NPC cell/NPC not found in the block on coordinates '%s',%d,%d\n", __func__, map->list[m].name, x, y);
		return 1;
	}
	switch(map->list[m].npc[i]->subtype) {
		case WARP:
...
int npc_scriptcont(struct map_session_data* sd, int id, bool closing) {
	...
	if( id != sd->npc_id ){
		...
		ShowDebug("%s: %s (sd->npc_id=%d) is not %s (id=%d).\n",
			__func__, nd_sd?(char*)nd_sd->name:"'Unknown NPC'", (int)sd->npc_id,
	...
	if(id != npc->fake_nd->bl.id) { // Not item script
		if ((npc->checknear(sd,target)) == NULL){
			ShowWarning("%s: failed npc->checknear test.\n", __func__);
			return 1;
		}
	}
	/**
...
int npc_buysellsel(struct map_session_data* sd, int id, int type) {
	...
	if ( nd->subtype != SHOP && !(nd->subtype == SCRIPT && nd->u.scr.shop && nd->u.scr.shop->items) ) {
		...
		if( nd->subtype == SCRIPT )
			ShowError("%s: trader '%s' has no shop list!\n", __func__, nd->exname);
		else
			ShowError("%s: no such shop npc %d (%s)\n", __func__, id,nd->exname);
...
void npc_market_fromsql(void) {
	...
	while ( SQL_SUCCESS == SQL->StmtNextRow(stmt) ) {
		...
		if( !(nd = npc->name2id(name)) ) {
			ShowError("%s: NPC '%s' not found! skipping...\n", __func__, name);
			...
			ShowError("%s: NPC '%s' is not proper for market, skipping...\n", __func__, name);
			npc->market_delfromsql_sub(name, USHRT_MAX);
			continue;
		}
		...
		if( i == nd->u.scr.shop->items ) {
			ShowError("%s: NPC '%s' does not sell item %d (qty %d), deleting...\n", __func__, name,itemid,amount);
...
void npc_trader_count_funds(struct npc_data *nd, struct map_session_data *sd) {
	...
	switch( nd->u.scr.shop->type ) {
		...
		case NST_CUSTOM:
			break;
		default:
			ShowError("%s: unsupported shop type %d\n", __func__, nd->u.scr.shop->type);
			return;
	}
	...
	if ( (ev = strdb_get(npc->ev_db, evname)) )
		...
		ShowError("%s: '%s' event '%s' not found, operation failed\n", __func__, nd->exname, evname);
...
int npc_cashshop_buy(struct map_session_data *sd, int nameid, int amount, int points) {
	...
	if( (double)shop[i].value * amount > INT_MAX ) {
		ShowWarning("%s: Item '%s' (%d) price overflow attempt!\n", __func__, item->name, nameid);
...
void npc_parsename(struct npc_data* nd, const char* name, const char* start, const char* buffer, const char* filepath) {
	...
	if( p ) { // <Display name>::<Unique name>
		size_t len = p-name;
		if( len > NAME_LENGTH ) {
			ShowWarning("%s: Display name of '%s' is too long (len=%u) in file '%s', line '%d'. Truncating to %u characters.\n",
				__func__, name, (unsigned int)len, filepath, strline(buffer,start-buffer), NAME_LENGTH);
		...
		len = strlen(p+2);
		if( len > NAME_LENGTH )
			ShowWarning("%s: Unique name of '%s' is too long (len=%u) in file '%s', line '%d'. Truncating to %u characters.\n",
				__func__, name, (unsigned int)len, filepath, strline(buffer,start-buffer), NAME_LENGTH);
		safestrncpy(nd->exname, p+2, sizeof(nd->exname));
	} else {// <Display name>
		size_t len = strlen(name);
		if( len > NAME_LENGTH )
			ShowWarning("%s: Name '%s' is too long (len=%u) in file '%s', line '%d'. Truncating to %u characters.\n",
				__func__, name, (unsigned int)len, filepath, strline(buffer,start-buffer), NAME_LENGTH);
		safestrncpy(nd->name, name, sizeof(nd->name));
	...
	if( *nd->exname == '\0' || strstr(nd->exname,"::") != NULL ) {// invalid
		...
		ShowWarning("%s: Invalid unique name in file '%s', line '%d'. Renaming '%s' to '%s'.\n",
			__func__, filepath, strline(buffer,start-buffer), nd->exname, newname);
	...
	if( (dnd=npc->name2id(nd->exname)) != NULL ) {// duplicate unique name, generate new one
		...
		ShowWarning("%s: Duplicate unique name in file '%s', line '%d'. Renaming '%s' to '%s'.\n",
			__func__, filepath, strline(buffer,start-buffer), nd->exname, newname);
		ShowDebug("this npc:\n   display name '%s'\n   unique name '%s'\n   map=%s, x=%d, y=%d\n",
...
int npc_parseview(const char* w4, const char* start, const char* buffer, const char* filepath) {
	...
	if(!npc->viewisid(viewid))
	{
		...
		if(!script->get_constant(viewid, &val)) {
			ShowWarning("%s: Invalid NPC constant '%s' specified in file '%s', line'%d'. Defaulting to INVISIBLE_CLASS. \n",
				__func__, viewid, filepath, strline(buffer,start-buffer));
			val = INVISIBLE_CLASS;
		}
	} else {
...
const char* npc_parse_warp(char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath, int *retval) {
	...
	if( sscanf(w1, "%31[^,],%d,%d", mapname, &x, &y) != 3
	 || sscanf(w4, "%d,%d,%31[^,],%d,%d", &xs, &ys, to_mapname, &to_x, &to_y) != 5
		...
		ShowError("%s: Invalid warp definition in file '%s', line '%d'.\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
			__func__, filepath, strline(buffer,start-buffer), w1, w2, w3, w4);
	...
	if( i == 0 ) {
		ShowError("%s: Unknown destination map in file '%s', line '%d' : %s\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
			__func__, filepath, strline(buffer,start-buffer), to_mapname, w1, w2, w3, w4);
		if (retval) *retval = EXIT_FAILURE;
		return strchr(start,'\n');// skip and continue
	}

	if( m != -1 && ( x < 0 || x >= map->list[m].xs || y < 0 || y >= map->list[m].ys ) ) {
		ShowError("%s: out-of-bounds coordinates (\"%s\",%d,%d), map is %dx%d, in file '%s', line '%d'\n",
			__func__, map->list[m].name, x, y, map->list[m].xs, map->list[m].ys,filepath,strline(buffer,start-buffer));
		if (retval) *retval = EXIT_FAILURE;
		return strchr(start,'\n');;//try next
	}
...
const char* npc_parse_shop(char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath, int *retval) {
	...
	if( strcmp(w1,"-") == 0 ) {
		...
		if( sscanf(w1, "%31[^,],%d,%d,%d", mapname, &x, &y, &dir) != 4
		 || strchr(w4, ',') == NULL
			...
			ShowError("%s: Invalid shop definition in file '%s', line '%d'.\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
				__func__, filepath, strline(buffer,start-buffer), w1, w2, w3, w4);
			if (retval) *retval = EXIT_FAILURE;
			return strchr(start,'\n');// skip and continue
		}
	...
	for( i = 0; p; ++i ) {
		...
		if( sscanf(p, ",%d:%d", &nameid, &value) != 2 ) {
			ShowError("%s: Invalid item definition in file '%s', line '%d'. Ignoring the rest of the line...\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
				__func__, filepath, strline(buffer,start-buffer), w1, w2, w3, w4);
			if (retval) *retval = EXIT_FAILURE;
			break;
		}
		...
		if( (id = itemdb->exists(nameid)) == NULL ) {
			ShowWarning("%s: Invalid sell item in file '%s', line '%d' (id '%d').\n",
				__func__, filepath, strline(buffer,start-buffer), nameid);
			p = strchr(p+1,',');
			if (retval) *retval = EXIT_FAILURE;
			continue;
		}
		...
		if( type == SHOP && value == 0 ) {
			...
			ShowWarning("%s: Item %s [%d] is being sold for FREE in file '%s', line '%d'.\n",
				__func__, id->name, nameid, filepath, strline(buffer,start-buffer));
			if (retval) *retval = EXIT_FAILURE;
		}
		if( type == SHOP && value*0.75 < id->value_sell*1.24 ) {
			// Exploit possible: you can buy and sell back with profit
			ShowWarning("%s: Item %s [%d] discounted buying price (%d->%d) is less than overcharged selling price (%d->%d) in file '%s', line '%d'.\n",
				__func__, id->name, nameid, value, (int)(value*0.75), id->value_sell, (int)(id->value_sell*1.24), filepath, strline(buffer,start-buffer));
			if (retval) *retval = EXIT_FAILURE;
		}
	...
	if( i == 0 ) {
		ShowWarning("%s: Ignoring empty shop in file '%s', line '%d'.\n", __func__, filepath, strline(buffer,start-buffer));
		aFree(items);
		if (retval) *retval = EXIT_FAILURE;
		return strchr(start,'\n');// continue
	}
...
const char* npc_skip_script(const char* start, const char* buffer, const char* filepath, int *retval) {
	...
	p = strchr(start,'{');
	if( p == NULL ) {
		ShowError("%s: Missing left curly in file '%s', line '%d'.\n", __func__, filepath, strline(buffer,start-buffer));
		if (retval) *retval = EXIT_FAILURE;
...
const char* npc_parse_script(char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath, int options, int *retval) {
	...
	{// floating npc
		...
		if( sscanf(w1, "%31[^,],%d,%d,%d", mapname, &x, &y, &dir) != 4 ) {
			ShowError("%s: Invalid placement format for a script in file '%s', line '%d'. Skipping the rest of file...\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
				__func__, filepath, strline(buffer,start-buffer), w1, w2, w3, w4);
			if (retval) *retval = EXIT_FAILURE;
	...
	{
		ShowError("%s: Missing left curly ',{' in file '%s', line '%d'. Skipping the rest of the file.\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
			__func__, filepath, strline(buffer,start-buffer), w1, w2, w3, w4);
		if (retval) *retval = EXIT_FAILURE;
		return NULL;// can't continue
	}
	...
	for (i = 0; i < nd->u.scr.label_list_num; i++) {
		if (npc->event_export(nd, i)) {
			ShowWarning("%s,: duplicate event %s::%s in file '%s'.\n",
			             __func__, nd->exname, nd->u.scr.label_list[i].name, filepath);
			if (retval) *retval = EXIT_FAILURE;
		}
...
const char* npc_parse_duplicate(char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath, int options, int *retval) {
	...
	if( (type==SHOP || type==CASHSHOP || type==SCRIPT) && strcmp(w1, "-") == 0 ) {// floating shop/chashshop/script
		...
		if( type == WARP && fields == 3 ) { // <map name>,<x>,<y>
			...
			ShowError("%s: Invalid placement format for duplicate in file '%s', line '%d'. Skipping line...\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
				__func__, filepath, strline(buffer,start-buffer), w1, w2, w3, w4);
			if (retval) *retval = EXIT_FAILURE;
			return end;// next line, try to continue
		}
	...
	if( type == WARP && sscanf(w4, "%d,%d", &xs, &ys) == 2 );// <spanx>,<spany>
		...
		ShowError("%s: Invalid span format for duplicate warp in file '%s', line '%d'. Skipping line...\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
			__func__, filepath, strline(buffer,start-buffer), w1, w2, w3, w4);
		if (retval) *retval = EXIT_FAILURE;
		return end;// next line, try to continue
	}
...
int npc_duplicate4instance(struct npc_data *snd, int16 m) {
	...
	if( npc->name2id(newname) != NULL ) { // Name already in use
		ShowError("%s: the npcname (%s) is already in use while trying to duplicate npc %s in instance %d.\n",
			__func__, newname, snd->exname, map->list[m].instance_id);
		return 1;
	}
	...
	if( snd->subtype == WARP ) { // Adjust destination, if instanced
		...
		if( ( im = instance->mapid2imapid(dm, map->list[m].instance_id) ) == -1 ) {
			ShowError("%s: warp (%s) leading to instanced map (%s), but instance map is not attached to current instance.\n", 
				__func__, map->list[dm].name, snd->exname);
			return 1;
		}
...
const char* npc_parse_function(char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath, int *retval) {
	...
	if( *w4 != '{' || script_start == NULL || (end != NULL && script_start > end) ) {
		ShowError("%s: Missing left curly '%%TAB%%{' in file '%s', line '%d'. Skipping the rest of the file.\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
			__func__, filepath, strline(buffer,start-buffer), w1, w2, w3, w4);
		if (retval) *retval = EXIT_FAILURE;
		return NULL;// can't continue
	}
	...
	if (func_db->put(func_db, DB->str2key(w3), DB->ptr2data(scriptroot), &old_data)) {
		...
		ShowWarning("%s: Overwriting user function [%s] in file '%s', line '%d'.\n", __func__, w3, filepath, strline(buffer,start-buffer));
...
const char* npc_parse_mob(char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath, int *retval) {
	...
	if( sscanf(w1, "%31[^,],%d,%d,%d,%d", mapname, &x, &y, &xs, &ys) < 3
	 ...
	 || sscanf(w4, "%d,%d,%u,%u,%127[^,],%d,%d[^\t\r\n]", &class_, &num, &mobspawn.delay1, &mobspawn.delay2, mobspawn.eventname, &size, &ai) < 2
		...
		ShowError("%s: Invalid mob definition in file '%s', line '%d'.\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
			__func__, filepath, strline(buffer,start-buffer), w1, w2, w3, w4);
		if (retval) *retval = EXIT_FAILURE;
		return strchr(start,'\n');// skip and continue
	}
	if( mapindex->name2id(mapname) == 0 ) {
		ShowError("%s: Unknown map '%s' in file '%s', line '%d'.\n", __func__, mapname, filepath, strline(buffer,start-buffer));
		if (retval) *retval = EXIT_FAILURE;
		return strchr(start,'\n');// skip and continue
	}
	m =  map->mapname2mapid(mapname);
	...
	if( x < 0 || x >= map->list[mobspawn.m].xs || y < 0 || y >= map->list[mobspawn.m].ys ) {
		ShowError("%s: Spawn coordinates out of range: %s (%d,%d), map size is (%d,%d) - %s %s in file '%s', line '%d'.\n", __func__, map->list[mobspawn.m].name, x, y, (map->list[mobspawn.m].xs-1), (map->list[mobspawn.m].ys-1), w1, w3, filepath, strline(buffer,start-buffer));
		if (retval) *retval = EXIT_FAILURE;
		return strchr(start,'\n');// skip and continue
	}
	...
	if( mob->db_checkid(class_) == 0 ) {
		ShowError("%s: Unknown mob ID %d in file '%s', line '%d'.\n", __func__, class_, filepath, strline(buffer,start-buffer));
		if (retval) *retval = EXIT_FAILURE;
		return strchr(start,'\n');// skip and continue
	}

	if( num < 1 || num > 1000 ) {
		ShowError("%s: Invalid number of monsters %d, must be inside the range [1,1000] in file '%s', line '%d'.\n",
			__func__, num, filepath, strline(buffer,start-buffer));
		if (retval) *retval = EXIT_FAILURE;
		return strchr(start,'\n');// skip and continue
	}

	if( (mobspawn.state.size < 0 || mobspawn.state.size > 2) && size != -1 ) {
		ShowError("%s: Invalid size number %d for mob ID %d in file '%s', line '%d'.\n",
			__func__, mobspawn.state.size, class_, filepath, strline(buffer, start - buffer));
		if (retval) *retval = EXIT_FAILURE;
		return strchr(start, '\n');
	}

	if( (mobspawn.state.ai < 0 || mobspawn.state.ai > 4) && ai != -1 ) {
		ShowError("%s: Invalid ai %d for mob ID %d in file '%s', line '%d'.\n",
			__func__, mobspawn.state.ai, class_, filepath, strline(buffer, start - buffer));
		if (retval) *retval = EXIT_FAILURE;
		return strchr(start, '\n');
	}

	if( (mob_lv == 0 || mob_lv > MAX_LEVEL) && mob_lv != -1 ) {
		ShowError("%s: Invalid level %d for mob ID %d in file '%s', line '%d'.\n",
			__func__, mob_lv, class_, filepath, strline(buffer, start - buffer));
		if (retval) *retval = EXIT_FAILURE;
		return strchr(start, '\n');
	}
	...
	if(mobspawn.delay1>0xfffffff || mobspawn.delay2>0xfffffff) {
		ShowError("%s: Invalid spawn delays %u %u in file '%s', line '%d'.\n",
			__func__, mobspawn.delay1, mobspawn.delay2, filepath, strline(buffer,start-buffer));
		if (retval) *retval = EXIT_FAILURE;
		return strchr(start,'\n');// skip and continue
	}
	...
	if( !mob->parse_dataset(&mobspawn) ) {
		ShowError("%s: Invalid dataset for monster ID %d in file '%s', line '%d'.\n",
			__func__, class_, filepath, strline(buffer,start-buffer));
		if (retval) *retval = EXIT_FAILURE;
		return strchr(start,'\n');// skip and continue
	}
...
const char* npc_parse_mapflag(char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath, int *retval) {
	...
	{
		ShowError("%s: Invalid mapflag definition in file '%s', line '%d'.\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
			__func__, filepath, strline(buffer,start-buffer), w1, w2, w3, w4);
		if (retval) *retval = EXIT_FAILURE;
		return strchr(start,'\n');// skip and continue
	}
	m = map->mapname2mapid(mapname);
	if( m < 0 ) {
		ShowWarning("%s: Unknown map in file '%s', line '%d' : %s\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
			__func__, mapname, filepath, strline(buffer,start-buffer), w1, w2, w3, w4);
		if (retval) *retval = EXIT_FAILURE;
		return strchr(start,'\n');// skip and continue
	}
	...
	if (!strcmpi(w3, "nosave")) {
		...
		if (state == 0)
			...
			if (!map->list[m].save.map) {
				ShowWarning("%s: Specified save point map '%s' for mapflag 'nosave' not found in file '%s', line '%d', using 'SavePoint'.\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
					__func__, savemap, filepath, strline(buffer,start-buffer), w1, w2, w3, w4);
				if (retval) *retval = EXIT_FAILURE;
				map->list[m].save.x = -1;
				map->list[m].save.y = -1;
			}
		...
		if( state && (map->list[m].flag.gvg || map->list[m].flag.gvg_dungeon || map->list[m].flag.gvg_castle) ) {
			...
			ShowWarning("%s: You can't set PvP and GvG flags for the same map! Removing GvG flags from %s in file '%s', line '%d'.\n",
				__func__, map->list[m].name, filepath, strline(buffer,start-buffer));
			if (retval) *retval = EXIT_FAILURE;
		}
		if( state && map->list[m].flag.battleground ) {
			map->list[m].flag.battleground = 0;
			ShowWarning("%s: You can't set PvP and BattleGround flags for the same map! Removing BattleGround flag from %s in file '%s', line '%d'.\n",
				__func__, map->list[m].name, filepath, strline(buffer,start-buffer));
			if (retval) *retval = EXIT_FAILURE;
		}
		...
		if( state && map->list[m].flag.pvp ) {
			...
			ShowWarning("%s: You can't set PvP and GvG flags for the same map! Removing PvP flag from %s in file '%s', line '%d'.\n",
				__func__, map->list[m].name, filepath, strline(buffer,start-buffer));
			if (retval) *retval = EXIT_FAILURE;
		}
		if( state && map->list[m].flag.battleground ) {
			map->list[m].flag.battleground = 0;
			ShowWarning("%s: You can't set GvG and BattleGround flags for the same map! Removing BattleGround flag from %s in file '%s', line '%d'.\n",
				__func__, map->list[m].name, filepath, strline(buffer,start-buffer));
			if (retval) *retval = EXIT_FAILURE;
		}
		...
		if( map->list[m].flag.battleground && map->list[m].flag.pvp ) {
			map->list[m].flag.pvp = 0;
			ShowWarning("%s: You can't set PvP and BattleGround flags for the same map! Removing PvP flag from %s in file '%s', line '%d'.\n",
				__func__, map->list[m].name, filepath, strline(buffer,start-buffer));
			if (retval) *retval = EXIT_FAILURE;
		}
		if( map->list[m].flag.battleground && (map->list[m].flag.gvg || map->list[m].flag.gvg_dungeon || map->list[m].flag.gvg_castle) ) {
			...
			map->list[m].flag.gvg_dungeon = 0;
			map->list[m].flag.gvg_castle = 0;
			ShowWarning("%s: You can't set GvG and BattleGround flags for the same map! Removing GvG flag from %s in file '%s', line '%d'.\n",
				__func__, map->list[m].name, filepath, strline(buffer,start-buffer));
			if (retval) *retval = EXIT_FAILURE;
		}
		...
		if( modifier[0] == '\0' ) {
			ShowWarning("%s: Missing 5th param for 'adjust_unit_duration' flag! removing flag from %s in file '%s', line '%d'.\n",
				__func__, map->list[m].name, filepath, strline(buffer,start-buffer));
			if (retval) *retval = EXIT_FAILURE;
		} else if( !( skill_id = skill->name2id(skill_name) ) || !skill->get_unit_id( skill->name2id(skill_name), 0) ) {
			ShowWarning("%s: Unknown skill (%s) for 'adjust_unit_duration' flag! removing flag from %s in file '%s', line '%d'.\n", 
				__func__, skill_name, map->list[m].name, filepath, strline(buffer,start-buffer));
			if (retval) *retval = EXIT_FAILURE;
		} else if ( atoi(modifier) < 1 || atoi(modifier) > USHRT_MAX ) {
			ShowWarning("%s: Invalid modifier '%d' for skill '%s' for 'adjust_unit_duration' flag! removing flag from %s in file '%s', line '%d'.\n",
				__func__, atoi(modifier), skill_name, map->list[m].name, filepath, strline(buffer,start-buffer));
			if (retval) *retval = EXIT_FAILURE;
		} else {
		...
		if( modifier[0] == '\0' ) {
			ShowWarning("%s: Missing 5th param for 'adjust_skill_damage' flag! removing flag from %s in file '%s', line '%d'.\n",
				__func__, map->list[m].name, filepath, strline(buffer,start-buffer));
			if (retval) *retval = EXIT_FAILURE;
		} else if( !( skill_id = skill->name2id(skill_name) ) ) {
			ShowWarning("%s: Unknown skill (%s) for 'adjust_skill_damage' flag! removing flag from %s in file '%s', line '%d'.\n",
				__func__, skill_name, map->list[m].name, filepath, strline(buffer,start-buffer));
			if (retval) *retval = EXIT_FAILURE;
		} else if ( atoi(modifier) < 1 || atoi(modifier) > USHRT_MAX ) {
			ShowWarning("%s: Invalid modifier '%d' for skill '%s' for 'adjust_skill_damage' flag! removing flag from %s in file '%s', line '%d'.\n",
				__func__, atoi(modifier), skill_name, map->list[m].name, filepath, strline(buffer,start-buffer));
			if (retval) *retval = EXIT_FAILURE;
		} else {
		...
		if( !(zone = strdb_get(map->zone_db, w4)) ) {
			ShowWarning("%s: Invalid zone '%s'! removing flag from %s in file '%s', line '%d'.\n",
				__func__, w4, map->list[m].name, filepath, strline(buffer,start-buffer));
			if (retval) *retval = EXIT_FAILURE;
		...
		ShowError("%s: unrecognized mapflag '%s' in file '%s', line '%d'.\n", __func__, w3, filepath, strline(buffer,start-buffer));
		if (retval) *retval = EXIT_FAILURE;
	}
...
int npc_parsesrcfile(const char* filepath, bool runOnInit) {
	...
	buffer[len] = '\0';
	if( ferror(fp) ) {
		ShowError("%s: Failed to read file '%s' - %s\n", __func__, filepath, strerror(errno));
		aFree(buffer);
		fclose(fp);
		return EXIT_FAILURE;
	}
	...
	if ((unsigned char)buffer[0] == 0xEF && (unsigned char)buffer[1] == 0xBB && (unsigned char)buffer[2] == 0xBF) {
		...
		ShowError("%s: Detected unsupported UTF-8 BOM in file '%s'. Stopping (please consider using another character set.)\n", __func__, filepath);
		aFree(buffer);
		fclose(fp);
		return EXIT_FAILURE;
	}
	...
	for( p = script->skip_space(buffer); p && *p ; p = script->skip_space(p) ) {
		...
		if( count < 0 )
		{
			ShowError("%s: Parse error in file '%s', line '%d'. Stopping...\n", __func__, filepath, strline(buffer,p-buffer));
			success = EXIT_FAILURE;
			break;
		}
		// fill w1
		if( pos[3]-pos[2] > ARRAYLENGTH(w1)-1 ) {
			ShowWarning("%s: w1 truncated, too much data (%d) in file '%s', line '%d'.\n", __func__, pos[3]-pos[2], filepath, strline(buffer,p-buffer));
			success = EXIT_FAILURE;
		}
		...
		if( pos[5]-pos[4] > ARRAYLENGTH(w2)-1 ) {
			ShowWarning("%s: w2 truncated, too much data (%d) in file '%s', line '%d'.\n", __func__, pos[5]-pos[4], filepath, strline(buffer,p-buffer));
			success = EXIT_FAILURE;
		}
		...
		if( pos[7]-pos[6] > ARRAYLENGTH(w3)-1 ) {
			ShowWarning("%s: w3 truncated, too much data (%d) in file '%s', line '%d'.\n", __func__, pos[7]-pos[6], filepath, strline(buffer,p-buffer));
			success = EXIT_FAILURE;
		}
		...
		if( pos[1]-pos[8] > ARRAYLENGTH(w4)-1 ) {
			ShowWarning("%s: w4 truncated, too much data (%d) in file '%s', line '%d'.\n", __func__, pos[1]-pos[8], filepath, strline(buffer,p-buffer));
			success = EXIT_FAILURE;
		}
		if( pos[8] != -1 ) {
		...
		{// w1 = <map name>,<x>,<y>,<facing>
			...
			if( !mapindex->name2id(mapname) ) {
				...
				ShowError("%s: Unknown map '%s' in file '%s', line '%d'. Skipping line...\n", __func__, mapname, filepath, strline(buffer,p-buffer));
				success = EXIT_FAILURE;
			...
			if (x < 0 || x >= map->list[m].xs || y < 0 || y >= map->list[m].ys) {
				ShowError("%s: Unknown coordinates ('%d', '%d') for map '%s' in file '%s', line '%d'. Skipping line...\n",
					__func__, x, y, mapname, filepath, strline(buffer,p-buffer));
				success = EXIT_FAILURE;
				if( strcmp(w2,"script") == 0 && count > 3 ) {
					if((p = npc->skip_script(p,buffer,filepath, &success)) == NULL) {
						break;
...
#endif // ENABLE_CASE_CHECK
			ShowError("%s: Unable to parse, probably a missing or extra TAB in file '%s', line '%d'. Skipping line...\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
				__func__, filepath, strline(buffer,p-buffer), w1, w2, w3, w4);
			p = strchr(p,'\n');// skip and continue
			success = EXIT_FAILURE;
		}
...
int npc_script_event(struct map_session_data* sd, enum npce_event type)
{
	...
	if (type == NPCE_MAX)
		return 0;
	if (!sd) {
		ShowError("%s: NULL sd. Event Type %d\n", __func__, type);
		return 0;
	}
...
void npc_read_event_script(void)
{
	...
	for (i = 0; i < NPCE_MAX; i++)
	{
		...
		for( data = iter->first(iter,&key); iter->exists(iter); data = iter->next(iter,&key) )
		{
			...
			{
				ShowWarning("%s: too many occurences of event '%s'!\n", __func__, config[i].event_name);
				break;
			}
	...
	if(battle_config.dynamic_mobs) {// dynamic check by [random]
		for (m = 0; m < map->count; m++) {
			...
			if (map->list[m].npc_num > 0)
				ShowWarning("%s: %d npcs weren't removed at map %s!\n", __func__, map->list[m].npc_num, map->list[m].name);
		}
	}
