 ...
 * @return int
 */
int clan_ally_count(const struct clan *c)
{
	int i, count;
 ...
 * @return int
 */
int clan_antagonist_count(const struct clan *c)
{
	int i, count;
	nullpo_ret(c);
	for (i = 0, count = 0; i < clan->maxalliances; i++) {
...
bool clan_read_db_sub(struct config_setting_t *clans, int n, const char *source)
{
	...
	int kicktime = 0, kickchecktime = 0, id, i;
	...
	if (allies != NULL) {
		...
		for (i = 0; i < libconfig->setting_length(allies) &&  i < clan->maxalliances; i++) {
			if ((ally_const = libconfig->setting_get_string_elem(allies, i)) != NULL)
				safestrncpy(c->allies[i].constant, ally_const, NAME_LENGTH);
		}
	}
	if (antagonists != NULL) {
		const char *antg_const;

		for (i = 0; i < libconfig->setting_length(antagonists) && i < clan->maxalliances; i++) {
			if ((antg_const = libconfig->setting_get_string_elem(antagonists, i)) != NULL)
				safestrncpy(c->antagonists[i].constant, antg_const, NAME_LENGTH);
		}
	}
