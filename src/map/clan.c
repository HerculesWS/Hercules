...
bool clan_config_read(bool clear)
{
	...
	if (data != NULL) {
		...
		int global_kicktime = 0, global_checktime = 0;
		...
		global_kicktime = 60 * 60 * kicktime;
		global_checktime = 60 * 60 * max(kickchecktime, 1) * 1000;
		...
		if ((clans = libconfig->setting_get_member(settings, "clans")) != NULL ) {
			...
			for (i = 0; i < clan_count; i++) {
				struct config_setting_t *clan_ = libconfig->setting_get_elem(clans, i);
				struct config_setting_t *allies = libconfig->setting_get_member(clan_, "Allies");
				struct config_setting_t *antagonists = libconfig->setting_get_member(clan_, "Antagonists");
				...
				if ((buff = libconfig->setting_get_member(clan_, "Buff")) != NULL) {
					...
					if (libconfig->setting_lookup_string(buff, "Script", &str)) {
						b->script = *str ? script->parse(str, config_filename, -b->icon, SCRIPT_IGNORE_EXTERNAL_BRACKETS, NULL) : NULL;
					}
				}
