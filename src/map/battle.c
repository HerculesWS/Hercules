...
static void battle_adjust_conf(void)
{
	...
	if (battle_config.enable_refinery_ui == 1) {
		ShowWarning("conf/map/battle/client.conf refinery ui is enabled but it requires PACKETVER 2016-10-05 or newer, disabling...\n");
		battle_config.enable_refinery_ui = 0;
	}

	if (battle_config.replace_refine_npcs == 1) {
		ShowWarning("conf/map/battle/client.conf replace refine npcs is enabled but it requires PACKETVER 2016-10-05 or newer, disabling...\n");
