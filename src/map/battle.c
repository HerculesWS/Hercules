...
static int battle_gettarget(struct block_list *bl)
{
	...
	switch (bl->type) {
		case BL_PC:
			return BL_UCCAST(BL_PC, bl)->ud.target;
		case BL_MOB:
			return BL_UCCAST(BL_MOB, bl)->target_id;
		case BL_PET:
			return BL_UCCAST(BL_PET, bl)->target_id;
		case BL_HOM:
			return BL_UCCAST(BL_HOM, bl)->ud.target;
		case BL_MER:
			return BL_UCCAST(BL_MER, bl)->ud.target;
		case BL_ELEM:
			return BL_UCCAST(BL_ELEM, bl)->ud.target;
		case BL_NUL:
		case BL_ITEM:
		case BL_NPC:
		case BL_SKILL:
...
static int battle_delay_damage_sub(int tid, int64 tick, int id, intptr_t data)
{
	...
	if (dat) {
		...
		if (target != NULL && !status->isdead(target)) {
			...
			if (src != NULL && ((dat->skill_id == MO_EXTREMITYFIST && (target->m != src->m || !battle_config.snap_dodge)) // Extremity fist always hits
			                    || (battle_config.fix_warp_hit_delay_abuse && target->m != src->m) || ((target->type != BL_PC || BL_UCAST(BL_PC, target)->invincible_timer == INVALID_TIMER) && (target->m == src->m && check_distance_bl(src, target, dat->distance))))) {
				map->freeblock_lock();
...
static int64 battle_attr_fix(struct block_list *src, struct block_list *target, int64 damage, int atk_elem, int def_type, int def_lv)
{
	...
	if (target && target->type == BL_SKILL) {
		if (atk_elem == ELE_FIRE && battle->get_current_skill(target) == GN_WALLOFTHORN) {
			...
			if (!su->alive || (sg = su->group) == NULL || sg->val3 == -1 || (sgsrc = map->id2bl(sg->src_id)) == NULL || status->isdead(sgsrc))
				return 0;
	...
	if (tsc && tsc->count) { // since an atk can only have one type let's optimize this a bit
		switch (atk_elem) {
			...
			case ELE_EARTH:
				...
				break;
			case ELE_NEUTRAL:
				if (tsc->data[SC_ANTI_MATERIAL_BLAST])
					ratio += tsc->data[SC_ANTI_MATERIAL_BLAST]->val2;
				break;
			case ELE_DARK:
				if (tsc->data[SC_SOULCURSE] != NULL)
					ratio += 100;
				break;
		}
	} // end tsc check
...
static int64 battle_addmastery(struct map_session_data *sd, struct block_list *target, int64 dmg, int type)
{
	...
	if ((skill_lv = pc->checkskill(sd, AL_DEMONBANE)) > 0 && target->type == BL_MOB && // This bonus doesn't work against players.
...
static int64 battle_calc_masteryfix(struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv, int64 damage, int div, bool left, bool weapon)
{
	...
	if (i < MAX_PC_FEELHATE && (skill2_lv = pc->checkskill(sd, pc->sg_info[i].anger_id)) > 0 && weapon) {
		...
		if (i == 2)
			ratio += status_get_str(src); // Star Anger
		if (skill2_lv < 4)
	...
	if ((sd->job & MAPID_THIRDMASK) == MAPID_ARCH_BISHOP) {
		if ((skill2_lv = pc->checkskill(sd, AB_EUCHARISTICA)) > 0 && (tstatus->race == RC_DEMON || tstatus->def_ele == ELE_DARK))
				...
				if (!(nk & NK_NO_ELEFIX)) {
					...
					for (i = 0; ARRAYLENGTH(tsd->subele2) > i && tsd->subele2[i].rate != 0; i++) {
						if (tsd->subele2[i].ele != s_ele)
							continue;
						if (!(tsd->subele2[i].flag & wflag & BF_WEAPONMASK && tsd->subele2[i].flag & wflag & BF_RANGEMASK && tsd->subele2[i].flag & wflag & BF_SKILLMASK))
							continue;
						...
						if (!(nk & NK_NO_ELEFIX)) {
							...
							for (i = 0; ARRAYLENGTH(sd->right_weapon.addele2) > i && sd->right_weapon.addele2[i].rate != 0; i++) {
								if (sd->right_weapon.addele2[i].ele != tstatus->def_ele)
									continue;
								if (!(sd->right_weapon.addele2[i].flag & wflag & BF_WEAPONMASK && sd->right_weapon.addele2[i].flag & wflag & BF_RANGEMASK && sd->right_weapon.addele2[i].flag & wflag & BF_SKILLMASK))
									continue;
								...
								if (!(nk & NK_NO_ELEFIX)) {
									...
									for (i = 0; ARRAYLENGTH(sd->left_weapon.addele2) > i && sd->left_weapon.addele2[i].rate != 0; i++) {
										if (sd->left_weapon.addele2[i].ele != tstatus->def_ele)
											continue;
										if (!(sd->left_weapon.addele2[i].flag & wflag & BF_WEAPONMASK && sd->left_weapon.addele2[i].flag & wflag & BF_RANGEMASK && sd->left_weapon.addele2[i].flag & wflag & BF_SKILLMASK))
											continue;
	...
	switch (attack_type) {
		case BF_WEAPON: {
			...
			if (tsd) {
				...
				if (src->type == BL_MOB && (i = pc->checkskill(tsd, RA_RANGERMAIN)) > 0 && (sstatus->race == RC_BRUTE || sstatus->race == RC_FISH || sstatus->race == RC_PLANT))
					vit_def += i * 5;
			} else { // Mob-Pet vit-eq
#ifndef RENEWAL
		...
		case BF_WEAPON:
			switch (skill_id) {
				...
				case GS_BULLSEYE:
					...
					if ((tst->race == RC_BRUTE || tst->race == RC_DEMIHUMAN) && !(tst->mode & MD_BOSS))
						skillratio += 400;
					break;
				case GS_TRACKING:
					skillratio += 100 * (skill_lv + 1);
				...
				case NC_ARMSCANNON:
					switch (tst->size) {
						case SZ_SMALL:
							skillratio = 300 + 350 * skill_lv;
							break; // Medium
						case SZ_MEDIUM:
							skillratio = 300 + 400 * skill_lv;
							break; // Small
						case SZ_BIG:
				...
				{
					int hp = status_get_max_hp(src) * (10 + 2 * skill_lv) / 100, sp = status_get_max_sp(src) * (6 + skill_lv) / 100;
				...
				case KO_SETSUDAN:
					...
					if (tsc != NULL) {
						...
						if ((sce = tsc->data[SC_SOULLINK]) != NULL || (sce = tsc->data[SC_SOULGOLEM]) != NULL || (sce = tsc->data[SC_SOULSHADOW]) != NULL || (sce = tsc->data[SC_SOULFALCON]) != NULL || (sce = tsc->data[SC_SOULFAIRY]) != NULL) // Bonus damage added when target is soul linked.
			...
			if (sc && skill_id != PA_SACRIFICE) {
#ifdef RENEWAL_EDP
				if (sc->data[SC_EDP]) {
					if (skill_id == AS_SONICBLOW || skill_id == GC_COUNTERSLASH || skill_id == GC_CROSSIMPACT)
						skillratio >>= 1;
				}
#endif
				...
				if ((!skill_id || skill_id == KN_AUTOCOUNTER) && sc->data[SC_CRUSHSTRIKE]) {
					if (sd) { // ATK [{Weapon Level * (Weapon Upgrade Level + 6) * 100} + (Weapon ATK) + (Weapon Weight)]%
						short index = sd->equip_index[EQI_HAND_R];
						if (index >= 0 && sd->inventory_data[index] && sd->inventory_data[index]->type == IT_WEAPON)
							skillratio += -100 + sd->inventory_data[index]->weight / 10 + st->rhw.atk + 100 * sd->inventory_data[index]->wlv * (sd->status.inventory[index].refine + 6);
					}
...
static int64 battle_calc_damage(struct block_list *src, struct block_list *bl, struct Damage *d, int64 damage, uint16 skill_id, uint16 skill_lv)
{
	...
	if (sc && sc->count) {
		...
		if (sc->data[SC_WHITEIMPRISON] && skill_id != HW_GRAVITATION) { // Gravitation and Pressure do damage without removing the effect
			if (skill_id == MG_NAPALMBEAT || skill_id == MG_SOULSTRIKE || skill_id == WL_SOULEXPANSION || (skill_id && skill->get_ele(skill_id, skill_lv) == ELE_GHOST) || (!skill_id && (status->get_status_data(src))->rhw.ele == ELE_GHOST)) {
		...
		if ((sce = sc->data[SC_AUTOGUARD]) && flag & BF_WEAPON && !(skill->get_nk(skill_id) & NK_NO_CARDFIX_ATK) && rnd() % 100 < sce->val2) {
			...
			if (sce_d) {
				...
				if (d_bl != NULL && check_distance_bl(bl, d_bl, sce_d->val3) && ((d_bl->type == BL_MER && d_md->master != NULL && d_md->master->bl.id == bl->id) || (d_bl->type == BL_PC && d_sd->devotion[sce_d->val2] == bl->id))) {
		...
		if ((sce = sc->data[SC_PARRYING]) && flag & BF_WEAPON && skill_id != WS_CARTTERMINATION && rnd() % 100 < sce->val2) { // attack blocked by Parrying
			clif->skill_nodamage(bl, bl, LK_PARRYING, sce->val1, 1);
			return 0;
		}

		if (sc->data[SC_DODGE_READY] && (!sc->opt1 || sc->opt1 == OPT1_BURNING) && (flag & BF_LONG || sc->data[SC_STRUP]) && rnd() % 100 < 20) {
			if (t_sd && pc_issit(t_sd))
				pc->setstand(t_sd); // Stand it to dodge.
		...
		if (((sce = sc->data[SC_NJ_UTSUSEMI]) || sc->data[SC_NJ_BUNSINJYUTSU]) && flag & BF_WEAPON && !(skill->get_nk(skill_id) & NK_NO_CARDFIX_ATK)) {
 ...
 **/
		...
		if ((sce = sc->data[SC_HAMI_BLOODLUST]) && flag & BF_WEAPON && damage > 0 && rnd() % 100 < sce->val3)
			status->heal(src, damage * sce->val4 / 100, 0, STATUS_HEAL_FORCED | STATUS_HEAL_SHOWEFFECT);

		if ((sce = sc->data[SC_FORCEOFVANGUARD]) && flag & BF_WEAPON && rnd() % 100 < sce->val2 && sc->fv_counter <= sce->val3)
			clif->millenniumshield(bl, sc->fv_counter++);

		if (sc->data[SC_STYLE_CHANGE] && rnd() % 2) {
			struct homun_data *hd = BL_CAST(BL_HOM, bl);
			if (hd)
				homun->addspiritball(hd, 10); // add a sphere
		}
	...
	if (s_sc && s_sc->count) {
		...
		if (bl->type == BL_MOB) {
			...
			if (((sce = s_sc->data[SC_MANU_ATK]) != NULL && (flag & BF_WEAPON)) || ((sce = s_sc->data[SC_MANU_MATK]) != NULL && (flag & BF_MAGIC))) {
				for (i = 0; i < ARRAYLENGTH(mob->manuk); i++)
			...
			if (((sce = s_sc->data[SC_SPL_ATK]) != NULL && (flag & BF_WEAPON)) || ((sce = s_sc->data[SC_SPL_MATK]) != NULL && (flag & BF_MAGIC))) {
				for (i = 0; i < ARRAYLENGTH(mob->splendide); i++)
...
static int battle_range_type(struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv)
{
	...
	if (battle_config.skillrange_by_distance && (src->type & battle_config.skillrange_by_distance)) { // based on distance between src/target [Skotlex]
...
#define MATK_ADD(a) (ad.damage += (a))
		...
		switch (skill_id) {
			...
			case PR_TURNUNDEAD:
				...
				i = 20 * skill_lv + sstatus->luk + sstatus->int_ + status->get_lv(src) + 200 - 200 * tstatus->hp / tstatus->max_hp; // there is no changed in success chance in renewal. [malufett]
				if (i > 700)
					i = 700;
		...
		if (sd) {
			...
			// Ignore Defense?
			if (!flag.imdef && (sd->bonus.ignore_mdef_ele & (1 << tstatus->def_ele) || sd->bonus.ignore_mdef_race & map->race_id2mask(tstatus->race) || sd->bonus.ignore_mdef_race & map->race_id2mask(is_boss(target) ? RC_BOSS : RC_NONBOSS)))
				flag.imdef = 1;
		}
...
static struct Damage battle_calc_weapon_attack(struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv, int wflag)
{
	...
	if ((sd && sd->state.arrow_atk) || (!sd && ((skill_id && skill->get_ammotype(skill_id)) || sstatus->rhw.range > 3)))
		flag.arrow = 1;
	...
	if (sd != NULL && skill_id == 0) {
		...
		if (((skill_lv = pc->checkskill(sd, TF_DOUBLE)) > 0 && sd->weapontype1 == W_DAGGER) || (sd->bonus.double_rate > 0 && sd->weapontype1 != W_FIST) // Will fail bare-handed
		...
		} else if (((sd->weapontype1 == W_REVOLVER && (skill_lv = pc->checkskill(sd, GS_CHAINACTION)) > 0) || (sc && sc->count && sc->data[SC_ETERNAL_CHAIN] && (skill_lv = sc->data[SC_ETERNAL_CHAIN]->val1) > 0)) && rnd() % 100 < 5 * skill_lv) {
	...
	// Check for critical
	if (!flag.cri && wd.type != BDT_MULTIHIT && sstatus->cri && (!skill_id || skill_id == KN_AUTOCOUNTER || skill_id == SN_SHARPSHOOTING || skill_id == MA_SHARPSHOOTING || skill_id == NJ_KIRIKAGE)) {
...
#define ATK_RATE(a)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           \
	do {                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      \
		int64 temp__ = (a);                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   \
		wd.damage = wd.damage * temp__ / 100;                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 \
		if (flag.lh)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          \
			wd.damage2 = wd.damage2 * temp__ / 100;                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           \
	} while (0)
...
#define ATK_ADD2(a, b)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        \
	...
	#define GET_NORMAL_ATTACK2(f, s) (wd.damage2 = battle->calc_base_damage(src, target, s, skill_lv, nk, n_ele, s_ele, s_ele_, EQI_HAND_L, (f), wd.flag))
		...
		switch (skill_id) {
			...
			default: {
				i = (flag.cri
				    ...
				    (flag.arrow ? 2 : 0) |
#ifndef RENEWAL
				    (skill_id == HW_MAGICCRASHER ? 4 : 0) | (skill_id == MO_EXTREMITYFIST ? 8 : 0) |
#endif
				    (!skill_id && sc && sc->data[SC_HLIF_CHANGE] ? 4 : 0) | (sc && sc->data[SC_WEAPONPERFECT] ? 8 : 0);
				if (flag.arrow && sd)
		...
		switch (skill_id) {
			...
			case GS_MAGICALBULLET:
				GET_NORMAL_ATTACK((sc && sc->data[SC_MAXIMIZEPOWER] ? 1 : 0) | (sc && sc->data[SC_WEAPONPERFECT] ? 8 : 0), skill_id);
				ATK_ADD(battle->attr_fix(src, target, battle->calc_cardfix(BF_MAGIC, src, target, nk, s_ele, 0, status->get_matk(src, 2), 0, wd.flag), ELE_NEUTRAL, tstatus->def_ele, tstatus->ele_lv));
				break;
			case GS_PIERCINGSHOT:
				GET_NORMAL_ATTACK((sc && sc->data[SC_MAXIMIZEPOWER] ? 1 : 0) | (sc && sc->data[SC_WEAPONPERFECT] ? 8 : 0), 0);
				if (wd.damage) {
			...
			case ML_SPIRALPIERCE: // [malufett]
				if (skill_id != NJ_TATAMIGAESHI) {
					...
					if (sd && index >= 0 && sd->inventory_data[index] && sd->inventory_data[index]->type == IT_WEAPON)
		...
		switch (skill_id) {
			case AS_SONICBLOW:
				if (sc && sc->data[SC_SOULLINK] && sc->data[SC_SOULLINK]->val2 == SL_ASSASIN)
		...
		if (sd) {
			...
			if ((i = pc->checkskill(sd, AB_EUCHARISTICA)) > 0 && (tstatus->race == RC_DEMON || tstatus->def_ele == ELE_DARK))
				ATK_ADDRATE(-i);
			if (skill_id != PA_SACRIFICE && skill_id != MO_INVESTIGATE && skill_id != CR_GRANDCROSS && skill_id != NPC_GRANDDARKNESS && skill_id != PA_SHIELDCHAIN && skill_id != KO_HAPPOKUNAI
#ifndef RENEWAL
			    && !flag.cri
#endif
			) {
				// Elemental/Racial adjustments
				if (sd->right_weapon.def_ratio_atk_ele & (1 << tstatus->def_ele) || sd->right_weapon.def_ratio_atk_race & map->race_id2mask(tstatus->race) || sd->right_weapon.def_ratio_atk_race & map->race_id2mask(is_boss(target) ? RC_BOSS : RC_NONBOSS))
					flag.pdef = 1;

				if (sd->left_weapon.def_ratio_atk_ele & (1 << tstatus->def_ele) || sd->left_weapon.def_ratio_atk_race & map->race_id2mask(tstatus->race) || sd->left_weapon.def_ratio_atk_race & map->race_id2mask(is_boss(target) ? RC_BOSS : RC_NONBOSS)) {
			...
			if (skill_id != CR_GRANDCROSS && skill_id != NPC_GRANDDARKNESS) {
				// Ignore Defense?
				if (!flag.idef && (sd->right_weapon.ignore_def_ele & (1 << tstatus->def_ele) || sd->right_weapon.ignore_def_race & map->race_id2mask(tstatus->race) || sd->right_weapon.ignore_def_race & map->race_id2mask(is_boss(target) ? RC_BOSS : RC_NONBOSS)))
					flag.idef = 1;

				if (!flag.idef2 && (sd->left_weapon.ignore_def_ele & (1 << tstatus->def_ele) || sd->left_weapon.ignore_def_race & map->race_id2mask(tstatus->race) || sd->left_weapon.ignore_def_race & map->race_id2mask(is_boss(target) ? RC_BOSS : RC_NONBOSS))) {
...
#ifndef RENEWAL
		// Refine bonus
		if (sd && flag.weapon && skill_id != MO_INVESTIGATE && skill_id != MO_EXTREMITYFIST) { // Counts refine bonus multiple times
	...
	if (wd.damage != 0 && tsc != NULL && tsc->data[SC_SWORDREJECT] != NULL && (sd == NULL || sd->weapontype1 == W_DAGGER || sd->weapontype1 == W_1HSWORD || sd->weapontype == W_2HSWORD) && rnd() % 100 < tsc->data[SC_SWORDREJECT]->val2) {
		ATK_RATER(50);
...
static struct Damage battle_calc_attack(int attack_type, struct block_list *bl, struct block_list *target, uint16 skill_id, uint16 skill_lv, int count)
{
	...
	switch (attack_type) {
		case BF_WEAPON:
			d = battle->calc_weapon_attack(bl, target, skill_id, skill_lv, count);
			break;
		case BF_MAGIC:
			d = battle->calc_magic_attack(bl, target, skill_id, skill_lv, count);
			break;
		case BF_MISC:
			d = battle->calc_misc_attack(bl, target, skill_id, skill_lv, count);
			break;
		default:
	...
	if (sd && d.damage + d.damage2 > 1) {
		// HPVanishRate
		if (sd->bonus.hp_vanish_rate && sd->bonus.hp_vanish_trigger && rnd() % 1000 < sd->bonus.hp_vanish_rate && ((d.flag & sd->bonus.hp_vanish_trigger & BF_WEAPONMASK) || (d.flag & sd->bonus.hp_vanish_trigger & BF_RANGEMASK) || (d.flag & sd->bonus.hp_vanish_trigger & BF_SKILLMASK)))
			status_percent_damage(&sd->bl, target, -sd->bonus.hp_vanish_per, 0, false);

		// SPVanishRate
		if (sd->bonus.sp_vanish_rate && sd->bonus.sp_vanish_trigger && rnd() % 1000 < sd->bonus.sp_vanish_rate && ((d.flag & sd->bonus.sp_vanish_trigger & BF_WEAPONMASK) || (d.flag & sd->bonus.sp_vanish_trigger & BF_RANGEMASK) || (d.flag & sd->bonus.sp_vanish_trigger & BF_SKILLMASK)))
	...
	if (sc) {
		if (wd->flag & BF_SHORT && !(skill->get_inf(skill_id) & (INF_GROUND_SKILL | INF_SELF_SKILL))) {
			if (sc->data[SC_CRESCENTELBOW] && !is_boss(src) && rnd() % 100 < sc->data[SC_CRESCENTELBOW]->val2) {
				...
				clif->skill_damage(target, src, tick, status_get_amotion(src), 0, rdamage, 1, SR_CRESCENTELBOW_AUTOSPELL, sc->data[SC_CRESCENTELBOW]->val1, BDT_SKILL); // This is how official does
	...
	if (wd->flag & BF_SHORT) {
		...
		if (wd->dmg_lv >= ATK_BLOCK) { /* yes block still applies, somehow gravity thinks it makes sense. */
			...
			if (sc) {
				...
				if (sc->data[SC_REFLECTSHIELD] && skill_id != WS_CARTTERMINATION && skill_id != GS_DESPERADO && !(d_bl && !(wd->flag & BF_SKILL)) // It should not be a basic attack if the target is under devotion
...
static enum damage_lv battle_weapon_attack(struct block_list *src, struct block_list *target, int64 tick, int flag)
{
	...
	if (sc) {
		...
		if (tsc && tsc->data[SC_MTF_MLEATKED] && rnd() % 100 < 20)
			clif->skill_nodamage(target, target, SM_ENDURE, 5, sc_start(target, target, SC_ENDURE, 100, 5, skill->get_time(SM_ENDURE, 5), SM_ENDURE));
	}
	...
	if (tsc) {
		if (tsc->data[SC_DEVOTION]) {
			...
			if (d_bl != NULL && ((d_bl->type == BL_MER && d_md->master != NULL && d_md->master->bl.id == target->id) || (d_sd != NULL && d_bl->type == BL_PC && d_sd->devotion[sce->val2] == target->id)) && check_distance_bl(target, d_bl, sce->val3)) {
	...
	if (sd) {
		if (wd.flag & BF_SHORT && sc && sc->data[SC__AUTOSHADOWSPELL] && rnd() % 100 < sc->data[SC__AUTOSHADOWSPELL]->val3 && sd->status.skill[skill->get_index(sc->data[SC__AUTOSHADOWSPELL]->val1)].id != 0 && sd->status.skill[skill->get_index(sc->data[SC__AUTOSHADOWSPELL]->val1)].flag == SKILL_FLAG_PLAGIARIZED) {
			...
			if (r_skill != AL_HOLYLIGHT && r_skill != PR_MAGNUS) {
				...
				if ((type = skill->get_casttype(r_skill)) == CAST_GROUND) {
					...
					if (!(BL_PC & battle_config.skill_reiteration) && skill->get_unit_flag(r_skill) & UF_NOREITERATION)
						type = -1;

					if (BL_PC & battle_config.skill_nofootset && skill->get_unit_flag(r_skill) & UF_NOFOOTSET)
						type = -1;

					if (BL_PC & battle_config.land_skill_limit && (maxcount = skill->get_maxcount(r_skill, r_lv)) > 0) {
						int v;
...
static int battle_check_target(struct block_list *src, struct block_list *target, int flag)
{
	...
	if ((flag & BCT_ENEMY) != 0 && (status_get_mode(s_bl) & MD_BOSS) == 0 && (map->getcell(m, src, src->x, src->y, CELL_CHKBASILICA) != 0 || map->getcell(m, src, target->x, target->y, CELL_CHKBASILICA) != 0)) {
		return -1;
	}
	...
	switch (target->type) { // Checks on actual target
		...
		case BL_MOB: {
			const struct mob_data *md = BL_UCCAST(BL_MOB, target);
			if (((md->special_state.ai == AI_SPHERE || (md->special_state.ai == AI_FLORA && battle_config.summon_flora & 1)) && s_bl->type == BL_PC && src->type != BL_MOB) || (md->special_state.ai == AI_ZANZOU && t_bl->id != s_bl->id)) {
		...
		case BL_SKILL: {
			...
			if (!su->group)
				return 0;
			if (skill->get_inf2(su->group->skill_id) & INF2_TRAP && su->group->unit_id != UNT_USED_TRAPS && su->group->unit_id != UNT_NETHERWORLD) { // Only a few skills can target traps...
				switch (battle->get_current_skill(src)) {
	...
	switch (t_bl->type) { // Checks on target master
		...
		case BL_MOB: {
			...
			if (!((map->agit_flag || map->agit2_flag) && map->list[m].flag.gvg_castle) && md->guardian_data && (md->guardian_data->g || md->guardian_data->castle->guild_id))
	...
	switch (s_bl->type) { // Checks on source master
		...
		case BL_MOB: {
			...
			if (md->special_state.ai == AI_NONE) {
				...
				if ((target_md != NULL && t_bl->type == BL_PC && target_md->special_state.ai != AI_ZANZOU && target_md->special_state.ai != AI_ATTACK) || (t_bl->type == BL_MOB && BL_UCCAST(BL_MOB, t_bl)->special_state.ai == AI_NONE))
	...
	if (map_flag_vs(m)) {
		...
		if (flag & (BCT_PARTY | BCT_ENEMY)) {
			...
			if (s_party != 0 && s_party == status->get_party_id(t_bl)) {
				...
				} else if (!(map->list[m].flag.pvp && map->list[m].flag.pvp_noparty) && (!map->list[m].flag.battleground || sbg_id == tbg_id)) {
					state |= BCT_PARTY;
		...
		if (flag & (BCT_GUILD | BCT_ENEMY)) {
			int s_guild = status->get_guild_id(s_bl);
			int t_guild = status->get_guild_id(t_bl);
			if (!(map->list[m].flag.pvp && map->list[m].flag.pvp_noguild) && s_guild && t_guild && (s_guild == t_guild || (!(flag & BCT_SAMEGUILD) && guild->isallied(s_guild, t_guild))) && (!map->list[m].flag.battleground || sbg_id == tbg_id) && (!map->list[m].flag.cvc || s_clan == t_clan)) {
				state |= BCT_GUILD;
			} else {
				state |= BCT_ENEMY;
			}
		...
		if (state & BCT_ENEMY && battle_config.pk_mode && !map_flag_gvg(m) && s_bl->type == BL_PC && t_bl->type == BL_PC) {
			...
			if ((s_sd->job & MAPID_UPPERMASK) == MAPID_NOVICE || (t_sd->job & MAPID_UPPERMASK) == MAPID_NOVICE || s_sd->status.base_level < battle_config.pk_min_level || t_sd->status.base_level < battle_config.pk_min_level || (battle_config.pk_level_range && abs(s_sd->status.base_level - t_sd->status.base_level) > battle_config.pk_level_range))
				state &= ~BCT_ENEMY;
		}
...
static const struct config_data_old battle_data[] = {
    ...
    {
        "trap_options/visibility",
        &battle_config.trap_visibility,
        2,
        0,
        2,
    },
    {
        "trap_options/display_on_trigger",
        &battle_config.trap_trigger,
        1,
        0,
        1,
    },
    ...
    },
    {
        ...
        &battle_config.pet_catch_rate_official_formula,
        1,
        0,
        1,
    },
    ...
    },
    {
        ...
        &battle_config.pet_remove_immediately,
        1,
        0,
        1,
    },
    ...
    {
        "combo_cache_skill",
        &battle_config.combo_cache_skill,
        0,
        0,
        1,
    },
    ...
    },
    {
        ...
        &battle_config.summer2_ignorepalette,
        0,
        0,
        1,
    },
    ...
    {
        "natural_heal_cap",
        &battle_config.natural_heal_cap,
        1000,
        1,
        INT_MAX,
    },
    ...
    {
        "party_change_leader_same_map",
        &battle_config.party_change_leader_same_map,
        0,
        0,
        1,
    },
    ...
    {
        "keep_dir_free_cell",
        &battle_config.keep_dir_free_cell,
        0,
        0,
        1,
    },
    {
        "check_occupied_cells",
        &battle_config.check_occupied_cells,
        1,
        0,
        1,
    },
    ...
    },
    {
        ...
        &battle_config.item_drop_add_chain_min,
        1,
        1,
        10000,
    },
    {
        "item_drop_add_chain_max",
        &battle_config.item_drop_add_chain_max,
        10000,
        1,
        10000,
    },
    ...
    {
        "item_rate_add_chain",
        &battle_config.item_rate_add_chain,
        100,
        0,
        1000000,
    },
    ...
    {
        "item_drop_bonus_max_threshold",
        &battle_config.item_drop_bonus_max_threshold,
        9000,
        0,
        10000,
    },
    ...
    {
        "show_katar_crit_bonus",
        &battle_config.show_katar_crit_bonus,
        0,
        0,
        1,
    },
    ...
    {
        "slave_chase_masters_chasetarget",
        &battle_config.slave_chase_masters_chasetarget,
        1,
        0,
        1,
    },
    ...
    {
        "client_emblem_max_blank_percent",
        &battle_config.client_emblem_max_blank_percent,
        100,
        0,
        100,
    },
    ...
    {
        "atcommand_max_stat_bypass",
        &battle_config.atcommand_max_stat_bypass,
        0,
        0,
        100,
    },
    ...
    {
        "trap_reflect",
        &battle_config.trap_reflect,
        1,
        0,
        1,
    },
    {
        "item_restricted_consumption_type",
        &battle_config.item_restricted_consumption_type,
        1,
        0,
        1,
    },
    {
        "unequip_restricted_equipment",
        &battle_config.unequip_restricted_equipment,
        0,
        0,
        3,
    },
    ...
    {
        "gm_ignore_warpable_area",
        &battle_config.gm_ignore_warpable_area,
        0,
        2,
        100,
    },
    {
        "packet_obfuscation",
        &battle_config.packet_obfuscation,
        1,
        0,
        3,
    },
    {
        "client_accept_chatdori",
        &battle_config.client_accept_chatdori,
        0,
        0,
        INT_MAX,
    },
    {
        "snovice_call_type",
        &battle_config.snovice_call_type,
        0,
        0,
        1,
    },
    ...
    {
        "idletime_criteria",
        &battle_config.idletime_criteria,
        0x25,
        1,
        INT_MAX,
    },
    ...
    {
        "case_sensitive_aegisnames",
        &battle_config.case_sensitive_aegisnames,
        1,
        0,
        1,
    },
    ...
    {
        "song_timer_reset",
        &battle_config.song_timer_reset,
        0,
        0,
        1,
    },
    ...
    {
        "show_monster_hp_bar",
        &battle_config.show_monster_hp_bar,
        1,
        0,
        1 | 2 | 4,
    },
    {
        "fix_warp_hit_delay_abuse",
    ...
    {
        "costume_refine_def",
        &battle_config.costume_refine_def,
        1,
        0,
        1,
    },
    {
        "shadow_refine_def",
        &battle_config.shadow_refine_def,
        1,
        0,
        1,
    },
    {
        "shadow_refine_atk",
        &battle_config.shadow_refine_atk,
        1,
        0,
        1,
    },
    ...
    {
        "player_warp_keep_direction",
        &battle_config.player_warp_keep_direction,
        0,
        0,
        1,
    },
    ...
    {
        "bow_unequip_arrow",
        &battle_config.bow_unequip_arrow,
        1,
        0,
        1,
    },
    ...
    {
        "boarding_halter_speed",
        &battle_config.boarding_halter_speed,
        25,
        0,
        100,
    },
    {
        "features/rodex",
        &battle_config.feature_rodex,
        1,
        0,
        1,
    },
    {
        "features/rodex_use_accountmail",
        &battle_config.feature_rodex_use_accountmail,
        0,
        0,
        1,
    },
    {
        ...
        &battle_config.feature_enable_homun_autofeed,
        1,
        0,
        1,
    },
    {
        ...
        &battle_config.feature_enable_pet_autofeed,
        1,
        0,
        1,
    },
    {
        "storage_use_item",
        &battle_config.storage_use_item,
        0,
        0,
        1,
    },
    {
        "features/enable_attendance_system",
        &battle_config.feature_enable_attendance_system,
        1,
        0,
        1,
    },
    {
        "features/feature_attendance_endtime",
        &battle_config.feature_attendance_endtime,
        1,
        0,
        99999999,
    },
    {
        "min_item_buy_price",
        &battle_config.min_item_buy_price,
        1,
        0,
        INT_MAX,
    },
    {
        "min_item_sell_price",
        &battle_config.min_item_sell_price,
        0,
        0,
        INT_MAX,
    },
    {
        ...
        &battle_config.display_fake_hp_when_dead,
        1,
        0,
        1,
    },
    ...
    {
        "skill_enabled_npc",
        &battle_config.skill_enabled_npc,
        0,
        0,
        INT_MAX,
    },
    {
        ...
        &battle_config.feature_enable_achievement,
        1,
        0,
        1,
    },
    ...
    {
        "option_drop_max_loop",
        &battle_config.option_drop_max_loop,
        10,
        1,
        100000,
    },
    {
        "enchant_ui_max_loop",
        &battle_config.enchant_ui_max_loop,
        10,
        1,
        100000,
    },
    ...
    },
    {
        ...
        &battle_config.display_rate_messages,
        1,
        0,
        7,
    },
    {
        ...
        &battle_config.display_config_messages,
        0x1F1,
        0,
        0x1F7,
    },
    {
        ...
        &battle_config.display_overweight_messages,
        3,
        0,
        3,
    },
    {
        "show_tip_window",
        &battle_config.show_tip_window,
        1,
        0,
        1,
    },
    {
        "features/enable_refinery_ui",
        &battle_config.enable_refinery_ui,
        1,
        0,
        1,
    },
    {
        "features/replace_refine_npcs",
        &battle_config.replace_refine_npcs,
        1,
        0,
        1,
    },
    ...
    {
        "hom_bonus_exp_from_master",
        &battle_config.hom_bonus_exp_from_master,
        10,
        0,
        100,
    },
    {
        ...
        &battle_config.allowed_actions_when_dead,
        0,
        0,
        3,
    },
    {
        ...
        &battle_config.teleport_close_storage,
        1,
        0,
        1,
    },
    {
        ...
        &battle_config.show_attendance_window,
        1,
        0,
        1,
    },
    {
        ...
        &battle_config.elem_natural_heal_hp,
        6000,
        NATURAL_HEAL_INTERVAL,
        INT_MAX,
    },
    {
        ...
        &battle_config.elem_natural_heal_sp,
        8000,
        NATURAL_HEAL_INTERVAL,
        INT_MAX,
    },
    {
        ...
        &battle_config.elem_natural_heal_cap,
        1000,
        1,
        INT_MAX,
    },
    {
        "hom_natural_heal_hp",
        &battle_config.hom_natural_heal_hp,
        2000,
        NATURAL_HEAL_INTERVAL,
        INT_MAX,
    },
    {
        "hom_natural_heal_sp",
        &battle_config.hom_natural_heal_sp,
        4000,
        NATURAL_HEAL_INTERVAL,
        INT_MAX,
    },
    {
        ...
        &battle_config.hom_natural_heal_cap,
        1000,
        1,
        INT_MAX,
    },
    ...
    },
    {
        ...
        &battle_config.merc_natural_heal_sp,
        8000,
        NATURAL_HEAL_INTERVAL,
        INT_MAX,
    },
    {
        ...
        &battle_config.merc_natural_heal_cap,
        1000,
        1,
        INT_MAX,
    },
    {
        "macro_detect_retry",
        &battle_config.macro_detect_retry,
        1,
        1,
        INT_MAX,
    },
    {
        "macro_detect_timeout",
        &battle_config.macro_detect_timeout,
        0,
        0,
        INT_MAX,
    },
    {
        "roulette_gold_step",
        &battle_config.roulette_gold_step,
        10,
        1,
        INT_MAX,
    },
    {
        "roulette_silver_step",
        &battle_config.roulette_silver_step,
        10,
        1,
        INT_MAX,
    },
    {
        "roulette_bronze_step",
        &battle_config.roulette_bronze_step,
        1,
        1,
        INT_MAX,
    },
    ...
    {
        "dynamic_npc_timeout",
        &battle_config.dynamic_npc_timeout,
        0,
        0,
        INT_MAX,
    },
    {
        "dynamic_npc_range",
        &battle_config.dynamic_npc_range,
        0,
        0,
        INT_MAX,
    },
    {
        "features/goldpc/enable",
        &battle_config.feature_goldpc_enable,
        0,
        0,
        1,
    },
    {
        "features/goldpc/default_mode",
        &battle_config.feature_goldpc_default_mode,
        1,
        0,
        INT_MAX,
    },
...
static void battle_config_check_deprecated(const char *filename, struct config_t *config)
{
	...
	const char *unit_params_keys[] = {"max_hp", "max_third_aspd", "max_extended_parameter", "max_summoner_parameter", "max_baby_parameter", "max_baby_third_parameter", "natural_heal_weight_rate"};
