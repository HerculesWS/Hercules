...
static struct Damage battle_calc_misc_attack(struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv, int mflag)
{
	...
	case CR_ACIDDEMONSTRATION:
		...
		{// [malufett]
			...
			int targetVit = min(120, (int)status_get_vit(target));
	...
	case SJ_NOVAEXPLOSING:
		...
		// Additional Damage
		md.damage += sstatus->max_hp / (6 - min(5, (int)skill_lv)) + status_get_max_sp(src) * (2 * skill_lv);
		break;
	case SP_SOULEXPLOSION:
	...
	if (sd != NULL && skill_id == 0) {
			...
			&& rnd() % 100 < 5 * skill_lv)
		{
			...
			&& (i = sd->equip_index[EQI_AMMO]) >= 0 && sd->inventory_data[i] && sd->status.inventory[i].amount > 1){
				...
				if ( wd.div_ > 1 ) {
					wd.div_ = min(wd.div_, (int)sd->status.inventory[i].amount);
...
#ifdef RENEWAL
#define NORMALIZE_RDAMAGE(d) ( trdamage += rdamage = max((int64)1, min((int64)max_reflect_damage, (d))) )
#else
...
static enum damage_lv battle_weapon_attack(struct block_list *src, struct block_list *target, int64 tick, int flag)
{
	...
	if (sc && sc->data[SC_AUTOSPELL] && rnd()%100 < sc->data[SC_AUTOSPELL]->val4) {
		...
		if (status->charge(src, 0, sp)) {
			skill->castend_type((enum cast_enum)skill->get_casttype(skill_id), src, target, skill_id, skill_lv, tick, flag);
		}
	}
	if (sd) {
