...
int64 battle_calc_masteryfix(struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv, int64 damage, int div, bool left, bool weapon) {
	...
	if( sc ){ // sc considered as masteries
		...
		if(sc->data[SC_DRUMBATTLE]){
			if(tstatus->size == MOBSIZE_SMALL)
				damage += sc->data[SC_DRUMBATTLE]->val2;
			else if(tstatus->size == MOBSIZE_MEDIUM)
	...
	switch(attack_type){
		case BF_MAGIC:
			switch(skill_id){
				...
				case HW_NAPALMVULCAN:
					...
					break;
				case SL_STIN:
					skillratio += (tst->size!=MOBSIZE_SMALL?-99:10*skill_lv); //target size must be small (0) for full damage.
					break;
				case SL_STUN:
					skillratio += (tst->size!=MOBSIZE_BIG?5*skill_lv:-99); //Full damage is dealt on small/medium targets
					break;
				case SL_SMA:
		...
		case BF_WEAPON:
			switch( skill_id )
			{
				...
				case NC_ARMSCANNON:
					switch( tst->size ) {
						case MOBSIZE_SMALL: skillratio = 300 + 400 * skill_lv; break;  // Small
						case MOBSIZE_MEDIUM: skillratio = 300 + 350 * skill_lv; break; // Medium
						case MOBSIZE_BIG: skillratio = 300 + 300 * skill_lv; break;    // Large
					}
					RE_LVL_DMOD(120);
					break;
		...
		switch (skill_id) {
			...
			case ML_SPIRALPIERCE:
				...
				switch (tstatus->size) { //Size-fix. Is this modified by weapon perfection?
					case MOBSIZE_SMALL: //Small: 125%
						ATK_RATE(125);
						break;
					//case MOBSIZE_MEDIUM: //Medium: 100%
					case MOBSIZE_BIG: //Large: 75%
						ATK_RATE(75);
						break;
				}
				break;
		...
		switch(skill_id){
			...
			case ML_SPIRALPIERCE: // [malufett]
				if( skill_id != NJ_TATAMIGAESHI ){
					...
					switch (tstatus->size) {
						case MOBSIZE_SMALL: //Small: 115%
							ATK_RATE(115);
							break;
						case MOBSIZE_BIG: //Large: 85%
							ATK_RATE(85);
					}
					wd.damage = battle->calc_masteryfix(src, target, skill_id, skill_lv, wd.damage, wd.div_, 0, flag.weapon);
					wd.damage = battle->calc_cardfix2(src, target, wd.damage, s_ele, nk, wd.flag);
