...
static void pc_inventory_rentals(struct map_session_data *sd)
{
	...
	nullpo_retv(sd);
	for (int i = 0; i < sd->status.inventorySize; i++) { // Check for Rentals on Inventory
...
static void pc_inventory_rental_add(struct map_session_data *sd, int seconds)
{
	...
	if (sd->rental_timer != INVALID_TIMER) {
		...
		if (DIFF_TICK(td->tick, timer->gettick()) > tick) { // Update Timer as this one ends first than the current one
...
static int pc_equippoint(struct map_session_data *sd, int n)
{
	...
	if (sd->inventory_data[n]->subtype == W_DAGGER || sd->inventory_data[n]->subtype == W_1HSWORD || sd->inventory_data[n]->subtype == W_1HAXE) {
		if (pc->checkskill(sd, AS_LEFT) > 0 || (sd->job & MAPID_UPPERMASK) == MAPID_ASSASSIN || (sd->job & MAPID_UPPERMASK) == MAPID_KAGEROUOBORO) {
	...
	switch (sd->weapontype1) {
		case W_DAGGER:
			switch (sd->weapontype2) {
				case W_DAGGER:
					sd->weapontype = W_DOUBLE_DD;
					break;
				case W_1HSWORD:
					sd->weapontype = W_DOUBLE_DS;
					break;
				case W_1HAXE:
...
static int pc_calc_skillpoint(struct map_session_data *sd)
{
	...
	for (i = 1; i < MAX_SKILL_DB; i++) {
		...
		if (skill_lv > 0) {
			...
			if ((!(inf2 & INF2_QUEST_SKILL) || battle_config.quest_skill_learn) && !(inf2 & (INF2_WEDDING_SKILL | INF2_SPIRIT_SKILL | INF2_GUILD_SKILL)) // Do not count wedding/link skills. [Skotlex]
			) {
...
static void pc_calc_skilltree_clear(struct map_session_data *sd)
{
	...
	for (i = 0; i < MAX_SKILL_DB; i++) {
		if (sd->status.skill[i].flag == SKILL_FLAG_PLAGIARIZED || sd->status.skill[i].flag == SKILL_FLAG_PERM_GRANTED || (sd->cloneskill_id != 0 && sd->status.skill[i].id == sd->cloneskill_id) || (sd->reproduceskill_id != 0 && sd->status.skill[i].id == sd->reproduceskill_id)) // Don't touch these
			continue;
		...
		for (int i = 0; i < MAX_SKILL_TREE && (id = pc->skill_tree[classidx][i].id) > 0; i++) {
			...
			if (!battle_config.skillfree) {
				...
				for (j = 0; j < VECTOR_LENGTH(pc->skill_tree[classidx][i].need); j++) {
					...
					int level;
					if (sd->status.skill[req->idx].id == 0 || sd->status.skill[req->idx].flag == SKILL_FLAG_TEMPORARY || sd->status.skill[req->idx].flag == SKILL_FLAG_PLAGIARIZED)
						level = 0;                                                        // Not learned.
			...
			if (satisfied) {
				...
				if (!sd->status.skill[idx].lv && ((inf2 & INF2_QUEST_SKILL && !battle_config.quest_skill_learn) || inf2 & INF2_WEDDING_SKILL || (inf2 & INF2_SPIRIT_SKILL && !sd->sc.data[SC_SOULLINK])))
...
static void pc_calc_skilltree_bonus(struct map_session_data *sd, int classidx)
{
	...
	//
	if (classidx > 0 && (sd->job & MAPID_UPPERMASK) == MAPID_TAEKWON && sd->status.base_level >= 90 && sd->status.skill_point == 0 && pc->fame_rank(sd->status.char_id, RANKTYPE_TAEKWON) > 0) {
...
static void pc_check_skilltree(struct map_session_data *sd, int skill_id)
{
	...
	do {
		...
		for (i = 0; i < MAX_SKILL_TREE && (id = pc->skill_tree[c][i].id) > 0; i++) {
			...
			if (!sd->status.skill[idx].lv && ((j & INF2_QUEST_SKILL && !battle_config.quest_skill_learn) || j & INF2_WEDDING_SKILL || (j & INF2_SPIRIT_SKILL && !sd->sc.data[SC_SOULLINK])))
...
static int pc_calc_skilltree_normalize_job(struct map_session_data *sd)
{
	...
	if (skill_point < novice_skills && (sd->job & MAPID_BASEMASK) != MAPID_SUMMONER) {
		...
		if (sd->change_level_2nd == 0) {
			if ((sd->job & JOBL_THIRD) != 0) {
				...
				if (sd->change_level_3rd == 0) {
					...
					sd->change_level_2nd = 1 + skill_point + sd->status.skill_point - (sd->status.job_level - 1) - (sd->change_level_3rd - 1) - novice_skills;
				}
			} else {
				sd->change_level_2nd = 1 + skill_point + sd->status.skill_point - (sd->status.job_level - 1) - novice_skills;
			}
		...
		if (skill_point < novice_skills + (sd->change_level_2nd - 1)) {
			...
			if (sd->change_level_3rd == 0) {
				sd->change_level_3rd = 1 + skill_point + sd->status.skill_point - (sd->status.job_level - 1) - (sd->change_level_2nd - 1) - novice_skills;
	...
	switch (type) {
		...
		case SP_AUTOSPELL:
			if (sd->state.lr_flag != 2) {
				...
				pc->bonus_autospell(sd->autospell, ARRAYLENGTH(sd->autospell), target ? -type2 : type2, type3, val, 0, status->current_equip_card_id);
			}
			break;
		case SP_AUTOSPELL_WHENHIT:
			if (sd->state.lr_flag != 2) {
				int target = skill->get_inf(type2); // Support or Self (non-auto-target) skills should pick self.
				target = target & INF_SUPPORT_SKILL || (target & INF_SELF_SKILL && !(skill->get_inf2(type2) & INF2_NO_TARGET_SELF));
				pc->bonus_autospell(sd->autospell2, ARRAYLENGTH(sd->autospell2), target ? -type2 : type2, type3, val, BF_NORMAL | BF_SKILL, status->current_equip_card_id);
			}
			break;
		case SP_SP_DRAIN_RATE:
			if (sd->state.lr_flag == 0) {
		...
		case SP_ADDEFF:
			...
			pc->bonus_addeff(sd->addeff, ARRAYLENGTH(sd->addeff), (sc_type)type2, sd->state.lr_flag != 2 ? type3 : 0, sd->state.lr_flag == 2 ? type3 : 0, val, 0);
			break;
...
static int pc_insert_card(struct map_session_data *sd, int idx_card, int idx_equip)
{
	...
	if (pc->delitem(sd, idx_card, 1, 1, DELITEM_NORMAL, LOG_TYPE_CARD) == 1) { // failed
...
static int pc_additem(struct map_session_data *sd, const struct item *item_data, int amount, e_log_pick_type log_type)
{
	...
	if (itemdb->isstackable2(data) && item_data->expire_time == 0) {
		for (i = 0; i < sd->status.inventorySize; i++) {
			if (sd->status.inventory[i].nameid == item_data->nameid && sd->status.inventory[i].bound == item_data->bound && sd->status.inventory[i].expire_time == 0 && sd->status.inventory[i].unique_id == item_data->unique_id && memcmp(&sd->status.inventory[i].card, &item_data->card, sizeof(item_data->card)) == 0) {
				if (amount > MAX_AMOUNT - sd->status.inventory[i].amount || (data->stack.inventory && amount > data->stack.amount - sd->status.inventory[i].amount))
					return 5;
...
static int pc_dropitem(struct map_session_data *sd, int n, int amount)
{
	...
	if (sd->status.inventory[n].nameid <= 0 || sd->status.inventory[n].amount <= 0 || sd->status.inventory[n].amount < amount || sd->state.trading || sd->state.vending || sd->state.prevend || !sd->inventory_data[n] // pc->delitem would fail on this case.
	)
		return 0;
	...
	if (fitem->first_get_charid > 0 && fitem->first_get_charid != sd->status.char_id) {
		...
		if (DIFF_TICK(tick, fitem->first_get_tick) < 0) {
			...
			if (DIFF_TICK(tick, fitem->second_get_tick) < 0) {
				if (!(p && p->party.item & 1 && ((first_sd && first_sd->status.party_id == sd->status.party_id) || (second_sd && second_sd->status.party_id == sd->status.party_id))))
					return 0;
				...
				if (DIFF_TICK(tick, fitem->third_get_tick) < 0) {
					if (!(p && p->party.item & 1 && ((first_sd && first_sd->status.party_id == sd->status.party_id) || (second_sd && second_sd->status.party_id == sd->status.party_id) || (third_sd && third_sd->status.party_id == sd->status.party_id))))
						return 0;
				}
			}
		}
...
static int pc_useitem(struct map_session_data *sd, int n)
{
	...
	if ((sd->npc_id != 0 && sd->state.using_megaphone == 0 && (sd->npc_item_flag & ITEMENABLEDNPC_CONSUME) == 0) || (sd->state.workinprogress & 1) != 0) {
	...
	if (nameid == ITEMID_MEGAPHONE && ((sd->state.workinprogress & 2) != 0 || sd->state.using_megaphone != 0 || sd->npc_id != 0)) {
#if PACKETVER >= 20110308
		clif->msgtable(sd, MSG_BUSY);
#else
		clif->messagecolor_self(sd->fd, COLOR_WHITE, msg_sd(sd, MSGTBL_WORK_IN_PROGRESS_TRY_AGAIN));
	...
	for (i = 0; i < map->list[sd->bl.m].zone->disabled_items_count; i++) {
		if (map->list[sd->bl.m].zone->disabled_items[i] == nameid) {
			...
			if (battle_config.item_restricted_consumption_type && sd->status.inventory[n].expire_time == 0) {
				...
				if ((nameid == ITEMID_EARTH_SCROLL_1_3 || nameid == ITEMID_EARTH_SCROLL_1_5) && sd->sc.count > 0 && sd->sc.data[SC_EARTHSCROLL] != NULL && rnd() % 100 > sd->sc.data[SC_EARTHSCROLL]->val2) {
					return 0;
				}
...
static void pc_autocast_remove(struct map_session_data *sd, enum autocast_type type, int skill_id, int skill_lv)
{
	...
	for (int i = 0; i < VECTOR_LENGTH(sd->auto_cast); i++) {
		if (VECTOR_INDEX(sd->auto_cast, i).type == type && VECTOR_INDEX(sd->auto_cast, i).skill_id == skill_id && VECTOR_INDEX(sd->auto_cast, i).skill_lv == skill_lv) {
...
static int pc_cart_additem(struct map_session_data *sd, struct item *item_data, int amount, e_log_pick_type log_type)
{
	...
	if (itemdb->isstackable2(data) && !item_data->expire_time) {
		ARR_FIND(0, MAX_CART, i, sd->status.cart[i].nameid == item_data->nameid && sd->status.cart[i].bound == item_data->bound && sd->status.cart[i].card[0] == item_data->card[0] && sd->status.cart[i].card[1] == item_data->card[1] && sd->status.cart[i].card[2] == item_data->card[2] && sd->status.cart[i].card[3] == item_data->card[3]);
	};
...
static int pc_steal_item(struct map_session_data *sd, struct block_list *bl, uint16 skill_lv)
{
	...
	if (md->master_id || md_status->mode & MD_BOSS || mob_is_treasure(md) || map->list[bl->m].flag.nomobloot || // check noloot map flag [Lorky]
...
static int pc_setpos(struct map_session_data *sd, unsigned short map_index, int x, int y, enum clr_type clrtype)
{
	...
	if (map->list[map_id].flag.src4instance != 0) {
		...
		if (sd->instances != 0) {
			...
			for (i = 0; i < sd->instances; i++) {
				if (sd->instance[i] >= 0) {
					ARR_FIND(0, instance->list[sd->instance[i]].num_map, j, map->list[instance->list[sd->instance[i]].map[j]].instance_src_map == map_id && !map->list[instance->list[sd->instance[i]].map[j]].custom_name);
		...
		if (!stop && sd->status.party_id != 0 && p != NULL && p->instances != 0) {
			...
			for (i = 0; i < p->instances; i++) {
				if (p->instance[i] >= 0) {
					ARR_FIND(0, instance->list[p->instance[i]].num_map, j, map->list[instance->list[p->instance[i]].map[j]].instance_src_map == map_id && !map->list[instance->list[p->instance[i]].map[j]].custom_name);
		...
		if (!stop && sd->status.guild_id != 0 && sd->guild != NULL && sd->guild->instances != 0) {
			...
			for (i = 0; i < sd->guild->instances; i++) {
				if (sd->guild->instance[i] >= 0) {
					ARR_FIND(0, instance->list[sd->guild->instance[i]].num_map, j, map->list[instance->list[sd->guild->instance[i]].map[j]].instance_src_map == map_id && !map->list[instance->list[sd->guild->instance[i]].map[j]].custom_name);
		...
		if (map->list[map_id].instance_id >= 0 && instance->list[map->list[map_id].instance_id].respawn.map == 0 && instance->list[map->list[map_id].instance_id].respawn.x == 0 && instance->list[map->list[map_id].instance_id].respawn.y == 0) {
	...
	if (sd->status.guild_id > 0 && (map->list[map_id].flag.gvg_castle == 1 || map->list[sd->bl.m].flag.gvg_castle == 1)) {
...
static int pc_checkallowskill(struct map_session_data *sd)
{
	const enum sc_type scw_list[] = {
	    SC_TWOHANDQUICKEN,  SC_ONEHANDQUICKEN,
	    SC_AURABLADE,       SC_PARRYING,
	    SC_SPEARQUICKEN,    SC_ADRENALINE,
	    SC_ADRENALINE2,     SC_DANCING,
	    SC_GS_GATLINGFEVER,
#ifdef RENEWAL
	    SC_LKCONCENTRATION, SC_EDP,
#endif
	    SC_FEARBREEZE,      SC_EXEEDBREAK,
	};
	const enum sc_type scs_list[] = {SC_AUTOGUARD, SC_DEFENDER, SC_REFLECTSHIELD, SC_LG_REFLECTDAMAGE};
	int i;
	nullpo_ret(sd);
	...
	for (i = 0; i < ARRAYLENGTH(scw_list); i++) {
		...
		if (scw_list[i] == SC_DANCING && !battle_config.dancing_weaponswitch_fix)
			continue;
		if (sd->sc.data[scw_list[i]] && !pc_check_weapontype(sd, skill->get_weapontype(status->sc2skill(scw_list[i]))))
...
static int pc_follow_timer(int tid, int64 tick, int id, intptr_t data)
{
	...
	if (sd->bl.prev != NULL && tbl->prev != NULL && sd->ud.skilltimer == INVALID_TIMER && sd->ud.attacktimer == INVALID_TIMER && sd->ud.walktimer == INVALID_TIMER) {
	...
	sd->followtimer = timer->add(tick + 1000, // increase time a bit to loosen up map's load
...
static bool pc_gainexp(struct map_session_data *sd, struct block_list *src, uint64 base_exp, uint64 job_exp, bool is_quest)
{
	...
	if (sd->state.showexp) {
		char output[256];
		sprintf(output, msg_sd(sd, MSGTBL_EXPERIENCE_GAINED), // Experience Gained Base:%llu (%.2f%%) Job:%llu (%.2f%%)
...
static int pc_skillup(struct map_session_data *sd, uint16 skill_id)
{
	...
	if (sd->status.skill_point > 0 && sd->status.skill[index].id && sd->status.skill[index].flag == SKILL_FLAG_PERMANENT && // Don't allow raising while you have granted skills. [Skotlex]
		...
		else if (sd->status.skill_point == 0 && (sd->job & MAPID_UPPERMASK) == MAPID_TAEKWON && sd->status.base_level >= 90 && pc->fame_rank(sd->status.char_id, RANKTYPE_TAEKWON) > 0)
...
static int pc_allskillup(struct map_session_data *sd)
{
	...
	if (pc_has_permission(sd, PC_PERM_ALL_SKILL)) { // Get ALL skills except npc/guild ones. [Skotlex]
		...
		for (i = 0; i < MAX_SKILL_TREE && (id = pc->skill_tree[pc->class2idx(sd->status.class)][i].id) > 0; i++) {
			...
			int inf2 = skill->dbs->db[idx].inf2;
			if ((inf2 & INF2_QUEST_SKILL && !battle_config.quest_skill_learn) || (inf2 & (INF2_WEDDING_SKILL | INF2_SPIRIT_SKILL)) || id == SG_DEVIL)
...
static int pc_dead(struct map_session_data *sd, struct block_list *src)
{
	...
	if (src != NULL) {
		switch (src->type) {
			case BL_MOB: {
				...
				if (battle_config.mobs_level_up != 0 && md->status.hp != 0 && md->level < pc->maxbaselv(sd) && md->guardian_data == NULL && md->special_state.ai == AI_NONE) { // Guardians/summons should not level up. [Skotlex]
	...
	if (battle_config.death_penalty_type != 0 && pc->isDeathPenaltyJob(sd->job) && !map_flag_gvg2(sd->bl.m) && map->list[sd->bl.m].flag.noexppenalty == 0 && sd->sc.data[SC_BABY] == NULL && sd->sc.data[SC_CASH_DEATHPENALTY] == NULL && !pc->auto_exp_insurance(sd)) {
		...
		for (int i = 0; i < map->list[sd->bl.m].drop_list_count; i++) {
			...
			if (id == -1) {
				...
				for (int j = 0; j < sd->status.inventorySize; j++) {
					...
					if (((type == 1 && !is_equipped) || (type == 2 && is_equipped) || type == 3) && sd->status.inventory[j].nameid == id && rnd() % 10000 < per) {
						if (is_equipped)
	...
	if ((map->list[sd->bl.m].flag.pvp != 0 || map->list[sd->bl.m].flag.gvg != 0) && (sd->state.standalone != 0 || sd->state.autotrade != 0)) {
	...
	if (map->list[sd->bl.m].flag.pvp != 0 && battle_config.pk_mode == 0 && map->list[sd->bl.m].flag.pvp_nocalcrank == 0) {
		sd->pvp_point -= 5;
		sd->pvp_lost++;
...
static int64 pc_readparam(const struct map_session_data *sd, int type)
{
	...
	switch (type) {
		...
		case SP_CLASS:
			...
			break;
		case SP_BASEJOB:
			val = pc->mapid2jobid(sd->job & MAPID_UPPERMASK, sd->status.sex);
			break; // Base job, extracting upper type.
		case SP_UPPER:
			val = (sd->job & JOBL_UPPER) != 0 ? 1 : ((sd->job & JOBL_BABY) != 0 ? 2 : 0);
			break;
		case SP_BASECLASS:
			val = pc->mapid2jobid(sd->job & MAPID_BASEMASK, sd->status.sex);
		...
		case SP_JOBEXP:
			...
			break;
		case SP_NEXTBASEEXP:
			val = pc->nextbaseexp(sd);
			break;
		case SP_NEXTJOBEXP:
			val = pc->nextjobexp(sd);
			break;
		case SP_HP:
		...
		case SP_KILLEDRID:
			...
			break;
		case SP_SLOTCHANGE:
			val = sd->status.slotchange;
			break;
		case SP_CHARRENAME:
			val = sd->status.rename;
			break;
		case SP_MOD_EXP:
			val = sd->status.mod_exp;
			break;
		case SP_MOD_DROP:
			val = sd->status.mod_drop;
			break;
		case SP_MOD_DEATH:
			val = sd->status.mod_death;
			break;
		case SP_CRITICAL:
...
static int pc_itemheal(struct map_session_data *sd, int itemid, int hp, int sp)
{
	...
	if (hp) {
		int i;
		bonus = 100 + (sd->battle_status.vit << 1) + pc->checkskill(sd, SM_RECOVERY) * 10 + pc->checkskill(sd, AM_LEARNINGPOTION) * 5;
	...
	if (sp) {
		bonus = 100 + (sd->battle_status.int_ << 1) + pc->checkskill(sd, MG_SRECOVERY) * 10 + pc->checkskill(sd, AM_LEARNINGPOTION) * 5;
		if (script->potion_flag > 1)
			bonus += bonus * (script->potion_flag - 1) * 50 / 100;
	...
	switch (type) {
		...
		case LOOK_HAIR: // Use the battle_config limits! [Skotlex]
			...
			if (sd->status.hair != val) {
				...
				if (sd->status.guild_id) // Update Guild Window. [Skotlex]
					intif->guild_change_memberinfo(sd->status.guild_id, sd->status.account_id, sd->status.char_id, GMI_HAIR, &sd->status.hair, sizeof(sd->status.hair));
			}
			break;
		case LOOK_WEAPON:
		...
		case LOOK_HAIR_COLOR: // Use the battle_config limits! [Skotlex]
			...
			if (sd->status.hair_color != val) {
				...
				if (sd->status.guild_id) // Update Guild Window. [Skotlex]
					intif->guild_change_memberinfo(sd->status.guild_id, sd->status.account_id, sd->status.char_id, GMI_HAIR_COLOR, &sd->status.hair_color, sizeof(sd->status.hair_color));
			}
			break;
...
static void pc_hide(struct map_session_data *sd, bool show_msg)
{
	...
	if (map->list[sd->bl.m].flag.pvp != 0 && map->list[sd->bl.m].flag.pvp_nocalcrank == 0 && sd->pvp_timer != INVALID_TIMER) { // Unregister the player for ranking.
		timer->delete(sd->pvp_timer, pc->calc_pvprank_timer);
		sd->pvp_timer = INVALID_TIMER;
	}
...
static bool pc_can_attack(struct map_session_data *sd, int target_id)
{
	...
	if (sd->sc.data[SC_BASILICA] || sd->sc.data[SC__SHADOWFORM] || sd->sc.data[SC__MANHOLE] || sd->sc.data[SC_CURSEDCIRCLE_ATKER] || sd->sc.data[SC_CURSEDCIRCLE_TARGET] || sd->sc.data[SC_COLD] || sd->sc.data[SC_ALL_RIDING] || // The client doesn't let you, this is to make cheat-safe
	    sd->sc.data[SC_TRICKDEAD] || (sd->sc.data[SC_SIREN] && sd->sc.data[SC_SIREN]->val2 == target_id) || sd->sc.data[SC_BLADESTOP] || sd->sc.data[SC_DEEP_SLEEP] || sd->sc.data[SC_FALLENEMPIRE] || sd->block_action.attack)
		return false;
...
static bool pc_can_talk(struct map_session_data *sd)
{
	...
	if (sd->sc.data[SC_BERSERK] || (sd->sc.data[SC_DEEP_SLEEP] && sd->sc.data[SC_DEEP_SLEEP]->val2) || pc_ismuted(&sd->sc, MANNER_NOCHAT) || sd->block_action.chat)
		return false;
...
static int pc_deleventtimer(struct map_session_data *sd, const char *name)
{
	...
	ARR_FIND(0, MAX_EVENTTIMER, i, sd->eventtimer[i] != INVALID_TIMER && (p = (char *)(timer->get(sd->eventtimer[i])->data)) != NULL && strcmp(p, name) == 0);
	if (i == MAX_EVENTTIMER)
		return 0; // not found
...
static int pc_addeventtimercount(struct map_session_data *sd, const char *name, int tick)
{
	...
	for (i = 0; i < MAX_EVENTTIMER; i++)
		if (sd->eventtimer[i] != INVALID_TIMER && strcmp((char *)(timer->get(sd->eventtimer[i])->data), name) == 0) {
...
{
	nullpo_retv(sd);
	if ((!map_no_view(sd->bl.m, EQP_SHADOW_WEAPON) && pos & EQP_SHADOW_WEAPON) || (pos & EQP_HAND_R)) {
		if (id != NULL) {
	...
	if ((!map_no_view(sd->bl.m, EQP_SHADOW_SHIELD) && pos & EQP_SHADOW_SHIELD) || (pos & EQP_HAND_L)) {
		if (id != NULL) {
...
static int pc_equipitem(struct map_session_data *sd, int n, int req_pos)
{
	...
	if (battle_config.battle_log != 0)
		ShowInfo("equip %d(%d) %x:%x\n", sd->status.inventory[n].nameid, n, sd->status.inventory[n].equip, (unsigned int)req_pos);
	...
	if (pc->isequip(sd, n) == 0 || (pos & req_pos) == 0 || sd->status.inventory[n].equip != 0 || (sd->status.inventory[n].attribute & ATTR_BROKEN) != 0) {
		clif->equipitemack(sd, n, 0, EIA_FAIL);
		return 0;
	}
...
static int pc_unequipitem(struct map_session_data *sd, int n, int flag)
{
	...
	if (sd->sc.count != 0 && (sd->sc.data[SC_BERSERK] != NULL || sd->sc.data[SC_NO_SWITCH_EQUIP] != NULL) && (flag & PCUNEQUIPITEM_FORCE) == 0) {
	...
	if ((pos & EQP_ARMS) != 0 && sd->weapontype1 == W_FIST && sd->weapontype2 == W_FIST && (sd->sc.data[SC_TK_SEVENWIND] == NULL || sd->sc.data[SC_ASPERSIO] != NULL)) { // Check for Seven Wind. (But not level seven!)
...
{
	if (sd == NULL || dstsd == NULL || sd->status.partner_id > 0 || dstsd->status.partner_id > 0 || (sd->job & JOBL_BABY) != 0 || (dstsd->job & JOBL_BABY) != 0)
		return -1;
...
{
	int heat = val, skill_lv, limit[] = {10, 20, 28, 46, 66};
...
static bool pc_read_exp_db_sub_class(struct config_setting_t *t, bool base)
{
	...
	if (libconfig->setting_lookup_int(t, "MaxLevel", &maxlv) == 0 || (maxlv <= 0 || maxlv > MAX_LEVEL)) {
...
static void pc_autotrade_load(void)
{
	...
	while (SQL_SUCCESS == SQL->NextRow(map->mysql_handle)) {
		...
		char_id = atoi(data);
		SQL->GetData(map->mysql_handle, 2, &data, NULL);
		sex = atoi(data);
		SQL->GetData(map->mysql_handle, 3, &data, NULL);
		safestrncpy(title, data, sizeof(title));
...
static void pc_autotrade_update(struct map_session_data *sd, enum e_pc_autotrade_update_action action)
{
	...
	switch (action) {
		...
		case PAUC_START: {
			...
			if (SQL_ERROR == SQL->Query(map->mysql_handle, "INSERT INTO `%s` (`account_id`,`char_id`,`sex`,`title`) VALUES ('%d','%d','%d','%s')", map->autotrade_merchants_db, sd->status.account_id, sd->status.char_id, sd->status.sex, title))
				Sql_ShowDebug(map->mysql_handle);
		}
			FALLTHROUGH
		case PAUC_REFRESH:
			for (i = 0; i < sd->vend_num; i++) {
				...
				if (SQL_ERROR == SQL->Query(map->mysql_handle, "INSERT INTO `%s` (`char_id`,`itemkey`,`amount`,`price`) VALUES ('%d','%d','%d','%u')", map->autotrade_data_db, sd->status.char_id, sd->status.cart[sd->vending[i].index].id, sd->vending[i].amount, sd->vending[i].value))
					Sql_ShowDebug(map->mysql_handle);
			}
			break;
	}
...
{
	return class < JOB_MAX_BASIC || (class >= JOB_NOVICE_HIGH && class <= JOB_DARK_COLLECTOR) || (class >= JOB_RUNE_KNIGHT && class <= JOB_MECHANIC_T2) || (class >= JOB_BABY_RUNE && class <= JOB_BABY_MECHANIC2) || (class >= JOB_SUPER_NOVICE_E && class <= JOB_SUPER_BABY_E) || (class >= JOB_KAGEROU && class <= JOB_OBORO) || (class == JOB_REBELLION) || (class >= JOB_SUMMONER && class < JOB_MAX);
}
