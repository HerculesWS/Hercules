...
static void macro_detector_process_answer(struct map_session_data *sd, const char *captcha_answer)
{
	...
	if (captcha_answer != NULL && strcmp(captcha_answer, cd->captcha_answer) == 0) {
		...
		// Grant a small buff
		sc_start(NULL, &sd->bl, skill->get_sc_type(&sd->bl, AL_INCAGI), 100, 10, 600000, 0); // skill_ids are set to 0 so GTB doesn't block for this check.
		sc_start(NULL, &sd->bl, skill->get_sc_type(&sd->bl, AL_BLESSING), 100, 10, 600000, 0); // skill_ids are set to 0 so GTB doesn't block for this check.
