...
static void chrif_authok(int fd)
{
	...
	if (core->runflag == MAPSERVER_ST_RUNNING && node->account_id == account_id && node->char_id == char_id && node->login_id1 == login_id1) { // Auth Ok
...
static void chrif_authfail(int fd)
{
	...
	if (node != NULL && node->account_id == account_id && node->char_id == char_id && node->login_id1 == login_id1 && node->sex == sex && node->state == ST_LOGIN) { // found a match
...
static bool chrif_char_ask_name_answer(int acc, const char *player_name, uint16 type, uint16 answer)
{
	...
	if (type == CHAR_ASK_NAME_CHARBAN)
		type = CHAR_ASK_NAME_BAN;
	if (type == CHAR_ASK_NAME_CHARUNBAN)
		type = CHAR_ASK_NAME_UNBAN;
	...
	switch (answer) {
		...
		case CHAR_ASK_NAME_ANS_NOTFOUND:
			sprintf(output, msg_sd(sd, MSGTBL_CHRIF_PLAYER_NOT_FOUND), NAME_LENGTH, player_name);
			break;
		case CHAR_ASK_NAME_ANS_GMLOW:
			sprintf(output, msg_sd(sd, MSGTBL_CHRIF_GM_UNAUTHORIZED), action, NAME_LENGTH, player_name);
			break;
		case CHAR_ASK_NAME_ANS_OFFLINE:
			sprintf(output, msg_sd(sd, MSGTBL_CHRIF_LOGIN_SERVER_OFFLINE), action, NAME_LENGTH, player_name);
			break;
		default:
			output[0] = '\0';
			break;
...
static int chrif_disconnectplayer(int fd)
{
	...
	switch (RFIFOB(fd, 6)) {
		case 1:
			...
			break; // server closed
		case 2:
			clif->authfail_fd(sd->fd, 2);
			break; // someone else logged in
		case 3:
			clif->authfail_fd(sd->fd, 4);
...
static bool chrif_save_scdata(struct map_session_data *sd)
{
	...
	for (i = 0; i < SC_MAX; i++) {
		...
		memcpy(WFIFOP(chrif->fd, 14 + count * sizeof(struct status_change_data)), &data, sizeof(struct status_change_data));
		count++;
	}
...
static bool chrif_load_scdata(int fd)
{
	...
	for (i = 0; i < count; i++) {
		...
		status->change_start_sub(NULL, &sd->bl, (sc_type)data->type, 10000, data->val1, data->val2, data->val3, data->val4, data->tick, data->total_tick, SCFLAG_NOAVOID | SCFLAG_FIXEDTICK | SCFLAG_LOADED | SCFLAG_FIXEDRATE, 0);
	}
	...
	while (RFIFOREST(fd) >= 2) {
		...
		switch (cmd) {
			...
			case 0x2afd:
				...
				break;
			case 0x2b00:
				map->setusers(RFIFOL(fd, 2));
				chrif->keepalive(fd);
				break;
			case 0x2b03:
				clif->charselectok(RFIFOL(fd, 2), RFIFOB(fd, 6));
				break;
			case 0x2b09:
				map->addnickdb(RFIFOL(fd, 2), RFIFOP(fd, 6));
				break;
			case 0x2b0a:
			...
			case 0x2b0d:
				...
				break;
			case 0x2b0f:
				chrif->char_ask_name_answer(RFIFOL(fd, 2), RFIFOP(fd, 6), RFIFOW(fd, 30), RFIFOW(fd, 32));
				break;
			case 0x2b12:
				chrif->divorceack(RFIFOL(fd, 2), RFIFOL(fd, 6));
				break;
			case 0x2b14:
				chrif->idbanned(fd);
				break;
			case 0x2b1b:
				chrif->recvfamelist(fd);
				break;
			case 0x2b1d:
				chrif->load_scdata(fd);
				break;
			case 0x2b1e:
				chrif->update_ip(fd);
				break;
			case 0x2b1f:
				chrif->disconnectplayer(fd);
				break;
			case 0x2b21:
				chrif->save_ack(fd);
				break;
			case 0x2b22:
				chrif->updatefamelist_ack(fd);
				break;
			case 0x2b24:
				chrif->keepalive_ack(fd);
				break;
			case 0x2b25:
				chrif->deadopt(RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10));
				break;
			case 0x2b27:
				chrif->authfail(fd);
				break;
