...
struct homun_beasts_system *homun_bs;
struct homun_beasts_evo_system *homun_bs_ev;

struct DBMap* beast_db;
struct DBMap* beast_evo_db;

static unsigned int beast_counter = 0;
static unsigned int beast_evo_counter = 0;
...
{
	struct homun_beasts_system *homun_bs;
	struct DBIterator *iter = db_iterator(beast_db);
	for (homun_bs = dbi_first(iter); dbi_exists(iter); homun_bs = dbi_next(iter))
	{
		if ( (flag == HBS_ALL && (homun_bs->beast_id == id || homun_bs->hom_class == id || homun_bs->mob_class == id)) || (flag == HBS_ID && homun_bs->beast_id == id) || (flag == HBS_HOMCLASS && homun_bs->hom_class == id) || (flag == HBS_MOBCLASS && homun_bs->mob_class == id) )
			return homun_bs;
	}
	dbi_destroy(iter);
	return NULL;
...
struct homun_beasts_system* beast_search2(int hom_id, int mob_id)
{
	...
	for (homun_bs = dbi_first(iter); dbi_exists(iter); homun_bs = dbi_next(iter))
	{
		if( homun_bs->hom_class == hom_id && homun_bs->mob_class )
			return homun_bs;
	}
	dbi_destroy(iter);
	return NULL;
...
{
	struct homun_beasts_evo_system *homun_bs_evo;
	struct DBIterator *iter = db_iterator(beast_evo_db);
	for (homun_bs_evo = dbi_first(iter); dbi_exists(iter); homun_bs_evo = dbi_next(iter))
	{
		if( id == homun_bs_evo->hom_class && sex == homun_bs_evo->sex )
			return homun_bs_evo;
	}
	dbi_destroy(iter);
	return NULL;
...
int homunculus_dead(struct homun_data *hd) {
	...
	homun->save(hd);
	homunculus_beast_change_item(hd, BSTATUS_DIE);
	homunculus_force_save(hd,0);
	sd->status.hom_id = 0;
...
bool homunculus_levelup(struct homun_data *hd) {
	...
	homun_bs_evo = beast_evo_search(hd->homunculus.class_, hd->homunculus.sex);
	if( homun_bs_evo != NULL ) {
		if( hd->homunculus.level >= homun_bs_evo->level && hd->homunculus.intimacy >= homun_bs_evo->intimacy ) {
			homunculus_evolve2(hd, homun_bs_evo->hom_evo);
		}
	}
	return true;
}
...
int homunculus_create2(struct map_session_data *sd, int class_, struct mob_data *md) {
	...
	bsc = beast_search2(class_, md->class_);

	if (bsc == NULL)
		return false;

	SQL->EscapeStringLen(map->mysql_handle, esc_name, md->name, strnlen(md->name, NAME_LENGTH));

	skill_point = md->level / 3;

	sex = (rnd() % 100<50 ? 0 : 1);
	if (SQL_ERROR == SQL->Query(map->mysql_handle, "INSERT INTO `%s` "
		"(`char_id`, `class`,`prev_class`,`name`,`level`,`exp`,`intimacy`,`hunger`, `str`, `agi`, `vit`, `int`, `dex`, `luk`, `hp`,`max_hp`,`sp`,`max_sp`,`skill_point`, `rename_flag`, `vaporize`, `sex`, `mobclass`) "
		"VALUES ('%d', '%d', '%d', '%s', '%d', '%u', '%u', '%d', '%d', %d, '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d','%d','%d')",
		"homunculus", sd->status.char_id, class_, 0, esc_name, md->level, 0, 2100, 32, md->status.str, md->status.agi, md->status.vit, md->status.int_, md->status.dex, md->status.luk,
		md->status.hp, md->status.max_hp, md->status.sp, md->status.max_sp, skill_point, 0, 0, sex, md->class_)) {
		Sql_ShowDebug(map->mysql_handle);
		return false;
	}
	hom_id = (int)SQL->LastInsertId(map->mysql_handle);
	homunculus_beast_get_item(sd, bsc->item_add, bsc->item_race_icon, hom_id, sex, BSTATUS_ALIVE);
	return hom_id;
}

bool homunculus_recv_data2(int homun_id, struct map_session_data *sd, struct s_homunculus* hd) {
	char* data;
	size_t len;
	...
	if (SQL_ERROR == SQL->Query(map->mysql_handle, "SELECT `homun_id`,`char_id`,`class`,`prev_class`,`name`,`level`,`exp`,`intimacy`,`hunger`, `str`, `agi`, `vit`, `int`, `dex`, `luk`, `hp`,`max_hp`,`sp`,`max_sp`,`skill_point`,`rename_flag`, `vaporize`, `mobclass`, `sex` FROM `homunculus` WHERE `homun_id`='%d'", homun_id))
	{
		Sql_ShowDebug(map->mysql_handle);
		return false;
	}

	if (!SQL->NumRows(map->mysql_handle)) {
		//No homunculus found.
	...
	hd->hom_id = homun_id;
	sd->status.hom_id = homun_id;
	hd->char_id = sd->status.char_id;
	SQL->GetData(map->mysql_handle, 2, &data, NULL); hd->class_ = atoi(data);
	SQL->GetData(map->mysql_handle, 3, &data, NULL); hd->prev_class = atoi(data);
	SQL->GetData(map->mysql_handle, 4, &data, &len); safestrncpy(hd->name, data, sizeof(hd->name));
	SQL->GetData(map->mysql_handle, 5, &data, NULL); hd->level = atoi(data);
	SQL->GetData(map->mysql_handle, 6, &data, NULL); hd->exp = atoi(data);
	SQL->GetData(map->mysql_handle, 7, &data, NULL); hd->intimacy = (unsigned int)strtoul(data, NULL, 10);
	SQL->GetData(map->mysql_handle, 8, &data, NULL); hd->hunger = atoi(data);
	SQL->GetData(map->mysql_handle, 9, &data, NULL); hd->str = atoi(data);
	SQL->GetData(map->mysql_handle, 10, &data, NULL); hd->agi = atoi(data);
	SQL->GetData(map->mysql_handle, 11, &data, NULL); hd->vit = atoi(data);
	SQL->GetData(map->mysql_handle, 12, &data, NULL); hd->int_ = atoi(data);
	SQL->GetData(map->mysql_handle, 13, &data, NULL); hd->dex = atoi(data);
	SQL->GetData(map->mysql_handle, 14, &data, NULL); hd->luk = atoi(data);
	SQL->GetData(map->mysql_handle, 15, &data, NULL); hd->hp = atoi(data);
	SQL->GetData(map->mysql_handle, 16, &data, NULL); hd->max_hp = atoi(data);
	SQL->GetData(map->mysql_handle, 17, &data, NULL); hd->sp = atoi(data);
	SQL->GetData(map->mysql_handle, 18, &data, NULL); hd->max_sp = atoi(data);
	SQL->GetData(map->mysql_handle, 19, &data, NULL); hd->skillpts = atoi(data);
	SQL->GetData(map->mysql_handle, 20, &data, NULL); hd->rename_flag = atoi(data);
	SQL->GetData(map->mysql_handle, 21, &data, NULL); hd->vaporize = atoi(data);
	SQL->GetData(map->mysql_handle, 22, &data, NULL); hd->mob_class = atoi(data);
	SQL->GetData(map->mysql_handle, 23, &data, NULL); hd->sex = atoi(data);
	SQL->FreeResult(map->mysql_handle);
	...
	if (SQL_ERROR == SQL->Query(map->mysql_handle, "SELECT `id`,`lv` FROM `skill_homunculus` WHERE `homun_id`=%d", homun_id)) {
		Sql_ShowDebug(map->mysql_handle);
		return false;
	}
	while (SQL_SUCCESS == SQL->NextRow(map->mysql_handle)) {
		...
		// lv
		SQL->GetData(map->mysql_handle, 1, &data, NULL);
...
bool homunculus_create_end(struct map_session_data *sd, const struct s_homunculus *hom)
{
	struct homun_data *hd;
	int i = 0;
...
bool homunculus_call2(int homun_id, struct map_session_data *sd) {
	...
	int hp, sp;
	bool result = homunculus_recv_data2(homun_id, sd, &hd);

	if (!result)
		return false;

	homun->vaporize(sd, HOM_ST_REST);
	homunculus_create_end(sd, &hd);
	homun->ressurect(sd, 1, sd->bl.x, sd->bl.y);
	hp = APPLY_RATE(sd->hd->homunculus.call_hp, 100);
	sp = APPLY_RATE(sd->hd->homunculus.call_sp, 100);
...
int homunculus_get_info(int homun_id, int type) {
	...
	if (SQL_ERROR == SQL->Query(map->mysql_handle, "SELECT `class`, `level`, `str`, `agi`, `vit`, `int`, `dex`, `luk`, `hp`,`max_hp`,`sp`,`max_sp`, `alive`, `mobclass`, `sex` FROM `homunculus` WHERE `homun_id`='%d'", homun_id))
	{
		Sql_ShowDebug(map->mysql_handle);
		return false;
	}
	...
	SQL->GetData(map->mysql_handle, type, &data, NULL);
	result = atoi(data);
	SQL->FreeResult(map->mysql_handle);
	return result;
}
...
const char* homunculus_beast_name(int homun_id) {
	...
	if (SQL_ERROR == SQL->Query(map->mysql_handle, "SELECT `name` FROM `homunculus` WHERE `homun_id`='%d'", homun_id))
	{
		Sql_ShowDebug(map->mysql_handle);
		return false;
	}
	...
	SQL->GetData(map->mysql_handle, 0, data, &len); safestrncpy(name, data, sizeof(name));
	SQL->FreeResult(map->mysql_handle);
...
bool homunculus_force_save(struct homun_data *hd, int alive) {
	if (SQL_ERROR == SQL->Query(map->mysql_handle,
		"UPDATE `homunculus` SET `alive`='%d', `char_id`='%d', `class`='%d', `prev_class`='%d', `level`='%d', `exp`='%u', `intimacy`='%u', `hunger`='%d', `str`='%d', `agi`='%d', `vit`='%d', `int`='%d', `dex`='%d', `luk`='%d', `hp`='%d', `max_hp`='%d', `sp`='%d', `max_sp`='%d', `skill_point`='%d', `rename_flag`='%d', `vaporize`='%d', `sex`='%d', `mobclass`='%d' WHERE `homun_id`='%d' LIMIT 1",
		alive, hd->homunculus.char_id, hd->homunculus.class_, hd->homunculus.prev_class, hd->homunculus.level, hd->homunculus.exp, hd->homunculus.intimacy, hd->homunculus.hunger, hd->homunculus.str, hd->homunculus.agi, hd->homunculus.vit, hd->homunculus.int_, hd->homunculus.dex, hd->homunculus.luk, hd->homunculus.hp, hd->homunculus.max_hp, hd->homunculus.sp, hd->homunculus.max_sp, hd->homunculus.skillpts, hd->homunculus.rename_flag, hd->homunculus.vaporize, hd->homunculus.sex, hd->homunculus.mob_class, hd->homunculus.hom_id)) {
		Sql_ShowDebug(map->mysql_handle);
		SQL->FreeResult(map->mysql_handle);
		return false;
	}
...
int homunculus_beast_get_item(struct map_session_data *sd, int item_id, int item_race_icon, int hom_id, int sex, int flag_type) {
	...
	c1 = flag_type;
	c2 = item_race_icon;
	c3 = (sex ? BSTATUS_FEMALE : BSTATUS_MALE);
	c4 = 0;
	...
	if (nameid > 0) {
		...
		item_tmp.unique_id = BEAST_UNIQUE_ID + hom_id;
		...
		for (i = 0; i < amount; i += get_count) {
			if ((flag = pc->additem(sd, &item_tmp, get_count, LOG_TYPE_NONE))) {
				clif->additem(sd, i, 1, flag);
...
int homunculus_beast_change_item(struct homun_data *hd, int flag) {
	...
	nullpo_retr(3, hd);
	sd = hd->master;
	unique_id = BEAST_UNIQUE_ID + sd->status.hom_id;
	bsc = beast_search2(hd->homunculus.class_, hd->homunculus.mob_class);
	...
	{
		if (sd->status.inventory[i].unique_id == unique_id)
		{
			pc->delitem(sd, i, 1, 1, DELITEM_MATERIALCHANGE, LOG_TYPE_NONE);
			clif->delitem(sd, i, 1, DELITEM_MATERIALCHANGE);
			homunculus_beast_get_item(sd, bsc->item_add, bsc->item_race_icon, sd->status.hom_id, hd->homunculus.sex, flag);
		}
	}
...
int homunculus_beast_used_item(struct map_session_data *sd, int homun_id) {
	...
	if (!alive)
		clif->message(sd->fd, "Você não pode invocar esta besta porque está morto.");
	else if (sd->hd && sd->status.hom_id && sd->status.hom_id != homun_id)
		clif->message(sd->fd, "Você não pode invocar duas bestas ao mesmo tempo, vaporize-o antes tentar utilizar.");
	else if (sd->status.hom_id == homun_id) {
		clif->messagecolor_self(sd->fd, COLOR_BEASTMSG, "Seu homunculus está guardado em segurança!");
		homun->vaporize(sd, HOM_ST_REST);
	}
	else {
		clif->messagecolor_self(sd->fd, COLOR_BEASTMSG, "Besta invocada com sucesso!");
		homunculus_call2(homun_id, sd);
	}
	return true;
}

int homunculus_beast_revive(int homun_id) {
	if (SQL_ERROR == SQL->Query(map->mysql_handle, "UPDATE `homunculus` SET `alive`='1' WHERE `homun_id`='%d' AND `alive`='0' LIMIT 1", homun_id)) {
		Sql_ShowDebug(map->mysql_handle);
		SQL->FreeResult(map->mysql_handle);
		return 0;
	}
...
int homunculus_beast_heal(int homun_id, int percent_hp, int percent_sp) {
	int hp, sp, max_hp, max_sp;

	hp = homunculus_get_info(homun_id, 8);
	sp = homunculus_get_info(homun_id, 10);

	max_hp = homunculus_get_info(homun_id, 9);
	max_sp = homunculus_get_info(homun_id, 11);

	hp += APPLY_RATE(max_hp, percent_hp);
	sp += APPLY_RATE(max_sp, percent_sp);
	...
	if (SQL_ERROR == SQL->Query(map->mysql_handle, "UPDATE `homunculus` SET `hp`='%d', `sp`='%d' WHERE `homun_id`='%d' LIMIT 1", hp, sp, homun_id)) {
		Sql_ShowDebug(map->mysql_handle);
		SQL->FreeResult(map->mysql_handle);
		return 0;
	}
...
bool homunculus_evolve2(struct homun_data *hd, int class_) {
	...
	if (homun_bs_evo == NULL)
	{
		ShowError("homunculus_evolve: Can't evolve homunc from %d to %d, data not found in evolution db.\n", hd->homunculus.class_, class_);
		return false;
	}

	bsc = beast_search2(homun_bs_evo->hom_evo, hd->homunculus.sex);

	if (bsc == NULL)
	{
		ShowError("homunculus_evolve: Can't evolve homunc from %d to %d, data not found in beast db.\n", hd->homunculus.class_, homun_bs_evo->hom_evo);
		return false;
	}

	if (!homun->change_class(hd, class_)) {
		ShowError("homunculus_evolve: Can't evolve homunc from %d to %d\n", hd->homunculus.class_, hd->homunculusDB->evo_class);
		return false;
	}

	unique_id = hd->homunculus.hom_id - BEAST_UNIQUE_ID;

	for (i = 0; i < MAX_INVENTORY; i++) {
		if (sd->status.inventory[i].unique_id == unique_id) {
			pc->delitem(sd, i, 1, 1, DELITEM_MATERIALCHANGE, LOG_TYPE_NONE);
			clif->delitem(sd, i, 1, DELITEM_MATERIALCHANGE);
			homunculus_beast_get_item(sd, bsc->item_add, bsc->item_race_icon, hd->homunculus.hom_id, hd->homunculus.sex, BSTATUS_ALIVE);
		}
	}
	...
	hd->homunculus.mob_class = bsc->mob_class;
	status->set_viewdata(&hd->bl, bsc->mob_class);
...
{
	char* data;
	char name[NAME_LENGTH], classname[NAME_LENGTH];
	char output[256];
	const char *race, *element;
	size_t len;
	int i, class_, level, str, agi, vit, int_, dex, luk, hp, max_hp, sp, max_sp, mob_class, sex, skill_id, skill_lv, alive;
	struct mob_db *monster;

	if (SQL_ERROR == SQL->Query(map->mysql_handle, "SELECT `name`, `class`, `level`, `str`, `agi`, `vit`, `int`, `dex`, `luk`, `hp`,`max_hp`,`sp`,`max_sp`, `mobclass`, `sex`, `alive` FROM `homunculus` WHERE `homun_id`='%d'", homun_id))
	{
		Sql_ShowDebug(map->mysql_handle);
		SQL->FreeResult(map->mysql_handle);
		return;
	...
	if (!SQL->NumRows(map->mysql_handle)) {
		//No homunculus found.
		SQL->FreeResult(map->mysql_handle);
		sprintf(output, "Besta (id: %d) não encontrada.", homun_id);
	...
	SQL->GetData(map->mysql_handle, 0, &data, &len); safestrncpy(name, data, sizeof(name));
	SQL->GetData(map->mysql_handle, 1, &data, NULL); class_ = atoi(data);
	SQL->GetData(map->mysql_handle, 2, &data, NULL); level = atoi(data);
	SQL->GetData(map->mysql_handle, 3, &data, NULL); str = atoi(data);
	SQL->GetData(map->mysql_handle, 4, &data, NULL); agi = atoi(data);
	SQL->GetData(map->mysql_handle, 5, &data, NULL); vit = atoi(data);
	SQL->GetData(map->mysql_handle, 6, &data, NULL); int_ = atoi(data);
	SQL->GetData(map->mysql_handle, 7, &data, NULL); dex = atoi(data);
	SQL->GetData(map->mysql_handle, 8, &data, NULL); luk = atoi(data);
	SQL->GetData(map->mysql_handle, 9, &data, NULL); hp = atoi(data);
	SQL->GetData(map->mysql_handle, 10, &data, NULL); max_hp = atoi(data);
	SQL->GetData(map->mysql_handle, 11, &data, NULL); sp = atoi(data);
	SQL->GetData(map->mysql_handle, 12, &data, NULL); max_sp = atoi(data);
	SQL->GetData(map->mysql_handle, 13, &data, NULL); mob_class = atoi(data);
	SQL->GetData(map->mysql_handle, 14, &data, NULL); sex = atoi(data);
	SQL->GetData(map->mysql_handle, 15, &data, NULL); alive = atoi(data);
	SQL->FreeResult(map->mysql_handle);
	...
	if (mob_class && !mob->db_checkid(mob_class))
		safestrncpy(classname, homun->dbs->db[i].name, sizeof(classname));
	else {
		monster = mob->db(mob_class);
		safestrncpy(classname, monster->jname, sizeof(classname));
	}

	switch (homun->dbs->db[i].race)
	{
	case 0: race = "Amorfo"; break;
	case 1: race = "Morto-Vivo"; break;
	case 2: race = "Besta"; break;
	case 3: race = "Planta"; break;
	case 4: race = "Inseto"; break;
	case 5: race = "Peixe"; break;
	case 6: race = "Demônio"; break;
	case 7: race = "Semi-Humano"; break;
	case 8: race = "Anjo"; break;
	case 9: race = "Dragão"; break;
	default: race = "Desconhecida"; break;
	}

	switch (homun->dbs->db[i].element)
	{
	case 0: element = "Neutro"; break;
	case 1: element = "Água"; break;
	case 2: element = "Terra"; break;
	case 3: element = "Fogo"; break;
	case 4: element = "Vento"; break;
	case 5: element = "Veneno"; break;
	case 6: element = "Sagrado"; break;
	case 7: element = "Sombrio"; break;
	case 8: element = "Fantasma"; break;
	case 9: element = "Morto-Vivo"; break;
	default: element = "Desconhecida"; break;
	}

	sprintf(output, "------------------------------------------------------------------------------------");
	clif->messagecolor_self(sd->fd, COLOR_BEASTMSG, output);
	sprintf(output, "[Informações] ID: %d | Nome: %s | Sexo: %s", homun_id, name, (sex ? "F" : "M"));
	clif->messagecolor_self(sd->fd, COLOR_BEASTMSG, output);
	sprintf(output, "[Estado] Nível: %d | HP: %d/%d | SP: %d/%d | Situação: %s", level, hp, max_hp, sp, max_sp, (alive ? "Vivo" : "Morto"));
	clif->messagecolor_self(sd->fd, COLOR_BEASTMSG, output);
	sprintf(output, "[Classes] Classe: %s | Raça: %s | Elemento: %s", classname, race, element);
	clif->messagecolor_self(sd->fd, COLOR_BEASTMSG, output);
	sprintf(output, "[Atributos] For: %d | Agi: %d | Vit: %d | Int: %d | Des: %d | Sor: %d", str, agi, vit, int_, dex, luk);
	clif->messagecolor_self(sd->fd, COLOR_BEASTMSG, output);
	...
	sprintf(output, "[Habilidades] Lista:");
	clif->messagecolor_self(sd->fd, COLOR_BEASTMSG, output);
	while (SQL_SUCCESS == SQL->NextRow(map->mysql_handle)) {
		// id
		SQL->GetData(map->mysql_handle, 0, &data, NULL);
		...
		SQL->GetData(map->mysql_handle, 1, &data, NULL);
		skill_lv = atoi(data);
		sprintf(output, "   - %s | Nível %d", skill->get_desc(skill_id), skill_lv);
		clif->messagecolor_self(sd->fd, COLOR_BEASTMSG, output);
	}
	SQL->FreeResult(map->mysql_handle);
}
...
void beast_inventory_atcmd_list(struct map_session_data *sd) {
	...
	const char *name;
	int i, c = 0, h, homun_id, level, class_, sex, alive, hp, max_hp, sp, max_sp, mob_class;
	struct mob_db *monster;

	clif->message(sd->fd, "[Beast_ID/Unique_ID] Nome, Nível, HP Atual/Máx. de HP, SP Atual/Máx. de SP, Classe, Sexo, Situação");
	for (i = 0; i < MAX_INVENTORY; i++) {
		if (sd->status.inventory[i].unique_id > BEAST_UNIQUE_ID) {
			homun_id = (int)sd->status.inventory[i].unique_id - BEAST_UNIQUE_ID;
			level = homunculus_get_info(homun_id, 1);
			alive = homunculus_get_info(homun_id, 12);
			sex = homunculus_get_info(homun_id, 14);
			class_ = homunculus_get_info(homun_id, 0);
			hp = homunculus_get_info(homun_id, 8);
			...
			max_sp = homunculus_get_info(homun_id, 11);
			mob_class = homunculus_get_info(homun_id, 13);
			name = homunculus_beast_name(homun_id);

			h = homun->db_search(class_, HOMUNCULUS_CLASS);

			if (h == INDEX_NOT_FOUND)
				continue;

			if (mob_class && !mob->db_checkid(mob_class))
				safestrncpy(classname, homun->dbs->db[h].name, sizeof(classname));
			else {
				monster = mob->db(mob_class);
				safestrncpy(classname, monster->jname, sizeof(classname));
			}

			sprintf(output, "[%d/%d] %s, %d, %d/%d, %d/%d %s, %s, %s", homun_id, (int)sd->status.inventory[i].unique_id, name, level, hp, max_hp, sp, max_sp, classname, (sex?"F":"M"), (alive?"Vivo":"Morto"));
			clif->message(sd->fd, output);
			c++;
		}
	}
	...
	if (!c)
		clif->message(sd->fd, "Nenhuma Besta encontrada em seu inventário.");
}
...
bool homunculus_read_beast_sub(char* str[], int columns, int current) {
	int beastid,classid,mobid,chance,itemidadd,itemidrace;
	struct homun_beasts_system *homun_bs, *homun_bsc;
	...
	classid = atoi(str[1]);
	mobid = atoi(str[2]);
	itemidadd = atoi(str[3]);
	itemidrace = atoi(str[4]);
	chance = atoi(str[5]);

	if (classid < HM_CLASS_BASE || classid > HM_CLASS_MAX) {
		ShowError("homunculus_read_beast_sub : Invalid class %d\n", classid);
		return false;
	}

	homun_bsc = beast_search2(classid, mobid);

	if (homun_bsc != NULL)
		ShowWarning("read_beast_db: Ignorando linha %d por conter o mesmo ID de Homunculus (%d) e Monstro (%d).\n", current, classid, mobid);
	else if ((homun_bsc = beast_search(beastid,HBS_ID)) != NULL)
		ShowWarning("read_beast_db: Ignorando linha %d por duplicar o ID (%d).\n", current, beastid);
	//else if ((homun_bsc = beast_search(itemid,HBS_CATCHITEM)) != NULL )
	//	ShowWarning("read_beast_db: Ignorando linha %d por duplicar o ID do Item de Caputura (%d).\n", current, itemid);
	else {
		beast_counter++;
		CREATE(homun_bs, struct homun_beasts_system, 1);
		homun_bs->beast_id = beastid;
		homun_bs->hom_class = classid;
		homun_bs->mob_class = mobid;
		homun_bs->item_add = itemidadd;
		homun_bs->item_race_icon = itemidrace;
		homun_bs->chance = chance;
		idb_put(beast_db, beast_counter, homun_bs);
	}
	return true;
}

bool homunculus_read_beast_evo_sub(char* str[], int columns, int current) {
	int hom_class, hom_evo, hom_level, hom_mob_class, hom_sex, hom_intimacy;
	struct homun_beasts_evo_system *homun_bs_evo, *homun_bsv;

	nullpo_retr(false, str);

	hom_class = atoi(str[0]);
	hom_evo = atoi(str[1]);
	hom_mob_class = atoi(str[2]);
	hom_level = atoi(str[3]);
	hom_sex = atoi(str[4]);
	hom_intimacy = atoi(str[5]);

	if (hom_class < HM_CLASS_BASE || hom_class > HM_CLASS_MAX) {
		ShowError("homunculus_read_beast_evo_sub : Invalid class %d\n", hom_class);
		return false;
	}

	if (hom_evo < HM_CLASS_BASE || hom_evo > HM_CLASS_MAX) {
		ShowError("homunculus_read_beast_evo_sub : Invalid evolution class %d\n", hom_evo);
		return false;
	}
	
	if (hom_sex < 0 || hom_sex > 1) {
		ShowError("homunculus_read_beast_evo_sub : Invalid gender %d\n", hom_sex);
		return false;
	}

	homun_bsv = beast_evo_search(hom_class,hom_sex);

	if (homun_bsv != NULL)
		ShowWarning("read_beast_evo_db: Ignorando linha %d por conter o mesmo ID de Homunculus (%d) com o mesmo Sexo.\n", current, hom_class);
	else {
		beast_evo_counter++;
		CREATE(homun_bs_evo, struct homun_beasts_evo_system, 1);
		homun_bs_evo->hom_class = hom_class;
		homun_bs_evo->hom_evo = hom_evo;
		homun_bs_evo->mob_class = hom_mob_class;
		homun_bs_evo->level = hom_level;
		homun_bs_evo->sex = hom_sex;
		homun_bs_evo->intimacy = hom_intimacy;
		idb_put(beast_evo_db, beast_evo_counter, homun_bs_evo);
	}
	return true;
}

void homunculus_read_beasts(void) {
	const char *filename = "homunculus_beasts_db.txt";
	const char *filename2 = "homunculus_beasts_evolutions.txt";
	char filepath[256];

	beast_db = idb_alloc(DB_OPT_RELEASE_DATA);
	beast_evo_db = idb_alloc(DB_OPT_RELEASE_DATA);
	...
	if (!exists(filepath))
		ShowError("read_bests: Falha ao abrir o arquivo '%s/%s' do sistema de bestas!\n", map->db_path, filename);
	else
		sv->readdb(map->db_path, filename, ',', 6, 6, MAX_BEASTS_CLASS, homunculus_read_beast_sub);

	sprintf(filepath, "%s/%s", map->db_path, filename2);

	if (!exists(filepath))
		ShowError("read_bests_evolutions: Falha ao abrir o arquivo '%s/%s' do sistema de bestas!\n", map->db_path, filename2);
	else
		sv->readdb(map->db_path, filename2, ',', 6, 6, MAX_BEASTS_EVO, homunculus_read_beast_evo_sub);
}
