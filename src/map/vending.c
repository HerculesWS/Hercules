...
static void vending_openvending(struct map_session_data *sd, const char *message, const uint8 *data, int count)
{
	...
	if (pc_isdead(sd) || sd->state.prevend == 0 || pc_istrading_except_npc(sd) || (sd->npc_id != 0 && sd->state.using_megaphone == 0)) {
...
static bool vending_searchall(struct map_session_data *sd, const struct s_search_store_search *s)
{
	...
	for (idx = 0; idx < s->item_count; idx++) {
		...
		if (!searchstore->result(s->search_sd, sd->vender_id, sd->status.account_id, sd->message, it->nameid, sd->vending[i].amount, sd->vending[i].value, it->card, it->refine, it->grade, it->option)) { // result set full
			return false;
		}
	}
