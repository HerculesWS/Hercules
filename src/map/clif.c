...
void clif_parse_cz_req_style_change(int fd, struct map_session_data *sd)
{
	...
	if (p->HeadStyle != 0) {
		entry = &VECTOR_INDEX(stylist_data[LOOK_HAIR], p->HeadStyle - 1);
		if (clif->style_change_validate_requirements(sd, LOOK_HAIR, p->HeadStyle - 1))
			pc->changelook(sd, LOOK_HAIR, entry->id);
	}
	if (p->HeadPalette != 0) {
		entry = &VECTOR_INDEX(stylist_data[LOOK_HAIR_COLOR], p->HeadPalette - 1);
		if (clif->style_change_validate_requirements(sd, LOOK_HAIR_COLOR, p->HeadPalette - 1))
			pc->changelook(sd, LOOK_HAIR_COLOR, entry->id);
	}
	if (p->BodyPalette != 0) {
		entry = &VECTOR_INDEX(stylist_data[LOOK_CLOTHES_COLOR], p->BodyPalette - 1);
		if (clif->style_change_validate_requirements(sd, LOOK_CLOTHES_COLOR, p->BodyPalette - 1))
			pc->changelook(sd, LOOK_CLOTHES_COLOR, entry->id);
	}
	if (p->TopAccessory != 0) {
