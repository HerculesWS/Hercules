...
static void clif_hom_food(struct map_session_data *sd, int foodid, int fail)
{
	...
	struct PACKET_ZC_FEED_MER *p = WP2PTR(struct PACKET_ZC_FEED_MER *, fd);
	p->packetType = 0x22f;
	p->result = fail;
	p->itemId = foodid;
...
static void clif_buylist(struct map_session_data *sd, struct npc_data *nd)
{
	...
	WFIFOHEAD(fd, len);
	struct PACKET_ZC_PC_PURCHASE_ITEMLIST *p = WP2PTR(struct PACKET_ZC_PC_PURCHASE_ITEMLIST *, fd);
...
static void clif_scriptmes(struct map_session_data *sd, int npcid, const char *mes)
{
	...
	WFIFOHEAD(fd, len);
	struct PACKET_ZC_SAY_DIALOG *p = WP2PTR(struct PACKET_ZC_SAY_DIALOG *, fd);
...
static void clif_scriptmes2(struct map_session_data *sd, int npcid, const char *mes, int type)
{
	...
	WFIFOHEAD(fd, len);
	struct PACKET_ZC_SAY_DIALOG2 *p = WP2PTR(struct PACKET_ZC_SAY_DIALOG2 *, fd);
...
static void clif_zc_quest_dialog(struct map_session_data *sd, int npcid, const char *mes)
{
	...
	int fd = sd->fd;
	WFIFOHEAD(fd, len);
	struct PACKET_ZC_QUEST_DIALOG *p = WP2PTR(struct PACKET_ZC_QUEST_DIALOG *, fd);
...
static void clif_zc_monolog_dialog(struct map_session_data *sd, int npcid, const char *mes)
{
	...
	int fd = sd->fd;
	WFIFOHEAD(fd, len);
	struct PACKET_ZC_MONOLOG_DIALOG *p = WP2PTR(struct PACKET_ZC_MONOLOG_DIALOG *, fd);
...
static void clif_scriptnext(struct map_session_data *sd, int npcid)
{
	...
	struct PACKET_ZC_WAIT_DIALOG *p = WP2PTR(struct PACKET_ZC_WAIT_DIALOG *, fd);
...
static void clif_scriptnext2(struct map_session_data *sd, int npcid, int type)
{
	...
	struct PACKET_ZC_WAIT_DIALOG2 *p = WP2PTR(struct PACKET_ZC_WAIT_DIALOG2 *, fd);
...
static void clif_zc_quest_dialog_menu_list(struct map_session_data *sd, int npcid, const char *mes)
{
	...
	int fd = sd->fd;
	WFIFOHEAD(fd, len);
	struct PACKET_ZC_QUEST_DIALOG_MENU_LIST *p = WP2PTR(struct PACKET_ZC_QUEST_DIALOG_MENU_LIST *, fd);
...
static void clif_cutin(struct map_session_data *sd, const char *image, int type)
{
	...
	WFIFOHEAD(fd, len);
	struct PACKET_ZC_SHOW_IMAGE *p = WP2PTR(struct PACKET_ZC_SHOW_IMAGE *, fd);
...
static void clif_item_movefailed(struct map_session_data *sd, int n, int amount)
{
	...
	WFIFOHEAD(fd, len);
	struct PACKET_ZC_MOVE_ITEM_FAILED *p = WP2PTR(struct PACKET_ZC_MOVE_ITEM_FAILED *, fd);
...
static void clif_inventoryExpansionInfo(struct map_session_data *sd)
{
	...
	struct PACKET_ZC_EXTEND_BODYITEM_SIZE *p = WP2PTR(struct PACKET_ZC_EXTEND_BODYITEM_SIZE *, fd);
...
static void clif_inventoryExpandAck(struct map_session_data *sd, enum expand_inventory result, int itemId)
{
	...
	struct PACKET_ZC_ACK_OPEN_MSGBOX_EXTEND_BODYITEM_SIZE *p = WP2PTR(struct PACKET_ZC_ACK_OPEN_MSGBOX_EXTEND_BODYITEM_SIZE *, fd);
...
static void clif_inventoryExpandResult(struct map_session_data *sd, enum expand_inventory_result result)
{
	...
	struct PACKET_ZC_ACK_EXTEND_BODYITEM_SIZE *p = WP2PTR(struct PACKET_ZC_ACK_EXTEND_BODYITEM_SIZE *, fd);
...
static void clif_arrow_create_list(struct map_session_data *sd)
{
	...
	WFIFOHEAD(fd, len);
	struct PACKET_ZC_MAKINGARROW_LIST *p = WP2PTR(struct PACKET_ZC_MAKINGARROW_LIST *, fd);
...
static void clif_statusupack(struct map_session_data *sd, int type, int ok, int val)
{
	...
	struct PACKET_ZC_STATUS_CHANGE_ACK *p = WP2PTR(struct PACKET_ZC_STATUS_CHANGE_ACK *, fd);
...
static void clif_useitemack(struct map_session_data *sd, int index, int amount, bool ok)
{
	...
	if (!ok) {
		...
		memcpy(WP2PTR(struct PACKET_ZC_USE_ITEM_ACK *, fd), &p, sizeof(p));
...
static void clif_tradeadditem(struct map_session_data *sd, struct map_session_data *tsd, int index, int amount)
{
	...
	struct PACKET_ZC_ADD_EXCHANGE_ITEM *p = WP2PTR(struct PACKET_ZC_ADD_EXCHANGE_ITEM *, fd);
...
static void clif_storageitemadded(struct map_session_data *sd, struct item *i, int index, int amount)
{
	...
	struct PACKET_ZC_ADD_ITEM_TO_STORE *p = WP2PTR(struct PACKET_ZC_ADD_ITEM_TO_STORE *, fd);
...
static void clif_getareachar_item(struct map_session_data *sd, struct flooritem_data *fitem)
{
	...
	struct PACKET_ZC_ITEM_ENTRY *p = WP2PTR(struct PACKET_ZC_ITEM_ENTRY *, fd);
	p->packetType = 0x9d;
	p->AID = fitem->bl.id;
...
static void clif_skillinfoblock(struct map_session_data *sd)
{
	...
	struct PACKET_ZC_SKILLINFO_LIST *p = WP2PTR(struct PACKET_ZC_SKILLINFO_LIST *, fd);
...
static void clif_addskill(struct map_session_data *sd, int id)
{
	...
	struct PACKET_ZC_ADD_SKILL *p = WP2PTR(struct PACKET_ZC_ADD_SKILL *, fd);
...
static void clif_skillinfo(struct map_session_data *sd, int skill_id, int inf)
{
	...
	struct PACKET_ZC_SKILLINFO_UPDATE2 *p = WP2PTR(struct PACKET_ZC_SKILLINFO_UPDATE2 *, fd);
...
static void clif_skill_fail(struct map_session_data *sd, uint16 skill_id, enum useskill_fail_cause cause, int btype, int32 item_id)
{
	...
	struct PACKET_ZC_ACK_TOUSESKILL *p = WP2PTR(struct PACKET_ZC_ACK_TOUSESKILL *, fd);
...
static void clif_skill_cooldown(struct map_session_data *sd, uint16 skill_id, unsigned int duration)
{
	...
	struct PACKET_ZC_SKILL_POSTDELAY *p = WP2PTR(struct PACKET_ZC_SKILL_POSTDELAY *, fd);
...
static void clif_skill_warppoint(struct map_session_data *sd, uint16 skill_id, uint16 skill_lv, unsigned short map1, unsigned short map2, unsigned short map3, unsigned short map4)
{
	...
	WFIFOHEAD(fd, len);
	struct PACKET_ZC_WARPLIST *p = WP2PTR(struct PACKET_ZC_WARPLIST *, fd);
	memset(p, 0, len);
...
static void clif_skill_produce_mix_list(struct map_session_data *sd, int skill_id, int trigger)
{
	...
	WFIFOHEAD(fd, len);
	struct PACKET_ZC_MAKABLEITEMLIST *p = WP2PTR(struct PACKET_ZC_MAKABLEITEMLIST *, fd);
	p->packetType = 0x18d;
...
static void clif_cooking_list(struct map_session_data *sd, int trigger, uint16 skill_id, int qty, int list_type)
{
	...
	WFIFOHEAD(fd, len);
	struct PACKET_ZC_MAKINGITEM_LIST *p = WP2PTR(struct PACKET_ZC_MAKINGITEM_LIST *, fd);
...
static void clif_upgrademessage(int fd, int result, int item_id)
{
	...
	struct PACKET_ZC_ACK_WEAPONREFINE *p = WP2PTR(struct PACKET_ZC_ACK_WEAPONREFINE *, fd);
	p->packetType = 0x223;
	p->result = result;
	p->itemId = item_id;
...
static void clif_wis_message(int fd, const char *nick, const char *mes, int mes_len)
{
	...
	WFIFOHEAD(fd, len);
	struct PACKET_ZC_WHISPER *p = WP2PTR(struct PACKET_ZC_WHISPER *, fd);
...
static void clif_item_repair_list(struct map_session_data *sd, struct map_session_data *dstsd, int lv)
{
	...
	WFIFOHEAD(fd, len);
	struct PACKET_ZC_REPAIRITEMLIST *p = WP2PTR(struct PACKET_ZC_REPAIRITEMLIST *, fd);
...
static void clif_item_refine_list(struct map_session_data *sd)
{
	...
	WFIFOHEAD(fd, len);
	struct PACKET_ZC_NOTIFY_WEAPONITEMLIST *p = WP2PTR(struct PACKET_ZC_NOTIFY_WEAPONITEMLIST *, fd);
	p->packetType = 0x221;
	for (i = c = 0; i < sd->status.inventorySize; i++) {
		if (sd->status.inventory[i].nameid > 0 && sd->status.inventory[i].identify
...
static void clif_item_skill(struct map_session_data *sd, uint16 skill_id, uint16 skill_lv)
{
	...
	struct PACKET_ZC_AUTORUN_SKILL *p = WP2PTR(struct PACKET_ZC_AUTORUN_SKILL *, fd);
...
static void clif_cart_additem(struct map_session_data *sd, int n, int amount, int fail)
{
	...
	struct PACKET_ZC_ADD_ITEM_TO_CART *p = WP2PTR(struct PACKET_ZC_ADD_ITEM_TO_CART *, fd);
...
static void clif_showvendingboard(struct block_list *bl, const char *message, int fd)
{
	...
	if (fd != 0) {
		...
		memcpy(WP2PTR(struct PACKET_ZC_STORE_ENTRY *, fd), &p, sizeof(struct PACKET_ZC_STORE_ENTRY));
...
static void clif_vendinglist(struct map_session_data *sd, unsigned int id, struct s_vending *vending_items)
{
	...
	WFIFOHEAD(fd, len);
	struct PACKET_ZC_PC_PURCHASE_ITEMLIST_FROMMC *p = WP2PTR(struct PACKET_ZC_PC_PURCHASE_ITEMLIST_FROMMC *, fd);
...
static void clif_openvending(struct map_session_data *sd, int id, struct s_vending *vending_items)
{
	...
	WFIFOHEAD(fd, len);
	struct PACKET_ZC_PC_PURCHASE_MYITEMLIST *p = WP2PTR(struct PACKET_ZC_PC_PURCHASE_MYITEMLIST *, fd);
...
static void clif_party_created(struct map_session_data *sd, int result)
{
	...
	struct PACKET_ZC_ACK_MAKE_GROUP *p = WP2PTR(struct PACKET_ZC_ACK_MAKE_GROUP *, fd);
...
static void clif_partyinvitationstate(struct map_session_data *sd)
{
	...
	struct PACKET_ZC_PARTY_CONFIG *p = WP2PTR(struct PACKET_ZC_PARTY_CONFIG *, fd);
...
static void clif_party_invite(struct map_session_data *sd, struct map_session_data *tsd)
{
	...
	struct PACKET_ZC_PARTY_JOIN_REQ *packet = WP2PTR(struct PACKET_ZC_PARTY_JOIN_REQ *, fd);
...
static void clif_party_inviteack(struct map_session_data *sd, const char *nick, int result)
{
	...
	struct PACKET_ZC_PARTY_JOIN_REQ_ACK *p = WP2PTR(struct PACKET_ZC_PARTY_JOIN_REQ_ACK *, fd);
...
static void clif_party_xy_single(int fd, struct map_session_data *sd)
{
	...
	struct PACKET_ZC_NOTIFY_POSITION_TO_GROUPM *p = WP2PTR(struct PACKET_ZC_NOTIFY_POSITION_TO_GROUPM *, fd);
...
static void clif_hpmeter_single(int fd, int id, unsigned int hp, unsigned int maxhp, unsigned int sp, unsigned int maxsp)
{
	...
	struct PACKET_ZC_NOTIFY_HP_TO_GROUPM *p = WP2PTR(struct PACKET_ZC_NOTIFY_HP_TO_GROUPM *, fd);
	p->PacketType = HEADER_ZC_NOTIFY_HP_TO_GROUPM;
	p->AID = id;
#if PACKETVER < 20100119
...
static void clif_movetoattack(struct map_session_data *sd, struct block_list *bl)
{
	...
	struct PACKET_ZC_ATTACK_FAILURE_FOR_DISTANCE *p = WP2PTR(struct PACKET_ZC_ATTACK_FAILURE_FOR_DISTANCE *, fd);
...
static void clif_produceeffect(struct map_session_data *sd, int flag, int nameid)
{
	...
	struct PACKET_ZC_ACK_REQMAKINGITEM *p = WP2PTR(struct PACKET_ZC_ACK_REQMAKINGITEM *, fd);
	p->packetType = 0x18f;
	p->result = flag;
...
static void clif_catch_process(struct map_session_data *sd)
{
	...
	struct PACKET_ZC_START_CAPTURE *p = WP2PTR(struct PACKET_ZC_START_CAPTURE *, fd);
 ...
 static void clif_pet_roulette(struct map_session_data *sd, int data)
{
	...
	struct PACKET_ZC_TRYCAPTURE_MONSTER *p = WP2PTR(struct PACKET_ZC_TRYCAPTURE_MONSTER *, fd);
...
static void clif_send_petstatus(struct map_session_data *sd)
{
	...
	struct PACKET_ZC_PROPERTY_PET *packet = WP2PTR(struct PACKET_ZC_PROPERTY_PET *, fd);
...
static void clif_pet_food(struct map_session_data *sd, int foodid, int fail)
{
	...
	struct PACKET_ZC_FEED_PET *p = WP2PTR(struct PACKET_ZC_FEED_PET *, fd);
	p->packetType = 0x1a3;
	p->result = fail;
	p->itemId = foodid;
...
static void clif_autospell(struct map_session_data *sd, uint16 skill_lv, int *skill_ids_list, int list_len)
{
	...
	struct PACKET_ZC_AUTOSPELLLIST *p = WP2PTR(struct PACKET_ZC_AUTOSPELLLIST *, fd);
...
static void clif_mvp_item(struct map_session_data *sd, int nameid)
{
	...
	struct PACKET_ZC_MVP_GETTING_ITEM *p = WP2PTR(struct PACKET_ZC_MVP_GETTING_ITEM *, fd);
	p->packetType = 0x10a;
	p->itemId = (view > 0) ? view : nameid;
...
static void clif_guild_belonginfo(struct map_session_data *sd, struct guild *g)
{
	...
	struct PACKET_ZC_UPDATE_GDID *p = WP2PTR(struct PACKET_ZC_UPDATE_GDID *, fd);
...
static void clif_guild_basicinfo(struct map_session_data *sd)
{
	...
	struct PACKET_ZC_GUILD_INFO *p = WP2PTR(struct PACKET_ZC_GUILD_INFO *, fd);
...
static void clif_guild_allianceinfo(struct map_session_data *sd)
{
	...
	struct PACKET_ZC_MYGUILD_BASIC_INFO *p = WP2PTR(struct PACKET_ZC_MYGUILD_BASIC_INFO *, fd);
...
static void clif_guild_memberlist(struct map_session_data *sd)
{
	...
	struct PACKET_ZC_MEMBERMGR_INFO *p = WP2PTR(struct PACKET_ZC_MEMBERMGR_INFO *, fd);
...
static void clif_guild_positionnamelist(struct map_session_data *sd)
{
	...
	struct PACKET_ZC_POSITION_ID_NAME_INFO *p = WP2PTR(struct PACKET_ZC_POSITION_ID_NAME_INFO *, fd);
...
static void clif_guild_positioninfolist(struct map_session_data *sd)
{
	...
	struct PACKET_ZC_POSITION_INFO *p = WP2PTR(struct PACKET_ZC_POSITION_INFO *, fd);
...
static void clif_guild_emblem_clear(struct map_session_data *sd, struct guild *g)
{
	...
	WFIFOHEAD(fd, len);
	struct PACKET_ZC_GUILD_EMBLEM_IMG *p = WP2PTR(struct PACKET_ZC_GUILD_EMBLEM_IMG *, fd);
...
static void clif_guild_skillinfo(struct map_session_data *sd)
{
	...
	struct PACKET_ZC_GUILD_SKILLINFO *p = WP2PTR(struct PACKET_ZC_GUILD_SKILLINFO *, fd);
...
static void clif_guild_expulsionlist(struct map_session_data *sd)
{
	...
	struct PACKET_ZC_BAN_LIST *packet = WP2PTR(struct PACKET_ZC_BAN_LIST *, fd);
...
#endif  // PACKETVER_MAIN_NUM >= 20220504
	WFIFOHEAD(fd, sz);
	struct PACKET_ZC_PLAY_NPC_BGM *p = WP2PTR(struct PACKET_ZC_PLAY_NPC_BGM *, fd);
...
static void clif_soundeffect(struct map_session_data *sd, struct block_list *bl, const char *name, enum play_sound_act type, int term)
{
	...
	struct PACKET_ZC_SOUND *p = WP2PTR(struct PACKET_ZC_SOUND *, fd);
...
static void clif_specialeffect_value_single(struct block_list *bl, int effect_id, uint64 num, int fd)
{
	...
	struct PACKET_ZC_NOTIFY_EFFECT3 *packet = WP2PTR(struct PACKET_ZC_NOTIFY_EFFECT3 *, fd);
	packet->packetType = HEADER_ZC_NOTIFY_EFFECT3;
	packet->aid = bl->id;
	packet->effectId = effect_id;
#if PACKETVER >= 20191127
...
static void clif_serviceMessageColor(struct map_session_data *sd, uint32 color, const char *msg)
{
	...
	const int fd = sd->fd;
	WFIFOHEAD(fd, len);
	struct PACKET_ZC_DEBUGMSG *p = WP2PTR(struct PACKET_ZC_DEBUGMSG *, fd);
...
static void clif_auth_error(int fd, int errorCode)
{
	...
	struct packet_ZC_REFUSE_LOGIN *p = WP2PTR(struct packet_ZC_REFUSE_LOGIN *, fd);
...
static void clif_parse_HotkeyRowShift1(int fd, struct map_session_data *sd)
{
	...
	const struct PACKET_CZ_SHORTCUTKEYBAR_ROTATE1 *p = RFIFOP(struct PACKET_CZ_SHORTCUTKEYBAR_ROTATE1 *, fd, 0);
...
{
#if PACKETVER_MAIN_NUM >= 20190522 || PACKETVER_RE_NUM >= 20190508 || PACKETVER_ZERO_NUM >= 20190605
	const struct PACKET_CZ_SHORTCUTKEYBAR_ROTATE2 *p = RFIFOP(struct PACKET_CZ_SHORTCUTKEYBAR_ROTATE2 *,fd, 0);
	if (p->tab == 0)
		sd->status.hotkey_rowshift = p->rowshift;
	else
...
static void clif_parse_Hotkey1(int fd, struct map_session_data *sd)
{
	...
	const struct PACKET_CZ_SHORTCUT_KEY_CHANGE1 *p = RFIFOP(struct PACKET_CZ_SHORTCUT_KEY_CHANGE1 *, fd, 0);
...
#ifdef HOTKEY_SAVING
#if PACKETVER_MAIN_NUM >= 20190522 || PACKETVER_RE_NUM >= 20190508 || PACKETVER_ZERO_NUM >= 20190605
	const struct PACKET_CZ_SHORTCUT_KEY_CHANGE2 *p = RFIFOP(struct PACKET_CZ_SHORTCUT_KEY_CHANGE2 *, fd, 0);
...
static void clif_parse_UnequipAllItems(int fd, struct map_session_data *sd)
{
	...
	// const struct PACKET_CZ_REQ_TAKEOFF_EQUIP_ALL *p = RFIFOP(struct PACKET_CZ_REQ_TAKEOFF_EQUIP_ALL *, fd, 0);
...
static void clif_parse_NpcClicked(int fd, struct map_session_data *sd)
{
	...
	const struct PACKET_CZ_CONTACTNPC *packet = RFIFOP(struct PACKET_CZ_CONTACTNPC *, fd, 0);
...
static void clif_parse_NpcBuyListSend(int fd, struct map_session_data *sd)
{
	...
	const struct PACKET_CZ_PC_PURCHASE_ITEMLIST *p = RFIFOP(struct PACKET_CZ_PC_PURCHASE_ITEMLIST *, fd, 0);
...
static void clif_parse_PutItemToCart(int fd, struct map_session_data *sd)
{
	...
	const struct PACKET_CZ_MOVE_ITEM_FROM_BODY_TO_CART *p = RFIFOP(struct PACKET_CZ_MOVE_ITEM_FROM_BODY_TO_CART *, fd, 0);
...
static void clif_parse_reqGearOff(int fd, struct map_session_data *sd)
{
	...
	const struct PACKET_CZ_UNINSTALLATION *p = RFIFOP(struct PACKET_CZ_UNINSTALLATION *, fd, 0);
...
{
#if PACKETVER_MAIN_NUM >= 20181002 || PACKETVER_RE_NUM >= 20181002 || PACKETVER_ZERO_NUM >= 20181010
	const struct PACKET_CZ_USE_SKILL_START *p = RFIFOP(struct PACKET_CZ_USE_SKILL_START *, fd, 0);
...
{
#if PACKETVER_MAIN_NUM >= 20181002 || PACKETVER_RE_NUM >= 20181002 || PACKETVER_ZERO_NUM >= 20181010
	const struct PACKET_CZ_USE_SKILL_END *p = RFIFOP(struct PACKET_CZ_USE_SKILL_END *, fd, 0);
...
{
	const struct PACKET_CZ_REQMAKINGITEM *p = RFIFOP(struct PACKET_CZ_REQMAKINGITEM *, fd, 0);
...
{
	const struct PACKET_CZ_REQ_MAKINGITEM *p = RFIFOP(struct PACKET_CZ_REQ_MAKINGITEM *, fd, 0);
	int type = p->type;
	int nameid = p->itemId;
...
{
	const struct PACKET_CZ_REQ_ITEMREPAIR1 *p = RFIFOP(struct PACKET_CZ_REQ_ITEMREPAIR1 *, fd, 0);
...
static void clif_parse_RepairItem2(int fd, struct map_session_data *sd)
{
	...
	const struct PACKET_CZ_REQ_ITEMREPAIR2 *p = RFIFOP(struct PACKET_CZ_REQ_ITEMREPAIR2 *, fd, 0);
...
static void clif_parse_NpcSelectMenuZero(int fd, struct map_session_data *sd)
{
	...
	const struct PACKET_CZ_CHOOSE_MENU_ZERO *packet = RFIFOP(struct PACKET_CZ_CHOOSE_MENU_ZERO *, fd, 0);
...
{
	const struct PACKET_CZ_REQ_GUILD_EMBLEM_IMG1 *p = RFIFOP(struct PACKET_CZ_REQ_GUILD_EMBLEM_IMG1 *, fd, 0);
...
static void clif_parse_GuildRequestEmblem2(int fd, struct map_session_data *sd)
{
	...
	const struct PACKET_CZ_REQ_GUILD_EMBLEM_IMG2 *p = RFIFOP(struct PACKET_CZ_REQ_GUILD_EMBLEM_IMG2 *, fd, 0);
	struct guild* g = guild->search(p->guild_id);
	if (g != NULL)
		clif->guild_emblem(sd, g);
...
static void clif_parse_GuildRequestEmblem3(int fd, struct map_session_data *sd)
{
	...
	const struct PACKET_CZ_REQ_GUILD_EMBLEM_IMG3 *p = RFIFOP(struct PACKET_CZ_REQ_GUILD_EMBLEM_IMG3 *, fd, 0);
	struct guild* g = guild->search(p->guild_id);
	if (g != NULL)
		clif->guild_emblem(sd, g);
#endif  // 20190724
...
static void clif_ranklist(struct map_session_data *sd, enum fame_list_type type)
{
	...
	struct PACKET_ZC_ACK_RANKING *p = WP2PTR(struct PACKET_ZC_ACK_RANKING *, fd);
...
static void clif_cashshop_show(struct map_session_data *sd, struct npc_data *nd)
{
	...
	WFIFOHEAD(fd, len);
	struct PACKET_ZC_PC_CASH_POINT_ITEMLIST *p = WP2PTR(struct PACKET_ZC_PC_CASH_POINT_ITEMLIST *, fd);
	p->packetType = 0x287;
...
static void clif_parse_cashshop_buy(int fd, struct map_session_data *sd)
{
	...
	int fail = 0;
	const struct PACKET_CZ_PC_BUY_CASH_POINT_ITEM *p = RFIFOP(struct PACKET_CZ_PC_BUY_CASH_POINT_ITEM *, fd, 0);
...
{
	const struct PACKET_CZ_PARTY_CONFIG *const p = RFIFOP(struct PACKET_CZ_PARTY_CONFIG *, fd, 0);
...
{ // '<ItemName>' item will disappear in <seconds/60> minutes.
	...
	struct PACKET_ZC_CASH_TIME_COUNTER *p = WP2PTR(struct PACKET_ZC_CASH_TIME_COUNTER *, fd);
	p->packetType = 0x298;
	p->itemId = nameid;
	p->seconds = seconds;
...
{ // '<ItemName>' item has been deleted from the Inventory
	...
	struct PACKET_ZC_CASH_ITEM_DELETE *p = WP2PTR(struct PACKET_ZC_CASH_ITEM_DELETE *, fd);
	p->packetType = 0x299;
	p->index = index + 2;
	p->itemId = nameid;
...
static void clif_parse_ReqOpenBuyingStore(int fd, struct map_session_data *sd)
{
	...
	const struct PACKET_CZ_REQ_OPEN_BUYING_STORE *p = RFIFOP(struct PACKET_CZ_REQ_OPEN_BUYING_STORE *, fd, 0);
...
static void clif_buyingstore_myitemlist(struct map_session_data *sd)
{
	...
	WFIFOHEAD(fd, len);
	struct PACKET_ZC_MYITEMLIST_BUYING_STORE *p = WP2PTR(struct PACKET_ZC_MYITEMLIST_BUYING_STORE *, fd);
	p->packetType = 0x813;
	p->packetLength = len;
	p->AID = sd->bl.id;
...
static void clif_buyingstore_entry_single(struct block_list *bl, const char *message, int fd)
{
	...
	struct PACKET_ZC_BUYING_STORE_ENTRY *p = WP2PTR(struct PACKET_ZC_BUYING_STORE_ENTRY *, fd);
	p->packetType = HEADER_ZC_BUYING_STORE_ENTRY;
	p->makerAID = bl->id;
	safestrncpy(p->storeName, message, MESSAGE_SIZE);
...
static void clif_buyingstore_disappear_entry_single(struct block_list *bl, int fd)
{
	...
	struct PACKET_ZC_DISAPPEAR_BUYING_STORE_ENTRY *p = WP2PTR(struct PACKET_ZC_DISAPPEAR_BUYING_STORE_ENTRY *, fd);
	p->packetType = HEADER_ZC_DISAPPEAR_BUYING_STORE_ENTRY;
	p->makerAID = bl->id;
...
static void clif_buyingstore_itemlist(struct map_session_data *sd, struct map_session_data *pl_sd)
{
	...
	WFIFOHEAD(fd, len);
	struct PACKET_ZC_ACK_ITEMLIST_BUYING_STORE *p  = WP2PTR(struct PACKET_ZC_ACK_ITEMLIST_BUYING_STORE *, fd);
	p->packetType = 0x818;
	p->packetLength = len;
	p->AID = pl_sd->bl.id;
...
static void clif_parse_ReqTradeBuyingStore(int fd, struct map_session_data *sd)
{
	...
	int count, packet_len;
	const struct PACKET_CZ_REQ_TRADE_BUYING_STORE *p = RFIFOP(struct PACKET_CZ_REQ_TRADE_BUYING_STORE *, fd, 0);
	packet_len = p->packetLength;
...
static void clif_buyingstore_update_item(struct map_session_data *sd, int nameid, unsigned short amount, uint32 char_id, int zeny)
{
	...
	struct PACKET_ZC_UPDATE_ITEM_FROM_BUYING_STORE *p = WP2PTR(struct PACKET_ZC_UPDATE_ITEM_FROM_BUYING_STORE *, fd);
...
static void clif_buyingstore_trade_failed_seller(struct map_session_data *sd, short result, int nameid)
{
	...
	struct PACKET_ZC_FAILED_TRADE_BUYING_STORE_TO_SELLER *p = WP2PTR(struct PACKET_ZC_FAILED_TRADE_BUYING_STORE_TO_SELLER *, fd);
	p->packetType = 0x824;
	p->result = result;
	p->itemId = nameid;
...
static void clif_parse_SearchStoreInfo(int fd, struct map_session_data *sd)
{
	...
	int i;
	const struct PACKET_CZ_SEARCH_STORE_INFO *p = RFIFOP(struct PACKET_CZ_SEARCH_STORE_INFO *, fd, 0);
	int32 *items_list;
	int32 *cards_list;
...
static void clif_search_store_info_ack(struct map_session_data *sd)
{
	...
	WFIFOHEAD(fd, len);
	struct PACKET_ZC_SEARCH_STORE_INFO_ACK *p = WP2PTR(struct PACKET_ZC_SEARCH_STORE_INFO_ACK *, fd);
...
static void clif_parse_SearchStoreInfoListItemClick(int fd, struct map_session_data *sd)
{
	...
	const struct PACKET_CZ_SSILIST_ITEM_CLICK *p = RFIFOP(struct PACKET_CZ_SSILIST_ITEM_CLICK *, fd, 0);
...
static int clif_autoshadowspell_list(struct map_session_data *sd)
{
	...
	WFIFOHEAD(fd, len);
	p = WP2PTR(struct PACKET_ZC_SKILL_SELECT_REQUEST *, fd);
...
#if PACKETVER >= 20191224
	const struct PACKET_CZ_SE_CASHSHOP_OPEN2 *p = RFIFOP(struct PACKET_CZ_SE_CASHSHOP_OPEN2 *, fd, 0);
...
static void clif_cashShopOpen(int fd, struct map_session_data *sd, int tab)
{
	...
	struct PACKET_ZC_SE_CASHSHOP_OPEN *p = WP2PTR(struct PACKET_ZC_SE_CASHSHOP_OPEN *, fd);
...
void clif_cashShopSchedule(int fd, struct map_session_data *sd)
{
	...
	for (i = 0; i < CASHSHOP_TAB_MAX; i++) {
		...
		WFIFOHEAD(fd, len);
		struct PACKET_ZC_ACK_SCHEDULER_CASHITEM *p = WP2PTR(struct PACKET_ZC_ACK_SCHEDULER_CASHITEM *, fd);
		p->packetType = 0x8ca;
		p->packetLength = len;
...
static void clif_cashShopBuyAck(int fd, struct map_session_data *sd, int itemId, enum CASH_SHOP_BUY_RESULT result)
{
	...
	struct PACKET_ZC_SE_PC_BUY_CASHITEM_RESULT *p = WP2PTR(struct PACKET_ZC_SE_PC_BUY_CASHITEM_RESULT *, fd);
	p->packetType = 0x849;
	p->itemId = itemId;
	p->result = result;
...
static void clif_partytickack(struct map_session_data *sd, bool flag)
{
	...
	struct PACKET_ZC_PARTY_CONFIG *p = WP2PTR(struct PACKET_ZC_PARTY_CONFIG *, sd->fd);
	p->packetType = HEADER_ZC_PARTY_CONFIG;
...
static void clif_show_modifiers(struct map_session_data *sd)
{
	...
	struct PACKET_ZC_PERSONAL_INFOMATION *p = WP2PTR(struct PACKET_ZC_PERSONAL_INFOMATION *, sd->fd);
...
static void clif_openmergeitem(int fd, struct map_session_data *sd)
{
	...
	WFIFOHEAD(fd, len);
	struct PACKET_ZC_MERGE_ITEM_OPEN *p = WP2PTR(struct PACKET_ZC_MERGE_ITEM_OPEN *, fd);
...
static void clif_mergeitems(int fd, struct map_session_data *sd, int index, int amount, enum mergeitem_reason reason)
{
	...
	struct PACKET_ZC_ACK_MERGE_ITEM *p = WP2PTR(struct PACKET_ZC_ACK_MERGE_ITEM *, fd);
...
static void clif_clan_basicinfo(struct map_session_data *sd)
{
	...
	struct PACKET_ZC_CLANINFO *packet = WP2PTR(struct PACKET_ZC_CLANINFO *, fd);
...
static void clif_parse_rodex_open_write_mail(int fd, struct map_session_data *sd)
{
	...
	const struct PACKET_CZ_REQ_OPEN_WRITE_MAIL *rPacket = RFIFOP(struct PACKET_CZ_REQ_OPEN_WRITE_MAIL *, fd, 0);
...
static void clif_rodex_open_write_mail(int fd, const char *receiver_name, int8 result)
{
	...
	sPacket = WP2PTR(struct PACKET_ZC_ACK_OPEN_WRITE_MAIL *, fd);
...
static void clif_parse_rodex_add_item(int fd, struct map_session_data *sd)
{
	...
	const struct PACKET_CZ_ADD_ITEM_TO_MAIL *rPacket = RFIFOP(struct PACKET_CZ_ADD_ITEM_TO_MAIL *, fd, 0);
...
// but used only packet versions with known struct
	...
	packet = WP2PTR(struct PACKET_ZC_ACK_ADD_ITEM_RODEX *, fd);
...
static void clif_parse_rodex_remove_item(int fd, struct map_session_data *sd)
{
	...
	const struct PACKET_CZ_REQ_REMOVE_ITEM_MAIL *rPacket = RFIFOP(struct PACKET_CZ_REQ_REMOVE_ITEM_MAIL *, fd, 0);
	int16 idx = rPacket->index - 2;
...
static void clif_rodex_remove_item_result(struct map_session_data *sd, int16 idx, int16 amount)
{
	...
	WFIFOHEAD(fd, sizeof(*packet));
	packet = WP2PTR(struct PACKET_ZC_ACK_REMOVE_ITEM_MAIL *, fd);
...
static void clif_parse_rodex_checkname1(int fd, struct map_session_data *sd)
{
	...
	const struct PACKET_CZ_CHECKNAME1 *rPacket = RFIFOP(struct PACKET_CZ_CHECKNAME1 *, fd, 0);
...
static void clif_parse_rodex_checkname2(int fd, struct map_session_data *sd)
{
	...
	const struct PACKET_CZ_CHECKNAME2 *rPacket = RFIFOP(struct PACKET_CZ_CHECKNAME2 *, fd, 0);
	int char_id = 0, base_level = 0;
	int class_ = 0;
	char name[NAME_LENGTH];
...
static void clif_rodex_checkname_result(struct map_session_data *sd, int char_id, int class_, int base_level, const char *name)
{
	...
	fd = sd->fd;
	WFIFOHEAD(fd, sizeof(*sPacket));
	sPacket = WP2PTR(struct PACKET_ZC_CHECKNAME *, fd);
...
static void clif_parse_rodex_send_mail(int fd, struct map_session_data *sd)
{
	...
	const struct PACKET_CZ_SEND_MAIL *rPacket = RFIFOP(struct PACKET_CZ_SEND_MAIL *, fd, 0);
	int8 result;
...
static void clif_rodex_send_mail_result(int fd, struct map_session_data *sd, int8 result)
{
	...
	WFIFOHEAD(fd, sizeof(*sPacket));
	sPacket = WP2PTR(struct PACKET_ZC_WRITE_MAIL_RESULT *, fd);
...
static void clif_rodex_send_maillist(int fd, struct map_session_data *sd, int8 open_type, int64 page_start)
{
	...
	packet = WP2PTR(struct PACKET_ZC_MAIL_LIST *, fd);
...
static void clif_parse_rodex_next_maillist(int fd, struct map_session_data *sd)
{
	...
	const struct PACKET_CZ_REQ_NEXT_MAIL_LIST *packet = RFIFOP(struct PACKET_CZ_REQ_NEXT_MAIL_LIST *, fd, 0);
...
static void clif_parse_rodex_read_mail(int fd, struct map_session_data *sd)
{
	...
	const struct PACKET_CZ_REQ_READ_MAIL *rPacket = RFIFOP(struct PACKET_CZ_REQ_READ_MAIL *, fd, 0);
...
// [4144] date unconfirmed
	...
	struct PACKET_ZC_ACK_READ_RODEX *sPacket = WP2PTR(struct PACKET_ZC_ACK_READ_RODEX *, fd);
...
static void clif_parse_rodex_delete_mail(int fd, struct map_session_data *sd)
{
	...
	const struct PACKET_CZ_REQ_DELETE_MAIL *rPacket = RFIFOP(struct PACKET_CZ_REQ_DELETE_MAIL *, fd, 0);
...
static void clif_rodex_delete_mail(struct map_session_data *sd, int8 opentype, int64 mail_id)
{
	...
	WFIFOHEAD(fd, sizeof(*sPacket));
	sPacket = WP2PTR(struct PACKET_ZC_ACK_DELETE_MAIL *, fd);
...
static void clif_parse_rodex_request_zeny(int fd, struct map_session_data *sd)
{
	...
	const struct PACKET_CZ_REQ_ZENY_FROM_MAIL *rPacket = RFIFOP(struct PACKET_CZ_REQ_ZENY_FROM_MAIL *, fd, 0);
...
static void clif_rodex_request_zeny(struct map_session_data *sd, int8 opentype, int64 mail_id, enum rodex_get_zeny result)
{
	...
	WFIFOHEAD(fd, sizeof(*sPacket));
	sPacket = WP2PTR(struct PACKET_ZC_ACK_ZENY_FROM_MAIL *, fd);
...
static void clif_parse_rodex_request_items(int fd, struct map_session_data *sd)
{
	...
	const struct PACKET_CZ_REQ_ITEM_FROM_MAIL *rPacket = RFIFOP(struct PACKET_CZ_REQ_ITEM_FROM_MAIL *, fd, 0);
...
static void clif_rodex_request_items(struct map_session_data *sd, int8 opentype, int64 mail_id, enum rodex_get_items result)
{
	...
	WFIFOHEAD(fd, sizeof(*sPacket));
	sPacket = WP2PTR(struct PACKET_ZC_ACK_ITEM_FROM_MAIL *, fd);
...
static void clif_parse_rodex_refresh_maillist(int fd, struct map_session_data *sd)
{
	...
	const struct PACKET_CZ_REQ_REFRESH_MAIL_LIST *packet = RFIFOP(struct PACKET_CZ_REQ_REFRESH_MAIL_LIST *, fd, 0);
#if PACKETVER >= 20170419
...
static void clif_parse_rodex_open_mailbox(int fd, struct map_session_data *sd)
{
	...
	const struct PACKET_CZ_REQ_OPEN_MAIL *packet = RFIFOP(struct PACKET_CZ_REQ_OPEN_MAIL *, fd, 0);
#if PACKETVER >= 20170419
...
static void clif_parse_cameraInfo(int fd, struct map_session_data *sd)
{
	...
	const struct PACKET_CZ_VIEW_CAMERAINFO *const p = RFIFOP(struct PACKET_CZ_VIEW_CAMERAINFO *, fd, 0);
	char command[200];
	if (p->action == 1) {
...
static void clif_captcha_upload_request(struct map_session_data *sd, const char *captcha_key, const int captcha_flag)
{
	...
	struct PACKET_ZC_ACK_UPLOAD_MACRO_DETECTOR *p = WP2PTR(struct PACKET_ZC_ACK_UPLOAD_MACRO_DETECTOR *, fd);
...
static void clif_captcha_upload_end(struct map_session_data *sd)
{
	...
	struct PACKET_ZC_COMPLETE_UPLOAD_MACRO_DETECTOR_CAPTCHA *p = WP2PTR(struct PACKET_ZC_COMPLETE_UPLOAD_MACRO_DETECTOR_CAPTCHA *, fd);
...
static void clif_captcha_preview_request_init(struct map_session_data *sd, const char *captcha_key, const int image_size, const int captcha_flag)
{
	...
	struct PACKET_ZC_ACK_PREVIEW_MACRO_DETECTOR *p = WP2PTR(struct PACKET_ZC_ACK_PREVIEW_MACRO_DETECTOR *, fd);
...
static void clif_captcha_preview_request_download(struct map_session_data *sd, const char *captcha_key, const int chunk_size, const char *chunk_data)
{
	...
	WFIFOHEAD(fd, len);
	struct PACKET_ZC_PREVIEW_MACRO_DETECTOR_CAPTCHA *p = WP2PTR(struct PACKET_ZC_PREVIEW_MACRO_DETECTOR_CAPTCHA *, fd);
...
static void clif_macro_detector_request_init(struct map_session_data *sd, const char *captcha_key, const int image_size)
{
	...
	struct PACKET_ZC_APPLY_MACRO_DETECTOR *p = WP2PTR(struct PACKET_ZC_APPLY_MACRO_DETECTOR *, fd);
...
static void clif_macro_detector_request_download(struct map_session_data *sd, const char *captcha_key, const int chunk_size, const char *chunk_data)
{
	...
	WFIFOHEAD(fd, len);
	struct PACKET_ZC_APPLY_MACRO_DETECTOR_CAPTCHA *p = WP2PTR(struct PACKET_ZC_APPLY_MACRO_DETECTOR_CAPTCHA *, fd);
...
static void clif_macro_detector_request_show(struct map_session_data *sd)
{
	...
	struct PACKET_ZC_REQ_ANSWER_MACRO_DETECTOR *p = WP2PTR(struct PACKET_ZC_REQ_ANSWER_MACRO_DETECTOR *, fd);
...
static void clif_macro_detector_status(struct map_session_data *sd, enum macro_detect_status stype)
{
	...
	struct PACKET_ZC_CLOSE_MACRO_DETECTOR *p = WP2PTR(struct PACKET_ZC_CLOSE_MACRO_DETECTOR *, fd);
...
static void clif_macro_reporter_select(struct map_session_data *sd, const struct macroaidlist *aid_list)
{
	...
	WFIFOHEAD(fd, len);
	struct PACKET_ZC_ACK_PLAYER_AID_IN_RANGE *p = WP2PTR(struct PACKET_ZC_ACK_PLAYER_AID_IN_RANGE *, fd);
...
static void clif_macro_reporter_status(struct map_session_data *sd, enum macro_report_status stype)
{
	...
	struct PACKET_ZC_ACK_APPLY_MACRO_DETECTOR *p = WP2PTR(struct PACKET_ZC_ACK_APPLY_MACRO_DETECTOR *, fd);
...
static void clif_sayDialogAlign(struct map_session_data *sd, int npcid, enum say_dialog_align align)
{
	...
	struct PACKET_ZC_DIALOG_TEXT_ALIGN *p = WP2PTR(struct PACKET_ZC_DIALOG_TEXT_ALIGN *, fd);
...
static void clif_grade_enchant_add_item_result_success(struct map_session_data *sd, int idx, const struct s_grade_info *gi)
{
	...
	struct PACKET_ZC_GRADE_ENCHANT_MATERIAL_LIST *p = WP2PTR(struct PACKET_ZC_GRADE_ENCHANT_MATERIAL_LIST *, fd);
...
static void clif_grade_enchant_result(struct map_session_data *sd, int16 index, enum grade_level gl, enum grade_ui_result result)
{
	...
	struct PACKET_ZC_GRADE_ENCHANT_ACK *p = WP2PTR(struct PACKET_ZC_GRADE_ENCHANT_ACK *, fd);
...
static void clif_set_npc_window_size(struct map_session_data *sd, int width, int height)
{
	...
	struct PACKET_ZC_DIALOG_WINDOW_SIZE *p = WP2PTR(struct PACKET_ZC_DIALOG_WINDOW_SIZE *, fd);
...
static void clif_set_npc_window_pos(struct map_session_data *sd, int x, int y)
{
	...
	struct PACKET_ZC_DIALOG_WINDOW_POS *p = WP2PTR(struct PACKET_ZC_DIALOG_WINDOW_POS *, fd);
...
static void clif_item_reform_open(struct map_session_data *sd, int itemId)
{
	...
	struct PACKET_ZC_OPEN_REFORM_UI *p = WP2PTR(struct PACKET_ZC_OPEN_REFORM_UI *, fd);
...
static void clif_item_reform_result(struct map_session_data *sd, int index, enum item_reform_status result)
{
	...
	struct PACKET_ZC_ITEM_REFORM_ACK *p = WP2PTR(struct PACKET_ZC_ITEM_REFORM_ACK *, fd);
...
static void clif_enchantui_status(struct map_session_data *sd, enum enchantui_status result, int itemId)
{
	...
	struct PACKET_ZC_RESPONSE_ENCHANT *p = WP2PTR(struct PACKET_ZC_RESPONSE_ENCHANT *, fd);
...
static void clif_special_popup(struct map_session_data *sd, int popupId)
{
	...
	struct PACKET_ZC_SPECIALPOPUP *p = WP2PTR(struct PACKET_ZC_SPECIALPOPUP *, fd);
...
static void clif_dynamicnpc_create_result(struct map_session_data *sd, enum dynamicnpc_create_result result)
{
	...
	struct PACKET_ZC_DYNAMICNPC_CREATE_RESULT *p = WP2PTR(struct PACKET_ZC_DYNAMICNPC_CREATE_RESULT *, fd);
...
static void clif_goldpc_info(struct map_session_data *sd)
{
	...
	struct PACKET_ZC_GOLDPCCAFE_POINT *p = WP2PTR(struct PACKET_ZC_GOLDPCCAFE_POINT *, fd);
...
static void clif_adventurerAgencyJoinReq(struct map_session_data *sd, struct map_session_data *tsd)
{
	...
	struct PACKET_ZC_ADVENTURER_AGENCY_JOIN_REQ *packet = WP2PTR(struct PACKET_ZC_ADVENTURER_AGENCY_JOIN_REQ *, fd);
...
#define RFIFOP_mutable(T, fd, pos) ((T)(void *)(sockt->session[fd]->rdata + sockt->session[fd]->rdata_pos + (pos)))
			int16 *packet_id = RFIFOP_mutable(int16 *, fd, 0);
