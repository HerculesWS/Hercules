...
void clif_blacksmith(struct map_session_data* sd)
{
	...
	for (i = 0; i < 10 && i < MAX_FAME_LIST; i++) {
		if (iPc->smith_fame_list[i].id > 0) {
			if (strcmp(iPc->smith_fame_list[i].name, "-") == 0 &&
				(name = iMap->charid2nick(iPc->smith_fame_list[i].id)) != NULL)
			{
				strncpy((char *)(WFIFOP(fd, 2 + 24 * i)), name, NAME_LENGTH);
			} else
				strncpy((char *)(WFIFOP(fd, 2 + 24 * i)), iPc->smith_fame_list[i].name, NAME_LENGTH);
		} else
			strncpy((char *)(WFIFOP(fd, 2 + 24 * i)), "None", 5);
		WFIFOL(fd, 242 + i * 4) = iPc->smith_fame_list[i].fame;
	}
...
void clif_alchemist(struct map_session_data* sd) {
	...
	//Packet size limits this list to 10 elements. [Skotlex]
	for (i = 0; i < 10 && i < MAX_FAME_LIST; i++) {
		if (iPc->chemist_fame_list[i].id > 0) {
			if (strcmp(iPc->chemist_fame_list[i].name, "-") == 0 &&
				(name = iMap->charid2nick(iPc->chemist_fame_list[i].id)) != NULL)
			{
				memcpy(WFIFOP(fd, 2 + 24 * i), name, NAME_LENGTH);
			} else
				memcpy(WFIFOP(fd, 2 + 24 * i), iPc->chemist_fame_list[i].name, NAME_LENGTH);
		} else
			memcpy(WFIFOP(fd, 2 + 24 * i), "None", NAME_LENGTH);
		WFIFOL(fd, 242 + i * 4) = iPc->chemist_fame_list[i].fame;
	}
	for(;i < 10; i++) { //In case the MAX is less than 10.
...
void clif_taekwon(struct map_session_data* sd) {
	...
	//Packet size limits this list to 10 elements. [Skotlex]
	for (i = 0; i < 10 && i < MAX_FAME_LIST; i++) {
		if (iPc->taekwon_fame_list[i].id > 0) {
			if (strcmp(iPc->taekwon_fame_list[i].name, "-") == 0 &&
				(name = iMap->charid2nick(iPc->taekwon_fame_list[i].id)) != NULL)
			{
				memcpy(WFIFOP(fd, 2 + 24 * i), name, NAME_LENGTH);
			} else
				memcpy(WFIFOP(fd, 2 + 24 * i), iPc->taekwon_fame_list[i].name, NAME_LENGTH);
		} else
			memcpy(WFIFOP(fd, 2 + 24 * i), "None", NAME_LENGTH);
		WFIFOL(fd, 242 + i * 4) = iPc->taekwon_fame_list[i].fame;
	}
	for(;i < 10; i++) { //In case the MAX is less than 10.
		memcpy(WFIFOP(fd, 2 + 24 * i), "Unavailable", NAME_LENGTH);
		WFIFOL(fd, 242 + i * 4) = 0;
...
int clif_instance(int instance_id, int type, int flag)
{
	...
	if( (p = iParty->search(instance[instance_id].party_id)) == NULL || (sd = iParty->getavailablesd(p)) == NULL )
		return 0;
