		...
		case BL_PC:
			{
				...
				if(sd->state.size==VIEWSIZE_BIG) // tiny/big players [Valaris]
					clif->specialeffect(bl,423,AREA);
				else if(sd->state.size==VIEWSIZE_SMALL)
		...
		case BL_MOB:
			{
				...
				if(md->special_state.size==VIEWSIZE_BIG) // tiny/big mobs [Valaris]
					clif->specialeffect(&md->bl,423,AREA);
				else if(md->special_state.size==VIEWSIZE_SMALL)
		...
		case BL_NPC:
			{
				...
				if( nd->size == VIEWSIZE_BIG )
					clif->specialeffect(&nd->bl,423,AREA);
				else if( nd->size == VIEWSIZE_SMALL )
		...
		case BL_PC:
			{
				...
				if(tsd->state.size==VIEWSIZE_BIG) // tiny/big players [Valaris]
					clif->specialeffect_single(bl,423,sd->fd);
				else if(tsd->state.size==VIEWSIZE_SMALL)
