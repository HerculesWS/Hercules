	...
	if( vd ) //temp hack to let Warp Portal change appearance
		switch(type) {
			...
			case LOOK_BASE:
				...
				if ((sd->sc.option & OPTION_WEDDING) != 0 && battle_config.wedding_ignorepalette == 1)
					vd->cloth_color = 0;
				if ((sd->sc.option & OPTION_XMAS) != 0 && battle_config.xmas_ignorepalette == 1)
					vd->cloth_color = 0;
				if ((sd->sc.option & OPTION_SUMMER) != 0 && battle_config.summer_ignorepalette == 1)
					vd->cloth_color = 0;
				if ((sd->sc.option & OPTION_HANBOK) != 0 && battle_config.hanbok_ignorepalette == 1)
					vd->cloth_color = 0;
				if ((sd->sc.option & OPTION_OKTOBERFEST) != 0 && battle_config.oktoberfest_ignorepalette == 1)
					vd->cloth_color = 0;
				if ((sd->sc.option & OPTION_SUMMER2) != 0 && battle_config.summer2_ignorepalette == 1)
					vd->cloth_color = 0;
...
static void clif_displaymessage_sprintf(const int fd, const char *mes, ...)
{
	...
	if (map->cpsd_active && fd == 0) {
		...
		/* process */
		va_start(ap,mes);
		len += vsnprintf(WFIFOP(char *, fd, 4), 255, mes, ap);
		va_end(ap);
...
static void clif_guild_basicinfo(struct map_session_data *sd)
{
	...
	p->exp = (uint32)std::clamp(g->exp, (uint64)0, (uint64)INT32_MAX);
	p->maxExp = g->next_exp;
...
static void clif_guild_memberlist(struct map_session_data *sd)
{
	...
	for (int i = 0; i < g->max_member; i++) {
		...
		p->guildMemberInfo[c].contributionExp = (int)std::clamp(m->exp, (uint64)0, (uint64)INT32_MAX);
...
static void clif_npc_expanded_barter_open(struct map_session_data *sd, struct npc_data *nd)
{
	...
	for (int i = 0; i < shop_size && buf_left >= ptr_size; i++) {
		if (shop[i].nameid) {
			...
			item->type   = itemtype((item_types)id->type);
			...
			for (int j = 0; j < count; j ++) {
				...
				packet_currency->type = itemtype((item_types)id2->type);
				currencies_count ++;
				item->currency_count ++;
			}
