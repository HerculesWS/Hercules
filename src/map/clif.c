...
bool clif_attendancedb_libconfig_sub(struct config_setting_t *it, int n, const char *source) {
	...
	if (!itemdb->lookup_const(it, "ItemID", &i32)) {
		ShowWarning("clif_attendancedb_libconfig_sub: Invalid or missing id in \"%s\", entry #%d, skipping.\n", source, n);
		return false;
	}
	clif->attendance_data.nameid[n] = (uint16)i32;
...
void do_final_clif(void)
{
	...
	for (i = 0; i < MAX_ATTENDANCE_DAYS; i++) {
		if (clif->attendance_data.nameid[i])
			aFree(clif->attendance_data.nameid[i]);
		if (clif->attendance_data.qty[i])
			aFree(clif->attendance_data.qty[i]);
	}
