...
struct achievement_objective {
	...
	/* Those criteria that do not make sense if stacked together
	 are held as a unique member. @see achievement_objective_unique */
	union {
		int achieve_id;
		int itemid;
...
// Achievements that require filtering by criteria
#define achievement_has_criteria(t) ( \
		   (t) == ACH_KILL_PC_JOB \
...
struct achievement_interface {
	...
	void (*progress_add) (struct map_session_data *sd, struct achievement_data *ad, const int obj_idx, const int progress);
	void (*progress_set) (struct map_session_data *sd, struct achievement_data *ad, const int obj_idx, const int progress);
	bool (*check_criteria) (const struct achievement_objective *objective, const struct achievement_objective *criteria);
	/* */
	bool (*validate) (struct map_session_data *sd, const int aid, const int obj_idx, const int progress, bool additive);
	...
	void (*validate_refine) (struct map_session_data *sd, int idx, bool success);
