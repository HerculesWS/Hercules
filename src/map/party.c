...
int party_recv_movemap(int party_id,int account_id,int char_id, unsigned short mapid,int online,int lv)
{
	...
	if( i == MAX_PARTY )
	{
		ShowError("%s: char %d/%d not found in party %s (id:%d)", __func__, account_id, char_id, p->party.name, party_id);
		return 0;
	}
...
int party_send_logout(struct map_session_data *sd)
{
	...
	if( i < MAX_PARTY )
		...
		ShowError("%s: Failed to locate member %d:%d in party %d!\n", __func__, sd->status.account_id, sd->status.char_id, p->party.party_id);
