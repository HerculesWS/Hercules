...
// A collection of id's per type for faster processing.
// We do this to avoid traversing through any other type when searching/validating.
VECTOR_DECL(int) achievement_category[ACH_TYPE_MAX];
 ...
 */
void achievement_progress_add(struct map_session_data *sd, struct achievement_data *ad, const int obj_idx, const int progress)
{
	struct achievement *ach = NULL;
	...
	Assert_retv(obj_idx >= 0 && obj_idx < VECTOR_LENGTH(ad->objective));
 ...
 */
void achievement_progress_set(struct map_session_data *sd, struct achievement_data *ad, const int obj_idx, const int progress)
{
	struct achievement *ach = NULL;
...
int achievement_validate_type(struct map_session_data *sd, const enum achievement_types type, const struct achievement_objective *criteria, const int progress, bool additive)
{
	...
	for (i = 0; i < VECTOR_LENGTH(achievement_category[type]); i++) {
		int j = 0;
		bool updated = false;
		struct achievement_data *ad = NULL;

		if ((ad = achievement->get(VECTOR_INDEX(achievement_category[type], i))) == NULL)
			continue;
 ...
 */
bool achievement_validate(struct map_session_data *sd, const int aid, const int index, const int progress, bool additive)
{
 ...
 * @see achievement_validate_type()
 */
void achievement_validate_refine(struct map_session_data *sd, int idx, bool success)
{
	struct achievement_objective criteria = { 0 };
...
void achievement_validate_achievement_rank(struct map_session_data *sd, const int rank)
{
	...
	if (rank > VECTOR_LENGTH(achievement->rank_exp))
		return;
	...
	while ((conf = libconfig->setting_get_elem(achdb, entry++))) {
		...
		if ((t = libconfig->setting_get_member(conf, "Objectives")) && config_setting_is_group(t)) {
			...
			for (i = 0; i < libconfig->setting_length(t) && i < MAX_ACHIEVEMENT_OBJECTIVES; i++) {
				...
				if ((tt = libconfig->setting_get_member(t, objnum)) && config_setting_is_group(tt)) {
					...
					if (libconfig->setting_lookup_mutable_string(tt, "Description", obj.description, OBJECTIVE_DESCRIPTION_LENGTH) == 0) {
						ShowError("achievement_readdb: Objective %d has no description for Achievement %d, skipping...\n", i + 1, t_ad.id);
						break;
					}

					/* Criteria */
					if ((c = libconfig->setting_get_member(tt, "Criteria")) && config_setting_is_group(tt)) {
						struct config_setting_t *cc = NULL; // for sub-settings (arrays and such).
						int job_id = 0;

						/* Monster */
						if (libconfig->setting_lookup_int(c, "Monster", &val)) {
							if (mob->db_checkid(val) == 0) {
								ShowError("achievement_readdb: Non-existant monster with ID %id provided (Achievement: %d, Objective: %d). Skipping...\n", val, t_ad.id, i + 1);
								break;
							}

							obj.mobid = val;
						} else if (libconfig->setting_lookup_string(c, "Monster", &string)) {
							if (!script->get_constant(string, &val)) {
								ShowError("achievement_readdb: Non-existant constant %s provided (Achievement: %d, Objective: %d). Skipping...\n", string, t_ad.id, i + 1);
								break;
							}

							obj.mobid = val;
						} else if (achievement_criteria_mobid(t_ad.type)) {
							ShowError("achievement_readdb: Achievement type of ID %d requires Monster as objective criteria, setting not provided. Skipping...\n", t_ad.id);
							break;
						}

						/* JobId */
						if (libconfig->setting_lookup_int(c, "JobId", &job_id)) {
							if (pc->jobid2mapid(job_id) == -1) {
								ShowError("achievement_readdb: Invalid JobId %d provided (Achievement: %d, Objective: %d). Skipping...\n", job_id, t_ad.id, i + 1);
								break;
							}

							VECTOR_ENSURE(obj.jobid, 1, 1);
							VECTOR_PUSH(obj.jobid, job_id);
						} else if (libconfig->setting_lookup_string(c, "JobId", &string)) {
							...
							VECTOR_ENSURE(obj.jobid, 1, 1);
							VECTOR_PUSH(obj.jobid, job_id);
						} else if ((cc = libconfig->setting_get_member(c, "JobId")) && config_setting_is_array(cc)) {
							int j = 0;

							while (j < libconfig->setting_length(cc)) {
								if ((job_id = libconfig->setting_get_int_elem(cc, j)) == 0) {
									if ((string = libconfig->setting_get_string_elem(cc, j)) != NULL && script->get_constant(string, &job_id) == false) {
										ShowError("achievement_readdb: Invalid JobId provided at index %d (Achievement: %d, Objective: %d). Skipping...\n", j, t_ad.id, i + 1);
										continue;
									}
								}
						...
						} else if (achievement_criteria_jobid(t_ad.type)) {
							ShowError("achievement_readdb: Achievement type of ID %d requires a JobId field in the objective criteria, setting not provided. Skipping...\n", t_ad.id);
							break;
						}

						/* Item */
						if (libconfig->setting_lookup_int(c, "Item", &val)) {
							if (itemdb->exists(val) == NULL) {
								ShowError("achievement_readdb: Invalid ItemID %d provided (Achievement: %d, Objective: %d). Skipping...\n", val, t_ad.id, i + 1);
								break;
							} else if (achievement_objective_unique(obj.unique)) {
								ShowError("achievement_readdb: A unique criteria has already been set. (Achievement: %d, Objective: %d). Skipping...\n", t_ad.id, i + 1);
								break;
							}

							obj.unique.itemid = val;
						} else if (libconfig->setting_lookup_string(c, "Item", &string)) {
							...
							obj.unique.itemid = val;
						} else if (achievement_criteria_itemid(t_ad.type)) {
							ShowError("achievement_readdb: Criteria requires a ItemId field (Achievement: %d, Objective: %d). Skipping...\n", t_ad.id, i + 1);
							break;
						}

						/* StatusType */
						if (libconfig->setting_lookup_int(c, "StatusType", &val)) {
							if (!achievement_valid_status_types(val)) {
								ShowError("achievement_readdb: Invalid StatusType %d provided (Achievement: %d, Objective: %d). Skipping...\n", val, t_ad.id, i + 1);
								break;
							} else if (achievement_objective_unique(obj.unique)) {
								ShowError("achievement_readdb: A unique criteria has already been set. (Achievement: %d, Objective: %d). Skipping...\n", t_ad.id, i + 1);
								break;
						...
						} else if (libconfig->setting_lookup_string(c, "StatusType", &string)) {
							...
							if (!achievement_valid_status_types(val)) {
								ShowError("achievement_readdb: Invalid StatusType %s provided (Achievement: %d, Objective: %d). Skipping...\n", string, t_ad.id, i + 1);
								break;
							} else if (achievement_objective_unique(obj.unique)) {
								ShowError("achievement_readdb: A unique criteria has already been set. (Achievement: %d, Objective: %d). Skipping...\n", t_ad.id, i + 1);
								break;
							...
							obj.unique.status_type = (enum status_point_types) val;
						} else if (achievement_criteria_stattype(t_ad.type)) {
							ShowError("achievement_readdb: Criteria requires a StatusType field (Achievement: %d, Objective: %d). Skipping...\n", t_ad.id, i + 1);
							break;
						}

						/* ItemType */
						if (libconfig->setting_lookup_int(c, "ItemType", &val)) {
							if (val < IT_HEALING || val >= IT_MAX) {
								ShowError("achievement_readdb: Invalid ItemType %d provided (Achievement: %d, Objective: %d). Skipping...\n", val, t_ad.id, i + 1);
								break;
							}

							obj.item_type |= (enum item_types) val;
						} else if (libconfig->setting_lookup_string(c, "ItemType", &string)) {
							...
							obj.item_type = (enum item_types) val;
						} else if ((cc = libconfig->setting_get_member(c, "ItemType")) && config_setting_is_array(cc)) {
							int j = 0;
							uint32 it_type = 0;

							while (j < libconfig->setting_length(cc)) {
								if ((val = libconfig->setting_get_int_elem(cc, j))) {
									if (val < IT_HEALING || val >= IT_MAX) {
										ShowError("achievement_readdb: Invalid ItemType %d provided (Achievement: %d, Objective: %d). Skipping...\n", val, t_ad.id, i + 1);
										break;
									}
									it_type |= val;
								} else if ((string = libconfig->setting_get_string_elem(cc, j))) {
									if      (strcmp(string, "IT_HEALING") == 0)       val = OBJ_IT_HEALING;
										...
										ShowError("achievement_readdb: Invalid ItemType %s provided (Achievement: %d, Objective: %d). Skipping...\n", string, t_ad.id, i + 1);
										break;
									}
						...
						} else if (achievement_criteria_itemtype(t_ad.type)) {
							ShowError("achievement_readdb: Criteria requires a ItemType field (Achievement: %d, Objective: %d). Skipping...\n", t_ad.id, i + 1);
							break;
						}

						/* WeaponLevel */
						if (libconfig->setting_lookup_int(c, "WeaponLevel", &val)) {
							if (val < 1 || val > 4) {
								ShowError("achievement_readdb: Invalid WeaponLevel %d provided (Achievement: %d, Objective: %d). Skipping...\n", val, t_ad.id, i + 1);
								break;
							} else if (achievement_objective_unique(obj.unique)) {
								ShowError("achievement_readdb: A unique criteria has already been set. (Achievement: %d, Objective: %d). Skipping...\n", t_ad.id, i + 1);
								break;
						...
						/* Achieve */
						if (libconfig->setting_lookup_int(c, "Achieve", &val)) {
							if (achievement->get(val) == NULL) {
								ShowError("achievement_readdb: Invalid Achievement %d provided as objective (Achievement %d, Objective %d). Skipping...\n", val, t_ad.id, i + 1);
								break;
							} else if (achievement_objective_unique(obj.unique)) {
								ShowError("achievement_readdb: A unique criteria has already been set. (Achievement: %d, Objective: %d). Skipping...\n", t_ad.id, i + 1);
								break;
						...
						} else if (t_ad.type == ACH_ACHIEVE) {
							ShowError("achievement_readdb: Achievement type of ID %d requires an Achieve field in the objective criteria, setting not provided. Skipping...\n", t_ad.id);
							break;
						}
					} else if (achievement_has_criteria(t_ad.type)) {
						ShowError("achievement_readdb: No criteria field added (Achievement: %d, Objective: %d)! Skipping...\n", t_ad.id, i + 1);
						break;
					}
					...
					/* Ensure size and allocation */
					VECTOR_ENSURE(t_ad.objective, 1, 1);

					/* Push buffer */
					VECTOR_PUSH(t_ad.objective, obj);
				} else { // Break if not in order, to comply with the client's objective order.
					ShowWarning("achievement_readdb: Objectives for Achievement %d are not in order (starting from *%d). Remaining objectives will be skipped.\n", t_ad.id, i + 1);
					break;
				}
			...
			if (libconfig->setting_length(t) > MAX_ACHIEVEMENT_OBJECTIVES)
				ShowWarning("achievement_readdb: Exceeded maximum number of objectives (%d) for Achievement %d. Remaining objectives will be skipped.\n", MAX_ACHIEVEMENT_OBJECTIVES, t_ad.id);
			if (i == 0) {
				ShowError("achievement_readdb: No Objectives provided for Achievement %d, skipping...\n", t_ad.id);
				continue;
			}
		} else {
			ShowError("achievement_readdb: No Objectives provided for Achievement %d, skipping...\n", t_ad.id);
			...
			if ((tt = libconfig->setting_get_member(t, "Items")) && config_setting_is_group(t)) {
				...
				for (i = 0; i < libconfig->setting_length(tt) && i < MAX_ACHIEVEMENT_ITEM_REWARDS; i++) {
					...
					if ((it = libconfig->setting_get_elem(tt, i)) != NULL) {
						...
						/* Ensure size and allocation */
						VECTOR_ENSURE(t_ad.rewards.item, 1, 1);

						item.id = val;
						item.amount = amount;

						/* push buffer */
						VECTOR_PUSH(t_ad.rewards.item, item);
					}
				} // end of looping through items.

				if (libconfig->setting_length(tt) > MAX_ACHIEVEMENT_ITEM_REWARDS)
					ShowError("achievement_readdb: Maximum amount of item rewards (%d) exceeded for Achievement %d. Remaining items will be skipped.\n", MAX_ACHIEVEMENT_ITEM_REWARDS, t_ad.id);

			} // end of "Items" setting.

			/* Buff/Bonus Script Code */
			if (libconfig->setting_lookup_string(t, "Bonus", &string))
				t_ad.rewards.bonus = string ? script->parse(string, filename, 0, SCRIPT_IGNORE_EXTERNAL_BRACKETS, NULL) : NULL;

			/* Title Id */
			// @TODO Check Title ID against title DB!
			libconfig->setting_lookup_int(t, "TitleId", &t_ad.rewards.title_id);
		...
		*p_ad = t_ad;
		...
		VECTOR_ENSURE(achievement_category[p_ad->type], 1, 1);
		VECTOR_PUSH(achievement_category[p_ad->type], p_ad->id);
...
void do_init_achievement(bool minimal)
{
	...
	for (i = 0; i < ACH_TYPE_MAX; i++)
		VECTOR_INIT(achievement_category[i]);
...
void do_final_achievement(void)
{
	...
	for (i = 0; i < ACH_TYPE_MAX; i++)
		VECTOR_CLEAR(achievement_category[i]);
