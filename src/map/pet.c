...
int pet_read_db_sub(struct config_setting_t *it, int n, const char *source)
{
	...
	if (libconfig->setting_lookup_string(it, "TamingItemID", &str)) {
	...
	if (libconfig->setting_lookup_string(it, "EggItemID", &str)) {
		if (!(data = itemdb->name2id(str))) {
			ShowWarning("pet_read_db_sub: Invalid item '%s' in pet %d of \"%s\", defaulting to 0.\n", str, pet->db[n].class_, source);
		} else {
	...
	if (libconfig->setting_lookup_string(it, "AccessoryItemID", &str)) {
		if (!(data = itemdb->name2id(str))) {
			ShowWarning("pet_read_db_sub: Invalid item '%s' in pet %d of \"%s\", defaulting to 0.\n", str, pet->db[n].class_, source);
		} else {
	...
	if (libconfig->setting_lookup_string(it, "FoodItemID", &str)) {
		if (!(data = itemdb->name2id(str))) {
			ShowWarning("pet_read_db_sub: Invalid item '%s' in pet %d of \"%s\", defaulting to 0.\n", str, pet->db[n].class_, source);
		} else {
	...
	if (libconfig->setting_lookup_int(it, "Fullness", &i32))
		pet->db[n].fullness = i32;

	if (libconfig->setting_lookup_int(it, "HungryDelay", &i32))
	...
	if ((t = libconfig->setting_get_member(it, "TalkConvertClass")) && (i32 = libconfig->setting_get_bool(t)))
		pet->db[n].talk_convert_class = i32;

	if (libconfig->setting_lookup_int(it, "AttackRate", &i32))
		pet->db[n].attack_rate = i32;

	if (libconfig->setting_lookup_int(it, "DefenceAttackRate", &i32))
...
bool pet_read_db_sub_intimacy(int idx, struct config_setting_t *t)
{
	...
	if (libconfig->setting_lookup_int(t, "Start", &i32))
		pet->db[idx].intimate = i32;

	if (libconfig->setting_lookup_int(t, "Feed", &i32))
		pet->db[idx].r_hungry = i32;

	if (libconfig->setting_lookup_int(t, "OverFeed", &i32))
		pet->db[idx].r_full = i32;

	if (libconfig->setting_lookup_int(t, "OwnerDie", &i32))
		pet->db[idx].die = i32;
