...
enum prevent_logout_trigger { PLT_NONE = 0x0, PLT_LOGIN = 0x1, PLT_ATTACK = 0x2, PLT_SKILL = 0x4, PLT_DAMAGE = 0x8 };
...
enum pc_checkitem_types { PCCHECKITEM_NONE = 0x0, PCCHECKITEM_INVENTORY = 0x1, PCCHECKITEM_CART = 0x2, PCCHECKITEM_STORAGE = 0x4, PCCHECKITEM_GSTORAGE = 0x8 };
...
#define pc_check_weapontype(sd, type) ((type) & ((sd)->weapontype < MAX_SINGLE_WEAPON_TYPE ? 1 << (sd)->weapontype : (1 << (sd)->weapontype1) | (1 << (sd)->weapontype2)))
...
// clientside display macros (values to the left/right of the "+")
...
#else
	#define pc_leftside_atk(sd) ((((sd)->battle_status.batk + (sd)->battle_status.rhw.atk + (sd)->battle_status.lhw.atk) * (sd)->battle_status.atk_percent) / 100)
	...
	#define pc_leftside_matk(sd) (((sd)->sc.data[SC_MAGICPOWER] && (sd)->sc.data[SC_MAGICPOWER]->val4) ? ((sd)->battle_status.matk_min * 100 + 50) / ((sd)->sc.data[SC_MAGICPOWER]->val3 + 100) : (sd)->battle_status.matk_min)
	#define pc_rightside_matk(sd) (((sd)->sc.data[SC_MAGICPOWER] && (sd)->sc.data[SC_MAGICPOWER]->val4) ? ((sd)->battle_status.matk_max * 100 + 50) / ((sd)->sc.data[SC_MAGICPOWER]->val3 + 100) : (sd)->battle_status.matk_max)
#endif
 ...
 * Exp types
 */
enum class_exp_type { CLASS_EXP_TABLE_BASE, CLASS_EXP_TABLE_JOB };
