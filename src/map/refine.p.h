...
struct refine_interface_private {
	...
	/**
	 * Processes a refine_db.conf requirements items entry.
	 *
	 ...
	 * @return true on success, false otherwise.
	 **/
	bool (*readdb_requirements_items) (const struct config_setting_t *elem, struct s_refine_requirement *req, const char *name, const char *source);

	/**
	 * Processes a refine_db.conf requirements entry.
	 *
	 * @param elem   Libconfig setting entry. It is expected to be valid and it
	 *               won't be freed (it is care of the caller to do so if
	 *               necessary)
	 ...
	 * @return true on success, false otherwise.
	 **/
	bool (*readdb_requirements_sub) (const struct config_setting_t *elem, int type, const char *name, const char *source);

	/**
	 * Reads a refine_db.conf requirements entry and sends it to be processed.
	 *
	 * @param r      Libconfig setting entry. It is expected to be valid and it
	 *               won't be freed (it is care of the caller to do so if
	 *               necessary)
	 ...
	 * @return true on success, false otherwise.
	 **/
	int (*readdb_requirements) (const struct config_setting_t *r, int type, const char *name, const char *source);
