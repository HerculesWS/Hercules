...
int mob_clone_spawn(struct map_session_data *sd, int16 m, int16 x, int16 y, const char *event, int master_id, int mode, int flag, unsigned int duration)
{
	...
	for (i=0,j = MAX_SKILL_TREE-1;j>=0 && i< MAX_MOBSKILL ;j--) {
		int idx = iPc->skill_tree[iPc->class2idx(sd->status.class_)][j].idx;
		skill_id = iPc->skill_tree[iPc->class2idx(sd->status.class_)][j].id;
...
static int mob_read_sqldb(void)
{
	...
	for( fi = 0; fi < ARRAYLENGTH(mob_db_name); ++fi ) {
		...
		if( SQL_ERROR == SQL->Query(iMap->mmysql_handle, "SELECT * FROM `%s`", mob_db_name[fi]) ) {
			Sql_ShowDebug(iMap->mmysql_handle);
			continue;
		}

		// process rows one by one
		while( SQL_SUCCESS == SQL->NextRow(iMap->mmysql_handle) ) {
			...
			for(i = 0, p = line; i < 31+2*MAX_MVP_DROP+2*MAX_MOB_DROP; i++)
			{
				char* data;
				size_t len;
				SQL->GetData(iMap->mmysql_handle, i, &data, &len);
...
static int mob_read_sqlskilldb(void)
{
	...
	for( fi = 0; fi < ARRAYLENGTH(mob_skill_db_name); ++fi ) {
		...
		if( SQL_ERROR == SQL->Query(iMap->mmysql_handle, "SELECT * FROM `%s`", mob_skill_db_name[fi]) ) {
			Sql_ShowDebug(iMap->mmysql_handle);
			continue;
		}
