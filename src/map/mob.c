					   ...
					   const char *event, unsigned int size, unsigned int ai, int npc_id)
{
	...
	if (bl != NULL && (x < 0 || y < 0))
		map->search_freecell(bl, m, &x, &y, 1, 1, SFC_DEFAULT);

	/** If none found, pick random position on map. **/
	if (x <= 0 || x >= map->list[m].xs || y <= 0 || y >= map->list[m].ys)
		map->search_freecell(NULL, m, &x, &y, -1, -1, SFC_XY_CENTER);
...
static int mob_warpslave_sub(struct block_list *bl, va_list ap)
{
	...
	map->search_freecell(master, 0, &x, &y, range, range, SFC_DEFAULT);
...
static int mob_summonslave(struct mob_data *md2, int *value, int amount, uint16 skill_id)
{
	...
	for (; k < amount; k++) {
		...
		short x;
		short y;
		map->search_freecell(&md2->bl, 0, &x, &y, MOB_SLAVEDISTANCE, MOB_SLAVEDISTANCE, SFC_DEFAULT);
		data.x = x;
		data.y = y;
	...
	for (int i = 0; i < md->db->maxskill; skill_idx++, i++) {
		...
		if (skill->get_casttype(ms[skill_idx].skill_id) == CAST_GROUND) { // Ground skill.
			...
			if (target_type >= MST_AROUND5 && target_type <= MST_AROUND) {
				...
				map->search_freecell(&md->bl, md->bl.m, &x, &y, range, range, (SFC_XY_CENTER | SFC_REACHABLE));
			}
