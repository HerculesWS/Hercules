...
BUILDIN(query_sql) {
	return buildin_query_sql_sub(st, iMap->mmysql_handle);
}
...
BUILDIN(query_logsql) {
	...
	return buildin_query_sql_sub(st, iMap->logmysql_handle);
}
...
BUILDIN(escape_sql)
{
	...
	SQL->EscapeStringLen(iMap->mmysql_handle, esc_str, str, len);
...
BUILDIN(instance_destroy)
{
	...
	else if( (sd = script_rid2sd(st)) != NULL && sd->status.party_id && (p = iParty->search(sd->status.party_id)) != NULL && p->instance_id )
...
BUILDIN(instance_detachmap)
{
	...
	if( (m = iMap->mapname2mapid(str)) < 0 || (m = instance_map2imap(m,instance_id)) < 0 )
	{
...
BUILDIN(instance_id)
{
	...
	if( script_hasdata(st, 2) )
	{
		...
		if( type == 0 )
			instance_id = st->instance_id;
		else if( type == 1 && (sd = script_rid2sd(st)) != NULL && sd->status.party_id && (p = iParty->search(sd->status.party_id)) != NULL )
			instance_id = p->instance_id;
		else
			instance_id = 0;
	}
...
BUILDIN(instance_announce)
{
	...
	for( i = 0; i < instance[instance_id].num_map; i++ )
		iMap->foreachinmap(buildin_announce_sub, instance[instance_id].map[i], BL_PC,
						 mes, strlen(mes)+1, flag&0xf0, fontColor, fontType, fontSize, fontAlign, fontY);
...
BUILDIN(has_instance)
{
	...
	if( !instance_id || (m = iMap->mapname2mapid(str)) < 0 || (m = instance_map2imap(m, instance_id)) < 0 )
	{
		script_pushconststr(st, "");
		return true;
	}
...
BUILDIN(instance_warpall)
{
	...
	if( script_hasdata(st,5) )
		...
		instance_id = st->instance_id;
	else if( (pl_sd = script_rid2sd(st)) != NULL && pl_sd->status.party_id && (p = iParty->search(pl_sd->status.party_id)) != NULL && p->instance_id )
		instance_id = p->instance_id;
	else return true;
	
	if( (m = iMap->mapname2mapid(mapn)) < 0 || (map[m].flag.src4instance && (m = instance_mapid2imapid(m, instance_id)) < 0) )
		return true;
	
	if( !(p = iParty->search(instance[instance_id].party_id)) )
		return true;
	
	mapindex = map_id2index(m);
	for( i = 0; i < MAX_PARTY; i++ )
		if( (pl_sd = p->data[i].sd) && map[pl_sd->bl.m].instance_id == st->instance_id ) iPc->setpos(pl_sd,mapindex,x,y,CLR_TELEPORT);
