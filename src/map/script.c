...
static BUILDIN(getpetinfo)
{
	...
	case PETINFO_INTIMACY:      script_pushint(st, pd->pet.intimate); break;
	case PETINFO_HUNGRY:        script_pushint(st, pd->pet.hungry); break;
	case PETINFO_RENAME:        script_pushint(st, pd->pet.rename_flag); break;
	case PETINFO_GID:           script_pushint(st, pd->bl.id); break;
	case PETINFO_EGGITEM:       script_pushint(st, pd->pet.egg_id); break;
	case PETINFO_FOODITEM:      script_pushint(st, pd->petDB->FoodID); break;
	case PETINFO_ACCESSORYITEM: script_pushint(st, pd->petDB->AcceID); break;
	case PETINFO_ACCESSORYFLAG: script_pushint(st, (pd->pet.equip != 0)? 1:0); break;
	case PETINFO_AUTOFEED:      script_pushint(st, pd->pet.autofeed); break;
...
static void script_parse_builtin(void)
{
	struct script_function BUILDIN[] = {
		...
		BUILDIN_DEF(setd,"sv"),
		// <--- [zBuffer] List of dynamic var commands
		BUILDIN_DEF_DEPRECATED(petstat, "i"), // Deprecated 2019-03-02
