...
static const char *parse_callfunc(const char *p, int require_paren, int is_custom)
{
	...
	if (*p == '"') {
		...
		if (p[nlen] == ':' && p[nlen + 1] == ':') {
			...
			p += nlen + flen; // skip to just before the ()
			aFree(npc_name);
			aFree(func_name);
		} else {
			...
			{// function <name> <line/block of code>
				...
				if( script->str_data[l].type == C_NOP || script->str_data[l].type == C_USERFUNC )// register only, if the name was not used by something else
				{
					...
					if( script->parse_options&SCRIPT_USE_LABEL_DB )
						script->label_add(l, VECTOR_LENGTH(script->buf), LABEL_IS_USERFUNC |
							(is_extern ? LABEL_IS_EXTERN : 0));
				}
				else
					disp_error_message("parse_syntax:function: function name is invalid", func_name);
...
static BUILDIN(foreigncall) {
	...
	for (i = 0; i < nd->u.scr.label_list_num; ++i) {
		if (strcmp(nd->u.scr.label_list[i].name, function_name) == 0) {
			...
			} else if (nd->u.scr.label_list[i].flags & LABEL_IS_USERFUNC) {
