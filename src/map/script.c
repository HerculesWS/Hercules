...
BUILDIN(beast_catch)
{
	...
	if (script_hasdata(st, 2))
		chance = script_getnum(st, 2);

	sd->beast_item_chance = chance;
	sd->beast_catch_flag = BEAST_UNIQUE_ID;
	pet->catch_process1(sd, BEAST_UNIQUE_ID);
	return true;
}

BUILDIN(beast_getinventorylist)
{
	struct map_session_data *sd = script->rid2sd(st);
	char card_var[NAME_LENGTH];
	...
	for (i = 0; i<MAX_INVENTORY; i++) {
		if (sd->status.inventory[i].nameid > 0 && sd->status.inventory[i].amount > 0 && sd->status.inventory[i].unique_id > BEAST_UNIQUE_ID ) {
			pc->setreg(sd, reference_uid(script->add_str("@b_inventorylist_id"), j), sd->status.inventory[i].nameid);
			pc->setreg(sd, reference_uid(script->add_str("@b_inventorylist_amount"), j), sd->status.inventory[i].amount);
			for (k = 0; k < MAX_SLOTS; k++) {
				sprintf(card_var, "@b_inventorylist_card%d", k + 1);
				pc->setreg(sd, reference_uid(script->add_str(card_var), j), sd->status.inventory[i].card[k]);
			}

			pc->setreg(sd, reference_uid(script->add_str("@b_inventorylist_unique_id"), j), (int)sd->status.inventory[i].unique_id);
			j++;
		}
	}
	pc->setreg(sd, script->add_str("@b_inventorylist_count"), j);
	return true;
}

BUILDIN(beast_revive)
{
	struct map_session_data *sd = script->rid2sd(st);
...
BUILDIN(beast_heal)
{
	...
	int unique_id, percent_hp, percent_sp, hp = 0, sp = 0;

	percent_hp = script_getnum(st, 2);
	percent_sp = script_getnum(st, 3);
	...
	if (script_hasdata(st, 4))
	{
		...
		script_pushint(st, homunculus_beast_heal((unique_id - BEAST_UNIQUE_ID), percent_hp, percent_sp));
		return true;
	}
	...
	if (!sd->hd || !sd->status.hom_id)
		...
		hp = APPLY_RATE(sd->hd->homunculus.max_hp, percent_hp);
		sp = APPLY_RATE(sd->hd->homunculus.max_sp, percent_sp);
...
void script_parse_builtin(void) {
	struct script_function BUILDIN[] = {
		...
		//[CreativeSD]: Beast System
		BUILDIN_DEF(beast_catch, "?"),
		BUILDIN_DEF(beast_getinventorylist, ""),
		BUILDIN_DEF(beast_revive, "i"),
		BUILDIN_DEF(beast_heal, "ii?"),
