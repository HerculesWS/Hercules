...
static void heap_push_node(struct node_heap *heap, struct path_node *node)
{
	...
	BHEAP_PUSH2(*heap, node, NODE_MINTOPCMP, HSWAP_PTR);
}
...
static int heap_update_node(struct node_heap *heap, struct path_node *node)
{
	...
	BHEAP_UPDATE(*heap, i, NODE_MINTOPCMP, HSWAP_PTR);
	return 0;
}
...
static bool path_search(struct walkpath_data *wpd, struct block_list *bl, int16 m, int16 x0, int16 y0, int16 x1, int16 y1, int flag, cell_chk cell)
{
	...
	for(;;) {
		...
		BHEAP_POP2(open_set, NODE_MINTOPCMP, HSWAP_PTR); // Remove it from 'open' set
