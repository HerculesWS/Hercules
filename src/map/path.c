...
int path_blownpos(int16 m,int16 x0,int16 y0,int16 dx,int16 dy,int count)
{
	...
	{
		if( !iMap->getcellp(md,x0+dx,y0+dy,CELL_CHKPASS) )
		{// attempt partial movement
			int fx = ( dx != 0 && iMap->getcellp(md,x0+dx,y0,CELL_CHKPASS) );
			int fy = ( dy != 0 && iMap->getcellp(md,x0,y0+dy,CELL_CHKPASS) );
			if( fx && fy )
			{
				if(rnd()&1)
					dx=0;
...
bool path_search_long(struct shootpath_data *spd,int16 m,int16 x0,int16 y0,int16 x1,int16 y1,cell_chk cell)
{
	...
	if (iMap->getcellp(md,x1,y1,cell))
		return false;
	...
	while (x0 != x1 || y0 != y1)
	{
		if (iMap->getcellp(md,x0,y0,cell))
			return false;
		wx += dx;
		wy += dy;
		if (wx >= weight) {
...
#else
	if( x0 < 0 || x0 >= md->xs || y0 < 0 || y0 >= md->ys /*|| iMap->getcellp(md,x0,y0,cell)*/ )
#endif
		return false;
	if( x1 < 0 || x1 >= md->xs || y1 < 0 || y1 >= md->ys || iMap->getcellp(md,x1,y1,cell) )
		return false;
