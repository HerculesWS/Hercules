...
ACMD(send)
{
	...
	if (!*message || !((sscanf(message, "len %x", (unsigned int *)&type) == 1 && (len = 1, true)) || sscanf(message, "%x", (unsigned int *)&type) == 1)) {
...
ACMD(mapmove)
{
	...
	if (!*message || (sscanf(message, "%15s %5hd %5hd", map_name, &x, &y) < 3 && sscanf(message, "%15[^,],%5hd,%5hd", map_name, &x, &y) < 1)) {
...
ACMD(where)
{
	...
	if (pl_sd == NULL || strncmp(pl_sd->status.name, atcmd_player_name, NAME_LENGTH) != 0 || (pc_has_permission(pl_sd, PC_PERM_HIDE_SESSION) && pc_get_group_level(pl_sd) > pc_get_group_level(sd) && !pc_has_permission(sd, PC_PERM_WHO_DISPLAY_AID))) {
...
ACMD(whogm)
{
	...
	for (pl_sd = BL_UCCAST(BL_PC, mapit->first(iter)); mapit->exists(iter); pl_sd = BL_UCCAST(BL_PC, mapit->next(iter))) {
		...
		snprintf(atcmd_output, sizeof(atcmd_output), msg_fd(fd, MSGTBL_WHOGM_BLEVEL_JLEVEL), // BLvl: %d | Job: %s (Lvl: %d)
		         pl_sd->status.base_level, pc->job_name(pl_sd->status.class), pl_sd->status.job_level);
		clif->message(fd, atcmd_output);
...
ACMD(option)
{
	...
	if (!*message || sscanf(message, "%12d %12d %12d", &param1, &param2, &param3) < 1 || param1 < 0 || param2 < 0 || param3 < 0) { // failed to match the parameters so inform the user of the options
		const char *text;
...
ACMD(jobchange)
{
	...
	if (job == JOB_KNIGHT2 || job == JOB_CRUSADER2 || job == JOB_WEDDING || job == JOB_XMAS || job == JOB_SUMMER || job == JOB_LORD_KNIGHT2 || job == JOB_PALADIN2 || job == JOB_BABY_KNIGHT2 || job == JOB_BABY_CRUSADER2 || job == JOB_STAR_GLADIATOR2 || job == JOB_BABY_STAR_GLADIATOR2 || (job >= JOB_RUNE_KNIGHT2 && job <= JOB_MECHANIC_T2) || (job >= JOB_BABY_RUNE2 && job <= JOB_BABY_MECHANIC2)) {
...
ACMD(item)
{
	...
	if (!strcmpi(info->command, "itembound") && (!*message || (sscanf(message, "\"%99[^\"]\" %12d %12d", item_name, &number, &bound) < 1 && sscanf(message, "%99s %12d %12d", item_name, &number, &bound) < 1))) {
		...
		return false;
	} else if (!*message || (sscanf(message, "\"%99[^\"]\" %12d", item_name, &number) < 1 && sscanf(message, "%99s %12d", item_name, &number) < 1)) {
...
ACMD(item2)
{
	...
	if (!strcmpi(info->command, "itembound2") && (!*message || (sscanf(message, "\"%99[^\"]\" %12d %12d %12d %12d %12d %12d %12d %12d %12d", item_name, &number, &identify, &refine_level, &attr, &c1, &c2, &c3, &c4, &bound) < 10 && sscanf(message, "%99s %12d %12d %12d %12d %12d %12d %12d %12d %12d", item_name, &number, &identify, &refine_level, &attr, &c1, &c2, &c3, &c4, &bound) < 10))) {
		...
		return false;
	} else if (!*message || (sscanf(message, "\"%99[^\"]\" %12d %12d %12d %12d %12d %12d %12d %12d", item_name, &number, &identify, &refine_level, &attr, &c1, &c2, &c3, &c4) < 1 && sscanf(message, "%99s %12d %12d %12d %12d %12d %12d %12d %12d", item_name, &number, &identify, &refine_level, &attr, &c1, &c2, &c3, &c4) < 1)) {
...
ACMD(model)
{
	...
	if (hair_style >= MIN_HAIR_STYLE && hair_style <= MAX_HAIR_STYLE && hair_color >= MIN_HAIR_COLOR && hair_color <= MAX_HAIR_COLOR && cloth_color >= MIN_CLOTH_COLOR && cloth_color <= MAX_CLOTH_COLOR) {
...
ACMD(monster)
{
	...
	if (sscanf(message, "\"%23[^\"]\" %23s %12d", name, monster, &number) > 1 || sscanf(message, "%23s \"%23[^\"]\" %12d", monster, name, &number) > 1) {
...
ACMD(produce)
{
	...
	if (!*message || (sscanf(message, "\"%99[^\"]\" %12d %12d", item_name, &attribute, &star) < 1 && sscanf(message, "%99s %12d %12d", item_name, &attribute, &star) < 1)) {
...
ACMD(gat)
{
	...
	for (y = 2; y >= -2; y--) {
		snprintf(atcmd_output, sizeof(atcmd_output), "%s (x= %d, y= %d) %02X %02X %02X %02X %02X", map->list[sd->bl.m].name, sd->bl.x - 2, sd->bl.y + y, (unsigned int)map->getcell(sd->bl.m, &sd->bl, sd->bl.x - 2, sd->bl.y + y, CELL_GETTYPE), (unsigned int)map->getcell(sd->bl.m, &sd->bl, sd->bl.x - 1, sd->bl.y + y, CELL_GETTYPE), (unsigned int)map->getcell(sd->bl.m, &sd->bl, sd->bl.x, sd->bl.y + y, CELL_GETTYPE), (unsigned int)map->getcell(sd->bl.m, &sd->bl, sd->bl.x + 1, sd->bl.y + y, CELL_GETTYPE), (unsigned int)map->getcell(sd->bl.m, &sd->bl, sd->bl.x + 2, sd->bl.y + y, CELL_GETTYPE));
...
ACMD(petrename)
{
	...
	ARR_FIND(0, sd->status.inventorySize, i, sd->status.inventory[i].card[0] == CARD0_PET && pd->pet.pet_id == MakeDWord(sd->status.inventory[i].card[1], sd->status.inventory[i].card[2]));
...
ACMD(char_ban)
{
	...
	chrif->char_ask_name(sd->status.account_id, atcmd_player_name, !strcmpi(info->command, "charban") ? CHAR_ASK_NAME_CHARBAN : CHAR_ASK_NAME_BAN, year, month, day, hour, minute, second);
	clif->message(fd, msg_fd(fd, MSGTBL_SENDING_REQUEST_TO_LOGIN)); // Character name sent to char-server to ask it.
...
ACMD(char_unban)
{
	...
	// send answer to login server via char-server
	chrif->char_ask_name(sd->status.account_id, atcmd_player_name, !strcmpi(info->command, "charunban") ? CHAR_ASK_NAME_CHARUNBAN : CHAR_ASK_NAME_UNBAN, 0, 0, 0, 0, 0, 0);
	clif->message(fd, msg_fd(fd, MSGTBL_SENDING_REQUEST_TO_LOGIN)); // Character name sent to char-server to ask it.
...
ACMD(mapinfo)
{
	...
	snprintf(atcmd_output, sizeof(atcmd_output), msg_fd(fd, MSGTBL_MAPINFO_NO_PENALTY),                                                                                                                                              // No Exp Penalty: %s | No Zeny Penalty: %s
	         (map->list[m_id].flag.noexppenalty != 0) ? msg_fd(fd, MSGTBL_MAPINFO_ON) : msg_fd(fd, MSGTBL_MAPINFO_OFF), (map->list[m_id].flag.nozenypenalty != 0) ? msg_fd(fd, MSGTBL_MAPINFO_ON) : msg_fd(fd, MSGTBL_MAPINFO_OFF)); // On / Off
	clif->message(fd, atcmd_output);
...
ACMD(useskill)
{
	...
	if (skill_id >= HM_SKILLBASE && skill_id < HM_SKILLBASE + MAX_HOMUNSKILL && homun_alive(sd->hd)) // (If used with @useskill, put the homunc as dest)
		bl = &sd->hd->bl;
	else
		bl = &sd->bl;
...
ACMD(autotrade)
{
	...
	if (battle_config.at_timeout) {
		...
		status->change_start(NULL, &sd->bl, SC_AUTOTRADE, 10000, 0, 0, 0, 0, ((timeout > 0) ? min(timeout, battle_config.at_timeout) : battle_config.at_timeout) * 60000, SCFLAG_NONE, 0);
	}
...
ACMD(mobinfo)
{
	...
	for (k = 0; k < count; k++) {
		...
		snprintf(atcmd_output, sizeof(atcmd_output), msg_fd(fd, MSGTBL_MOBINFO_ATTRIBUTES), //  DEF:%d  MDEF:%d  STR:%d  AGI:%d  VIT:%d  INT:%d  DEX:%d  LUK:%d
		         monster->status.def, monster->status.mdef, monster->status.str, monster->status.agi, monster->status.vit, monster->status.int_, monster->status.dex, monster->status.luk);
		clif->message(fd, atcmd_output);
		...
		snprintf(atcmd_output, sizeof(atcmd_output), msg_fd(fd, MSGTBL_MOBINFO_ADDITIONAL_INFO), //  ATK : %d~%d MATK : %d~%d Range : %d~%d~%d  Size : %s  Race : %s  Element : %s(Lv : %d)
		         MOB_ATK1(monster), MOB_ATK2(monster), MOB_MATK1(monster), MOB_MATK2(monster), monster->status.rhw.range, monster->range2, monster->range3, msize[monster->status.size], mrace[monster->status.race], melement[monster->status.def_ele], monster->status.ele_lv);
#else
		snprintf(atcmd_output, sizeof(atcmd_output), msg_fd(fd, MSGTBL_MOBINFO_ATTACK), //  ATK:%d~%d  Range:%d~%d~%d  Size:%s  Race: %s  Element: %s (Lv:%d)
		         monster->status.rhw.atk, monster->status.rhw.atk2, monster->status.rhw.range, monster->range2, monster->range3, msize[monster->status.size], mrace[monster->status.race], melement[monster->status.def_ele], monster->status.ele_lv);
#endif
		clif->message(fd, atcmd_output);
...
ACMD(hominfo)
{
	...
	snprintf(atcmd_output, sizeof(atcmd_output), msg_fd(fd, MSGTBL_HOMINFO_STATS_DETAIL), // Stats: Str %d / Agi %d / Vit %d / Int %d / Dex %d / Luk %d
...
ACMD(homstats)
{
	...
	snprintf(atcmd_output, sizeof(atcmd_output), msg_fd(fd, MSGTBL_HOMSTATS_GROWTH), lv, db->name); // Homunculus growth stats (Lv %d %s):
	clif->message(fd, atcmd_output);
	...
	for (i = 0; i < count; i++) {
		...
		if (item_data != NULL) {
			...
			snprintf(atcmd_output, sizeof(atcmd_output), msg_fd(fd, MSGTBL_ITEMINFO_DETAILS),                                                                                                                             // Item: '%s'/'%s' (%d) Type: %s | Extra Effect: %s
			         item_data->name, StrBuf->Value(&buf), item_data->nameid, itemdb->typename(item_data->type), (item_data->script == NULL) ? msg_fd(fd, MSGTBL_ITEMINFO_NONE) : msg_fd(fd, MSGTBL_ITEMINFO_WITH_SCRIPT) // None / With script
			);
			StrBuf->Clear(&buf);
			clif->message(fd, atcmd_output);
...
ACMD(mutearea)
{
	...
	map->foreachinarea(atcommand->mutearea_sub, sd->bl.m, sd->bl.x - AREA_SIZE, sd->bl.y - AREA_SIZE, sd->bl.x + AREA_SIZE, sd->bl.y + AREA_SIZE, BL_PC, sd->bl.id, time);
...
ACMD(accept)
{
	...
	if (duel->list[sd->duel_invite].max_players_limit > 0 && duel->list[sd->duel_invite].members_count >= duel->list[sd->duel_invite].max_players_limit) {
		// "Duel: Limit of players is reached."
		clif->message(fd, msg_fd(fd, MSGTBL_DUEL_LIMIT_REACHED));
		return false;
	}
...
ACMD(stats)
{
	...
	struct {
		const char *format;
		int value;
	} output_table[] = {{"Base Level - %d", 0}, {NULL, 0}, {"Hp - %d", 0}, {"MaxHp - %d", 0}, {"Sp - %d", 0}, {"MaxSp - %d", 0}, {"Str - %3d", 0}, {"Agi - %3d", 0}, {"Vit - %3d", 0}, {"Int - %3d", 0}, {"Dex - %3d", 0}, {"Luk - %3d", 0}, {"Zeny - %d", 0}, {"Free SK Points - %d", 0}, {"JobChangeLvl (2nd) - %d", 0}, {"JobChangeLvl (3rd) - %d", 0}, {NULL, 0}};
...
static void atcommand_commands_sub(struct map_session_data *sd, const int fd, AtCommandType type)
{
	...
	if (atcommand->binding_count > 0) {
		...
		for (i = 0; i < atcommand->binding_count; i++) {
			if (gm_lvl >= ((type == COMMAND_ATCOMMAND) ? atcommand->binding[i]->group_lv : atcommand->binding[i]->group_lv_char) || (type == COMMAND_ATCOMMAND && atcommand->binding[i]->at_groups[pcg->get_idx(sd->group)] > 0) || (type == COMMAND_CHARCOMMAND && atcommand->binding[i]->char_groups[pcg->get_idx(sd->group)] > 0)) {
...
static bool atcommand_exec(const int fd, struct map_session_data *sd, const char *message, bool player_invoked)
{
	...
	if (player_invoked && atcommand->binding_count > 0) {
		...
		if (binding != NULL && binding->npc_event[0] != '\0' && ((is_atcommand && pc_get_group_level(sd) >= binding->group_lv) || (!is_atcommand && pc_get_group_level(sd) >= binding->group_lv_char) || (is_atcommand && binding->at_groups[pcg->get_idx(sd->group)] > 0) || (!is_atcommand && binding->char_groups[pcg->get_idx(sd->group)] > 0))) {
	...
	if (player_invoked) {
		int i;
		if ((is_atcommand && info->at_groups[pcg->get_idx(sd->group)] == 0) || (!is_atcommand && info->char_groups[pcg->get_idx(sd->group)] == 0))
			return false;
		...
		for (i = 0; i < sz; i++) {
			...
			if (commands != NULL) {
				...
				if (config_setting_lookup_bool(commands, atcmd->command, &result) && result) {
					...
					// <commandname> : [ <bool>, <bool> ] ([ atcommand, charcommand ])
					if ((cmd = config_setting_get_member(commands, atcmd->command)) != NULL && config_setting_is_aggregate(cmd) && config_setting_length(cmd) == 2) {
...
static bool atcommand_can_use(struct map_session_data *sd, const char *command)
{
	...
	if ((acmd_d = atcommand->get_info_byname(atcommand->check_alias(command + 1))) != NULL) {
		return ((*command == atcommand->at_symbol && acmd_d->at_groups[pcg->get_idx(sd->group)] > 0) || (*command == atcommand->char_symbol && acmd_d->char_groups[pcg->get_idx(sd->group)] > 0));
	} else if ((bcmd_d = atcommand->get_bind_byname(atcommand->check_alias(command + 1))) != NULL) {
		return ((*command == atcommand->at_symbol && bcmd_d->at_groups[pcg->get_idx(sd->group)] > 0) || (*command == atcommand->char_symbol && bcmd_d->char_groups[pcg->get_idx(sd->group)] > 0));
	}
...
static bool atcommand_can_use2(struct map_session_data *sd, const char *command, AtCommandType type)
{
	...
	if ((acmd_d = atcommand->get_info_byname(atcommand->check_alias(command))) != NULL) {
		return ((type == COMMAND_ATCOMMAND && acmd_d->at_groups[pcg->get_idx(sd->group)] > 0) || (type == COMMAND_CHARCOMMAND && acmd_d->char_groups[pcg->get_idx(sd->group)] > 0));
	} else if ((bcmd_d = atcommand->get_bind_byname(atcommand->check_alias(command))) != NULL) {
		return ((type == COMMAND_ATCOMMAND && bcmd_d->at_groups[pcg->get_idx(sd->group)] > 0) || (type == COMMAND_CHARCOMMAND && bcmd_d->char_groups[pcg->get_idx(sd->group)] > 0));
	}
