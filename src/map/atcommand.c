...
ACMD(mapmove)
{
	...
	if ((x || y) && map_getcell(m, x, y, CELL_CHKNOPASS) && pc->get_group_level(sd) < battle_config.gm_ignore_warpable_area)
...
/*==========================================
 * @reloadpcdb - reloads exp.txt pc->skill_tree.txt attr_fix.txt statpoint.txt
 *------------------------------------------*/
ACMD(reloadpcdb)
{
	pc->readdb();
...
ACMD(servertime)
{
	...
	if (battle_config.night_duration == 0 && battle_config.day_duration == 0) {
		...
		if (night_flag == 1) { // we start with night
			timer_data = get_timer(pc->day_timer_tid);
			...
			if (night_flag == 0) { // we start with day
				timer_data = get_timer(pc->night_timer_tid);
				...
				if (night_flag == 0) {
					timer_data = get_timer(pc->night_timer_tid);
					timer_data2 = get_timer(pc->day_timer_tid);
					sprintf(temp, msg_txt(235), txt_time(DIFF_TICK(timer_data->tick,gettick())/1000)); // Game time: The game is actualy in daylight for %s.
					clif->message(fd, temp);
					...
					timer_data = get_timer(pc->day_timer_tid);
					timer_data2 = get_timer(pc->night_timer_tid);
					sprintf(temp, msg_txt(233), txt_time(DIFF_TICK(timer_data->tick,gettick()) / 1000)); // Game time: The game is actualy in night for %s.
					clif->message(fd, temp);
					if (DIFF_TICK(timer_data->tick,timer_data2->tick) > 0)
...
ACMD(changeleader)
{
	...
	if (party->changeleader(sd, map_nick2sd((char *) message)))
		return true;
	return false;
}
