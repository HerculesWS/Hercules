...
/*==========================================
 * @reloadpcdb - reloads exp.txt iPc->skill_tree.txt attr_fix.txt statpoint.txt
 *------------------------------------------*/
ACMD(reloadpcdb)
{
	iPc->readdb();
...
ACMD(servertime)
{
	...
	if (battle_config.night_duration == 0 && battle_config.day_duration == 0) {
		...
		if (iMap->night_flag == 1) { // we start with night
			timer_data = get_timer(iPc->day_timer_tid);
			...
			if (iMap->night_flag == 0) { // we start with day
				timer_data = get_timer(iPc->night_timer_tid);
				...
				if (iMap->night_flag == 0) {
					timer_data = get_timer(iPc->night_timer_tid);
					timer_data2 = get_timer(iPc->day_timer_tid);
					sprintf(temp, msg_txt(235), txt_time(DIFF_TICK(timer_data->tick,gettick())/1000)); // Game time: The game is actualy in daylight for %s.
					clif->message(fd, temp);
					...
					timer_data = get_timer(iPc->day_timer_tid);
					timer_data2 = get_timer(iPc->night_timer_tid);
					sprintf(temp, msg_txt(233), txt_time(DIFF_TICK(timer_data->tick,gettick()) / 1000)); // Game time: The game is actualy in night for %s.
					clif->message(fd, temp);
					if (DIFF_TICK(timer_data->tick,timer_data2->tick) > 0)
...
ACMD(skilltree)
{
	...
	ARR_FIND( 0, MAX_SKILL_TREE, j, iPc->skill_tree[c][j].id == 0 || iPc->skill_tree[c][j].id == skill_id );
	if( j == MAX_SKILL_TREE || iPc->skill_tree[c][j].id == 0 )
	{
		clif->message(fd, msg_txt(1169)); // The player cannot use that skill.
		return true;
	}
	
	ent = &iPc->skill_tree[c][j];
