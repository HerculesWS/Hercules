...
ACMD(channel)
{
	...
	if (strcmpi(subcmd,"create") == 0 && (channel->config->allow_user_channel_creation || pc_has_permission(sd, PC_PERM_HCHSYS_ADMIN))) {
		...
		if (sub3[0] == '\0') {
			...
			int v = atoi(sub3);
			if (k == HCS_OPT_MSG_DELAY) {
				if (v < 0 || v > battle->bc->channel_opt_msg_delay) {
					safesnprintf(atcmd_output, sizeof(atcmd_output), msg_fd(fd,1451), v, opt_str[k], battle->bc->channel_opt_msg_delay);// value '%d' for option '%s' is out of range (limit is 0-%d)
					clif->message(fd, atcmd_output);
					return false;
				}
				if (v == 0) {
