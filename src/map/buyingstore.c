...
bool buyingstore_setup(struct map_session_data* sd, unsigned char slots)
{
	...
	{
		ShowWarning("%s: Requested %d slots, but server supports only %d slots.\n", __func__, (int)slots, MAX_BUYINGSTORE_SLOTS);
...
void buyingstore_create(struct map_session_data* sd, int zenylimit, unsigned char result, const char* storename, const uint8* itemlist, unsigned int count)
{
	...
	{// itemlist: <name id>.W <amount>.W <price>.L
		...
		{// duplicate check. as the client does this too, only malicious intent should be caught here
			...
			if( listidx != i )
			{// duplicate
				ShowWarning("%s: Found duplicate item on buying list (nameid=%hu, amount=%hu, account_id=%d, char_id=%d).\n", __func__, nameid, amount, sd->status.account_id, sd->status.char_id);
				break;
			}
		}
...
void buyingstore_trade(struct map_session_data* sd, int account_id, unsigned int buyer_id, const uint8* itemlist, unsigned int count)
{
	...
	{// itemlist: <index>.W <name id>.W <amount>.W
		...
		{// duplicate check. as the client does this too, only malicious intent should be caught here
			...
			if( k != i )
			{// duplicate
				ShowWarning("%s: Found duplicate item on selling list (prevnameid=%hu, prevamount=%hu, nameid=%hu, amount=%hu, account_id=%d, char_id=%d).\n",
					__func__, RBUFW(itemlist,k*6+2), RBUFW(itemlist,k*6+4), nameid, amount, sd->status.account_id, sd->status.char_id);
