...
static int itemdb_read_sqldb(void) {
	...
	for( fi = 0; fi < ARRAYLENGTH(item_db_name); ++fi ) {
		...
		if( SQL_ERROR == SQL->Query(iMap->mmysql_handle, "SELECT * FROM `%s`", item_db_name[fi]) ) {
			Sql_ShowDebug(iMap->mmysql_handle);
			continue;
		}

		// process rows one by one
		while( SQL_SUCCESS == SQL->NextRow(iMap->mmysql_handle) ) {// wrap the result into a TXT-compatible format
			char* str[22];
			char* dummy = "";
			int i;
			++lines;
			for( i = 0; i < 22; ++i ) {
				SQL->GetData(iMap->mmysql_handle, i, &str[i], NULL);
				if( str[i] == NULL )
		...
		SQL->FreeResult(iMap->mmysql_handle);
...
int itemdb_uid_load(){
	...
	char * uid;
	if (SQL_ERROR == SQL->Query(iMap->mmysql_handle, "SELECT `value` FROM `interreg` WHERE `varname`='unique_id'"))
		Sql_ShowDebug(iMap->mmysql_handle);

	if( SQL_SUCCESS != SQL->NextRow(iMap->mmysql_handle) )
	{
	...
	SQL->GetData(iMap->mmysql_handle, 0, &uid, NULL);
