...
static const char *itemdb_typename(enum item_types type)
{
	switch (type) {
		...
		case IT_SELECTPACKAGE:
			return "Selection Package Usable";
		case IT_CASH:
			return "Cash Usable";
		case IT_UNKNOWN2:
		case IT_MAX:
...
static int itemdb_validate_entry(struct item_data *entry, int n, const char *source)
{
	...
	if (entry->type < 0 || entry->type == IT_UNKNOWN || entry->type == IT_UNKNOWN2 || (entry->type > IT_SELECTPACKAGE && entry->type < IT_CASH) || entry->type >= IT_MAX) {
...
static void itemdb_item_reform(struct map_session_data *sd, const struct item_reform *ir, int idx)
{
	...
	if ((itemdb_type(itr->nameid) == IT_ARMOR || itemdb_type(itr->nameid) == IT_WEAPON) && (itr->refine < ir->NeedRefineMin || itr->refine > ir->NeedRefineMax))
		return;
