...
static bool itemdb_read_libconfig_lapineddukddak(void)
{
	...
	if (!libconfig->load_file(&item_lapineddukddak, config_filename))
		return false;

	while ((it = libconfig->setting_get_elem(item_lapineddukddak.root, i++))) {
...
static bool itemdb_read_libconfig_lapineddukddak_sub(struct config_setting_t *it, const char *source)
{
	...
	if (libconfig->setting_lookup_int(it, "NeedCount", &i32))
		data->lapineddukddak->NeedCount = (int16)i32;

	if (libconfig->setting_lookup_int(it, "NeedRefineMin", &i32))
...
static bool itemdb_read_libconfig_lapineddukddak_sub_sources(struct config_setting_t *sources, struct item_data *data)
{
	...
	while ((entry = libconfig->setting_get_elem(sources, i++))) {
		...
		if ((i32 = libconfig->setting_get_int(entry)) && (i32 <= 0 || i32 > MAX_AMOUNT)) {
