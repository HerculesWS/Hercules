...
const char* itemdb_typename(int type)
{
 ...
 /**
 * Converts the JobID to the format used by map-server to check
 * item restriction as per job
...
/**
 * Converts the JobMask to the format used by map-server to check
 * item restriction as per job
 * @param *bclass  Pointer to the variable containing the new format
 ...
 *
 * @param *t      Libconfig setting entry. It is expected to be valid and it
 *                won't be freed (it is care of the caller to do so if
 *                necessary)
 * @param *id     item_data entry
 */
void itemdb_readdb_job_sub(struct item_data* id, config_setting_t *t)
{
	int idx = 0;
	config_setting_t *it = NULL;
	id->class_base[0] = id->class_base[1] = id->class_base[2] = 0;
	while ((it = libconfig->setting_get_elem(t, idx++))) {
