...
/// @copydoc refine_interface_private::readdb_requirements_items()
static bool refine_readdb_requirements_items(const struct config_setting_t *elem, struct s_refine_requirement *req, const char *name, const char *source)
{
...
/// @copydoc refine_interface_private::readdb_requirements_sub()
static bool refine_readdb_requirements_sub(const struct config_setting_t *elem, int type, const char *name, const char *source)
{
	nullpo_retr(false, elem);
	nullpo_retr(false, name);
	nullpo_retr(false, source);
	...
	for (int i = 0; i < libconfig->setting_length(items_t); ++i) {
		...
		if (!refine->p->readdb_requirements_items(item_t, &req, name, source))
			continue;
...
/// @copydoc refine_interface_private::readdb_requirements()
static int refine_readdb_requirements(const struct config_setting_t *r, int type, const char *name, const char *source)
{
	nullpo_retr(0, r);
	nullpo_retr(0, name);
	...
	while ((elem = libconfig->setting_get_elem(r, i++)) != NULL) {
		refine->p->readdb_requirements_sub(elem, type, name, source);
	}
...
static int refine_readdb_refine_libconfig_sub(struct config_setting_t *r, const char *name, const char *source)
{
	...
	struct config_setting_t *requirements;
	if ((requirements = libconfig->setting_get_member(r, "Requirements")) == NULL) {
		ShowWarning("status_readdb_refine_libconfig_sub: Missing Requirements for entry '%s' in \"%s\", skipping.\n", name, source);
		return 0;
	}

	if (refine->p->readdb_requirements(requirements, type, name, source) != MAX_REFINE) {
...
void refine_defaults(void)
{
	...
	refine->p->readdb_requirements_items = refine_readdb_requirements_items;
	refine->p->readdb_requirements_sub = refine_readdb_requirements_sub;
	refine->p->readdb_requirements = refine_readdb_requirements;
