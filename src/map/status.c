...
#define set_sc_with_vfx(skill, sc, icon, flag) do { set_sc((skill), (sc), (icon), (flag)); if((icon) < SI_MAX) status->dbs->RelevantBLTypes[(icon)] |= BL_SCEFFECT; } while(0)
	...
	**/
	...
	set_sc( SU_SPRITEMABLE			, SC_SPRITEMABLE	, SI_SPRITEMABLE	, SCB_NONE);
...
unsigned int status_get_base_maxsp(const struct map_session_data *sd, const struct status_data *st)
{
	...
	if((sd->class_&MAPID_BASEMASK) == MAPID_SUMMONER) {	// Summoner starts at 8 SP and gain 2 SP per even base lv and 3 SP per odd base lv.
		val -= 4;
...
unsigned int status_get_base_maxhp(const struct map_session_data *sd, const struct status_data *st)
{
	...
	if((sd->class_&MAPID_BASEMASK) == MAPID_SUMMONER) {	// Summoner starts with 60 HP and gains HP in according to hp_coefficient 1 and 2.
...
int status_calc_pc_(struct map_session_data* sd, enum e_status_calc_opt opt) {
	...
	if( (skill_lv = pc_checkskill(sd,SU_SPRITEMABLE)))
