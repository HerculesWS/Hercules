...
static int status_base_amotion_pc(struct map_session_data *sd, struct status_data *st)
{
	...
	amotion = status->dbs->aspd_base[pc->class2idx(sd->status.class_)][primary_weapon_type];
	if (sd->weapontype > MAX_SINGLE_WEAPON_TYPE)
		amotion += status->dbs->aspd_base[pc->class2idx(sd->status.class_)][secondary_weapon_type] / 4;
	if (sd->has_shield)
	...
	amotion = (sd->weapontype < MAX_SINGLE_WEAPON_TYPE)
		...
		: (status->dbs->aspd_base[pc->class2idx(sd->status.class_)][primary_weapon_type] + status->dbs->aspd_base[pc->class2idx(sd->status.class_)][secondary_weapon_type]) * 7 / 10; // dual-wield
