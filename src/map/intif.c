	...
	int pet_equip, short intimate, short hungry, char rename_flag, char incubate, char *pet_name)
{
	...
	struct PACKET_INTER_CREATE_PET *p = WFIFOP(struct PACKET_INTER_CREATE_PET *, inter_fd, 0);
...
{
	const struct PACKET_CHARMAP_GUILD_INFO_EMBLEM *p = RFIFOP(struct PACKET_CHARMAP_GUILD_INFO_EMBLEM *, fd, 0);
...
static void intif_parse_GuildInfo(int fd)
{
	if (RFIFOW(fd, 2) == sizeof(struct PACKET_CHARMAP_GUILD_INFO_EMPTY)) {
		const struct PACKET_CHARMAP_GUILD_INFO_EMPTY *empty = RFIFOP(struct PACKET_CHARMAP_GUILD_INFO_EMPTY *, fd, 0);
		ShowWarning("intif: guild noinfo %d\n", empty->guild_id);
		guild->recv_noinfo(empty->guild_id);
		return;
	}
	const struct PACKET_CHARMAP_GUILD_INFO *p = RFIFOP(struct PACKET_CHARMAP_GUILD_INFO *, fd, 0);
...
static void intif_request_agency_join_party(int char_id, int party_id, int map_index)
{
	...
	struct PACKET_MAPCHAR_AGENCY_JOIN_PARTY_REQ *p = WFIFOP(struct PACKET_MAPCHAR_AGENCY_JOIN_PARTY_REQ *, inter_fd, 0);
	p->packetType = 0x3084;
	p->char_id = char_id;
	p->party_id = party_id;
...
{
	const struct PACKET_CHARMAP_AGENCY_JOIN_PARTY *p = RFIFOP(struct PACKET_CHARMAP_AGENCY_JOIN_PARTY *, fd, 0);
