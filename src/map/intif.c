...
// pet
static int intif_create_pet(int account_id, int char_id, int pet_class, int pet_lv, int pet_egg_id, int pet_equip, short intimate, short hungry, char rename_flag, char incubate, char *pet_name)
{
...
static void intif_parse_GuildInfo(int fd)
{
	...
	if (intif->emblem_tmp_done == false || intif->emblem_tmp_guild_id != p->g.guild_id || intif->emblem_tmp_emblem_id != p->g.emblem_id) {
...
static void intif_parse_GuildMemberInfoChanged(int fd)
{
	...
	switch (type) {
		case GMI_POSITION:
			...
			guild->memberposition_changed(g, idx, RFIFOW(fd, 18));
			break;
		case GMI_EXP:
		...
		case GMI_GENDER:
			...
			break;
		case GMI_CLASS:
			g->member[idx].class = RFIFOW(fd, 18);
			break;
		case GMI_LEVEL:
...
static void intif_parse_GuildEmblem(int fd)
{
	...
	RFIFO_CHUNKED_COMPLETE(p)
	{
		guild->emblem_changed(intif->emblem_tmp.data_size, p->guild_id, p->emblem_id, intif->emblem_tmp.data);
		fifo_chunk_buf_clear(intif->emblem_tmp);
	}
}
...
{
	struct map_session_data *sd = NULL;
	if ((sd = map->id2sd(RFIFOL(fd, 2))) == NULL || sd->status.char_id != RFIFOL(fd, 6))
		return;
...
static void intif_parse_AuctionCancel(int fd)
{
	...
	switch (result) {
		case 0:
			clif->auction_message(sd->fd, 2);
			break;
		case 1:
			clif->auction_close(sd->fd, 2);
			break;
		case 2:
			clif->auction_close(sd->fd, 1);
			break;
		case 3:
			clif->auction_message(sd->fd, 3);
			break;
	}
}
...
static int intif_parse(int fd)
{
	...
	switch (cmd) {
		case 0x3804:
			...
			break;
		case 0x3805:
			intif->pAccountStorage(fd);
			break;
		case 0x3806:
			intif->pChangeNameOk(fd);
			break;
		case 0x3807:
			intif->pMessageToFD(fd);
			break;
		case 0x3808:
			intif->pAccountStorageSaveAck(fd);
			break;
		case 0x3810:
			intif->pAchievementsLoad(fd);
			break;
		case 0x3818:
			intif->pLoadGuildStorage(fd);
			break;
		case 0x3819:
			intif->pSaveGuildStorage(fd);
			break;
		case 0x3820:
			intif->pPartyCreated(fd);
			break;
		case 0x3821:
			intif->pPartyInfo(fd);
			break;
		case 0x3822:
			intif->pPartyMemberAdded(fd);
			break;
		case 0x3823:
			intif->pPartyOptionChanged(fd);
			break;
		case 0x3824:
			intif->pPartyMemberWithdraw(fd);
			break;
		case 0x3825:
			intif->pPartyMove(fd);
			break;
		case 0x3826:
			intif->pPartyBroken(fd);
			break;
		case 0x3830:
			intif->pGuildCreated(fd);
			break;
		case HEADER_CHARMAP_GUILD_INFO:
			intif->pGuildInfo(fd);
			break;
		case HEADER_CHARMAP_GUILD_INFO_EMBLEM:
			intif->pGuildInfoEmblem(fd);
			break;
		case 0x3832:
			intif->pGuildMemberAdded(fd);
			break;
		case 0x3834:
			intif->pGuildMemberWithdraw(fd);
			break;
		case 0x3835:
			intif->pGuildMemberInfoShort(fd);
			break;
		case 0x3836:
			intif->pGuildBroken(fd);
			break;
		case 0x3839:
			intif->pGuildBasicInfoChanged(fd);
			break;
		case 0x383a:
			intif->pGuildMemberInfoChanged(fd);
			break;
		case 0x383b:
			intif->pGuildPosition(fd);
			break;
		case 0x383c:
			intif->pGuildSkillUp(fd);
			break;
		case 0x383d:
			intif->pGuildAlliance(fd);
			break;
		case 0x383e:
			intif->pGuildNotice(fd);
			break;
		case HEADER_CHARMAP_GUILD_EMBLEM:
			intif->pGuildEmblem(fd);
			break;
		case 0x3840:
			intif->pGuildCastleDataLoad(fd);
			break;
		case 0x3843:
			intif->pGuildMasterChanged(fd);
			break;
		...
		// Mail System
		case 0x3848:
			intif->pMailInboxReceived(fd);
			break;
		case 0x3849:
			intif->pMailNew(fd);
			break;
		case 0x384a:
			intif->pMailGetAttach(fd);
			break;
		case 0x384b:
			intif->pMailDelete(fd);
			break;
		case 0x384c:
			intif->pMailReturn(fd);
			break;
		case 0x384d:
			intif->pMailSend(fd);
			break;
		// Auction System
		case 0x3850:
			intif->pAuctionResults(fd);
			break;
		case 0x3851:
			intif->pAuctionRegister(fd);
			break;
		case 0x3852:
			intif->pAuctionCancel(fd);
			break;
		case 0x3853:
			intif->pAuctionClose(fd);
			break;
		case 0x3854:
			intif->pAuctionMessage(fd);
			break;
		case 0x3855:
			intif->pAuctionBid(fd);
			break;
		// Bound items
		case 0x3856:
#ifdef GP_BOUND_ITEMS
		...
		// Mercenary System
		case 0x3870:
			intif->pMercenaryReceived(fd);
			break;
		case 0x3871:
			intif->pMercenaryDeleted(fd);
			break;
		case 0x3872:
			intif->pMercenarySaved(fd);
			break;
		// Elemental System
		case 0x387c:
			intif->pElementalReceived(fd);
			break;
		case 0x387d:
			intif->pElementalDeleted(fd);
			break;
		case 0x387e:
			intif->pElementalSaved(fd);
			break;
		...
		case 0x3880:
			...
			break;
		case 0x3881:
			intif->pRecvPetData(fd);
			break;
		case 0x3882:
			intif->pSavePetOk(fd);
			break;
		case 0x3883:
			intif->pDeletePetOk(fd);
			break;
		case 0x3890:
			intif->pCreateHomunculus(fd);
			break;
		case 0x3891:
			intif->pRecvHomunculusData(fd);
			break;
		case 0x3892:
			intif->pSaveHomunculusOk(fd);
			break;
		case 0x3893:
			intif->pDeleteHomunculusOk(fd);
			break;

		// RoDEX
		case 0x3895:
			intif->pRequestRodexOpenInbox(fd);
			break;
		case 0x3896:
			intif->pRodexHasNew(fd);
			break;
		case 0x3897:
			intif->pRodexSendMail(fd);
			break;
		case 0x3898:
			intif->pRodexCheckName(fd);
			break;
		case 0x3899:
			intif->pGetZenyAck(fd);
			break;
		case 0x389a:
			intif->pGetItemsAck(fd);
			break;

		case 0x389b:
			intif->pAgencyJoinResult(fd);
			break;

		// Clan System
		case 0x3858:
			intif->pRecvClanMemberAction(fd);
			break;
