...
enum windows_product_type {
	...
	msPRODUCT_MEDIUMBUSINESS_SERVER_MANAGEMENT
	...
	msPRODUCT_STORAGE_WORKGROUP_EVALUATION_SERVER
	    = 0x0000005F, ///< Storage Server Workgroup (evaluation installation)
	msPRODUCT_STORAGE_STANDARD_EVALUATION_SERVER
...
// Compiler detection <http://sourceforge.net/p/predef/wiki/Compilers/>
  ...
  #define SYSINFO_COMPILER \
	  "Clang v" EXPAND_AND_QUOTE(__clang_major__) "." EXPAND_AND_QUOTE(__clang_minor__) "." EXPAND_AND_QUOTE(__clang_patchlevel__)
#elif defined(__INTEL_COMPILER)
  #define SYSINFO_COMPILER "Intel CC v" EXPAND_AND_QUOTE(__INTEL_COMPILER)
#elif defined(__MINGW32__)
  #if defined(__MINGW64__)
    #define SYSINFO_COMPILER \
	    "MinGW-w64 64 Bit v" EXPAND_AND_QUOTE(__MINGW64_VERSION_MAJOR) "." EXPAND_AND_QUOTE(__MINGW64_VERSION_MINOR) " (MinGW " EXPAND_AND_QUOTE(__MINGW32_MAJOR_VERSION) "." EXPAND_AND_QUOTE(__MINGW32_MINOR_VERSION) ")"
  #elif defined(__MINGW64_VERSION_MAJOR)
    #define SYSINFO_COMPILER \
	    "MinGW-w64 32 Bit v" EXPAND_AND_QUOTE(__MINGW64_VERSION_MAJOR) "." EXPAND_AND_QUOTE(__MINGW64_VERSION_MINOR) " (MinGW " EXPAND_AND_QUOTE(__MINGW32_MAJOR_VERSION) "." EXPAND_AND_QUOTE(__MINGW32_MINOR_VERSION) ")"
  #else
    #define SYSINFO_COMPILER \
	    "MinGW32 v" EXPAND_AND_QUOTE(__MINGW32_MAJOR_VERSION) "." EXPAND_AND_QUOTE(__MINGW32_MINOR_VERSION)
  #endif
#elif defined(__GNUC__)
  #define SYSINFO_COMPILER \
	  "GCC v" EXPAND_AND_QUOTE(__GNUC__) "." EXPAND_AND_QUOTE(__GNUC_MINOR__) "." EXPAND_AND_QUOTE(__GNUC_PATCHLEVEL__)
...
static bool sysinfo_svn_get_revision(char **out)
{
  ...
  #ifndef SVNNODEPATH // not sure how to handle branches, so I'll leave this overridable define until a better solution
	              // comes up
...
static void sysinfo_osversion_retrieve(void)
{
	...
	/*
	 * #pragma rantmode (on)
	 * Some engineer at Microsoft moronically decided that, since some applications use this information to do
	 * version checks and refuse to run if they detect a new, unknown version of Windows, now nobody will be able to
	 * rely on this information anymore, not even those who need it for reporting or logging. The correct fix was to
	 * let those applications break, and their developer fix them (and in the meanwhile let the users use the
	 * Compatibility settings to run them) but no, they decided they'd deprecate the API, and make it lie for those
	 * who use it, reporting windows 8 even if they're running on 8.1 or newer. The API wasn't broken, applications
	 * were. Now we have broken applications, and a broken API. Great move, Microsoft.  Oh right, there's the
	 * Version API helper functions. Or maybe not, since you can only do 'are we running on at least version X?'
	 * checks with those, it's not what we need. You know what? I'll just silence your deprecation warning for the
	 * time being. Maybe by the time you release the next version of Windows, you'll have provided a less crippled
	 * API or something. #pragma rantmode (off)
	 */
  #pragma warning(push)
  ...
  #pragma warning(pop)
	...
	if (
	    VER_PLATFORM_WIN32_NT == osvi.dwPlatformId // Windows NT Family
	    && ((osvi.dwMajorVersion > 4 && osvi.dwMajorVersion < 6)
	        || (osvi.dwMajorVersion == 6 && osvi.dwMinorVersion <= 3)) // Between XP and 8.1
	) {
		if (osvi.dwMajorVersion == 6 && osvi.dwMinorVersion <= 3) { // Between Vista and 8.1
			...
			if (osvi.dwMinorVersion == 0) {
				...
				*(FARPROC *)&RtlGetVersion
				    = GetProcAddress(GetModuleHandle(TEXT("ntdll")), "RtlGetVersion");
				...
				StrBuf->Printf(
				    &buf, "Windows%s %d", osvi.wProductType == VER_NT_WORKSTATION ? "" : " Server",
				    osvi.dwMajorVersion
				);
				if (osvi.dwMinorVersion > 0)
...
static void sysinfo_cpu_retrieve(void)
{
	...
	if (si.wProcessorArchitecture == PROCESSOR_ARCHITECTURE_INTEL
		...
		StrBuf->Printf(
		    &buf, "%s CPU, Family %d, Model %d, Stepping %d",
		    si.wProcessorArchitecture == PROCESSOR_ARCHITECTURE_INTEL ? "x86" : "x86_64", si.wProcessorLevel,
		    (si.wProcessorRevision & 0xFF00) >> 8, (si.wProcessorRevision & 0xFF)
		);
	} else {
