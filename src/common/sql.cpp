...
static int
    Sql_Connect(struct Sql *self, const char *user, const char *passwd, const char *host, uint16 port, const char *db)
{
	if (self == NULL)
		return SQL_ERROR;
	...
	if (!mysql_real_connect(
	        &self->handle, host, user, passwd, db, (unsigned int)port, NULL /*unix_socket*/, 0 /*clientflag*/
	    )) {
...
static int Sql_P_Keepalive(struct Sql *self)
{
	...
	return timer->add_interval(
	    timer->gettick() + ping_interval * 1000, Sql_P_KeepaliveTimer, 0, (intptr_t)self, ping_interval * 1000
	);
}
...
static void Sql_Free(struct Sql *self)
{
	if (self) {
		...
		if (self->keepalive != INVALID_TIMER)
			timer->delete_(self->keepalive, Sql_P_KeepaliveTimer);
...
)
{
	...
	// [4144] probably can be many types and not supported by all mysql versions because this ignore warning about
	// missing type enum values
...
static void SqlStmt_P_ShowDebugTruncatedColumn(struct SqlStmt *self, size_t i)
{
	...
	if (column->buffer_type == MYSQL_TYPE_STRING)
		Sql_P_ShowDebugMysqlFieldInfo(
		    "buffer - ", column->buffer_type, column->is_unsigned, column->buffer_length, "+1(null-terminator)"
		);
	else
		Sql_P_ShowDebugMysqlFieldInfo(
...
static int SqlStmt_BindColumn(
    struct SqlStmt *self, size_t idx, enum SqlDataType buffer_type, void *buffer, size_t buffer_len, uint32 *out_length,
    int8 *out_is_null
)
{
	if (self == NULL)
	...
	if (buffer_type == SQLDT_STRING || buffer_type == SQLDT_ENUM) {
		if (buffer_len < 1) {
			ShowDebug(
			    "SqlStmt_BindColumn: buffer_len(%" PRIuS
...
static bool Sql_inter_server_read(const char *filename, bool imported)
{
	...
	if ((setting = libconfig->lookup(&config, "inter_configuration/mysql_reconnect")) == NULL) {
		...
		if (imported)
			return true;
		ShowError(
		    "Sql_inter_server_read: inter_configuration/mysql_reconnect was not found in %s!\n", filename
		);
		return false;
	}
	...
	if (libconfig->setting_lookup_int(setting, "type", &mysql_reconnect_type) == CONFIG_TRUE) {
		if (mysql_reconnect_type != 1 && mysql_reconnect_type != 2) {
			ShowError(
			    "%s::inter_configuration/mysql_reconnect/type is set to %d which is not valid, defaulting to 1...\n",
	...
	if (libconfig->lookup_string(&config, "import", &import) == CONFIG_TRUE) {
		if (strcmp(import, filename) == 0
		    || strcmp(import, "conf/common/inter-server.conf") == 0) { // FIXME: Hardcoded path
...
void Sql_HerculesUpdateCheck(struct Sql *self)
{
	...
	while (fgets(line, sizeof(line), ifp)) {
		...
		if (fseek(ufp, 1, SEEK_SET) == 0 /* woo. skip the # */
			...
			if (SQL_ERROR
			    == SQL->Query(
			        self, "SELECT 1 FROM `sql_updates` WHERE `timestamp` = '%u' LIMIT 1", timestampui
			    ))
				Sql_ShowDebug(self);
			if (Sql_NumRows(self) != 1) {
				StrBuf->Printf(
				    &buf, CL_MAGENTA "[SQL]" CL_RESET ": -- '" CL_WHITE "%s" CL_RESET "'\n", path
				);
				performed++;
			}
		}
...
void Sql_HerculesUpdateSkip(struct Sql *self, const char *filename)
{
	...
	if (fseek(ifp, 1, SEEK_SET) == 0 /* woo. skip the # */
		...
		unsigned int timestampui = (unsigned int)atol(timestamp);
		if (SQL_ERROR
		    == SQL->Query(self, "SELECT 1 FROM `sql_updates` WHERE `timestamp` = '%u' LIMIT 1", timestampui))
			Sql_ShowDebug(self);
			...
			if (SQL_ERROR
			    == SQL->Query(
			        self, "INSERT INTO `sql_updates` (`timestamp`,`ignored`) VALUES ('%u','Yes') ",
			        timestampui
			    ))
				Sql_ShowDebug(self);
			else {
