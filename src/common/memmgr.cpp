...
static void *aCalloc_(size_t num, size_t size, const char *file, int line, const char *func)
    __attribute__((alloc_size(1, 2))) GCCATTR((returns_nonnull));
...
static void *aCalloc_(size_t num, size_t size, const char *file, int line, const char *func)
{
	...
	memset(ret, 0x0, (size * num) + sizeof(*ret));
	ret->size = (size * num) + sizeof(*ret);

	return ret + 1;
}

static void *aRealloc_(void *p, size_t size, const char *file, int line, const char *func)
    __attribute__((alloc_size(2))) __attribute__((nonnull(1))) GCCATTR((returns_nonnull));
...
static void *aReallocz_(void *p, size_t size, const char *file, int line, const char *func)
    __attribute__((alloc_size(2))) GCCATTR((returns_nonnull));

static void *aReallocz_(void *p, size_t size, const char *file, int line, const char *func)
{
	struct nomemmgr_block *ret = NULL;
	...
	if (p != NULL) {
		...
		if (ret != NULL) {
			...
			if (new_size > old_size) {
				memset((uint8 *)ret + old_size, 0, new_size - old_size);
			}
		}
	} else {
...
static char *aStrndup_(const char *p, size_t size, const char *file, int line, const char *func)
    __attribute__((nonnull(1))) GCCATTR((returns_nonnull));
...
static void aFree_(void *p, const char *file, int line, const char *func)
{
	...
	if (p != NULL)
		FREE((struct nomemmgr_block *)p - 1, file, line, func);
	// p = NULL;
}
...
static void *mmalloc_(size_t size, const char *file, int line, const char *func) __attribute__((alloc_size(1)))
GCCATTR((returns_nonnull));
...
static void *mmalloc_(size_t size, const char *file, int line, const char *func)
{
	...
	if (hash2size(size_hash) > BLOCK_DATA_SIZE - sizeof(struct unit_head)) {
		struct unit_head_large *p
		    = (struct unit_head_large *)MALLOC(sizeof(struct unit_head_large) + size, file, line, func);
		...
		if (p != NULL) {
			...
			*(long *)((char *)p + sizeof(struct unit_head_large) - sizeof(long) + size) = 0xDEADBEAF;
			...
			ShowFatalError(
			    "Memory manager::memmgr_alloc failed (allocating %" PRIuS "+%" PRIuS " bytes at %s:%d).\n",
  ...
  #ifdef DEBUG_MEMMGR
	{
		...
		for (i = 0; i < sz; i++) {
			if (((unsigned char *)head)[sizeof(struct unit_head) - sizeof(long) + i] != 0xFD) {
				if (head->line != 0xFDFD) {
					ShowError(
					    "Memory manager: freed-data is changed. (freed in %s line %d)\n",
					    head->file, head->line
					);
				} else {
...
static void *mrealloc_(void *memblock, size_t size, const char *file, int line, const char *func)
{
	...
	if (old_size == 0) {
		old_size
		    = ((struct unit_head_large *)((char *)memblock - sizeof(struct unit_head_large) + sizeof(long)))
		          ->size;
	}
	if (old_size > size) {
...
static char *mstrdup_(const char *p, const char *file, int line, const char *func) __attribute__((nonnull(1)))
GCCATTR((returns_nonnull));
...
static void mfree_(void *ptr, const char *file, int line, const char *func)
{
	...
	if (head->size == 0) {
		...
		struct unit_head_large *head_large
		    = (struct unit_head_large *)((char *)ptr - sizeof(struct unit_head_large) + sizeof(long));
		if (*(long *)((char *)head_large + sizeof(struct unit_head_large) - sizeof(long) + head_large->size)
		    != 0xDEADBEAF) {
			ShowError(
			    "Memory manager: args of aFree 0x%p is overflowed pointer %s line %d\n", ptr, file, line
			);
		} else {
			head->size = 0xFFFF;
			if (head_large->prev) {
		...
		if ((char *)head - (char *)block > (ssize_t)sizeof(struct block)) {
			ShowError(
			    "Memory manager: args of aFree 0x%p is invalid pointer %s line %d\n", ptr, file, line
			);
		} else if (head->block == NULL) {
			ShowError(
			    "Memory manager: args of aFree 0x%p is freed pointer %s:%d@%s\n", ptr, file, line, func
			);
		} else if (
		    *(long *)((char *)head + sizeof(struct unit_head) - sizeof(long) + head->size) != 0xDEADBEAF
		) {
			ShowError(
			    "Memory manager: args of aFree 0x%p is overflowed pointer %s line %d\n", ptr, file, line
			);
			...
			if (--block->unit_used == 0) {
				...
				block->unit_unfill
				    = (unsigned short)(((uintptr_t)head - (uintptr_t)block->data) / block->unit_size);
			}
		}
	}
}
...
static bool memmgr_verify(void *ptr)
{
	...
	// search small blocks
	while (block) {
		if ((char *)ptr >= (char *)block
		    && (char *)ptr < ((char *)block) + sizeof(struct block)) { // found memory block
			if (block->unit_used
			    && (char *)ptr >= block->data) { // memory block is being used and ptr points to a sub-unit
				size_t i               = (size_t)((char *)ptr - block->data) / block->unit_size;
				struct unit_head *head = block2unit(block, i);
				if (i < block->unit_maxused
				    && head->block != NULL) { // memory unit is allocated, check if ptr points to the
					                      // usable part
					return (
	...
	while (large) {
		if ((char *)ptr >= (char *)large
		    && (char *)ptr < ((char *)large)
		                         + large->size) { // found memory block, check if ptr points to the usable part
			return (
			    (char *)ptr >= ((char *)large) + sizeof(struct unit_head_large) - sizeof(long)
			    && (char *)ptr
			           < ((char *)large) + sizeof(struct unit_head_large) - sizeof(long) + large->size
			);
		}
		large = large->next;
	}
...
static void memmgr_final(void)
{
  ...
  #endif /* LOG_MEMMGR */
	snprintf(
	    vcsinfo, sizeof(vcsinfo), "%s rev '%s'", sysinfo->vcstype(), sysinfo->vcsrevision_src()
		...
		if (block->unit_used) {
			...
			for (i = 0; i < block->unit_maxused; i++) {
				...
				if (head->block != NULL) {
					...
					char buf[1024];
					sprintf(
					    buf, "%04d : %s line %d size %lu address 0x%p\n", ++count, head->file,
					    head->line, (unsigned long)head->size, ptr
					);
	...
	while (large) {
		...
		char buf[1024];
		sprintf(
		    buf, "%04d : %s line %d size %lu address 0x%p\n", ++count, large->unit_head.file,
		    large->unit_head.line, (unsigned long)large->size, &large->unit_head.checksum
		);
		memmgr_log(buf, vcsinfo);
  #endif /* LOG_MEMMGR */
		large2 = large->next;
...
void memmgr_report(int extra)
{
	...
	for (j = 0; j < 100; j++) {
		if (data[j].size != 0) {
			ShowMessage(
			    "[malloc] : " CL_WHITE "%s" CL_RESET ":" CL_WHITE "%d" CL_RESET
			    " %u instances => %.2f MB\n",
	...
	ShowMessage(
	    "[malloc] : internal usage %.2f MB | %.2f MB\n",
	    (double)((memmgr_usage_bytes_t - memmgr_usage_bytes) / 1024) / 1024,
	    (double)((memmgr_usage_bytes_t) / 1024) / 1024
	);
