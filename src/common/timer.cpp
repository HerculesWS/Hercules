...
static int timer_add_func_list(TimerFunc func, const char *name)
{
	...
	if (name) {
		for (tfl = tfl_root; tfl != NULL; tfl = tfl->next) { // check suspicious cases
			if (func == tfl->func)
				ShowWarning(
				    "timer_add_func_list: duplicating function %p(%s) as %s.\n", tfl->func, tfl->name,
				    name
				);
			else if (strcmp(name, tfl->name) == 0)
				ShowWarning(
				    "timer_add_func_list: function %p has the same name as %p(%s)\n", func, tfl->func,
				    tfl->name
				);
		}
...
static int timer_add(int64 tick, TimerFunc func, int id, intptr_t data)
{
	...
	if (timer_data[tid].type != 0 && timer_data[tid].type != TIMER_REMOVE_HEAP) {
		ShowError(
		    "timer_add error: wrong tid type: %d, [%d]%p(%s) -> %p(%s)\n", timer_data[tid].type, tid, func,
		    search_timer_func_list(func), timer_data[tid].func, search_timer_func_list(timer_data[tid].func)
		);
		Assert_retr(INVALID_TIMER, 0);
	}
	if (timer_data[tid].func != NULL) {
		ShowError(
		    "timer_add error: func non NULL: [%d]%p(%s) -> %p(%s)\n", tid, func, search_timer_func_list(func),
...
static int timer_add_interval(int64 tick, TimerFunc func, int id, intptr_t data, int interval)
{
	...
	if (interval < 1) {
		ShowError(
		    "timer_add_interval: invalid interval (tick=%" PRId64 " %p[%s] id=%d data=%" PRIdPTR
		    " diff_tick=%" PRId64 ")\n",
	...
	if (timer_data[tid].type != 0 && timer_data[tid].type != TIMER_REMOVE_HEAP) {
		ShowError(
		    "timer_add_interval: wrong tid type: %d, [%d]%p(%s) -> %p(%s)\n", timer_data[tid].type, tid, func,
		    search_timer_func_list(func), timer_data[tid].func, search_timer_func_list(timer_data[tid].func)
		);
		Assert_retr(INVALID_TIMER, 0);
		return INVALID_TIMER;
	...
	if (timer_data[tid].func != NULL) {
		ShowError(
		    "timer_add_interval: func non NULL: [%d]%p(%s) -> %p(%s)\n", tid, func,
		    search_timer_func_list(func), timer_data[tid].func, search_timer_func_list(timer_data[tid].func)
		);
		Assert_retr(INVALID_TIMER, 0);
		return INVALID_TIMER;
...
static int timer_do_delete(int tid, TimerFunc func)
{
	...
	if (tid < 1 || tid >= timer_data_num) {
		ShowError(
		    "timer_do_delete error : no such timer [%d](%p(%s))\n", tid, func, search_timer_func_list(func)
		);
		Assert_retr(-1, 0);
		return -1;
	}
	if (timer_data[tid].func != func) {
		ShowError(
		    "timer_do_delete error : function mismatch [%d]%p(%s) != %p(%s)\n", tid, timer_data[tid].func,
		    search_timer_func_list(timer_data[tid].func), func, search_timer_func_list(func)
		);
		Assert_retr(-2, 0);
		return -2;
	}
	...
	if (timer_data[tid].type == 0 || timer_data[tid].type == TIMER_REMOVE_HEAP) {
		ShowError(
		    "timer_do_delete: timer already deleted: %d, [%d]%p(%s) -> %p(%s)\n", timer_data[tid].type, tid,
		    func, search_timer_func_list(func), func, search_timer_func_list(func)
		);
		Assert_retr(-3, 0);
		return -3;
	}
...
static int64 timer_settick(int tid, int64 tick)
{
	...
	if (i == BHEAP_LENGTH(timer_heap)) {
		ShowError(
		    "timer_settick: no such timer [%d](%p(%s))\n", tid, timer_data[tid].func,
	...
	if (timer_data[tid].type == 0 || timer_data[tid].type == TIMER_REMOVE_HEAP) {
		ShowError(
		    "timer_settick error: set tick for deleted timer %d, [%d](%p(%s))\n", timer_data[tid].type, tid,
		    timer_data[tid].func, search_timer_func_list(timer_data[tid].func)
		);
		Assert_retr(-1, 0);
		return -1;
	...
	if (timer_data[tid].func == NULL) {
		ShowError(
		    "timer_settick error: set tick for timer with wrong func [%d](%p(%s))\n", tid, timer_data[tid].func,
		    search_timer_func_list(timer_data[tid].func)
		);
		Assert_retr(-1, 0);
		return -1;
...
static bool timer_get_current_clocksource(char *buf, int buf_size)
{
	...
	FILE *fp = fopen("/sys/devices/system/clocksource/clocksource0/current_clocksource", "rt");
	if (fp == NULL) {
		ShowClockError("Cant open /sys/devices/system/clocksource/clocksource0/current_clocksource\n");
		return false;
	}
	if (fgets(buf, buf_size, fp) == NULL) {
		ShowClockError("Cant read /sys/devices/system/clocksource/clocksource0/current_clocksource\n");
		fclose(fp);
		return false;
	}
...
static bool timer_get_available_clocksource(char *buf, int buf_size)
{
	...
	FILE *fp = fopen("/sys/devices/system/clocksource/clocksource0/available_clocksource", "rt");
	if (fp == NULL) {
		ShowClockError("Cant open /sys/devices/system/clocksource/clocksource0/available_clocksource\n");
		return false;
	}
	if (fgets(buf, buf_size, fp) == NULL) {
		ShowClockError("Cant read /sys/devices/system/clocksource/clocksource0/available_clocksource\n");
		fclose(fp);
		return false;
	}
	const size_t sz = strlen(buf);
...
#if defined(__linux) || defined(__linux__)
	char bufAll[1024];
	if (!timer->get_available_clocksource(bufAll, sizeof(bufAll)))
		return;
	ShowInfo("Found clock sources: '%s'\n", bufAll);
	char bufCur[1024];
	if (!timer->get_current_clocksource(bufCur, sizeof(bufCur)))
		return;
	const bool haveTsc = (strstr(bufAll, "tsc ") != NULL);
	if (strcmp(bufCur, "tsc") != 0) {
		if (haveTsc
		    || (strcmp(bufCur, "hyperv_clocksource_tsc_page") != 0 && strcmp(bufCur, "kvm-clock") != 0)) {
			ShowClockError(
			    "Unoptimal clock source detected: '%s'. " "Please set clock source to 'tsc' or 'hyperv_clocksource_tsc_page' or 'kvm-clock'\n",
			    bufCur
			);
			return;
		}
...
void timer_defaults(void)
{
	...
	timer->check_timers              = timer_check_timers;
	timer->get_current_clocksource   = timer_get_current_clocksource;
	timer->get_available_clocksource = timer_get_available_clocksource;
}
