...
static void *ers_obj_alloc_entry(ERS *self)
{
	...
	if (instance->Cache->ReuseList != NULL) {
		...
		ret = &instance->Cache->Blocks[instance->Cache->Used - 1]
		                              [instance->Cache->Free * (size_t)instance->Cache->ObjectSize
...
static void ers_obj_free_entry(ERS *self, void *entry)
{
	...
	if (instance->Cache->Options & ERS_OPT_CLEAN)
		memset(
		    (unsigned char *)reuse + sizeof(struct ers_list), 0,
...
static void ers_obj_destroy(ERS *self)
{
	...
	if (instance->Count > 0)
		if (!(instance->Options & ERS_OPT_CLEAR))
			ShowWarning(
			    "Memory leak detected at ERS '%s', %u objects not freed.\n", instance->Name, instance->Count
			);
...
static void ers_cache_size(ERS *self, unsigned int new_size)
{
	...
	if (!(instance->Cache->Options & ERS_OPT_FLEX_CHUNK)) {
		ShowWarning(
		    "ers_cache_size: '%s' has adjusted its chunk size to '%u', however ERS_OPT_FLEX_CHUNK is missing!\n",
...
ERS *ers_new(uint32 size, const char *name, enum ERSOptions options) // FIXME: change this to a flag type
{
...
void ers_report(void)
{
	...
	for (instance = InstanceList; instance; instance = instance->Next) {
		...
		instance_c_d++;
		ShowMessage(
		    CL_BOLD "[ERS Instance " CL_NORMAL CL_WHITE "%s" CL_NORMAL CL_BOLD " report]\n" CL_NORMAL,
		    instance->Name
		);
  ...
  #ifdef DEBUG
	ShowInfo(
	    "ers_report: '" CL_WHITE "%u" CL_NORMAL "' instances in use, '" CL_WHITE "%u" CL_NORMAL "' displayed\n",
