 ...
 *
 * Copyright (C) 2020-2026 Hercules Dev Team
...
#define WFIFO_CHUNKED_FINAL_START_RAW1(p, pname, _dataField, chunk_size) \
...
#define WFIFO_CHUNKED_BLOCK_START(p, pname) \
	WFIFO_CHUNKED_BLOCK_START_RAW1(p, pname, data, WFIFO_CHUNK_SIZE); \
	WFIFO_CHUNKED_BLOCK_START_RAW_FLAG(p)

#define WFIFO_CHUNKED_BLOCK_END() \
	WFIFO_CHUNKED_BLOCK_END_RAW()

#define WFIFO_CHUNKED_FINAL_START(p, pname) \
	WFIFO_CHUNKED_FINAL_START_RAW1(p, pname, data, WFIFO_CHUNK_SIZE); \
...
#define WFIFO_CLIENT_CHUNKED_BLOCK_START(p, pname, _dataField) \
	WFIFO_CHUNKED_BLOCK_START_RAW1(p, pname, _dataField, WFIFO_CLIENT_CHUNK_SIZE)

#define WFIFO_CLIENT_CHUNKED_BLOCK_END() \
	WFIFO_CHUNKED_BLOCK_END_RAW()

#define WFIFO_CLIENT_CHUNKED_FINAL_START(p, pname, _dataField) \
	WFIFO_CHUNKED_FINAL_START_RAW1(p, pname, _dataField, WFIFO_CLIENT_CHUNK_SIZE)
