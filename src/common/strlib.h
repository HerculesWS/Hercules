...
T HSTRRCHR(T src, int c)
{
	static_assert(
	    std::is_same_v<std::decay_t<T>, char *> || std::is_same_v<std::decay_t<T>, const char *>,
	    "Invalid type passed to strrchr"
	);
...
struct strlib_interface {
	...
	int (*jmemescapecpy)(char *pt, const char *spt, int size)
	    GCC10ATTR((access(write_only, 1), access(read_only, 2, 3)));
	...
	const char *(*stristr_)(const char *haystack, const char *needle)
	    GCC10ATTR((access(read_only, 1), access(read_only, 2)));

	/* only used when '!(defined(WIN32) && defined(_MSC_VER)) && !defined(HAVE_STRNLEN)', needs to be defined at all
	 * times however  */
	...
	char *(*strtok_r_)(char *s1, const char *s2, char **lasts)
	    GCC10ATTR((access(read_write, 1), access(read_only, 2), access(read_write, 3)));
	...
	char *(*safestrncpy_)(char *dst, const char *src, size_t n)
	    GCC10ATTR((access(write_only, 1, 3), access(read_only, 2)));
...
struct sv_interface {
	...
	/// Opens and parses a file containing delim-separated columns, feeding them to the specified callback function
	/// row by row. Tracks the progress of the operation (current line number, number of successfully processed
	/// rows). Returns 'true' if it was able to process the specified file, or 'false' if it could not be read.
	bool (*readdb)(
	    const char *directory, const char *filename, char delim, int mincols, int maxcols, int maxrows,
