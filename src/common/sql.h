...
struct sql_interface {
	...
	int (*Connect)(
	    struct Sql *self, const char *user, const char *passwd, const char *host, uint16 port, const char *db
	);
	...
	*-------------------------------------*/
	...
	/// @return SQL_SUCCESS or SQL_ERROR
	int (*StmtBindParam)(
	    struct SqlStmt *self, size_t idx, enum SqlDataType buffer_type, const void *buffer, size_t buffer_len
	);
	...
	/// @return SQL_SUCCESS or SQL_ERROR
	int (*StmtBindColumn)(
	    struct SqlStmt *self, size_t idx, enum SqlDataType buffer_type, void *buffer, size_t buffer_len,
	    uint32 *out_length, int8 *out_is_null
	);
