...
// Uncomment the following line if your client is ragexeRE instead of ragexe (required because of conflicting packets in
// ragexe vs ragexeRE). #define ENABLE_PACKETVER_RE
...
// Comment the following line to disable sc_data saving. [Skotlex]
...
#if PACKETVER_MAIN_NUM >= 20070711 \
  ...
  #endif
#endif // PACKETVER_MAIN_NUM >= 20070711 || PACKETVER_RE_NUM >= 20080827 || PACKETVER_AD_NUM >= 20070711 ||
       // PACKETVER_SAK_NUM >= 20070628 || defined(PACKETVER_ZERO)
...
// to support more characters as well.
...
#endif
#ifndef MAX_SKILL_DB
  #define MAX_SKILL_DB 1367 ///< Maximum number of skills in the skill DB (compacted array size)
#endif
#ifndef MAX_SKILL_ID
...
static_assert(
    MAX_ACHIEVEMENT_OBJECTIVES <= 10,
    "This value is limited by the client and database layout and should only be increased if you know the consequences."
);
  #define MAX_ACHIEVEMENT_RANKS 20 // Achievement Ranks
static_assert(
    MAX_ACHIEVEMENT_RANKS <= 255,
    "This value is limited by the client and database layout and should only be increased if you know the consequences."
);
...
#define SCRIPT_VARNAME_LENGTH \
	32 ///< Maximum length of a script variable's name including affixes and excluding NULL-terminator.
static_assert(
    SCRIPT_VARNAME_LENGTH <= 32,
    "This value is limited by the inter-server communication and database layout and should only be increased if you know the consequences."
);
#define SCRIPT_STRING_VAR_LENGTH \
	255 ///< Maximum length of strings stored in script variables excluding NULL-terminator.
static_assert(
    SCRIPT_STRING_VAR_LENGTH <= 255,
    "This value is limited by the inter-server communication and database layout and should only be increased if you know the consequences."
);
 ...
 **/
static_assert(
    MAX_STORAGE * sizeof(struct item) + 8 <= 0xFFFF,
    "The maximum amount of item slots per account storage is limited by the inter-server communication layout. Use a smaller value!"
);
...
enum e_skill_flag {
	...
	SKILL_FLAG_UNUSED, ///< needed to maintain the order since the values are saved, can be renamed and used if a
	                   ///< new flag is necessary
	...
	SKILL_FLAG_REPLACED_LV_0, // Temporary skill overshadowing permanent skill of level 'N -
...
enum e_option {
	...
	OPTION_COSTUME
	    = OPTION_WEDDING | OPTION_XMAS | OPTION_SUMMER | OPTION_HANBOK | OPTION_OKTOBERFEST | OPTION_SUMMER2,
};
 ...
 **/
static_assert(
    20 + sizeof(struct item) * MAX_GUILD_STORAGE <= 0xFFFF,
    "The maximum amount of item slots per guild storage is limited by the inter-server communication layout. Use a smaller value!"
);
...
struct clan {
	...
	int req_count_tid; ///< Timer ID for the timer that handles the timeout of requests for interserver to count
	                   ///< members
	int req_kick_tid;  ///< Timer ID for the timer that handles the timeout of requests for interserver to kick
	                   ///< inactive members
