...
static void setsocketopts(int fd, struct hSockOpt *opt)
{
	...
	int yes = 1;
#endif // WIN32
	struct linger lopt ZERO_INITIALIZED;
...
#else // not WIN32
		struct timeval timeout ZERO_INITIALIZED;
		timeout.tv_sec = 5;
#endif // WIN32
...
{
	struct sockaddr_in server_address ZERO_INITIALIZED;
	int fd;
	int result;
...
{
	struct sockaddr_in remote_address ZERO_INITIALIZED;
	int fd;
	int result;
