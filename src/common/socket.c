...
static int realloc_writefifo(int fd, size_t addition)
{
	...
	if (sockt->session[fd]->wdata_size + addition > sockt->session[fd]->max_wdata) {
		...
		while (sockt->session[fd]->wdata_size + addition > newsize)
			newsize += WFIFO_SIZE;
	} else if (sockt->session[fd]->max_wdata >= (size_t)2 * (sockt->session[fd]->flag.server ? FIFOSIZE_SERVERLINK : WFIFO_SIZE) && (sockt->session[fd]->wdata_size + addition) * 4 < sockt->session[fd]->max_wdata) {
...
enum aco { ACO_DENY_ALLOW, ACO_ALLOW_DENY, ACO_MUTUAL_FAILURE };
...
static void socket_datasync(int fd, bool send)
{
	struct {
	    ...
	    {sizeof(struct mmo_charstatus)}, {sizeof(struct quest)}, {sizeof(struct item)}, {sizeof(struct point)}, {sizeof(struct s_skill)}, {sizeof(struct status_change_data)}, {sizeof(struct storage_data)}, {sizeof(struct guild_storage)}, {sizeof(struct s_pet)}, {sizeof(struct s_mercenary)}, {sizeof(struct s_homunculus)}, {sizeof(struct s_elemental)}, {sizeof(struct s_friend)}, {sizeof(struct mail_message)}, {sizeof(struct mail_data)}, {sizeof(struct party_member)}, {sizeof(struct party)}, {sizeof(struct guild_member)}, {sizeof(struct guild_position)}, {sizeof(struct guild_alliance)}, {sizeof(struct guild_expulsion)}, {sizeof(struct guild_skill)}, {sizeof(struct guild)}, {sizeof(struct guild_castle)}, {sizeof(struct fame_list)}, {PACKETVER}, {PACKETVER_MAIN_NUM}, {PACKETVER_RE_NUM}, {PACKETVER_ZERO_NUM}, {PACKETVER_AD_NUM}, {PACKETVER_SAK_NUM},
	};
	unsigned short i;
