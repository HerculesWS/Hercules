...
static char *sErr(int code)
{
	...
	if (FormatMessage(
	        ...
	        MAKELANGID(LANG_ENGLISH, SUBLANG_DEFAULT), (LPTSTR)&sbuf, sizeof(sbuf), NULL
	    )
	    == 0)
...
static void setsocketopts(int fd, struct hSockOpt *opt)
{
	...
	// lag. The RO protocol is mainly single-packet request/response, plus the FIFO model already does packet
	// grouping anyway.
...
static int send_from_fifo(int fd)
{
	...
	if (len > 0) {
		...
		if ((size_t)len < sockt->session[fd]->wdata_size)
			memmove(
			    sockt->session[fd]->wdata, sockt->session[fd]->wdata + len,
...
static int connect_client(int listen_fd)
{
	...
	if (epoll_ctl(epfd, EPOLL_CTL_ADD, fd, &epevent) == SOCKET_ERROR) {
		ShowError(
		    "connect_client: New Socket #%d failed to add to epoll event dispatcher: %s\n", fd, error_msg()
		);
		sClose(fd);
		return -1;
	}
	...
	sockt->create_session(
	    fd, recv_to_fifo, send_from_fifo, default_func_parse, default_func_client_connected, default_func_delete
	);
...
static int realloc_writefifo(int fd, size_t addition)
{
	...
	if (sockt->session[fd]->wdata_size + addition > sockt->session[fd]->max_wdata) {
		...
		newsize = WFIFO_SIZE;
		while (sockt->session[fd]->wdata_size + addition > newsize)
			newsize += WFIFO_SIZE;
	} else if (
	    sockt->session[fd]->max_wdata
	        >= (size_t)2 * (sockt->session[fd]->flag.server ? FIFOSIZE_SERVERLINK : WFIFO_SIZE)
...
static int rfifoskip(int fd, size_t len)
{
	...
	if (s->rdata_size < s->rdata_pos + len) {
		ShowError(
		    "RFIFOSKIP: skipped past end of read buffer! Adjusting from %" PRIuS " to %" PRIuS
		    " (session #%d)\n",
		    len, RFIFOREST(fd), fd
		);
		Assert_report(0);
					...
					if (packet_len == -1) {
						if (lenRest < 4) {
							...
							if (packet_len != lenRest) {
								ShowError(
								    "Skip packet 0x%04X with dynamic size %" PRIdS
								    ", but must be size %d\n",
						...
						ShowError(
						    "Skip packet 0x%04X with size %" PRIdS ", but must be size %d\n",
						    cmd, lenRest, packet_len
						);
						Assert_report(0);
					}
...
static int wfifoset(int fd, size_t len, bool validate)
{
	...
	if (s->wdata_size + len > s->max_wdata) {
		...
		ShowFatalError(
		    "WFIFOSET: Write Buffer Overflow. Connection %d (%u.%u.%u.%u) has written %u bytes on a %u/%u bytes buffer.\n",
	...
	if (!s->flag.server) {
		if (len > socket_max_client_packet) { // see declaration of socket_max_client_packet for details
			ShowError(
			    "WFIFOSET: Dropped too large client packet 0x%04x (length=%" PRIuS ", max=%" PRIuS ").\n",
...
static int do_sockets(int next)
{
	...
	for (i = 0; i < ret; i++) {
		...
		if ((it->events & EPOLLERR) || (it->events & EPOLLHUP) || (!(it->events & EPOLLIN))) {
	...
	for (i = 1; i < sockt->fd_max; i++) {
		...
		if (sockt->session[i]->flag.eof) { // func_send can't free a session, this is safe.
			// Finally, even if there is no data to parse, connections signaled eof should be closed, so we
			// call parse_func [Skotlex]
	...
	for (i = 1; i < sockt->fd_max; i++) {
		...
		if (sockt->session[i]->rdata_tick
		    && DIFF_TICK(sockt->last_tick, sockt->session[i]->rdata_tick) > sockt->stall_time) {
			if (sockt->session[i]->flag.server) { /* server is special */
				if (sockt->session[i]->flag.ping
	...
	if (sockt->last_tick != socket_data_last_tick) {
		...
		sprintf(
		    ...
		    socket_data_co / 1024., socket_data_qo / 1024., iMalloc->usage() / 1024.
		);
  #ifdef _WIN32
		SetConsoleTitle(buf);
  #else  // _WIN32
...
static int connect_check(uint32 ip)
{
	...
	if (access_debug) {
		ShowInfo(
		    "connect_check: Connection from %u.%u.%u.%u %s\n", CONVIP(ip), result ? "allowed." : "denied!"
		);
	}
	return result;
}
...
static int connect_check_(uint32 ip)
{
	...
	for (i = 0; i < VECTOR_LENGTH(access_allow); ++i) {
		...
		if (SUBNET_MATCH(ip, entry->ip, entry->mask)) {
			if (access_debug) {
				ShowInfo(
				    "connect_check: Found match from allow list:%u.%u.%u.%u IP:%u.%u.%u.%u Mask:%u.%u.%u.%u\n",
				    CONVIP(ip), CONVIP(entry->ip), CONVIP(entry->mask)
				);
			}
			is_allowip = 1;
			break;
	...
	for (i = 0; i < VECTOR_LENGTH(access_deny); ++i) {
		...
		if (SUBNET_MATCH(ip, entry->ip, entry->mask)) {
			if (access_debug) {
				ShowInfo(
				    "connect_check: Found match from deny list:%u.%u.%u.%u IP:%u.%u.%u.%u Mask:%u.%u.%u.%u\n",
				    CONVIP(ip), CONVIP(entry->ip), CONVIP(entry->mask)
				);
			}
			is_denyip = 1;
...
static int connect_check_clear(int tid, int64 tick, int id, intptr_t data)
{
	...
	for (
	    struct connect_history *hist = (struct connect_history *)dbi_first(iter); dbi_exists(iter);
	    hist                         = (struct connect_history *)dbi_next(iter)
	) {
...
static int access_ipmask(const char *str, struct access_control *acc)
{
	...
	if (strcmp(str, "all") == 0) {
		...
		unsigned int m[4];
		int n;
		if (((n = sscanf(str, "%u.%u.%u.%u/%u.%u.%u.%u", a, a + 1, a + 2, a + 3, m, m + 1, m + 2, m + 3)) != 8
		     && // not an ip + standard mask
		     (n = sscanf(str, "%u.%u.%u.%u/%u", a, a + 1, a + 2, a + 3, m)) != 5
		     && // not an ip + bit mask
 ...
 */
static bool
    access_list_add(struct config_setting_t *setting, const char *list_name, struct access_control_list *access_list)
{
...
static bool socket_config_read_iprules(const char *filename, struct config_t *config, bool imported)
{
	...
	if (libconfig->setting_lookup_string(setting, "order", &temp) == CONFIG_TRUE) {
		if (strcmpi(temp, "deny,allow") == 0) {
			...
			ShowWarning(
			    "socket_config_read: invalid value '%s' for socket_configuration/ip_rules/order.\n", temp
			);
		}
	}
	...
	if ((setting = libconfig->lookup(config, "socket_configuration/ip_rules/allow_list")) == NULL) {
		if (!imported)
			ShowError(
			    "socket_config_read: socket_configuration/ip_rules/allow_list was not found in %s!\n",
			    filename
			);
	} else {
	...
	if ((setting = libconfig->lookup(config, "socket_configuration/ip_rules/deny_list")) == NULL) {
		if (!imported)
			ShowError(
			    "socket_config_read: socket_configuration/ip_rules/deny_list was not found in %s!\n",
			    filename
			);
	} else {
...
static void socket_close(int fd)
{
	...
	sClose(fd); // We don't really care if these closing functions return an error, we are just shutting down and
	            // not reusing this socket.
	if (sockt->session[fd])
		...
		if (0 == getrlimit(RLIMIT_NOFILE, &rlp)) {
			...
			if (0
				...
				if (0 != setrlimit(RLIMIT_NOFILE, &rlp)) { // failed
					...
					ShowWarning(
					    "socket_init: failed to set socket limit to %d, setting to maximum allowed (original limit=%d, current limit=%d, maximum allowed=%d, %s).\n",
...
static void send_shortlist_add_fd(int fd)
{
	...
	if (send_shortlist_count >= ARRAYLENGTH(send_shortlist_array)) {
		ShowDebug(
		    "send_shortlist_add_fd: shortlist is full, ignoring... (fd=%d shortlist.count=%d shortlist.length=%d)\n",
...
static uint32 socket_lan_subnet_check(uint32 ip, struct s_subnet *info)
{
	int i;
	ARR_FIND(
	    0, VECTOR_LENGTH(sockt->lan_subnets), i,
	    SUBNET_MATCH(ip, VECTOR_INDEX(sockt->lan_subnets, i).ip, VECTOR_INDEX(sockt->lan_subnets, i).mask)
	);
...
static bool socket_allowed_ip_check(uint32 ip)
{
	int i;
	ARR_FIND(
	    0, VECTOR_LENGTH(sockt->allowed_ips), i,
	    SUBNET_MATCH(ip, VECTOR_INDEX(sockt->allowed_ips, i).ip, VECTOR_INDEX(sockt->allowed_ips, i).mask)
	);
...
static bool socket_trusted_ip_check(uint32 ip)
{
	int i;
	ARR_FIND(
	    0, VECTOR_LENGTH(sockt->trusted_ips), i,
	    SUBNET_MATCH(ip, VECTOR_INDEX(sockt->trusted_ips, i).ip, VECTOR_INDEX(sockt->trusted_ips, i).mask)
	);
 ...
 */
static int socket_net_config_read_sub(
...
)
{
	...
	for (i = 0; i < len; ++i) {
		...
		if (sscanf(subnet, "%63[^:]:%63[^:]", ipbuf, maskbuf) != 2) {
			ShowWarning(
			    "Invalid IP:Subnet entry in configuration file %s: '%s' (%s)\n", filename, subnet, groupname
			);
			continue;
		}
...
static void socket_net_config_read(const char *filename)
{
	...
	if (!libconfig->load_file(&network_config, filename)) {
		ShowError(
		    "LAN Support configuration file is not found: '%s'. This server won't be able to accept connections from any servers.\n",
		    filename
		);
		return;
	}
	...
	VECTOR_CLEAR(sockt->lan_subnets);
	if (sockt->net_config_read_sub(
	        libconfig->lookup(&network_config, "lan_subnets"), &sockt->lan_subnets, filename, "lan_subnets"
	    )
	    > 0)
	...
	VECTOR_CLEAR(sockt->trusted_ips);
	if (sockt->net_config_read_sub(
	        libconfig->lookup(&network_config, "trusted"), &sockt->trusted_ips, filename, "trusted"
	    )
	    > 0)
		ShowStatus("Read information about %d trusted IP ranges.\n", (int)VECTOR_LENGTH(sockt->trusted_ips));
	ARR_FIND(
	    0, VECTOR_LENGTH(sockt->trusted_ips), i,
	    SUBNET_MATCH(0, VECTOR_INDEX(sockt->trusted_ips, i).ip, VECTOR_INDEX(sockt->trusted_ips, i).mask)
	);
	if (i != VECTOR_LENGTH(sockt->trusted_ips)) {
	...
	VECTOR_CLEAR(sockt->allowed_ips);
	if (sockt->net_config_read_sub(
	        libconfig->lookup(&network_config, "allowed"), &sockt->allowed_ips, filename, "allowed"
	    )
	    > 0)
		ShowStatus(
		    "Read information about %d allowed server IP ranges.\n", (int)VECTOR_LENGTH(sockt->allowed_ips)
		);
	if (VECTOR_LENGTH(sockt->allowed_ips) + VECTOR_LENGTH(sockt->trusted_ips) == 0) {
		ShowError(
		    "No allowed server IP ranges configured. This server won't be able to accept connections from any char servers.\n"
		);
	}
	ARR_FIND(
	    0, VECTOR_LENGTH(sockt->allowed_ips), i,
	    SUBNET_MATCH(0, VECTOR_INDEX(sockt->allowed_ips, i).ip, VECTOR_INDEX(sockt->allowed_ips, i).mask)
	);
#ifndef BUILDBOT
	if (i != VECTOR_LENGTH(sockt->allowed_ips)) {
...
static void socket_validateWfifo(int fd, size_t len)
{
	...
	if (packet_len == -1) {
		...
		if (packet_len != len2) {
			ShowError(
			    "Sent packet 0x%04X with dynamic size %d, but must be size %d\n", cmd, len2, packet_len
			);
			Assert_retv(0);
		}
	...
	if (last_head_size < (uint32)packet_len) {
		ShowError(
		    "Reserved too small packet buffer for packet 0x%04X with size %u, but must be size %d\n", cmd,
		    last_head_size, packet_len
		);
		Assert_retv(0);
	}
}
