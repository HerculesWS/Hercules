...
#define FILELIST_ADDS 1024 // number increment of file lists `
	...
	if (filelist_entrys >= filelist_maxentry) {
		filelist = (struct grf_filelist *)aRealloc(
		    filelist, (filelist_maxentry + FILELIST_ADDS) * sizeof(struct grf_filelist)
		);
...
static void *grfio_reads(const char *fname, int *size)
{
	...
	if (entry == NULL || entry->gentry <= 0) {
		...
		if (in != NULL) {
			...
			hseek(in, 0, SEEK_SET);
			unsigned char *buf
			    = (unsigned char *)aMalloc(declen + 1); // +1 for resnametable zero-termination
			buf[declen] = '\0';
	...
	if (entry != NULL && entry->gentry > 0) {
		...
		if (in != NULL) {
			...
			if (hseek(in, entry->srcpos, SEEK_SET) != 0 || fread(buf, 1, fsize, in) != (size_t)fsize) {
...
static int grfio_entryread(const char *grfname, int gentry)
{
	...
	if (strcmp((const char *)grf_header, "Master of Magic") != 0
	    || hseek(fp, getlong(grf_header + 0x1E), SEEK_CUR) != 0) {
		fclose(fp);
	...
	if (grf_version == 0x01) {
		...
		for (entry = 0, ofs = 0; entry < entrys; ++entry) {
			...
			if (type & FILELIST_TYPE_FILE) {
				...
				type |= grfio_is_full_encrypt(fname) ? FILELIST_TYPE_ENCRYPT_MIXED
