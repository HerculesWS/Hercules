...
packetLen(
    0x2AFC, 10
) /* M->H, chrif_scdata_request -> request sc_data for pc->authok'ed char. <- new command reuses previous one. */
...
packetLen(0x2B02, 18)  /* M->H, chrif_charselectreq -> 'player returns from ingame to charserver to select another
                          char.., this packets includes sessid etc' ? (not 100% sure) */
...
packetLen(
    0x2B1E, 2
) /* H->M, chrif_update_ip -> 'Request forwarded from char-server for interserver IP sync.' [Lance] */
packetLen(
    0x2B1F, 7
) /* H->M, chrif_disconnectplayer -> 'disconnects a player (aid X) with the message XY ... 0x81 ..' [Sirius] */
packetLen(0x2B20, 0) /* FREE */
packetLen(
    0x2B21, 10
) /* H->M, chrif_save_ack. Returned after a character has been "final saved" on the char-server. [Skotlex] */
