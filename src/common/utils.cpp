...
void findfile(const char *p, const char *pat, void(func)(const char *, void *context), void *context)
{
	...
	while ((entry = readdir(dir))) {
		...
		if (snprintf(tmppath, sizeof(tmppath), "%s%c%s", path, PATHSEP, entry->d_name)
		    >= (ssize_t)sizeof(tmppath)) {
...
uint64 get_percentage64(const uint64 A, const uint64 B)
{
	...
	if (result > UINT_MAX) {
		ShowError(
		    "get_percentage(): result percentage too high! (A=%" PRIu64 ",B=%" PRIu64 ",result=%g)\n", A, B,
		    result
		);
		return UINT_MAX;
	}
