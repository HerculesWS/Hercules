	...
	for (i = 0, j = 0; i < len;) {
		if (src[i] == '\\') {
			...
			if (i >= len)
				...
				out_dest[j++] = (char)c;
			} else if (src[i] == '0' || src[i] == '1' || src[i] == '2' || src[i] == '3') { // octal escape
				                                                                       // sequence
				                                                                       // (255=0377)
...
)
{
	...
	while (fgets(line, sizeof(line), fp)) {
		...
		columns = sv_split(
		    line, (int)strlen(line), 0, delim, fields, fields_length,
