...
/* ATCMD_FUNC(mobinfo) HIT, FLEE, ATK, ATK2, MATK and MATK2 calculations */
  ...
  #define RE_SKILL_REDUCTION() \
	  do { \
		  wd.damage = battle->calc_elefix( \
		      src, target, skill_id, skill_lv, \
		      battle->calc_cardfix(BF_WEAPON, src, target, nk, s_ele, s_ele_, wd.damage, 0, wd.flag), nk, \
		      n_ele, s_ele, s_ele_, false, flag.arrow \
		  ); \
		  if (flag.lh) \
			  wd.damage2 = battle->calc_elefix( \
			      src, target, skill_id, skill_lv, \
			      battle->calc_cardfix(BF_WEAPON, src, target, nk, s_ele, s_ele_, wd.damage2, 1, wd.flag), \
			      nk, n_ele, s_ele, s_ele_, true, flag.arrow \
			  ); \
	  } while (0)
#else
