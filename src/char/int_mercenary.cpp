...
static bool inter_mercenary_owner_fromsql(int char_id, struct mmo_charstatus *status)
{
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle,
	        "SELECT `merc_id`, `arch_calls`, `arch_faith`, `spear_calls`, `spear_faith`, `sword_calls`, `sword_faith` FROM `%s` WHERE `char_id` = '%d'",
	        mercenary_owner_db, char_id
	    )) {
...
static bool inter_mercenary_owner_tosql(int char_id, struct mmo_charstatus *status)
{
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle,
	        "REPLACE INTO `%s` (`char_id`, `merc_id`, `arch_calls`, `arch_faith`, `spear_calls`, `spear_faith`, `sword_calls`, `sword_faith`) VALUES ('%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d')",
	        mercenary_owner_db, char_id, status->mer_id, status->arch_calls, status->arch_faith,
...
{
	if (SQL_ERROR
	    == SQL->Query(inter->sql_handle, "DELETE FROM `%s` WHERE `char_id` = '%d'", mercenary_owner_db, char_id))
		Sql_ShowDebug(inter->sql_handle);

	if (SQL_ERROR
	    == SQL->Query(inter->sql_handle, "DELETE FROM `%s` WHERE `char_id` = '%d'", mercenary_db, char_id))
		Sql_ShowDebug(inter->sql_handle);
...
{
	if (SQL_ERROR
	    == SQL->Query(inter->sql_handle, "DELETE FROM `%s` WHERE `mer_id` = '%d'", mercenary_db, merc_id)) {
		Sql_ShowDebug(inter->sql_handle);
		return false;
	}
...
static int inter_mercenary_parse_frommap(int fd)
{
	...
	switch (cmd) {
	case 0x3070:
		mapif->parse_mercenary_create(fd, RFIFOP(struct s_mercenary *, fd, 4));
		break;
	case 0x3071:
	...
	case 0x3072:
		...
		break;
	case 0x3073:
		mapif->parse_mercenary_save(fd, RFIFOP(struct s_mercenary *, fd, 4));
		break;
	default:
		return 0;
	}
