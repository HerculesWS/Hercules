...
static void inter_accinfo(int u_fd, int aid, int castergroup, const char *query, int map_fd)
{
	...
	if (account_id < START_ACCOUNT_NUM) {
		...
		if (SQL_ERROR
		        == SQL->Query(
		            inter->sql_handle,
		            "SELECT `account_id`,`name`,`class`,`base_level`,`job_level`,`online` FROM `%s` WHERE `name` LIKE '%s' LIMIT 10",
		            char_db, query_esq
		        )
		    || SQL->NumRows(inter->sql_handle) == 0) {
			if (SQL->NumRows(inter->sql_handle) == 0) {
				inter->msg_to_fd(
				    map_fd, u_fd, aid, "No matches were found for your criteria, '%s'", query
				);
			} else {
			...
			if (SQL->NumRows(inter->sql_handle) == 1) { // we found a perfect match
				...
				while (SQL_SUCCESS == SQL->NextRow(inter->sql_handle)) {
					...
					inter->msg_to_fd(
					    map_fd, u_fd, aid, "[AID: %d] %s | %s | Level: %d/%d | %s", account_id,
					    name, inter->job_name(class_), base_level, job_level,
...
// enable this if you really know what you doing.
	...
	inter->msg_to_fd(
	    map_fd, u_fd, u_aid, "This user has logged %d times, the last time were at %s", logincount, lastlogin
	);
	...
	if (SQL_ERROR
	        == SQL->Query(
	            inter->sql_handle,
	            "SELECT `char_id`, `name`, `char_num`, `class`, `base_level`, `job_level`, `online` " "FROM `%s` WHERE `account_id` = '%d' ORDER BY `char_num` LIMIT %d",
		...
		while (SQL_SUCCESS == SQL->NextRow(inter->sql_handle)) {
			...
			inter->msg_to_fd(
			    map_fd, u_fd, u_aid, "[Slot/CID: %d/%d] %s | %s | Level: %d/%d | %s", char_num, char_id,
			    name, inter->job_name(class_), base_level, job_level, online ? "On" : "Off"
			);
		}
	}
	SQL->FreeResult(inter->sql_handle);
 ...
 **/
static void
    inter_savereg(int account_id, int char_id, const char *key, unsigned int index, intptr_t val, bool is_string)
{
	char val_esq[1000];
	nullpo_retv(key);
	...
	if (key[0] == '#' && key[1] == '#') { /* global account reg */
		if (sockt->session_is_valid(chr->login_fd))
			...
			ShowError(
			    "Login server unavailable, cant perform update on '%s' variable for AID:%d CID:%d\n", key,
			    account_id, char_id
			);
		}
		...
		if (is_string) {
			if (val) {
				...
				if (SQL_ERROR
				    == SQL->Query(
				        ...
				        acc_reg_str_db, account_id, key, index, val_esq
				    ))
					Sql_ShowDebug(inter->sql_handle);
			} else {
				if (SQL_ERROR
				    == SQL->Query(
				        ...
				        acc_reg_str_db, account_id, key, index
				    ))
					Sql_ShowDebug(inter->sql_handle);
			}
		} else {
			if (val) {
				if (SQL_ERROR
				    == SQL->Query(
				        inter->sql_handle,
				        "REPLACE INTO `%s` (`account_id`,`key`,`index`,`value`) VALUES ('%d','%s','%u','%d')",
				        acc_reg_num_db, account_id, key, index, (int)val
				    ))
					Sql_ShowDebug(inter->sql_handle);
			} else {
				if (SQL_ERROR
				    == SQL->Query(
				        inter->sql_handle,
				        "DELETE FROM `%s` WHERE `account_id` = '%d' AND `key` = '%s' AND `index` = '%u' LIMIT 1",
				        acc_reg_num_db, account_id, key, index
				    ))
					Sql_ShowDebug(inter->sql_handle);
			}
		}
		...
		if (is_string) {
			if (val) {
				...
				if (SQL_ERROR
				    == SQL->Query(
				        inter->sql_handle,
				        "REPLACE INTO `%s` (`char_id`,`key`,`index`,`value`) VALUES ('%d','%s','%u','%s')",
				        char_reg_str_db, char_id, key, index, val_esq
				    ))
					Sql_ShowDebug(inter->sql_handle);
			} else {
				if (SQL_ERROR
				    == SQL->Query(
				        inter->sql_handle,
				        "DELETE FROM `%s` WHERE `char_id` = '%d' AND `key` = '%s' AND `index` = '%u' LIMIT 1",
				        char_reg_str_db, char_id, key, index
				    ))
					Sql_ShowDebug(inter->sql_handle);
			}
		} else {
			if (val) {
				if (SQL_ERROR
				    == SQL->Query(
				        inter->sql_handle,
				        "REPLACE INTO `%s` (`char_id`,`key`,`index`,`value`) VALUES ('%d','%s','%u','%d')",
				        char_reg_num_db, char_id, key, index, (int)val
				    ))
					Sql_ShowDebug(inter->sql_handle);
			} else {
				if (SQL_ERROR
				    == SQL->Query(
				        inter->sql_handle,
				        "DELETE FROM `%s` WHERE `char_id` = '%d' AND `key` = '%s' AND `index` = '%u' LIMIT 1",
				        char_reg_num_db, char_id, key, index
				    ))
					Sql_ShowDebug(inter->sql_handle);
			}
		}
...
static int inter_accreg_fromsql(int account_id, int char_id, int fd, int type)
{
	...
	case 3: // char reg
		if (SQL_ERROR
		    == SQL->Query(
		        inter->sql_handle, "SELECT `key`, `index`, `value` FROM `%s` WHERE `char_id`='%d'",
		        char_reg_str_db, char_id
		    ))
			Sql_ShowDebug(inter->sql_handle);
		break;
	case 2: // account reg
		if (SQL_ERROR
		    == SQL->Query(
		        inter->sql_handle, "SELECT `key`, `index`, `value` FROM `%s` WHERE `account_id`='%d'",
		        acc_reg_str_db, account_id
		    ))
			Sql_ShowDebug(inter->sql_handle);
		break;
	case 1: // account2 reg
		ShowError(
		    "inter->accreg_fromsql: Char server shouldn't handle type 1 registry values (##). That is the login server's work!\n"
		);
		return 0;
	default:
...
static bool inter_config_read_connection(const char *filename, const struct config_t *config, bool imported)
{
	...
	libconfig->setting_lookup_mutable_string(
	    setting, "default_codepage", default_codepage, sizeof(default_codepage)
	);
...
static int inter_vlog(const char *fmt, va_list ap)
{
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle, "INSERT INTO `%s` (`time`, `log`) VALUES (NOW(),  '%s')", interlog_db, esc_str
	    ))
		Sql_ShowDebug(inter->sql_handle);
...
static int inter_init_sql(const char *file)
{
	...
	if (SQL_ERROR
	    == SQL->Connect(
	        inter->sql_handle, char_server_id, char_server_pw, char_server_ip, (uint16)char_server_port,
	        char_server_db
	    )) {
		Sql_ShowDebug(inter->sql_handle);
...
static int inter_parse_frommap(int fd)
{
	...
	if (cmd < MIN_INTER_PACKET_DB
	    || cmd >= MAX_INTER_PACKET_DB
	    || packets->inter_db[cmd - MIN_INTER_PACKET_DB] == 0)
		return 0;
