...
static bool inter_elemental_create(struct s_elemental *ele)
{
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle,
	        "INSERT INTO `%s` (`char_id`,`class`,`mode`,`hp`,`sp`,`max_hp`,`max_sp`,`atk1`,`atk2`,`matk`,`aspd`,`def`,`mdef`,`flee`,`hit`,`life_time`)" "VALUES ('%d','%d','%u','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d')",
	        elemental_db, ele->char_id, ele->class_, ele->mode, ele->hp, ele->sp, ele->max_hp, ele->max_sp,
	        ele->atk, ele->atk2, ele->matk, ele->amotion, ele->def, ele->mdef, ele->flee, ele->hit, ele->life_time
	    )) {
...
static bool inter_elemental_save(const struct s_elemental *ele)
{
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle,
	        "UPDATE `%s` SET `char_id` = '%d', `class` = '%d', `mode` = '%u', `hp` = '%d', `sp` = '%d'," "`max_hp` = '%d', `max_sp` = '%d', `atk1` = '%d', `atk2` = '%d', `matk` = '%d', `aspd` = '%d', `def` = '%d'," "`mdef` = '%d', `flee` = '%d', `hit` = '%d', `life_time` = '%d' WHERE `ele_id` = '%d'",
	        elemental_db, ele->char_id, ele->class_, ele->mode, ele->hp, ele->sp, ele->max_hp, ele->max_sp,
	        ele->atk, ele->atk2, ele->matk, ele->amotion, ele->def, ele->mdef, ele->flee, ele->hit, ele->life_time,
	        ele->elemental_id
	    )) {
...
{
	if (SQL_ERROR
	    == SQL->Query(inter->sql_handle, "DELETE FROM `%s` WHERE `ele_id` = '%d'", elemental_db, ele_id)) {
		Sql_ShowDebug(inter->sql_handle);
		return false;
	}
...
static int inter_elemental_parse_frommap(int fd)
{
	...
	switch (cmd) {
	case 0x307C:
		mapif->parse_elemental_create(fd, RFIFOP(struct s_elemental *, fd, 4));
		break;
	case 0x307D:
	...
	case 0x307E:
		...
		break;
	case 0x307F:
		mapif->parse_elemental_save(fd, RFIFOP(struct s_elemental *, fd, 4));
		break;
	default:
		return 0;
	}
