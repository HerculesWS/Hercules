...
static int inter_auction_count(int char_id, bool buy)
{
	...
	for (
	    struct auction_data *auction = (struct auction_data *)dbi_first(iter); dbi_exists(iter);
	    auction                      = (struct auction_data *)dbi_next(iter)
	) {
...
static void inter_auction_save(struct auction_data *auction)
{
	...
	StrBuf->Printf(
	    &buf,
	    "UPDATE `%s` SET `seller_id` = '%d', `seller_name` = ?, `buyer_id` = '%d', `buyer_name` = ?, `price` = '%d', `buynow` = '%d', `hours` = '%d', `timestamp` = '%lu', `nameid` = '%d', `item_name` = ?, `type` = '%d', `refine` = '%d', `grade` = '%d', `attribute` = '%d'",
	    auction_db, auction->seller_id, auction->buyer_id, auction->price, auction->buynow, auction->hours,
	    (unsigned long)auction->timestamp, auction->item.nameid, auction->type, auction->item.refine,
	    auction->item.grade, auction->item.attribute
	);
	for (j = 0; j < MAX_SLOTS; j++)
		StrBuf->Printf(&buf, ", `card%d` = '%d'", j, auction->item.card[j]);
	for (j = 0; j < MAX_ITEM_OPTIONS; j++)
		StrBuf->Printf(
		    &buf, ", `opt_idx%d` = '%d', `opt_val%d` = '%d'", j, auction->item.option[j].index, j,
		    auction->item.option[j].value
		);
	...
	if (SQL_SUCCESS != SQL->StmtPrepareStr(stmt, StrBuf->Value(&buf))
	    || SQL_SUCCESS
	           != SQL->StmtBindParam(
	               stmt, 0, SQLDT_STRING, auction->seller_name, strnlen(auction->seller_name, NAME_LENGTH)
	           )
	    || SQL_SUCCESS
	           != SQL->StmtBindParam(
	               stmt, 1, SQLDT_STRING, auction->buyer_name, strnlen(auction->buyer_name, NAME_LENGTH)
	           )
	    || SQL_SUCCESS
	           != SQL->StmtBindParam(
	               stmt, 2, SQLDT_STRING, auction->item_name, strnlen(auction->item_name, ITEM_NAME_LENGTH)
	           )
...
static unsigned int inter_auction_create(struct auction_data *auction)
{
	...
	StrBuf->Printf(
	    &buf,
	    "INSERT INTO `%s` (`seller_id`,`seller_name`,`buyer_id`,`buyer_name`,`price`,`buynow`,`hours`,`timestamp`,`nameid`,`item_name`,`type`,`refine`,`grade`,`attribute`,`unique_id`",
	    auction_db
	);
	for (j = 0; j < MAX_SLOTS; j++)
	...
	StrBuf->Printf(
	    &buf, ") VALUES ('%d',?,'%d',?,'%d','%d','%d','%lu','%d',?,'%d','%d','%d','%d','%" PRIu64 "'",
	    auction->seller_id, auction->buyer_id, auction->price, auction->buynow, auction->hours,
	    (unsigned long)auction->timestamp, auction->item.nameid, auction->type, auction->item.refine,
	    auction->item.grade, auction->item.attribute, auction->item.unique_id
	);
	for (j = 0; j < MAX_SLOTS; j++)
	...
	if (SQL_SUCCESS != SQL->StmtPrepareStr(stmt, StrBuf->Value(&buf))
		...
		auction->auction_end_timer
		    = timer->add(timer->gettick() + tick, inter_auction->end_timer, auction->auction_id, 0);
		ShowInfo(
		    "New Auction %u | time left %" PRId64 " ms | By %s.\n", auction->auction_id, tick,
		    auction->seller_name
		);
	...
	if ((auction = (struct auction_data *)idb_get(inter_auction->db, id)) != NULL) {
		if (auction->buyer_id) {
			inter_mail->sendmail(
			    ...
			    "Thanks, you won the auction!.", 0, &auction->item
			);
			mapif->auction_message(auction->buyer_id, 6); // You have won the auction
			inter_mail->sendmail(
			    0, "Auction Manager", auction->seller_id, auction->seller_name, "Auction",
			    "Payment for your auction!.", auction->price, NULL
			);
		} else
			inter_mail->sendmail(
			    0, "Auction Manager", auction->seller_id, auction->seller_name, "Auction",
			    "No buyers have been found for your auction.", 0, &auction->item
			);
...
static void inter_auction_delete(struct auction_data *auction)
{
	...
	if (SQL_ERROR
	    == SQL->Query(inter->sql_handle, "DELETE FROM `%s` WHERE `auction_id` = '%u'", auction_db, auction_id))
...
static void inter_auctions_fromsql(void)
{
	...
	StrBuf->AppendStr(
	    &buf,
	    "SELECT `auction_id`,`seller_id`,`seller_name`,`buyer_id`,`buyer_name`," "`price`,`buynow`,`hours`,`timestamp`,`nameid`,`item_name`,`type`,`refine`,`grade`,`attribute`,`unique_id`"
	);
