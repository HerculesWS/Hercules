 ...
 *
 * Table structure:
 * `pet` (`pet_id`, `class`, `name`, `account_id`, `char_id`, `level`, `egg_id`, `equip`, `intimate`, `hungry`,
 * `rename_flag`, `incubate`, `autofeed`)
 *
 * @remark In case of newly created pet, the pet ID is not updated to reflect the newly assigned ID. The caller must do
 * so.
 *
...
static int inter_pet_tosql(const struct s_pet *p)
{
	...
	if (p->pet_id == 0) { // New pet.
		...
		if (SQL_ERROR == SQL->StmtPrepare(stmt, query, pet_db)
		    ...
		    || SQL_ERROR
		           == SQL->StmtBindParam(stmt, 1, SQLDT_STRING, &p->name, strnlen(p->name, sizeof(p->name)))
...
static int inter_pet_fromsql(int pet_id, struct s_pet *p)
{
	...
	if (SQL_ERROR == SQL->StmtPrepare(stmt, query, pet_db)
	    ...
	    || SQL_ERROR
	           == SQL->StmtBindColumn(stmt, 9, SQLDT_INT8, &p->rename_flag, sizeof(p->rename_flag), NULL, NULL)
 ...
 *
 **/
static struct s_pet *inter_pet_create(
    int account_id, int char_id, int pet_class, int pet_lv, int pet_egg_id, int pet_equip, short intimate, short hungry,
