...
static int inter_clan_kick_inactive_members(int clan_id, int kick_interval)
{
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle,
	        "UPDATE `%s` SET " "`clan_id` = 0 WHERE `clan_id` = '%d' AND `online` = 0 AND `last_login` < %" PRId64,
...
static int inter_clan_count_members(int clan_id, int kick_interval)
{
	...
	if (SQL_ERROR
	        == SQL->StmtPrepare(
	            stmt, "SELECT COUNT(*) FROM `%s` WHERE `clan_id` = ? AND `last_login` >= %" PRId64, char_db,
	            (int64)(time(NULL) - kick_interval)
	        )
