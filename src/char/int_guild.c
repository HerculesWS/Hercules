...
struct guild * inter_guild_fromsql(int guild_id) {
	...
	SQL->GetData(sql_handle,  5, &data, NULL); g->max_storage = atoi(data);
	...
	SQL->GetData(sql_handle,  6, &data, NULL); g->average_lv = atoi(data);
	SQL->GetData(sql_handle,  7, &data, NULL); g->exp = strtoull(data, NULL, 10);
	SQL->GetData(sql_handle,  8, &data, NULL); g->next_exp = (unsigned int)strtoul(data, NULL, 10);
	SQL->GetData(sql_handle,  9, &data, NULL); g->skill_point = atoi(data);
	SQL->GetData(sql_handle, 10, &data, &len); memcpy(g->mes1, data, min(len, sizeof(g->mes1)));
	SQL->GetData(sql_handle, 11, &data, &len); memcpy(g->mes2, data, min(len, sizeof(g->mes2)));
	SQL->GetData(sql_handle, 12, &data, &len); g->emblem_len = atoi(data);
	SQL->GetData(sql_handle, 13, &data, &len); g->emblem_id = atoi(data);
	SQL->GetData(sql_handle, 14, &data, &len);
