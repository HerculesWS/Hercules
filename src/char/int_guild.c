...
static struct guild *inter_guild_fromsql(int guild_id)
{
	...
	g->guild_id = guild_id;
	SQL->GetData(inter->sql_handle,  0, &data, &len); memcpy(g->name, data, HMIN(len, (size_t)NAME_LENGTH));
	SQL->GetData(inter->sql_handle,  1, &data, &len); memcpy(g->master, data, HMIN(len, (size_t)NAME_LENGTH));
	...
	SQL->GetData(inter->sql_handle, 10, &data, &len); memcpy(g->mes1, data, HMIN(len, sizeof(g->mes1)));
	SQL->GetData(inter->sql_handle, 11, &data, &len); memcpy(g->mes2, data, HMIN(len, sizeof(g->mes2)));
	...
	for( i = 0; i < g->max_member && SQL_SUCCESS == SQL->NextRow(inter->sql_handle); ++i )
	{
		...
		SQL->GetData(inter->sql_handle, 11, &data, &len); memcpy(m->name, data, HMIN(len, (size_t)NAME_LENGTH));
	...
	while( SQL_SUCCESS == SQL->NextRow(inter->sql_handle) )
	{
		...
		SQL->GetData(inter->sql_handle, 1, &data, &len); memcpy(pos->name, data, HMIN(len, (size_t)NAME_LENGTH));
	...
	for( i = 0; i < MAX_GUILDALLIANCE && SQL_SUCCESS == SQL->NextRow(inter->sql_handle); ++i )
	{
		...
		SQL->GetData(inter->sql_handle, 2, &data, &len); memcpy(a->name, data, HMIN(len, (size_t)NAME_LENGTH));
	}
	...
	for( i = 0; i < MAX_GUILDEXPULSION && SQL_SUCCESS == SQL->NextRow(inter->sql_handle); ++i )
	{
		...
		SQL->GetData(inter->sql_handle, 2, &data, &len); memcpy(e->name, data, HMIN(len, (size_t)NAME_LENGTH));
		SQL->GetData(inter->sql_handle, 3, &data, &len); memcpy(e->mes, data, HMIN(len, sizeof(e->mes)));
	}
