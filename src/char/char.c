...
static int char_mmo_char_tosql(int char_id, struct mmo_charstatus *p)
{
	...
	if ((p->base_exp != cp->base_exp) || (p->base_level != cp->base_level) || (p->job_level != cp->job_level) || (p->job_exp != cp->job_exp) || (p->zeny != cp->zeny) || (p->last_point.map != cp->last_point.map) || (p->last_point.x != cp->last_point.x) || (p->last_point.y != cp->last_point.y) || (p->max_hp != cp->max_hp) || (p->hp != cp->hp) || (p->max_sp != cp->max_sp) || (p->sp != cp->sp) || (p->status_point != cp->status_point) || (p->skill_point != cp->skill_point) || (p->str != cp->str) || (p->agi != cp->agi) || (p->vit != cp->vit) || (p->int_ != cp->int_) || (p->dex != cp->dex) || (p->luk != cp->luk) || (p->option != cp->option) || (p->party_id != cp->party_id) || (p->guild_id != cp->guild_id) || (p->pet_id != cp->pet_id) || (p->look.weapon != cp->look.weapon) || (p->hom_id != cp->hom_id) || (p->ele_id != cp->ele_id) || (p->look.shield != cp->look.shield) || (p->look.head_top != cp->look.head_top) || (p->look.head_mid != cp->look.head_mid) || (p->look.head_bottom != cp->look.head_bottom) || (p->delete_date != cp->delete_date) || (p->rename != cp->rename) || (p->slotchange != cp->slotchange) || (p->look.robe != cp->look.robe) || (p->show_equip != cp->show_equip) || (p->allow_party != cp->allow_party) || (p->font != cp->font) || (p->uniqueitem_counter != cp->uniqueitem_counter) || (p->hotkey_rowshift != cp->hotkey_rowshift) || (p->hotkey_rowshift2 != cp->hotkey_rowshift2) || (p->clan_id != cp->clan_id) || (p->last_login != cp->last_login) || (p->title_id != cp->title_id) || (p->inventorySize != cp->inventorySize) || (p->allow_call != cp->allow_call)) {
		// Save status
		unsigned int opt = 0;
		...
		if (SQL_ERROR == SQL->Query(inter->sql_handle,
		                            ...
		                            char_db, p->base_level, p->job_level, p->base_exp, p->job_exp, p->zeny, p->max_hp, p->hp, p->max_sp, p->sp, p->status_point, p->skill_point, p->str, p->agi, p->vit, p->int_, p->dex, p->luk, p->option, p->party_id, p->guild_id, p->pet_id, p->hom_id, p->ele_id, p->look.weapon, p->look.shield, p->look.head_top, p->look.head_mid, p->look.head_bottom, mapindex_id2name(p->last_point.map), p->last_point.x, p->last_point.y, mapindex_id2name(p->save_point.map), p->save_point.x, p->save_point.y, p->rename,
		                            (unsigned long)p->delete_date, // FIXME: platform-dependent size
		                            p->look.robe, p->slotchange, opt, p->font, p->uniqueitem_counter, p->hotkey_rowshift, p->hotkey_rowshift2, p->clan_id, p->last_login, p->title_id, p->inventorySize, p->account_id, p->char_id)) {
			Sql_ShowDebug(inter->sql_handle);
			errors++;
		} else
	...
	if ((p->hair != cp->hair) || (p->hair_color != cp->hair_color) || (p->clothes_color != cp->clothes_color) || (p->body != cp->body) || (p->class != cp->class) || (p->partner_id != cp->partner_id) || (p->father != cp->father) || (p->mother != cp->mother) || (p->child != cp->child) || (p->karma != cp->karma) || (p->manner != cp->manner) || (p->fame != cp->fame)) {
		if (SQL_ERROR == SQL->Query(inter->sql_handle,
		                            ...
		                            " WHERE  `account_id`='%d' AND `char_id` = '%d'",
		                            char_db, p->class, p->hair, p->hair_color, p->clothes_color, p->body, p->partner_id, p->father, p->mother, p->child, p->karma, p->manner, p->fame, p->account_id, p->char_id)) {
			Sql_ShowDebug(inter->sql_handle);
			errors++;
		} else
	...
	/* Mercenary Owner */
	if ((p->mer_id != cp->mer_id) || (p->arch_calls != cp->arch_calls) || (p->arch_faith != cp->arch_faith) || (p->spear_calls != cp->spear_calls) || (p->spear_faith != cp->spear_faith) || (p->sword_calls != cp->sword_calls) || (p->sword_faith != cp->sword_faith)) {
...
static int char_getitemdata_from_sql(struct item *items, int max, int guid, enum inventory_table_type table)
{
	...
	if (SQL_ERROR == SQL->StmtPrepareStr(stmt, StrBuf->Value(&buf)) || SQL_ERROR == SQL->StmtBindParam(stmt, 0, SQLDT_INT, &guid, sizeof guid) || SQL_ERROR == SQL->StmtExecute(stmt)) {
	...
	if (SQL_ERROR == SQL->StmtBindColumn(stmt, 0, SQLDT_INT, &item.id, sizeof item.id, NULL, NULL) || SQL_ERROR == SQL->StmtBindColumn(stmt, 1, SQLDT_INT, &item.nameid, sizeof item.nameid, NULL, NULL) || SQL_ERROR == SQL->StmtBindColumn(stmt, 2, SQLDT_SHORT, &item.amount, sizeof item.amount, NULL, NULL) || SQL_ERROR == SQL->StmtBindColumn(stmt, 3, SQLDT_UINT, &item.equip, sizeof item.equip, NULL, NULL) || SQL_ERROR == SQL->StmtBindColumn(stmt, 4, SQLDT_CHAR, &item.identify, sizeof item.identify, NULL, NULL) || SQL_ERROR == SQL->StmtBindColumn(stmt, 5, SQLDT_CHAR, &item.refine, sizeof item.refine, NULL, NULL) || SQL_ERROR == SQL->StmtBindColumn(stmt, 6, SQLDT_CHAR, &item.grade, sizeof item.grade, NULL, NULL) || SQL_ERROR == SQL->StmtBindColumn(stmt, 7, SQLDT_CHAR, &item.attribute, sizeof item.attribute, NULL, NULL) || SQL_ERROR == SQL->StmtBindColumn(stmt, 8, SQLDT_UINT, &item.expire_time, sizeof item.expire_time, NULL, NULL) || SQL_ERROR == SQL->StmtBindColumn(stmt, 9, SQLDT_UCHAR, &item.bound, sizeof item.bound, NULL, NULL) || SQL_ERROR == SQL->StmtBindColumn(stmt, 10, SQLDT_UINT64, &item.unique_id, sizeof item.unique_id, NULL, NULL)) {
		SqlStmt_ShowDebug(stmt);
	}
	...
	for (i = 0; i < MAX_ITEM_OPTIONS; i++) {
		if (SQL_ERROR == SQL->StmtBindColumn(stmt, 11 + MAX_SLOTS + i * 2, SQLDT_INT16, &item.option[i].index, sizeof item.option[i].index, NULL, NULL) || SQL_ERROR == SQL->StmtBindColumn(stmt, 12 + MAX_SLOTS + i * 2, SQLDT_INT16, &item.option[i].value, sizeof item.option[i].index, NULL, NULL))
			SqlStmt_ShowDebug(stmt);
	}
...
static int char_memitemdata_to_sql(const struct item *p_items, int current_size, int guid, enum inventory_table_type table)
{
	...
	if ((db_size = chr->getitemdata_from_sql(cp_items, max_size, guid, table)) > 0) {
		...
		for (int i = 0; i < db_size; i++) {
			...
			int j = 0;
			ARR_FIND(0, current_size, j, matched_p[j] != true && p_items[j].nameid != 0 && cp_it->nameid == p_items[j].nameid && cp_it->unique_id == p_items[j].unique_id && memcmp(p_items[j].card, cp_it->card, sizeof(int) * MAX_SLOTS) == 0 && memcmp(p_items[j].option, cp_it->option, 5 * MAX_ITEM_OPTIONS) == 0);
			...
			if (j < current_size) {  // Item found.
				...
				if (memcmp(cp_it, &p_items[j], sizeof(struct item)) != 0) {
					...
					StrBuf->Printf(&buf, "%s('%d', '%d', '%d', '%d', '%u', '%d', '%d', '%d', '%d'", total_updates > 0 ? ", " : "", cp_it->id, guid, p_items[j].nameid, p_items[j].amount, p_items[j].equip, p_items[j].identify, p_items[j].refine, p_items[j].grade, p_items[j].attribute);
					for (int k = 0; k < MAX_SLOTS; k++)
	...
	for (int i = 0; i < current_size; i++) {
		...
		StrBuf->Printf(&buf, "%s('%d', '%d', '%d', '%u', '%d', '%d', '%d', '%d', '%u', '%d', '%" PRIu64 "'", total_inserts > 0 ? ", " : "", guid, p_it->nameid, p_it->amount, p_it->equip, p_it->identify, p_it->refine, p_it->grade, p_it->attribute, p_it->expire_time, p_it->bound, p_it->unique_id);
...
static int char_mmo_char_fromsql(int char_id, struct mmo_charstatus *p, bool load_everything)
{
	...
	if (SQL_ERROR == SQL->StmtPrepare(stmt, "SELECT `map`,`x`,`y` FROM `%s` WHERE `char_id`=? ORDER by `memo_id` LIMIT %d", memo_db, MAX_MEMOPOINTS) || SQL_ERROR == SQL->StmtBindParam(stmt, 0, SQLDT_INT, &char_id, sizeof char_id) || SQL_ERROR == SQL->StmtExecute(stmt) || SQL_ERROR == SQL->StmtBindColumn(stmt, 0, SQLDT_STRING, &point_map, sizeof point_map, NULL, NULL) || SQL_ERROR == SQL->StmtBindColumn(stmt, 1, SQLDT_INT16, &tmp_point.x, sizeof tmp_point.x, NULL, NULL) || SQL_ERROR == SQL->StmtBindColumn(stmt, 2, SQLDT_INT16, &tmp_point.y, sizeof tmp_point.y, NULL, NULL))
		SqlStmt_ShowDebug(stmt);
	...
	if (SQL_ERROR == SQL->StmtPrepare(stmt, "SELECT `id`, `lv`,`flag` FROM `%s` WHERE `char_id`=? LIMIT %d", skill_db, MAX_SKILL_DB) || SQL_ERROR == SQL->StmtBindParam(stmt, 0, SQLDT_INT, &char_id, sizeof char_id) || SQL_ERROR == SQL->StmtExecute(stmt) || SQL_ERROR == SQL->StmtBindColumn(stmt, 0, SQLDT_USHORT, &tmp_skill.id, sizeof tmp_skill.id, NULL, NULL) || SQL_ERROR == SQL->StmtBindColumn(stmt, 1, SQLDT_UCHAR, &tmp_skill.lv, sizeof tmp_skill.lv, NULL, NULL) || SQL_ERROR == SQL->StmtBindColumn(stmt, 2, SQLDT_UCHAR, &tmp_skill.flag, sizeof tmp_skill.flag, NULL, NULL)) {
		SqlStmt_ShowDebug(stmt);
	}
	...
	if (SQL_ERROR == SQL->StmtPrepare(stmt, "SELECT c.`account_id`, c.`char_id`, c.`name` FROM `%s` c LEFT JOIN `%s` f ON f.`friend_account` = c.`account_id` AND f.`friend_id` = c.`char_id` WHERE f.`char_id`=? LIMIT %d", char_db, friend_db, MAX_FRIENDS) || SQL_ERROR == SQL->StmtBindParam(stmt, 0, SQLDT_INT, &char_id, sizeof char_id) || SQL_ERROR == SQL->StmtExecute(stmt) || SQL_ERROR == SQL->StmtBindColumn(stmt, 0, SQLDT_INT, &tmp_friend.account_id, sizeof tmp_friend.account_id, NULL, NULL) || SQL_ERROR == SQL->StmtBindColumn(stmt, 1, SQLDT_INT, &tmp_friend.char_id, sizeof tmp_friend.char_id, NULL, NULL) || SQL_ERROR == SQL->StmtBindColumn(stmt, 2, SQLDT_STRING, &tmp_friend.name, sizeof tmp_friend.name, NULL, NULL)) {
		SqlStmt_ShowDebug(stmt);
	}
	...
	if (SQL_ERROR == SQL->StmtPrepare(stmt, "SELECT `bank_vault`,`base_exp`,`base_drop`,`base_death`,`attendance_count`, `attendance_timer` FROM `%s` WHERE `account_id`=? LIMIT 1", account_data_db) || SQL_ERROR == SQL->StmtBindParam(stmt, 0, SQLDT_INT, &account_id, sizeof account_id) || SQL_ERROR == SQL->StmtExecute(stmt) || SQL_ERROR == SQL->StmtBindColumn(stmt, 0, SQLDT_INT, &p->bank_vault, sizeof p->bank_vault, NULL, NULL) || SQL_ERROR == SQL->StmtBindColumn(stmt, 1, SQLDT_USHORT, &p->mod_exp, sizeof p->mod_exp, NULL, NULL) || SQL_ERROR == SQL->StmtBindColumn(stmt, 2, SQLDT_USHORT, &p->mod_drop, sizeof p->mod_drop, NULL, NULL) || SQL_ERROR == SQL->StmtBindColumn(stmt, 3, SQLDT_USHORT, &p->mod_death, sizeof p->mod_death, NULL, NULL) || SQL_ERROR == SQL->StmtBindColumn(stmt, 4, SQLDT_SHORT, &p->attendance_count, sizeof p->attendance_count, NULL, NULL) || SQL_ERROR == SQL->StmtBindColumn(stmt, 5, SQLDT_INT64, &p->attendance_timer, sizeof p->attendance_timer, NULL, NULL)) {
		SqlStmt_ShowDebug(stmt);
	}
...
static bool char_char_slotchange(struct char_session_data *sd, int fd, unsigned short from, unsigned short to)
{
	...
	if (sd->found_char[to] > 0) { /* moving char to occupied slot */
		bool result = false;
		/* update both at once */
		if (SQL_SUCCESS != SQL->QueryStr(inter->sql_handle, "START TRANSACTION") || SQL_SUCCESS != SQL->Query(inter->sql_handle, "UPDATE `%s` SET `char_num`='%d' WHERE `char_id`='%d' LIMIT 1", char_db, from, sd->found_char[to]) || SQL_SUCCESS != SQL->Query(inter->sql_handle, "UPDATE `%s` SET `char_num`='%d' WHERE `char_id`='%d' LIMIT 1", char_db, to, sd->found_char[from]))
			Sql_ShowDebug(inter->sql_handle);
		else
			result = true;
...
static int char_rename_char_sql(struct char_session_data *sd, int char_id)
{
	...
	if (chr->enable_logs) {
		if (SQL_ERROR == SQL->Query(inter->sql_handle,
		                            ...
		                            " '%d', '%d'"
		                            ")",
		                            charlog_db, sd->account_id, char_dat.char_id, char_dat.slot, char_dat.class, esc_name, char_dat.str, char_dat.agi, char_dat.vit, char_dat.int_, char_dat.dex, char_dat.luk, char_dat.hair, char_dat.hair_color))
			Sql_ShowDebug(inter->sql_handle);
	}
...
static int char_make_new_char_sql(struct char_session_data *sd, const char *name_, int str, int agi, int vit, int int_, int dex, int luk, int slot, int hair_color, int hair_style, int starting_class, uint8 sex)
{
	...
	if (SQL_ERROR == SQL->Query(inter->sql_handle,
	                            "INSERT INTO `%s` (`account_id`, `char_num`, `name`, `class`, `zeny`, `status_point`,`str`, `agi`, `vit`, `int`, `dex`, `luk`, `max_hp`, `hp`,"
	                            ...
	                            char_db, sd->account_id, slot, esc_name, starting_class, start_zeny, 48, str, agi, vit, int_, dex, luk, (40 * (100 + vit) / 100), (40 * (100 + vit) / 100), (11 * (100 + int_) / 100), (11 * (100 + int_) / 100), hair_style, hair_color, mapindex_id2name(start_point.map), start_point.x, start_point.y, mapindex_id2name(start_point.map), start_point.x, start_point.y, sex, FIXED_INVENTORY_SIZE)) {
		Sql_ShowDebug(inter->sql_handle);
	...
	// Insert the new char entry to the database
	if (SQL_ERROR == SQL->Query(inter->sql_handle,
	                            "INSERT INTO `%s` (`account_id`, `char_num`, `name`, `class`, `zeny`, `str`, `agi`, `vit`, `int`, `dex`, `luk`, `max_hp`, `hp`,"
	                            ...
	                            char_db, sd->account_id, slot, esc_name, starting_class, start_zeny, str, agi, vit, int_, dex, luk, (40 * (100 + vit) / 100), (40 * (100 + vit) / 100), (11 * (100 + int_) / 100), (11 * (100 + int_) / 100), hair_style, hair_color, mapindex_id2name(start_point.map), start_point.x, start_point.y, mapindex_id2name(start_point.map), start_point.x, start_point.y, FIXED_INVENTORY_SIZE)) {
		Sql_ShowDebug(inter->sql_handle);
		return -2; // No, stop the procedure!
	}
#endif
	...
	for (i = 0; i < VECTOR_LENGTH(start_items); i++) {
		...
		if (item->stackable) {
			if (SQL_ERROR == SQL->Query(inter->sql_handle, "INSERT INTO `%s` (`char_id`,`nameid`, `amount`, `identify`) VALUES ('%d', '%d', '%d', '%d')", inventory_db, char_id, item->id, item->amount, 1))
				Sql_ShowDebug(inter->sql_handle);
		} else {
			// Non-stackable items should have their own entries (issue: 7279)
			int l, loc = item->loc;
			for (l = 0; l < item->amount; l++) {
				if (SQL_ERROR == SQL->Query(inter->sql_handle, "INSERT INTO `%s` (`char_id`,`nameid`, `amount`, `equip`, `identify`) VALUES ('%d', '%d', '%d', '%d', '%d')", inventory_db, char_id, item->id, 1, loc, 1))
					Sql_ShowDebug(inter->sql_handle);
			}
		}
	}
...
static int char_char_family(int cid1, int cid2, int cid3)
{
	...
	if (SQL_ERROR == SQL->Query(inter->sql_handle, "SELECT `char_id`,`partner_id`,`child` FROM `%s` WHERE `char_id` IN ('%d','%d','%d')", char_db, cid1, cid2, cid3))
		...
		while (SQL_SUCCESS == SQL->NextRow(inter->sql_handle)) {
			...
			if ((cid1 == charid && ((cid2 == partnerid && cid3 == childid) || (cid2 == childid && cid3 == partnerid))) || (cid1 == partnerid && ((cid2 == charid && cid3 == childid) || (cid2 == childid && cid3 == charid))) || (cid1 == childid && ((cid2 == charid && cid3 == partnerid) || (cid2 == partnerid && cid3 == charid)))) {
				SQL->FreeResult(inter->sql_handle);
				return childid;
			}
		}
...
static void char_parse_fromlogin_auth_state(int fd)
{
	...
	if (sockt->session_is_active(request_id) && (sd = (struct char_session_data *)sockt->session[request_id]->session_data) && !sd->auth && sd->account_id == account_id && sd->login_id1 == login_id1 && sd->login_id2 == login_id2 && sd->sex == sex) {
...
static void char_parse_fromlogin_account_data(int fd)
{
	...
	if (i < sockt->fd_max) {
		...
		if ((max_connect_user == 0 && sd->group_id != gm_allow_group) || (max_connect_user > 0 && chr->count_users() >= max_connect_user && sd->group_id != gm_allow_group)) {
...
static void char_change_sex_sub(int sex, int acc, int char_id, int class, int guild_id)
{
	...
	if (SQL_ERROR == SQL->StmtPrepare(stmt, query_inv, inventory_db) || SQL_ERROR == SQL->StmtBindParam(stmt, 0, SQLDT_INT32, &char_id, sizeof(char_id)) || SQL_ERROR == SQL->StmtExecute(stmt)) {
		SqlStmt_ShowDebug(stmt);
		SQL->StmtFree(stmt);
		return;
	}
	...
	if (SQL_ERROR == SQL->StmtPrepare(stmt, query_char, char_db) || SQL_ERROR == SQL->StmtBindParam(stmt, 0, SQLDT_INT32, &class, sizeof(class)) || SQL_ERROR == SQL->StmtBindParam(stmt, 1, SQLDT_ENUM, &gender, sizeof(gender)) || SQL_ERROR == SQL->StmtBindParam(stmt, 2, SQLDT_INT32, &char_id, sizeof(char_id)) || SQL_ERROR == SQL->StmtExecute(stmt)) {
		SqlStmt_ShowDebug(stmt);
		SQL->StmtFree(stmt);
		return;
	}
...
static int char_parse_fromlogin_changesex_reply(int fd)
{
	...
	// get characters
	stmt = SQL->StmtMalloc(inter->sql_handle);
	if (SQL_ERROR == SQL->StmtPrepare(stmt, "SELECT `char_id`,`class`,`guild_id` FROM `%s` WHERE `account_id` = '%d'", char_db, acc) || SQL_ERROR == SQL->StmtExecute(stmt) || SQL_ERROR == SQL->StmtBindColumn(stmt, 0, SQLDT_INT, &char_id, sizeof char_id, NULL, NULL) || SQL_ERROR == SQL->StmtBindColumn(stmt, 1, SQLDT_INT, &class, sizeof class, NULL, NULL) || SQL_ERROR == SQL->StmtBindColumn(stmt, 2, SQLDT_INT, &guild_id, sizeof guild_id, NULL, NULL)) {
		SqlStmt_ShowDebug(stmt);
		SQL->StmtFree(stmt);
	}
...
{
	inter->accinfo2(false, RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10), RFIFOL(fd, 14), NULL, NULL, NULL, NULL, NULL, NULL, NULL, -1, 0, 0);
	RFIFOSKIP(fd, 18);
}

static void char_parse_fromlogin_accinfo2_ok(int fd)
{
	inter->accinfo2(true, RFIFOL(fd, 167), RFIFOL(fd, 171), RFIFOL(fd, 175), RFIFOL(fd, 179), RFIFOP(fd, 2), RFIFOP(fd, 26), RFIFOP(fd, 59), RFIFOP(fd, 99), RFIFOP(fd, 119), RFIFOP(fd, 151), RFIFOP(fd, 156), RFIFOL(fd, 115), RFIFOL(fd, 143), RFIFOL(fd, 147));
	RFIFOSKIP(fd, 183);
}
...
#ifdef ENABLE_SC_SAVING
	if (SQL_ERROR == SQL->Query(inter->sql_handle,
	                            "SELECT `type`, `tick`, `total_tick`, `val1`, `val2`, `val3`, `val4` "
...
static void char_parse_frommap_remove_friend(int fd)
{
	...
	if (SQL_ERROR == SQL->Query(inter->sql_handle, "DELETE FROM `%s` WHERE `char_id`='%d' AND `friend_id`='%d' LIMIT 1", friend_db, char_id, friend_id)) {
		Sql_ShowDebug(inter->sql_handle);
	}
	RFIFOSKIP(fd, 10);
}
...
static void char_ban(int account_id, int char_id, time_t *unban_time, short year, short month, short day, short hour, short minute, short second)
{
	...
	if (SQL_SUCCESS != SQL->StmtPrepare(stmt, "UPDATE `%s` SET `unban_time` = ? WHERE `char_id` = ? LIMIT 1", char_db) || SQL_SUCCESS != SQL->StmtBindParam(stmt, 0, SQLDT_TIME, &timestamp, sizeof timestamp) || SQL_SUCCESS != SQL->StmtBindParam(stmt, 1, SQLDT_INT, &char_id, sizeof char_id) || SQL_SUCCESS != SQL->StmtExecute(stmt)) {
		SqlStmt_ShowDebug(stmt);
	}
...
static void char_parse_frommap_ragsrvinfo(int fd)
{
	...
	if (SQL_ERROR == SQL->Query(inter->sql_handle, "INSERT INTO `%s` SET `index`='%d',`name`='%s',`exp`='%u',`jexp`='%u',`drop`='%u'", ragsrvinfo_db, fd, esc_server_name, RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10))) {
		Sql_ShowDebug(inter->sql_handle);
	}
	RFIFOSKIP(fd, 14);
}
...
static void char_parse_frommap_save_status_change_data(int fd)
{
	...
	if (count > 0) {
		...
		for (i = 0; i < count; ++i) {
			...
			if (i > 0)
				StrBuf->AppendStr(&buf, ", ");
			StrBuf->Printf(&buf, "('%d','%d','%hu','%d','%d','%d','%d','%d','%d')", aid, cid, data.type, data.tick, data.total_tick, data.val1, data.val2, data.val3, data.val4);
		}
		if (SQL_ERROR == SQL->QueryStr(inter->sql_handle, StrBuf->Value(&buf)))
			Sql_ShowDebug(inter->sql_handle);
		StrBuf->Destroy(&buf);
...
{ // CH: <0827>.W <char id>.L
	...
	if (char_aegis_delete) {
		...
		if (SQL_SUCCESS != SQL->Query(inter->sql_handle, "SELECT `party_id`, `guild_id` FROM `%s` WHERE `char_id`='%d'", char_db, char_id) || SQL_SUCCESS != SQL->NextRow(inter->sql_handle)) {
			Sql_ShowDebug(inter->sql_handle);
			chr->delete2_ack(fd, char_id, 3, 0); // 3: A database error occurred
			return;
		}
...
{ // CH: <0829>.W <char id>.L <birth date:YYMMDD>.6B
	...
	if (SQL_SUCCESS != SQL->Query(inter->sql_handle, "SELECT `base_level`,`delete_date` FROM `%s` WHERE `char_id`='%d'", char_db, char_id) || SQL_SUCCESS != SQL->NextRow(inter->sql_handle)) { // data error
		Sql_ShowDebug(inter->sql_handle);
		chr->delete2_accept_ack(fd, char_id, 3); // 3: A database error occurred
		return;
	}
...
static void char_parse_char_select(int fd, struct char_session_data *sd, uint32 ipl)
{
	...
	if (SQL_SUCCESS != SQL->Query(inter->sql_handle, "SELECT `char_id` FROM `%s` WHERE `account_id`='%d' AND `char_num`='%d'", char_db, sd->account_id, slot) || SQL_SUCCESS != SQL->NextRow(inter->sql_handle) || SQL_SUCCESS != SQL->GetData(inter->sql_handle, 0, &data, NULL)) {
	...
	if (chr->enable_logs) {
		...
		if (SQL_ERROR == SQL->Query(inter->sql_handle,
		                            ...
		                            " '%d', '%d', '%d', '%d', '%d', '%d',"
		                            " '%d', '%d')",
		                            charlog_db, sd->account_id, cd->char_id, slot, char_dat.class, esc_name, char_dat.str, char_dat.agi, char_dat.vit, char_dat.int_, char_dat.dex, char_dat.luk, char_dat.hair, char_dat.hair_color))
			Sql_ShowDebug(inter->sql_handle);
	}
...
static void char_creation_failed(int fd, int result)
{
	...
	switch (result) {
		...
		case -4:
			WFIFOB(fd, 2) = 0x03;
			break; // 'You are not eligible to open the Character Slot.'
		case -5:
			WFIFOB(fd, 2) = 0x02;
...
static void char_parse_char_login_map_server(int fd, uint32 ipl)
{
	...
	if (core->runflag != CHARSERVER_ST_RUNNING || chr->map_server.fd > 0 || strcmp(l_user, chr->userid) != 0 || strcmp(l_pass, chr->passwd) != 0 || !sockt->allowed_ip_check(ipl)) {
