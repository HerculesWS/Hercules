...
static void char_set_account_online(int account_id, bool standalone)
{
	...
	struct PACKET_CHARLOGIN_SET_ACCOUNT_ONLINE *p = WFIFOP(struct PACKET_CHARLOGIN_SET_ACCOUNT_ONLINE *, chr->login_fd, 0);
...
static void char_send_HC_ACK_CHARINFO_PER_PAGE(int fd, struct char_session_data *sd)
{
	...
	struct PACKET_HC_ACK_CHARINFO_PER_PAGE *p = WFIFOP(struct PACKET_HC_ACK_CHARINFO_PER_PAGE *, fd, 0);
	int count = 0;
...
static void char_parse_frommap_auth_request(int fd)
{
	...
	const struct PACKET_MAPCHAR_AUTH_REQ *p = RFIFOP(struct PACKET_MAPCHAR_AUTH_REQ *, fd, 0);
...
static int char_send_accounts_tologin(int tid, int64 tick, int id, intptr_t data)
{
	if (chr->login_fd > 0 && sockt->session[chr->login_fd] != NULL) {
		...
		p = WFIFOP(struct PACKET_CHARLOGIN_ONLINE_ACCOUNTS *, chr->login_fd, 0);
