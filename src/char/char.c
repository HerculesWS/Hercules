...
int char_mmo_char_tosql(int char_id, struct mmo_charstatus* p)
{
	...
	if (
		...
		if( SQL_ERROR == SQL->Query(inter->sql_handle, "UPDATE `%s` SET `base_level`='%d', `job_level`='%d',"
			...
			"`hotkey_rowshift`='%d',`clan_id`='%d',`last_login`='%lu'"
			...
			p->hotkey_rowshift,p->clan_id,(unsigned long)p->last_login,
...
int char_mmo_char_fromsql(int char_id, struct mmo_charstatus* p, bool load_everything)
{
	...
	if (SQL_ERROR == SQL->StmtPrepare(stmt, "SELECT "
	 ...
	 || SQL_ERROR == SQL->StmtBindColumn(stmt, 59, SQLDT_INT,    &p->clan_id, 0, NULL, NULL)
	 || SQL_ERROR == SQL->StmtBindColumn(stmt, 60, SQLDT_UINT64, &p->last_login, 0, NULL, NULL)
	) {
		SqlStmt_ShowDebug(stmt);
		SQL->StmtFree(stmt);
		return 0;
