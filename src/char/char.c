 ...
 * @param items     [in|out]   reference to the item list of a character/account/guild.
 * @param max       [in]       Max amount of items to be pulled into the list.
 * @param guid      [in]       Unique ID of the player object (account_id, char_id, guild_id).
 * @param table     [in]       Table to be used for the transaction.
...
int char_getitemdata_from_sql(struct item *items, int max, int guid, enum inventory_table_type table)
{
	...
	default:
		ShowError("char_getitemdata_from_sql: Invalid table name!\n");
		Assert_retr(-1, table);
	}
	...
	StrBuf->Printf(&buf, " FROM `%s` WHERE `%s`='%d'", tablename, selectoption, guid);
	...
	for (i = 0; i < MAX_SLOTS; i++)
		SQL->StmtBindColumn(stmt, 10 + i, SQLDT_SHORT, &item.card[i], 0, NULL, NULL);

	for (i = 0; i < MAX_ITEM_OPTIONS; i++) {
		SQL->StmtBindColumn(stmt, 10 + MAX_SLOTS + i * 2, SQLDT_INT16, &item.option[i].index, 0, NULL, NULL);
		SQL->StmtBindColumn(stmt, 11 + MAX_SLOTS + i * 2, SQLDT_INT16, &item.option[i].value, 0, NULL, NULL);
	}
 ...
 * @param guid        The character/account/guild ID (depending on table).
 ...
 */
int char_memitemdata_to_sql(const struct item *p_items, int max, int guid, enum inventory_table_type table)
{
	StringBuf buf;
	int i = 0, j = 0;
	const char *tablename = NULL;
	...
	nullpo_ret(p_items);
	Assert_retr(1, max > 0); // Anything below wouldn't work without a max limit.
	...
	cp_items = aCalloc(max, sizeof(struct item));
	deletes = aCalloc(max, sizeof(struct item));
	matched_p = aCalloc(max, sizeof(bool));
	 ...
	 */
	if (chr->getitemdata_from_sql(cp_items, max, guid, table) > 0) {
		for (i = 0; i < max; i++) {
...
int char_mmo_char_fromsql(int char_id, struct mmo_charstatus* p, bool load_everything)
{
	...
	if (chr->memitemdata_to_sql(p->cart, MAX_CART, p->char_id, TABLE_CART) > 0)
		strcat(t_msg, " cart");
