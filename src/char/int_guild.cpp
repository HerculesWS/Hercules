...
static int inter_guild_save_timer(int tid, int64 tick, int id, intptr_t data)
{
	...
	int state = 0; // 0: Have not reached last guild. 1: Reached last guild, ready for save. 2: Some guild saved,
	...
	for (
	    g = (struct guild *)DB->data2ptr(iter->first(iter, &key)); dbi_exists(iter);
	    g = (struct guild *)DB->data2ptr(iter->next(iter, &key))
	) {
		if (!g)
			continue;
...
static int inter_guild_removemember_tosql(int account_id, int char_id)
{
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle, "DELETE from `%s` where `account_id` = '%d' and `char_id` = '%d'", guild_member_db,
	        account_id, char_id
	    ))
		Sql_ShowDebug(inter->sql_handle);
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle, "UPDATE `%s` SET `guild_id` = '0' WHERE `char_id` = '%d'", char_db, char_id
	    ))
		Sql_ShowDebug(inter->sql_handle);
	return 0;
}
	...
	*t_info = '\0';
	...
	if (flag & GS_BASIC && g->guild_id == -1) {
		...
		if (SQL_ERROR
		    == SQL->Query(
		        inter->sql_handle,
		        "INSERT INTO `%s` " "(`name`,`master`,`guild_lv`,`max_member`,`average_lv`,`char_id`) " "VALUES ('%s', '%s', '%d', '%d', '%d', '%d')",
	...
	if (((flag & GS_BASIC_MASK) && !new_guild) || ((flag & (GS_BASIC_MASK & ~GS_BASIC)) && new_guild)) {
		...
		if (flag & GS_EMBLEM) {
			...
			*pData = 0;
			StrBuf->Printf(
			    &buf, "`emblem_len`=%d, `emblem_id`=%d, `emblem_data`='%s'", g->emblem_len, g->emblem_id,
			    emblem_data
			);
			aFree(emblem_data);
			add_comma = true;
		...
		if (flag & GS_LEVEL) {
			...
			StrBuf->Printf(
			    ...
			    ", `next_exp`=%u, `max_member`=%d, `max_storage`=%hd",
	...
	if (flag & GS_MEMBER) {
		...
		for (i = 0; i < g->max_member; i++) {
			...
			if (!m->modified)
				continue;
			if (m->account_id) {
				// Since nothing references guild member table as foreign keys, it's safe to use REPLACE
				// INTO
				SQL->EscapeStringLen(
				    inter->sql_handle, esc_name, m->name, strnlen(m->name, NAME_LENGTH)
				);
				if (SQL_ERROR
				    == SQL->Query(
				        inter->sql_handle,
				        "REPLACE INTO `%s` (`guild_id`,`account_id`,`char_id`,`hair`,`hair_color`,`gender`,`class`,`lv`,`exp`,`exp_payper`,`online`,`position`,`name`) " "VALUES ('%d','%d','%d','%d','%d','%d','%d','%d','%" PRIu64
				        "','%d','%d','%d','%s')",
				        guild_member_db, g->guild_id, m->account_id, m->char_id, m->hair, m->hair_color,
				        m->gender, m->class_, m->lv, m->exp, m->exp_payper, m->online, m->position,
				        esc_name
				    ))
					Sql_ShowDebug(inter->sql_handle);
				if (m->modified & GS_MEMBER_NEW || new_guild == 1) {
					if (SQL_ERROR
					    == SQL->Query(
					        inter->sql_handle,
					        "UPDATE `%s` SET `guild_id` = '%d' WHERE `char_id` = '%d'", char_db,
					        g->guild_id, m->char_id
					    ))
						Sql_ShowDebug(inter->sql_handle);
				}
	...
	if (flag & GS_POSITION) {
		...
		for (i = 0; i < MAX_GUILDPOSITION; i++) {
			...
			if (SQL_ERROR
			    == SQL->Query(
			        inter->sql_handle,
			        "REPLACE INTO `%s` (`guild_id`,`position`,`name`,`mode`,`exp_mode`) VALUES ('%d','%d','%s','%d','%d')",
	...
	if (flag & GS_ALLIANCE) {
		...
		// [Skotlex]
		// if( SQL_ERROR == SQL->Query(inter->sql_handle, "DELETE FROM `%s` WHERE `guild_id`='%d' OR
		// `alliance_id`='%d'", guild_alliance_db, g->guild_id, g->guild_id) )
		if (SQL_ERROR
		    == SQL->Query(
		        inter->sql_handle, "DELETE FROM `%s` WHERE `guild_id`='%d'", guild_alliance_db, g->guild_id
		    )) {
			Sql_ShowDebug(inter->sql_handle);
		} else {
			...
			for (i = 0; i < MAX_GUILDALLIANCE; i++) {
				...
				if (a->guild_id > 0) {
					SQL->EscapeStringLen(
					    inter->sql_handle, esc_name, a->name, strnlen(a->name, NAME_LENGTH)
					);
					if (SQL_ERROR
					    == SQL->Query(
					        inter->sql_handle,
					        "REPLACE INTO `%s` (`guild_id`,`opposition`,`alliance_id`,`name`) " "VALUES ('%d','%d','%d','%s')",
	...
	if (flag & GS_EXPULSION) {
		...
		for (i = 0; i < MAX_GUILDEXPULSION; i++) {
			...
			if (e->account_id > 0) {
				...
				SQL->EscapeStringLen(
				    inter->sql_handle, esc_name, e->name, strnlen(e->name, NAME_LENGTH)
				);
				SQL->EscapeStringLen(
				    inter->sql_handle, esc_mes, e->mes, strnlen(e->mes, sizeof(e->mes))
				);
				if (SQL_ERROR
				    == SQL->Query(
				        inter->sql_handle,
				        "REPLACE INTO `%s` (`guild_id`,`account_id`, `char_id`, `name`,`mes`) " "VALUES ('%d','%d','%d','%s','%s')",
...
static struct guild *inter_guild_fromsql(int guild_id)
{
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle,
	        "SELECT g.`name`,c.`name`,g.`guild_lv`,g.`connect_member`,g.`max_member`,g.`max_storage`," "g.`average_lv`,g.`exp`,g.`next_exp`,g.`skill_point`,g.`mes1`,g.`mes2`,g.`emblem_len`,g.`emblem_id`,g.`emblem_data` " "FROM `%s` g LEFT JOIN `%s` c ON c.`char_id` = g.`char_id` WHERE g.`guild_id`='%d'",
	...
	memcpy(g->name, data, std::min(len, (size_t)NAME_LENGTH));
	SQL->GetData(inter->sql_handle, 1, &data, &len);
	memcpy(g->master, data, std::min(len, (size_t)NAME_LENGTH));
	...
	memcpy(g->mes1, data, std::min(len, sizeof(g->mes1)));
	SQL->GetData(inter->sql_handle, 11, &data, &len);
	memcpy(g->mes2, data, std::min(len, sizeof(g->mes2)));
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle,
	        "SELECT g.`account_id`,g.`char_id`,g.`hair`,g.`hair_color`,g.`gender`,g.`class`,g.`lv`,g.`exp`,g.`exp_payper`,g.`online`,g.`position`,g.`name`,c.`last_login`" "FROM `%s` g LEFT JOIN `%s` c ON c.`char_id` = g.`char_id` WHERE g.`guild_id`='%d' ORDER BY `position`",
	...
	for (i = 0; i < g->max_member && SQL_SUCCESS == SQL->NextRow(inter->sql_handle); ++i) {
		...
		if (data != NULL) {
			...
			if (m->last_login > 2114283600) {
				ShowError(
				    "Last login time bigger than allowd value in %d:%s: %u\n", guild_id, g->name,
				    m->last_login
				);
				m->last_login = 0;
			}
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle, "SELECT `position`,`name`,`mode`,`exp_mode` FROM `%s` WHERE `guild_id`='%d'",
	...
	while (SQL_SUCCESS == SQL->NextRow(inter->sql_handle)) {
		...
		SQL->GetData(inter->sql_handle, 1, &data, &len);
		memcpy(pos->name, data, std::min(len, (size_t)NAME_LENGTH));
		SQL->GetData(inter->sql_handle, 2, &data, NULL);
		pos->mode = atoi(data);
		SQL->GetData(inter->sql_handle, 3, &data, NULL);
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle, "SELECT `opposition`,`alliance_id`,`name` FROM `%s` WHERE `guild_id`='%d'",
	...
	for (i = 0; i < MAX_GUILDALLIANCE && SQL_SUCCESS == SQL->NextRow(inter->sql_handle); ++i) {
		...
		memcpy(a->name, data, std::min(len, (size_t)NAME_LENGTH));
	}

	// printf("- Read guild_expulsion %d from sql \n",guild_id);
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle, "SELECT `account_id`,`char_id`,`name`,`mes` FROM `%s` WHERE `guild_id`='%d'",
	...
	for (i = 0; i < MAX_GUILDEXPULSION && SQL_SUCCESS == SQL->NextRow(inter->sql_handle); ++i) {
		...
		e->char_id = atoi(data);
		SQL->GetData(inter->sql_handle, 2, &data, &len);
		memcpy(e->name, data, std::min(len, (size_t)NAME_LENGTH));
		SQL->GetData(inter->sql_handle, 3, &data, &len);
		memcpy(e->mes, data, std::min(len, sizeof(e->mes)));
	}

	// printf("- Read guild_skill %d from sql \n",guild_id);
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle, "SELECT `id`,`lv` FROM `%s` WHERE `guild_id`='%d' ORDER BY `id`", guild_skill_db,
	        guild_id
	    )) {
		Sql_ShowDebug(inter->sql_handle);
		aFree(g->emblem_data);
...
static int inter_guild_castle_tosql(struct guild_castle *gc)
{
	...
	StrBuf->Printf(
	    &buf,
	    "REPLACE INTO `%s` SET `castle_id`='%d', `guild_id`='%d', `economy`='%d', `defense`='%d', " "`triggerE`='%d', `triggerD`='%d', `nextTime`='%d', `payTime`='%d', `createTime`='%d', `visibleC`='%d'",
	    guild_castle_db, gc->castle_id, gc->guild_id, gc->economy, gc->defense, gc->triggerE, gc->triggerD,
	    gc->nextTime, gc->payTime, gc->createTime, gc->visibleC
	);
...
static struct guild_castle *inter_guild_castle_fromsql(int castle_id)
{
	...
	StrBuf->AppendStr(
	    &buf,
	    "SELECT `castle_id`, `guild_id`, `economy`, `defense`, `triggerE`, " "`triggerD`, `nextTime`, `payTime`, `createTime`, `visibleC`"
	);
	for (i = 0; i < MAX_GUARDIANS; ++i)
...
static bool inter_guild_exp_parse_row(char *split[], int column, int current)
{
	...
	if (exp < 0 || exp >= UINT_MAX) {
		ShowError(
		    "exp_guild: Invalid exp %" PRId64 " (valid range: 0 - %u) at line %d\n", exp, UINT_MAX, current
		);
		return false;
	}
...
static int inter_guild_CharOnline(int char_id, int guild_id)
{
	...
	if (guild_id == -1) {
		...
		if (SQL_ERROR
		    == SQL->Query(
		        inter->sql_handle, "SELECT guild_id FROM `%s` WHERE char_id='%d'", char_db, char_id
		    )) {
			Sql_ShowDebug(inter->sql_handle);
			return 0;
		}
...
static int inter_guild_search_guildname(const char *str)
{
	...
	if (SQL_ERROR
	    == SQL->Query(inter->sql_handle, "SELECT guild_id FROM `%s` WHERE name='%s'", guild_db, esc_name)) {
		Sql_ShowDebug(inter->sql_handle);
		return -1;
	}
		...
		if (g->member[i].account_id > 0) {
			if (g->member[i].lv >= 0) {
				...
				ShowWarning(
				    ...
				    g->member[i].char_id, g->member[i].name, g->member[i].lv
				);
			}
...
static bool inter_guild_leave(int guild_id, int account_id, int char_id, int flag, const char *mes)
{
	...
	if (g == NULL) {
		...
		if (SQL_ERROR
		    == SQL->Query(
		        inter->sql_handle, "UPDATE `%s` SET `guild_id`='0' WHERE `account_id`='%d' AND `char_id`='%d'",
...
// Change member info
static bool
    inter_guild_update_member_info_short(int guild_id, int account_id, int char_id, int online, int lv, int class_)
{
...
static bool inter_guild_disband(int guild_id)
{
	...
	if (SQL_ERROR
	    == SQL->Query(inter->sql_handle, "DELETE FROM `%s` WHERE `guild_id` = '%d'", guild_member_db, guild_id))
		Sql_ShowDebug(inter->sql_handle);

	if (SQL_ERROR
	    == SQL->Query(inter->sql_handle, "DELETE FROM `%s` WHERE `guild_id` = '%d'", guild_castle_db, guild_id))
		Sql_ShowDebug(inter->sql_handle);
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle, "DELETE FROM `%s` WHERE `guild_id` = '%d' OR `alliance_id` = '%d'",
	        guild_alliance_db, guild_id, guild_id
	    ))
		Sql_ShowDebug(inter->sql_handle);

	if (SQL_ERROR
	    == SQL->Query(inter->sql_handle, "DELETE FROM `%s` WHERE `guild_id` = '%d'", guild_position_db, guild_id))
		Sql_ShowDebug(inter->sql_handle);

	if (SQL_ERROR
	    == SQL->Query(inter->sql_handle, "DELETE FROM `%s` WHERE `guild_id` = '%d'", guild_skill_db, guild_id))
		Sql_ShowDebug(inter->sql_handle);

	if (SQL_ERROR
	    == SQL->Query(inter->sql_handle, "DELETE FROM `%s` WHERE `guild_id` = '%d'", guild_expulsion_db, guild_id))
		Sql_ShowDebug(inter->sql_handle);

	// printf("- Update guild %d of char\n",guild_id);
	if (SQL_ERROR
	    == SQL->Query(inter->sql_handle, "UPDATE `%s` SET `guild_id`='0' WHERE `guild_id`='%d'", char_db, guild_id))
		Sql_ShowDebug(inter->sql_handle);
...
static bool inter_guild_update_basic_info(int guild_id, enum guild_basic_info type, const void *data, int len)
{
	...
	default:
		ShowError(
		    "int_guild: GuildBasicInfoChange: Unknown type %u, see mmo.h::guild_basic_info for more information\n",
		    (unsigned int)type
		);
		return false;
	}
...
static bool inter_guild_update_member_info(
    int guild_id, int account_id, int char_id, enum guild_member_info type, const char *data, int len
)
{
...
{
	return inter_guild->update_member_info(
	    guild_id, account_id, char_id, GMI_GENDER, (const char *)&gender, sizeof(gender)
	);
}
...
static bool inter_guild_change_alliance(int guild_id1, int guild_id2, int account_id1, int account_id2, int flag)
{
	...
	if (g[0]
		...
		return inter_guild->remove_alliance(
...
static bool inter_guild_is_guild_master(int char_id, int guild_id)
{
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle,
	        "SELECT g.* FROM `%s` g LEFT JOIN `%s` c ON g.`char_id` = c.`char_id` " "WHERE c.char_id = '%d' AND g.guild_id = '%d' AND g.`master` = c.`name`",
...
static int inter_guild_parse_frommap(int fd)
{
	...
	case 0x3030:
		mapif->parse_CreateGuild(
		    fd, RFIFOL(fd, 4), RFIFOP(char *, fd, 8), RFIFOP(struct guild_member *, fd, 32)
		);
		break;
	case 0x3031:
		...
		break;
	case 0x3032:
		mapif->parse_GuildAddMember(fd, RFIFOL(fd, 4), RFIFOP(struct guild_member *, fd, 8));
		break;
	case 0x3033:
		mapif->parse_GuildMasterChange(fd, RFIFOL(fd, 4), RFIFOP(char *, fd, 8), RFIFOW(fd, 2) - 8);
		break;
	case 0x3034:
		mapif->parse_GuildLeave(
		    fd, RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10), RFIFOB(fd, 14), RFIFOP(char *, fd, 15)
		);
		break;
	case 0x3035:
		mapif->parse_GuildChangeMemberInfoShort(
		    fd, RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10), RFIFOB(fd, 14), RFIFOL(fd, 15), RFIFOL(fd, 19)
		);
		break;
	case 0x3036:
	...
	case 0x3039:
		mapif->parse_GuildBasicInfoChange(
		    fd, RFIFOL(fd, 4), RFIFOW(fd, 8), RFIFOP(void *, fd, 10), RFIFOW(fd, 2) - 10
		);
		break;
	case 0x303A:
		mapif->parse_GuildMemberInfoChange(
		    fd, RFIFOL(fd, 4), RFIFOL(fd, 8), RFIFOL(fd, 12), RFIFOW(fd, 16), RFIFOP(char *, fd, 18),
		    RFIFOW(fd, 2) - 18
		);
		break;
	case 0x303B:
		mapif->parse_GuildPosition(fd, RFIFOL(fd, 4), RFIFOL(fd, 8), RFIFOP(struct guild_position *, fd, 12));
		break;
	case 0x303C:
		mapif->parse_GuildSkillUp(fd, RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10), RFIFOL(fd, 14));
		break;
	case 0x303D:
		mapif->parse_GuildAlliance(
		    fd, RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10), RFIFOL(fd, 14), RFIFOB(fd, 18)
		);
		break;
	case 0x303E:
		mapif->parse_GuildNotice(fd, RFIFOL(fd, 2), RFIFOP(char *, fd, 6), RFIFOP(char *, fd, 66));
		break;
	case HEADER_MAPCHAR_GUILD_EMBLEM:
		mapif->parse_GuildEmblem(fd);
		break;
	case 0x3040:
		mapif->parse_GuildCastleDataLoad(fd, RFIFOW(fd, 2), RFIFOP(int *, fd, 4));
		break;
	case 0x3041:
