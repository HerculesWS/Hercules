...
static int inter_party_check_lv(struct party_data *p)
{
	...
	for (int i = 0; i < MAX_PARTY; i++) {
		if (p->party.member[i].online == 0 || p->party.member[i].char_id == p->family)
			continue; /// If not online OR if it's a family party and this is the child, don't affect exp
			          /// range.
...
static int inter_party_is_family_party(struct party_data *p)
{
	...
	for (int i = 0; i < MAX_PARTY - 1; i++) {
		...
		struct mmo_charstatus *char_i
		    = (struct mmo_charstatus *)idb_get(chr->char_db_, p->party.member[i].char_id);
		...
		for (int j = i + 1; j < MAX_PARTY; j++) {
			...
			struct mmo_charstatus *char_j
			    = (struct mmo_charstatus *)idb_get(chr->char_db_, p->party.member[j].char_id);
	...
	if (child_id != 0 && p->size > 2) {
		for (int i = 0; i < MAX_PARTY; i++) {
			struct mmo_charstatus *party_member
			    = (struct mmo_charstatus *)idb_get(chr->char_db_, p->party.member[i].char_id);

			/// Check if there is a stranger within the party.
			if (party_member != NULL
			    && party_member->char_id != child_id
			    && party_member->child != child_id) {
...
static int inter_party_tosql(struct party *p, int flag, int index)
{
	...
	if (flag & PS_BREAK) { // Break the party
		...
		if (SQL_ERROR
		    == SQL->Query(
		        inter->sql_handle, "UPDATE `%s` SET `party_id`='0' WHERE `party_id`='%d'", char_db, party_id
		    ))
			Sql_ShowDebug(inter->sql_handle);
		if (SQL_ERROR
		    == SQL->Query(inter->sql_handle, "DELETE FROM `%s` WHERE `party_id`='%d'", party_db, party_id))
			Sql_ShowDebug(inter->sql_handle);
		// Remove from memory
	...
	if (flag & PS_CREATE) { // Create party
		if (SQL_ERROR
		    == SQL->Query(
		        inter->sql_handle,
		        "INSERT INTO `%s` " "(`name`, `exp`, `item`, `leader_id`, `leader_char`) " "VALUES ('%s', '%d', '%d', '%d', '%d')",
	...
	if (flag & PS_BASIC) { // Update party info.
		if (SQL_ERROR
		    == SQL->Query(
		        inter->sql_handle, "UPDATE `%s` SET `name`='%s', `exp`='%d', `item`='%d' WHERE `party_id`='%d'",
	...
	if (flag & PS_LEADER) { // Update leader
		if (SQL_ERROR
		    == SQL->Query(
		        inter->sql_handle,
		        "UPDATE `%s`  SET `leader_id`='%d', `leader_char`='%d' WHERE `party_id`='%d'", party_db,
		        p->member[index].account_id, p->member[index].char_id, party_id
		    ))
			Sql_ShowDebug(inter->sql_handle);
	}
	...
	if (flag & PS_ADDMEMBER) { // Add one party member.
		if (SQL_ERROR
		    == SQL->Query(
		        inter->sql_handle, "UPDATE `%s` SET `party_id`='%d' WHERE `account_id`='%d' AND `char_id`='%d'",
...
static struct party_data *inter_party_fromsql(int party_id)
{
	...
	memcpy(p->party.name, data, std::min(len, (size_t)NAME_LENGTH));
	...
	for (i = 0; i < MAX_PARTY && SQL_SUCCESS == SQL->NextRow(inter->sql_handle); ++i) {
		...
		memcpy(m->name, data, std::min(len, (size_t)NAME_LENGTH));
		SQL->GetData(inter->sql_handle, 3, &data, NULL);
		m->lv = atoi(data);
		SQL->GetData(inter->sql_handle, 4, &data, NULL);
		m->map = mapindex->name2id(data);
		SQL->GetData(inter->sql_handle, 5, &data, NULL);
...
static struct party_data *inter_party_search_partyname(const char *const str)
{
	...
	if (SQL_ERROR
	    == SQL->Query(inter->sql_handle, "SELECT `party_id` FROM `%s` WHERE `name`='%s'", party_db, esc_name))
		Sql_ShowDebug(inter->sql_handle);
...
static int inter_party_parse_frommap(int fd)
{
	...
	switch (RFIFOW(fd, 0)) {
	case 0x3020:
		mapif->parse_CreateParty(
		    fd, RFIFOP(char *, fd, 4), RFIFOB(fd, 28), RFIFOB(fd, 29), RFIFOP(struct party_member *, fd, 30)
		);
		break;
	case 0x3021:
		mapif->parse_PartyInfo(fd, RFIFOL(fd, 2), RFIFOL(fd, 6));
		break;
	case 0x3022:
		mapif->parse_PartyAddMember(fd, RFIFOL(fd, 4), RFIFOP(struct party_member *, fd, 8));
		break;
	case 0x3023:
		mapif->parse_PartyChangeOption(fd, RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOW(fd, 10), RFIFOW(fd, 12));
		break;
	case 0x3024:
		mapif->parse_PartyLeave(fd, RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10));
		break;
	case 0x3025:
		mapif->parse_PartyChangeMap(
		    fd, RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10), RFIFOW(fd, 14), RFIFOB(fd, 16), RFIFOW(fd, 17)
		);
		break;
	case 0x3026:
		mapif->parse_BreakParty(fd, RFIFOL(fd, 2));
		break;
	case 0x3029:
		mapif->parse_PartyLeaderChange(fd, RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10));
		break;
	default:
		return 0;
	}
...
static int inter_party_CharOnline(int char_id, int party_id)
{
	if (party_id == INDEX_NOT_FOUND) { /// Get party_id from the database.
		...
		if (SQL_ERROR
		    == SQL->Query(
		        inter->sql_handle, "SELECT party_id FROM `%s` WHERE char_id='%d'", char_db, char_id
		    )) {
			Sql_ShowDebug(inter->sql_handle);
			return 0;
		}
...
static int inter_party_CharOffline(int char_id, int party_id)
{
	...
	if (p->party.count
	    == 0) /// Parties don't have any data that needs be saved at this point... so just remove it from memory.
		idb_remove(inter_party->db, party_id);
