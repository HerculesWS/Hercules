...
static int inter_storage_tosql(int account_id, int storage_id, const struct storage_data *p)
{
			...
			if (j < VECTOR_LENGTH(p->item)) {
				...
				if (memcmp(cp_it, p_it, sizeof(struct item)) != 0) {
					if (total_updates == 0) {
						StrBuf->Printf(
						    &buf,
						    "REPLACE INTO `%s` (`id`, `account_id`, `storage_id`, `nameid`, `amount`, `equip`, `identify`, `refine`, `grade`, `attribute`",
						    storage_db
						);
						...
						StrBuf->AppendStr(
						    &buf, ", `expire_time`, `bound`, `unique_id`) VALUES"
						);
					}

					StrBuf->Printf(
					    &buf, "%s('%d', '%d', '%d', '%d', '%d', '%u', '%d', '%d', '%d', '%d'",
					    total_updates > 0 ? ", " : "", cp_it->id, account_id, storage_id,
					    p_it->nameid, p_it->amount, p_it->equip, p_it->identify, p_it->refine,
					    p_it->grade, p_it->attribute
					);
					for (k = 0; k < MAX_SLOTS; k++)
						StrBuf->Printf(&buf, ", '%d'", p_it->card[k]);
					for (k = 0; k < MAX_ITEM_OPTIONS; ++k)
						StrBuf->Printf(
						    &buf, ", '%d', '%d'", p_it->option[k].index, p_it->option[k].value
						);
					StrBuf->Printf(
					    &buf, ", '%u', '%d', '%" PRIu64 "')", p_it->expire_time, p_it->bound,
					    p_it->unique_id
					);
	...
	for (i = 0; i < VECTOR_LENGTH(p->item); i++) {
		...
		if (total_inserts == 0) {
			...
			StrBuf->Printf(
			    &buf,
			    "INSERT INTO `%s` (`account_id`, `storage_id`, `nameid`, `amount`, `equip`, `identify`, `refine`, `grade`, `attribute`, `expire_time`, `bound`, `unique_id`",
			    storage_db
			);
		...
		StrBuf->Printf(
		    &buf, "%s('%d', '%d', '%d', '%d', '%u', '%d',  '%d', '%d', '%d', '%u', '%d', '%" PRIu64 "'",
		    total_inserts > 0 ? ", " : "", account_id, storage_id, p_it->nameid, p_it->amount, p_it->equip,
		    p_it->identify, p_it->refine, p_it->grade, p_it->attribute, p_it->expire_time, p_it->bound,
		    p_it->unique_id
		);
		for (j = 0; j < MAX_SLOTS; ++j)
	...
	ShowInfo(
	    "Storage #%d save complete - id: %d (replace: %d, insert: %d, delete: %d)\n", storage_id, account_id,
...
static int inter_storage_fromsql(int account_id, int storage_id, struct storage_data *p)
{
	...
	// storage
	// {`account_id`/`storage_id`/`id`/`nameid`/`amount`/`equip`/`identify`/`refine`/`grade`/`attribute`/`card0`/`card1`/`card2`/`card3`}
	StrBuf->Init(&buf);
	StrBuf->AppendStr(
	    &buf,
	    "SELECT `id`,`nameid`,`amount`,`equip`,`identify`,`refine`,`grade`,`attribute`,`expire_time`,`bound`,`unique_id`"
	);
	...
	StrBuf->Printf(
	    &buf, " FROM `%s` WHERE `account_id`='%d' AND `storage_id`='%d' ORDER BY `nameid`", storage_db, account_id,
	    storage_id
	);
	...
	int num_rows
	    = SQL->NumRows(inter->sql_handle) > MAX_STORAGE ? MAX_STORAGE : (int)SQL->NumRows(inter->sql_handle);
	...
	ShowInfo(
	    "Storage #%d load complete from DB - id: %d (total: %d)\n", storage_id, account_id, VECTOR_LENGTH(p->item)
	);
...
static bool inter_storage_guild_storage_tosql(int guild_id, const struct guild_storage *gstor)
{
	...
	if (SQL_ERROR
	    == SQL->Query(inter->sql_handle, "SELECT `guild_id` FROM `%s` WHERE `guild_id`='%d'", guild_db, guild_id)) {
		Sql_ShowDebug(inter->sql_handle);
		return false;
	...
	int changes
	    = chr->memitemdata_to_sql(gstor->items.data, gstor->items.capacity, gstor->guild_id, TABLE_GUILD_STORAGE);
	if (changes == -1) {
...
static int inter_storage_guild_storage_fromsql(int guild_id, struct guild_storage *gstor)
{
	...
	// storage
	// {`guild_id`/`id`/`nameid`/`amount`/`equip`/`identify`/`refine`/`grade`/`attribute`/`card0`/`card1`/`card2`/`card3`}
	StrBuf->Init(&buf);
	StrBuf->AppendStr(
	    &buf, "SELECT `id`,`nameid`,`amount`,`equip`,`identify`,`refine`,`grade`,`attribute`,`bound`,`unique_id`"
	);
	for (j = 0; j < MAX_SLOTS; ++j)
		StrBuf->Printf(&buf, ",`card%d`", j);
	for (j = 0; j < MAX_ITEM_OPTIONS; ++j)
	...
	if (gstor->items.amount < gstor->items.capacity) {
		if (gstor->items.amount > 0) {
			struct item *temp = (struct item *)aRealloc(
			    gstor->items.data, sizeof(gstor->items.data[0]) * gstor->items.amount
			);
			if (temp != NULL)
...
{
	if (SQL_ERROR
	    == SQL->Query(inter->sql_handle, "DELETE FROM `%s` WHERE `account_id`='%d'", storage_db, account_id))
		Sql_ShowDebug(inter->sql_handle);
	return 0;
}
...
{
	if (SQL_ERROR
	    == SQL->Query(inter->sql_handle, "DELETE FROM `%s` WHERE `guild_id`='%d'", guild_storage_db, guild_id))
		Sql_ShowDebug(inter->sql_handle);
	return 0;
}
...
static bool inter_storage_retrieve_bound_items(int char_id, int account_id, int guild_id)
{
	...
	for (j = 0; j < MAX_ITEM_OPTIONS; ++j) {
		SQL->StmtBindColumn(
		    stmt, 11 + MAX_SLOTS + j * 2, SQLDT_INT16, &item.option[j].index, sizeof item.option[j].index, NULL,
		    NULL
		);
		SQL->StmtBindColumn(
		    stmt, 12 + MAX_SLOTS + j * 2, SQLDT_INT16, &item.option[j].value, sizeof item.option[j].value, NULL,
		    NULL
		);
	}
	...
	if (bound_qt) {
  #define CHECK_REMOVE(var, mask, token) \
	  do { /* Verifies equip bitmasks (see item.equip) and handles the sql statement */ \
		  if ((var) & (mask)) { \
			  if ((var) != (mask) && s) \
				  StrBuf->AppendStr(&buf, ","); \
  ...
  #undef CHECK_REMOVE
	...
	StrBuf->Printf(
	    &buf,
	    "INSERT INTO `%s` (`guild_id`,`nameid`,`amount`,`equip`,`identify`,`refine`," "`grade`, `attribute`,`expire_time`,`bound`,`unique_id`",
	    guild_storage_db
	);
	...
	for (j = 0; j < i; ++j) {
		...
		StrBuf->Printf(
		    &buf, "('%d', '%d', '%d', '%u', '%d', '%d', '%d', '%d', '%u', '%d', '%" PRIu64 "'", guild_id,
		    items[j].nameid, items[j].amount, items[j].equip, items[j].identify, items[j].refine,
		    items[j].grade, items[j].attribute, items[j].expire_time, items[j].bound, items[j].unique_id
		);
		for (s = 0; s < MAX_SLOTS; ++s)
...
static int inter_storage_parse_frommap(int fd)
{
	...
	switch (RFIFOW(fd, 0)) {
	case 0x3010:
		mapif->pAccountStorageLoad(fd);
		break;
	case 0x3011:
		mapif->pAccountStorageSave(fd);
		break;
	case 0x3018:
		mapif->parse_LoadGuildStorage(fd);
		break;
	case 0x3019:
		mapif->parse_SaveGuildStorage(fd);
		break;
#ifdef GP_BOUND_ITEMS
	case 0x3056:
		mapif->parse_ItemBoundRetrieve(fd);
		break;
#endif
	default:
		return 0;
