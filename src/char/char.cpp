...
int char_name_option = 0; // Option to know which letters/symbols are authorized in the name of a character (0: all, 1:
...
static bool char_aegis_delete = false; ///< Verify if char is in guild/party or char and reacts as Aegis does (disallow
                                       ///< deletion), @see chr->delete2_req.
...
static void char_set_account_online(int account_id, bool standalone)
{
	...
	struct PACKET_CHARLOGIN_SET_ACCOUNT_ONLINE *p
	    = WP2PTR(struct PACKET_CHARLOGIN_SET_ACCOUNT_ONLINE *, chr->login_fd);
...
{
	struct online_char_data *character
	    = (struct online_char_data *)idb_ensure(chr->online_char_db, account_id, chr->create_online_char_data);
...
static void char_set_char_online(bool is_initializing, int char_id, int account_id, bool standalone)
{
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle, "UPDATE `%s` SET `online`='1' WHERE `char_id`='%d' LIMIT 1", char_db, char_id
	    ))
...
static void char_set_char_offline(int char_id, int account_id)
{
	...
	if (char_id == -1) {
		if (SQL_ERROR
		    == SQL->Query(
		        inter->sql_handle, "UPDATE `%s` SET `online`='0' WHERE `account_id`='%d'", char_db, account_id
		    ))
			Sql_ShowDebug(inter->sql_handle);
	} else {
		struct mmo_charstatus *cp = (struct mmo_charstatus *)idb_get(chr->char_db_, char_id);
		/* Character Achievements */
		struct char_achievements *c_ach
		    = (struct char_achievements *)idb_get(inter_achievement->char_achievements, char_id);
		...
		if (SQL_ERROR
		    == SQL->Query(
		        inter->sql_handle, "UPDATE `%s` SET `online`='0' WHERE `char_id`='%d' LIMIT 1", char_db, char_id
		    ))
			Sql_ShowDebug(inter->sql_handle);
	}
	...
	if (chr->login_fd > 0
	    && !sockt->session[chr->login_fd]->flag.eof
	    && (char_id == -1 || character == NULL || character->fd == -1))
...
static int char_mmo_char_tosql(int char_id, struct mmo_charstatus *p)
{
	...
	if ((p->base_exp != cp->base_exp)
	    ...
	    || (p->pet_id != cp->pet_id)
	    || (p->look.weapon != cp->look.weapon)
	    || (p->hom_id != cp->hom_id)
	    || (p->ele_id != cp->ele_id)
	    || (p->look.shield != cp->look.shield)
	    || (p->look.head_top != cp->look.head_top)
	    || (p->look.head_mid != cp->look.head_mid)
	    || (p->look.head_bottom != cp->look.head_bottom)
	    || (p->delete_date != cp->delete_date)
	    || (p->rename != cp->rename)
	    || (p->slotchange != cp->slotchange)
	    || (p->look.robe != cp->look.robe)
	    || (p->show_equip != cp->show_equip)
	    || (p->allow_party != cp->allow_party)
	    || (p->font != cp->font)
	    ...
	    || (p->title_id != cp->title_id)
	    || (p->inventorySize != cp->inventorySize)
		...
		if (p->inventorySize <= 0 || p->inventorySize > MAX_INVENTORY) {
			ShowError(
			    "Wrong inventorySize field: %d. Must be in range 1 to %d. Character %s (CID: %d, AID: %d)\n",
		...
		if (SQL_ERROR
		    == SQL->Query(
		        inter->sql_handle,
		        "UPDATE `%s` SET `base_level`='%d', `job_level`='%d'," "`base_exp`='%" PRIu64
		        "', `job_exp`='%" PRIu64
		        "', `zeny`='%d'," "`max_hp`='%d',`hp`='%d',`max_sp`='%d',`sp`='%d',`status_point`='%d',`skill_point`='%d'," "`str`='%d',`agi`='%d',`vit`='%d',`int`='%d',`dex`='%d',`luk`='%d'," "`option`='%u',`party_id`='%d',`guild_id`='%d',`pet_id`='%d',`homun_id`='%d',`elemental_id`='%d'," "`weapon`='%d',`shield`='%d',`head_top`='%d',`head_mid`='%d',`head_bottom`='%d'," "`last_map`='%s',`last_x`='%d',`last_y`='%d',`save_map`='%s',`save_x`='%d',`save_y`='%d', `rename`='%d'," "`delete_date`='%lu',`robe`='%d',`slotchange`='%d', `char_opt`='%u', `font`='%u', `uniqueitem_counter` ='%u'," "`hotkey_rowshift`='%d',`hotkey_rowshift2`='%d',`clan_id`='%d',`last_login`='%" PRId64
		        "'," "`title_id`='%d', `inventory_size`='%d'" " WHERE  `account_id`='%d' AND `char_id` = '%d'",
		        char_db, p->base_level, p->job_level, p->base_exp, p->job_exp, p->zeny, p->max_hp, p->hp,
		        p->max_sp, p->sp, p->status_point, p->skill_point, p->str, p->agi, p->vit, p->int_, p->dex,
		        p->luk, p->option, p->party_id, p->guild_id, p->pet_id, p->hom_id, p->ele_id, p->look.weapon,
		        p->look.shield, p->look.head_top, p->look.head_mid, p->look.head_bottom,
		        mapindex_id2name(p->last_point.map), p->last_point.x, p->last_point.y,
		        mapindex_id2name(p->save_point.map), p->save_point.x, p->save_point.y, p->rename,
		        (unsigned long)p->delete_date, // FIXME: platform-dependent size
		        p->look.robe, p->slotchange, opt, p->font, p->uniqueitem_counter, p->hotkey_rowshift,
		        p->hotkey_rowshift2, p->clan_id, p->last_login, p->title_id, p->inventorySize, p->account_id,
		        p->char_id
		    )) {
			Sql_ShowDebug(inter->sql_handle);
			errors++;
	...
	if (p->bank_vault != cp->bank_vault
	    || p->mod_exp != cp->mod_exp
	    || p->mod_drop != cp->mod_drop
	    || p->mod_death != cp->mod_death
	    || p->attendance_count != cp->attendance_count
	    || p->attendance_timer != cp->attendance_timer) {
		if (SQL_ERROR
		    == SQL->Query(
		        inter->sql_handle,
		        "REPLACE INTO `%s` (`account_id`,`bank_vault`,`base_exp`,`base_drop`,`base_death`,`attendance_count`,`attendance_timer`) VALUES ('%d','%d','%d','%d','%d','%d','%" PRId64
		        "')",
		        account_data_db, p->account_id, p->bank_vault, p->mod_exp, p->mod_drop, p->mod_death,
		        p->attendance_count, p->attendance_timer
		    )) {
			Sql_ShowDebug(inter->sql_handle);
			errors++;
		} else
	...
	if ((p->hair != cp->hair)
		...
		if (SQL_ERROR
		    == SQL->Query(
		        inter->sql_handle,
		        "UPDATE `%s` SET `class`='%d'," "`hair`='%d', `hair_color`='%d', `clothes_color`='%d', `body`='%d'," "`partner_id`='%d', `father`='%d', `mother`='%d', `child`='%d'," "`karma`='%d', `manner`='%d', `fame`='%d'" " WHERE  `account_id`='%d' AND `char_id` = '%d'",
		        char_db, p->class_, p->hair, p->hair_color, p->clothes_color, p->body, p->partner_id, p->father,
		        p->mother, p->child, p->karma, p->manner, p->fame, p->account_id, p->char_id
		    )) {
			Sql_ShowDebug(inter->sql_handle);
			errors++;
		} else
	...
	if ((p->mer_id != cp->mer_id)
	    || (p->arch_calls != cp->arch_calls)
	    || (p->arch_faith != cp->arch_faith)
	    || (p->spear_calls != cp->spear_calls)
	    || (p->spear_faith != cp->spear_faith)
	    || (p->sword_calls != cp->sword_calls)
	...
	if (memcmp(p->memo_point, cp->memo_point, sizeof(p->memo_point))) {
		...
		if (SQL_ERROR
		    == SQL->Query(inter->sql_handle, "DELETE FROM `%s` WHERE `char_id`='%d'", memo_db, p->char_id)) {
			Sql_ShowDebug(inter->sql_handle);
			errors++;
		}
		...
		for (int i = 0; i < MAX_MEMOPOINTS; ++i) {
			if (p->memo_point[i].map) {
				...
				SQL->EscapeString(
				    inter->sql_handle, esc_mapname, mapindex_id2name(p->memo_point[i].map)
				);
				StrBuf->Printf(
				    &buf, "('%d', '%s', '%d', '%d')", char_id, esc_mapname, p->memo_point[i].x,
				    p->memo_point[i].y
				);
				++count;
			}
	...
	if (memcmp(p->skill, cp->skill, sizeof(p->skill))) {
		//`skill` (`char_id`, `id`, `lv`)
		if (SQL_ERROR
		    == SQL->Query(inter->sql_handle, "DELETE FROM `%s` WHERE `char_id`='%d'", skill_db, p->char_id)) {
			Sql_ShowDebug(inter->sql_handle);
			errors++;
		}
		...
		for (int i = 0; i < MAX_SKILL_DB; ++i) {
			...
			int saved_lv   = (p->skill[i].flag > SKILL_FLAG_REPLACED_LV_0)
			                   ? p->skill[i].flag - SKILL_FLAG_REPLACED_LV_0
			                   : p->skill[i].lv;
			int saved_flag = p->skill[i].flag == SKILL_FLAG_PERM_GRANTED
			                   ? p->skill[i].flag
	...
	if (diff == 1) {
		// Save friends
		if (SQL_ERROR
		    == SQL->Query(inter->sql_handle, "DELETE FROM `%s` WHERE `char_id`='%d'", friend_db, char_id)) {
			Sql_ShowDebug(inter->sql_handle);
			errors++;
		}
		...
		for (int i = 0; i < MAX_FRIENDS; ++i) {
			if (p->friends[i].char_id > 0) {
				if (count)
					StrBuf->AppendStr(&buf, ",");
				StrBuf->Printf(
				    &buf, "('%d','%d','%d')", char_id, p->friends[i].account_id, p->friends[i].char_id
				);
				count++;
			}
		}
	...
	StrBuf->Clear(&buf);
	StrBuf->Printf(
	    &buf, "REPLACE INTO `%s` (`char_id`, `hotkey`, `type`, `itemskill_id`, `skill_lvl`) VALUES ", hotkey_db
	);
	diff = 0;
	for (int i = 0; i < ARRAYLENGTH(p->hotkeys); i++) {
		if (memcmp(&p->hotkeys[i], &cp->hotkeys[i], sizeof(struct hotkey))) {
			...
			StrBuf->Printf(
			    &buf, "('%d','%u','%u','%u','%u')", char_id, (unsigned int)i,
			    (unsigned int)p->hotkeys[i].type, p->hotkeys[i].id, (unsigned int)p->hotkeys[i].lv
			);
			diff = 1;
		}
	}
...
static int char_getitemdata_from_sql(struct item *items, int max, int guid, enum inventory_table_type table)
{
	...
	for (i = 0; i < MAX_SLOTS; i++) {
		if (SQL_ERROR
		    == SQL->StmtBindColumn(stmt, 11 + i, SQLDT_INT, &item.card[i], sizeof item.card[i], NULL, NULL))
			SqlStmt_ShowDebug(stmt);
	}
	...
	for (i = 0; i < MAX_ITEM_OPTIONS; i++) {
		if (SQL_ERROR
		        == SQL->StmtBindColumn(
		            stmt, 11 + MAX_SLOTS + i * 2, SQLDT_INT16, &item.option[i].index,
		            sizeof item.option[i].index, NULL, NULL
		        )
		    || SQL_ERROR
		           == SQL->StmtBindColumn(
		               stmt, 12 + MAX_SLOTS + i * 2, SQLDT_INT16, &item.option[i].value,
		               sizeof item.option[i].index, NULL, NULL
		           ))
			SqlStmt_ShowDebug(stmt);
	}
	...
	if (has_favorite) {
		if (SQL_ERROR
		    == SQL->StmtBindColumn(
		        stmt, 11 + MAX_SLOTS + MAX_ITEM_OPTIONS * 2, SQLDT_CHAR, &item.favorite, sizeof item.favorite,
		        NULL, NULL
		    ))
			SqlStmt_ShowDebug(stmt);
	}
 ...
 * @param[in] current_size The current size of the items array (-1 to automatically use the maximum size, for fixed size
 * inventories).
 ...
 */
static int
    char_memitemdata_to_sql(const struct item *p_items, int current_size, int guid, enum inventory_table_type table)
{
	const char *tablename    = NULL;
	const char *selectoption = NULL;
	bool has_favorite        = false;
			...
			if (j < current_size) {      // Item found.
				...
				if (memcmp(cp_it, &p_items[j], sizeof(struct item)) != 0) {
					if (total_updates == 0) {
						...
						StrBuf->Printf(
						    &buf,
						    "REPLACE INTO `%s` (`id`, `%s`, `nameid`, `amount`, `equip`, `identify`, `refine`, `grade`, `attribute`",
					...
					StrBuf->Printf(
					    &buf, "%s('%d', '%d', '%d', '%d', '%u', '%d', '%d', '%d', '%d'",
					    total_updates > 0 ? ", " : "", cp_it->id, guid, p_items[j].nameid,
					    p_items[j].amount, p_items[j].equip, p_items[j].identify, p_items[j].refine,
					    p_items[j].grade, p_items[j].attribute
					);
					for (int k = 0; k < MAX_SLOTS; k++)
						StrBuf->Printf(&buf, ", '%d'", p_items[j].card[k]);
					for (int k = 0; k < MAX_ITEM_OPTIONS; ++k)
						StrBuf->Printf(
						    &buf, ", '%d', '%d'", p_items[j].option[k].index,
						    p_items[j].option[k].value
						);
					StrBuf->Printf(
					    &buf, ", '%u', '%d', '%" PRIu64 "'", p_items[j].expire_time,
					    p_items[j].bound, p_items[j].unique_id
					);
					if (has_favorite)
	...
	for (int i = 0; i < current_size; i++) {
		...
		if (total_inserts == 0) {
			...
			StrBuf->Printf(
			    &buf,
			    "INSERT INTO `%s` (`%s`, `nameid`, `amount`, `equip`, `identify`, `refine`, `grade`, `attribute`, `expire_time`, `bound`, `unique_id`",
			    tablename, selectoption
			);
		...
		StrBuf->Printf(
		    &buf, "%s('%d', '%d', '%d', '%u', '%d', '%d', '%d', '%d', '%u', '%d', '%" PRIu64 "'",
		    total_inserts > 0 ? ", " : "", guid, p_it->nameid, p_it->amount, p_it->equip, p_it->identify,
		    p_it->refine, p_it->grade, p_it->attribute, p_it->expire_time, p_it->bound, p_it->unique_id
		);
	...
	ShowInfo(
	    "%s save complete - guid: %d (replace: %d, insert: %d, delete: %d)\n", tablename, guid, total_updates,
...
static int char_mmo_gender(const struct char_session_data *sd, const struct mmo_charstatus *p, char sex)
{
	...
	if (sex == 'M' || sex == 'F') {
		if (!sd) {
			...
			ShowWarning(
			    "Character '%s' (CID: %d, AID: %d) has sex '%c', but PACKETVER does not support per-character sex. Defaulting to 'U'.\n",
		...
		if ((sex == 'M' && sd->sex == SEX_FEMALE) || (sex == 'F' && sd->sex == SEX_MALE)) {
			ShowWarning(
			    "Changing sex of character '%s' (CID: %d, AID: %d) to 'U' due to incompatible PACKETVER.\n",
			    p->name, p->char_id, p->account_id
			);
			chr->changecharsex(p->char_id, sd->sex);
		} else {
			ShowInfo(
			    "Resetting sex of character '%s' (CID: %d, AID: %d) to 'U' due to incompatible PACKETVER.\n",
			    p->name, p->char_id, p->account_id
			);
		}
		if (SQL_ERROR
		    == SQL->Query(
		        inter->sql_handle, "UPDATE `%s` SET `sex` = 'U' WHERE `char_id` = '%d'", char_db, p->char_id
		    )) {
			Sql_ShowDebug(inter->sql_handle);
		}
	}
...
static int char_mmo_chars_fromsql(struct char_session_data *sd, uint8 *buf, int *count)
{
	...
	if (SQL_ERROR
	        == SQL->StmtPrepare(
	            stmt,
	            "SELECT " "`char_id`,`char_num`,`name`,`class`,`base_level`,`job_level`,`base_exp`,`job_exp`,`zeny`," "`str`,`agi`,`vit`,`int`,`dex`,`luk`,`max_hp`,`hp`,`max_sp`,`sp`," "`status_point`,`skill_point`,`option`,`karma`,`manner`,`hair`,`hair_color`," "`clothes_color`,`body`,`weapon`,`shield`,`head_top`,`head_mid`,`head_bottom`,`last_map`,`rename`,`delete_date`," "`robe`,`slotchange`,`unban_time`,`sex`,`title_id`,`inventory_size`" " FROM `%s` WHERE `account_id`='%d' AND `char_num` < '%d'",
	    ...
	    || SQL_ERROR
	           == SQL->StmtBindColumn(stmt, 26, SQLDT_SHORT, &p.clothes_color, sizeof p.clothes_color, NULL, NULL)
	    ...
	    || SQL_ERROR
	           == SQL->StmtBindColumn(stmt, 30, SQLDT_INT, &p.look.head_top, sizeof p.look.head_top, NULL, NULL)
	    || SQL_ERROR
	           == SQL->StmtBindColumn(stmt, 31, SQLDT_INT, &p.look.head_mid, sizeof p.look.head_mid, NULL, NULL)
	    || SQL_ERROR
	           == SQL->StmtBindColumn(
	               stmt, 32, SQLDT_INT, &p.look.head_bottom, sizeof p.look.head_bottom, NULL, NULL
	           )
	    ...
	    || SQL_ERROR
	           == SQL->StmtBindColumn(stmt, 41, SQLDT_INT, &p.inventorySize, sizeof p.inventorySize, NULL, NULL)) {
		SqlStmt_ShowDebug(stmt);
		SQL->StmtFree(stmt);
		return 0;
	}
...
static int char_mmo_char_fromsql(int char_id, struct mmo_charstatus *p, bool load_everything)
{
	...
	if (SQL_ERROR
	    ...
	    || SQL_ERROR
	           == SQL->StmtBindColumn(stmt, 20, SQLDT_INT, &p->status_point, sizeof p->status_point, NULL, NULL)
	    ...
	    || SQL_ERROR
	           == SQL->StmtBindColumn(stmt, 32, SQLDT_SHORT, &p->clothes_color, sizeof p->clothes_color, NULL, NULL)
	    ...
	    || SQL_ERROR
	           == SQL->StmtBindColumn(stmt, 36, SQLDT_INT, &p->look.head_top, sizeof p->look.head_top, NULL, NULL)
	    || SQL_ERROR
	           == SQL->StmtBindColumn(stmt, 37, SQLDT_INT, &p->look.head_mid, sizeof p->look.head_mid, NULL, NULL)
	    || SQL_ERROR
	           == SQL->StmtBindColumn(
	               stmt, 38, SQLDT_INT, &p->look.head_bottom, sizeof p->look.head_bottom, NULL, NULL
	           )
	    || SQL_ERROR == SQL->StmtBindColumn(stmt, 39, SQLDT_STRING, &last_map, sizeof last_map, NULL, NULL)
	    || SQL_ERROR
	           == SQL->StmtBindColumn(stmt, 40, SQLDT_INT16, &p->last_point.x, sizeof p->last_point.x, NULL, NULL)
	    || SQL_ERROR
	           == SQL->StmtBindColumn(stmt, 41, SQLDT_INT16, &p->last_point.y, sizeof p->last_point.y, NULL, NULL)
	    || SQL_ERROR == SQL->StmtBindColumn(stmt, 42, SQLDT_STRING, &save_map, sizeof save_map, NULL, NULL)
	    || SQL_ERROR
	           == SQL->StmtBindColumn(stmt, 43, SQLDT_INT16, &p->save_point.x, sizeof p->save_point.x, NULL, NULL)
	    || SQL_ERROR
	           == SQL->StmtBindColumn(stmt, 44, SQLDT_INT16, &p->save_point.y, sizeof p->save_point.y, NULL, NULL)
	    ...
	    || SQL_ERROR
	           == SQL->StmtBindColumn(stmt, 51, SQLDT_TIME, &p->delete_date, sizeof p->delete_date, NULL, NULL)
	    || SQL_ERROR == SQL->StmtBindColumn(stmt, 52, SQLDT_INT, &p->look.robe, sizeof p->look.robe, NULL, NULL)
	    || SQL_ERROR
	           == SQL->StmtBindColumn(stmt, 53, SQLDT_USHORT, &p->slotchange, sizeof p->slotchange, NULL, NULL)
	    ...
	    || SQL_ERROR
	           == SQL->StmtBindColumn(
	               stmt, 56, SQLDT_UINT32, &p->uniqueitem_counter, sizeof p->uniqueitem_counter, NULL, NULL
	           )
	    || SQL_ERROR == SQL->StmtBindColumn(stmt, 57, SQLDT_ENUM, &sex, sizeof sex, NULL, NULL)
	    || SQL_ERROR
	           == SQL->StmtBindColumn(
	               stmt, 58, SQLDT_UCHAR, &p->hotkey_rowshift, sizeof p->hotkey_rowshift, NULL, NULL
	           )
	    || SQL_ERROR
	           == SQL->StmtBindColumn(
	               stmt, 59, SQLDT_UCHAR, &p->hotkey_rowshift2, sizeof p->hotkey_rowshift2, NULL, NULL
	           )
	    ...
	    || SQL_ERROR
	           == SQL->StmtBindColumn(
	               stmt, 63, SQLDT_INT, &p->inventorySize, sizeof p->inventorySize, NULL, NULL
	           )) {
		SqlStmt_ShowDebug(stmt);
		SQL->StmtFree(stmt);
		return 0;
	...
	if (SQL_ERROR
	        == SQL->StmtPrepare(
	            stmt, "SELECT `map`,`x`,`y` FROM `%s` WHERE `char_id`=? ORDER by `memo_id` LIMIT %d", memo_db,
	            MAX_MEMOPOINTS
	        )
	    || SQL_ERROR == SQL->StmtBindParam(stmt, 0, SQLDT_INT, &char_id, sizeof char_id)
	    || SQL_ERROR == SQL->StmtExecute(stmt)
	...
	if (SQL_ERROR
	        == SQL->StmtPrepare(
	            stmt, "SELECT `id`, `lv`,`flag` FROM `%s` WHERE `char_id`=? LIMIT %d", skill_db, MAX_SKILL_DB
	        )
	    || SQL_ERROR == SQL->StmtBindParam(stmt, 0, SQLDT_INT, &char_id, sizeof char_id)
	    || SQL_ERROR == SQL->StmtExecute(stmt)
	    ...
	    || SQL_ERROR
	           == SQL->StmtBindColumn(stmt, 2, SQLDT_UCHAR, &tmp_skill.flag, sizeof tmp_skill.flag, NULL, NULL)) {
		SqlStmt_ShowDebug(stmt);
	}
	...
	for (i = 0; i < MAX_SKILL_DB && SQL_SUCCESS == SQL->StmtNextRow(stmt); ++i) {
		if (skillid2idx[tmp_skill.id])
			...
			ShowWarning(
			    "chr->mmo_char_fromsql: ignoring invalid skill (id=%u,lv=%u) of character %s (AID=%d,CID=%d)\n",
	...
	if (SQL_ERROR
	    ...
	    || SQL_ERROR
	           == SQL->StmtBindColumn(
	               stmt, 0, SQLDT_INT, &tmp_friend.account_id, sizeof tmp_friend.account_id, NULL, NULL
	           )
	    || SQL_ERROR
	           == SQL->StmtBindColumn(
	               stmt, 1, SQLDT_INT, &tmp_friend.char_id, sizeof tmp_friend.char_id, NULL, NULL
	           )
	    || SQL_ERROR
	           == SQL->StmtBindColumn(
	               stmt, 2, SQLDT_STRING, &tmp_friend.name, sizeof tmp_friend.name, NULL, NULL
	           )) {
		SqlStmt_ShowDebug(stmt);
	}
	...
	if (SQL_ERROR
	        == SQL->StmtPrepare(
	            stmt, "SELECT `hotkey`, `type`, `itemskill_id`, `skill_lvl` FROM `%s` WHERE `char_id`=?", hotkey_db
	        )
	    || SQL_ERROR == SQL->StmtBindParam(stmt, 0, SQLDT_INT, &char_id, sizeof char_id)
	    || SQL_ERROR == SQL->StmtExecute(stmt)
	    || SQL_ERROR == SQL->StmtBindColumn(stmt, 0, SQLDT_INT, &hotkey_num, sizeof hotkey_num, NULL, NULL)
	    || SQL_ERROR
	           == SQL->StmtBindColumn(stmt, 1, SQLDT_UCHAR, &tmp_hotkey.type, sizeof tmp_hotkey.type, NULL, NULL)
	    || SQL_ERROR == SQL->StmtBindColumn(stmt, 2, SQLDT_UINT, &tmp_hotkey.id, sizeof tmp_hotkey.id, NULL, NULL)
	    || SQL_ERROR
	           == SQL->StmtBindColumn(stmt, 3, SQLDT_USHORT, &tmp_hotkey.lv, sizeof tmp_hotkey.lv, NULL, NULL)) {
		SqlStmt_ShowDebug(stmt);
	}
	...
	while (SQL_SUCCESS == SQL->StmtNextRow(stmt)) {
		if (hotkey_num >= 0 && hotkey_num < MAX_HOTKEYS_DB)
			...
			ShowWarning(
			    "chr->mmo_char_fromsql: ignoring invalid hotkey (hotkey=%d,type=%u,id=%u,lv=%u) of character %s (AID=%d,CID=%d)\n",
	...
	//`account_data` (`account_id`,`bank_vault`,`base_exp`,`base_drop`,`base_death`,`attendance_count`,
	//`attendance_timer`)
	if (SQL_ERROR
	        == SQL->StmtPrepare(
	            stmt,
	            "SELECT `bank_vault`,`base_exp`,`base_drop`,`base_death`,`attendance_count`, `attendance_timer` FROM `%s` WHERE `account_id`=? LIMIT 1",
	            account_data_db
	        )
	    ...
	    || SQL_ERROR
	           == SQL->StmtBindColumn(
	               stmt, 4, SQLDT_SHORT, &p->attendance_count, sizeof p->attendance_count, NULL, NULL
	           )
	    || SQL_ERROR
	           == SQL->StmtBindColumn(
	               stmt, 5, SQLDT_INT64, &p->attendance_timer, sizeof p->attendance_timer, NULL, NULL
	           )) {
		SqlStmt_ShowDebug(stmt);
	}
...
static bool char_char_slotchange(struct char_session_data *sd, int fd, unsigned short from, unsigned short to)
{
	...
	if (from >= MAX_CHARS
	    || to >= MAX_CHARS
	    || (sd->char_slots && to > sd->char_slots)
	    || sd->found_char[from] <= 0)
		return false;
	...
	if (sd->found_char[to] > 0) { /* moving char to occupied slot */
		...
		if (SQL_SUCCESS != SQL->QueryStr(inter->sql_handle, "START TRANSACTION")
		    || SQL_SUCCESS
		           != SQL->Query(
		               inter->sql_handle, "UPDATE `%s` SET `char_num`='%d' WHERE `char_id`='%d' LIMIT 1",
		               char_db, from, sd->found_char[to]
		           )
		    || SQL_SUCCESS
		           != SQL->Query(
		               inter->sql_handle, "UPDATE `%s` SET `char_num`='%d' WHERE `char_id`='%d' LIMIT 1",
		               char_db, to, sd->found_char[from]
		           ))
			Sql_ShowDebug(inter->sql_handle);
		else
			result = true;
		...
		if (SQL_ERROR
		    == SQL->Query(
		        inter->sql_handle, "UPDATE `%s` SET `char_num`='%d' WHERE `char_id`='%d' LIMIT 1", char_db, to,
		        sd->found_char[from]
		    )) {
			Sql_ShowDebug(inter->sql_handle);
			return false;
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle, "UPDATE `%s` SET `slotchange`=`slotchange`-1 WHERE `char_id`='%d' LIMIT 1", char_db,
	        from_id
	    )) {
		Sql_ShowDebug(inter->sql_handle);
		return false;
...
static int char_rename_char_sql(struct char_session_data *sd, int char_id)
{
	...
	if (SQL_ERROR
	    == SQL->Query(inter->sql_handle, "SELECT 1 FROM `%s` WHERE `name` LIKE '%s' LIMIT 1", char_db, esc_name)) {
		Sql_ShowDebug(inter->sql_handle);
		return 4;
	}
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle, "UPDATE `%s` SET `name` = '%s', `rename` = '%d' WHERE `char_id` = '%d'", char_db,
	        esc_name, --char_dat.rename, char_id
	    )) {
		Sql_ShowDebug(inter->sql_handle);
		return 3;
	}
	...
	if (chr->enable_logs) {
		if (SQL_ERROR
		    == SQL->Query(
		        inter->sql_handle,
		        "INSERT INTO `%s` (" " `time`, `char_msg`, `account_id`, `char_id`, `char_num`, `class`, `name`," " `str`, `agi`, `vit`, `int`, `dex`, `luk`," " `hair`, `hair_color`" ") VALUES (" " NOW(), 'change char name', '%d', '%d', '%d', '%d', '%s'," " '%d', '%d', '%d', '%d', '%d', '%d'," " '%d', '%d'" ")",
		        charlog_db, sd->account_id, char_dat.char_id, char_dat.slot, char_dat.class_, esc_name,
...
static bool char_name_exists(const char *name, const char *esc_name)
{
	...
	if (name_ignoring_case) {
		if (SQL_ERROR
		    == SQL->Query(
		        inter->sql_handle, "SELECT 1 FROM `%s` WHERE BINARY `name` = '%s' LIMIT 1", char_db, esc_name
		    )) {
			Sql_ShowDebug(inter->sql_handle);
			return true;
		}
		...
		if (SQL_ERROR
		    == SQL->Query(
		        inter->sql_handle, "SELECT 1 FROM `%s` WHERE `name` = '%s' LIMIT 1", char_db, esc_name
		    )) {
			Sql_ShowDebug(inter->sql_handle);
			return true;
		}
 ...
 *  char_id: Success
 **/
static int char_make_new_char_sql(
    struct char_session_data *sd, const char *name_, int str, int agi, int vit, int int_, int dex, int luk, int slot,
    int hair_color, int hair_style, int starting_class, uint8 sex
)
{
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle,
	        "INSERT INTO `%s` (`account_id`, `char_num`, `name`, `class`, `zeny`, `status_point`,`str`, `agi`, `vit`, `int`, `dex`, `luk`, `max_hp`, `hp`," "`max_sp`, `sp`, `hair`, `hair_color`, `last_map`, `last_x`, `last_y`, `save_map`, `save_x`, `save_y`, `sex`, `inventory_size`) VALUES (" "'%d', '%d', '%s', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d','%d', '%d','%d', '%d', '%s', '%d', '%d', '%s', '%d', '%d', '%c', '%d')",
	        ...
	        (11 * (100 + int_) / 100), hair_style, hair_color, mapindex_id2name(start_point.map), start_point.x,
	        start_point.y, mapindex_id2name(start_point.map), start_point.x, start_point.y, sex,
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle,
	        "INSERT INTO `%s` (`account_id`, `char_num`, `name`, `class`, `zeny`, `str`, `agi`, `vit`, `int`, `dex`, `luk`, `max_hp`, `hp`," "`max_sp`, `sp`, `hair`, `hair_color`, `last_map`, `last_x`, `last_y`, `save_map`, `save_x`, `save_y`, `inventory_size`) VALUES (" "'%d', '%d', '%s', '%d',  '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d','%d', '%d','%d', '%d', '%s', '%d', '%d', '%s', '%d', '%d', '%d')",
	        char_db, sd->account_id, slot, esc_name, starting_class, start_zeny, str, agi, vit, int_, dex, luk,
	        (40 * (100 + vit) / 100), (40 * (100 + vit) / 100), (11 * (100 + int_) / 100),
	        (11 * (100 + int_) / 100), hair_style, hair_color, mapindex_id2name(start_point.map), start_point.x,
	        start_point.y, mapindex_id2name(start_point.map), start_point.x, start_point.y, FIXED_INVENTORY_SIZE
	    )) {
		Sql_ShowDebug(inter->sql_handle);
		return -2; // No, stop the procedure!
	}
	...
	if (chr->enable_logs) {
		if (SQL_ERROR
		    == SQL->Query(
		        inter->sql_handle,
		        "INSERT INTO `%s` (`time`, `char_msg`, `account_id`, `char_id`, `char_num`, `class`, `name`, `str`, `agi`, `vit`, `int`, `dex`, `luk`, `hair`, `hair_color`)" "VALUES (NOW(), '%s', '%d', '%d', '%d', '%d', '%s', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d')",
		        charlog_db, "make new char", sd->account_id, char_id, slot, starting_class, esc_name, str, agi,
		        vit, int_, dex, luk, hair_style, hair_color
		    ))
			Sql_ShowDebug(inter->sql_handle);
	}
	...
	ShowInfo(
	    "Created char: account: %d, char: %d, slot: %d, name: %s, sex: %c\n", sd->account_id, char_id, slot, name,
	    sex
	);
	return char_id;
}
...
static int char_divorce_char_sql(int partner_id1, int partner_id2)
{
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle, "UPDATE `%s` SET `partner_id`='0' WHERE `char_id`='%d' OR `char_id`='%d' LIMIT 2",
	        char_db, partner_id1, partner_id2
	    ))
		Sql_ShowDebug(inter->sql_handle);
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle,
	        "DELETE FROM `%s` WHERE (`nameid`='%d' OR `nameid`='%d') AND (`char_id`='%d' OR `char_id`='%d') LIMIT 2",
	        inventory_db, WEDDING_RING_M, WEDDING_RING_F, partner_id1, partner_id2
	    ))
		Sql_ShowDebug(inter->sql_handle);
...
static int char_delete_char_sql(int char_id)
{
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle,
	        "SELECT `name`,`account_id`,`party_id`,`guild_id`,`base_level`,`homun_id`,`partner_id`,`father`,`mother`,`elemental_id` FROM `%s` WHERE `char_id`='%d'",
	        char_db, char_id
	    ))
		Sql_ShowDebug(inter->sql_handle);
	...
	if (father_id || mother_id) { // Char is Baby
		...
		if (SQL_ERROR
		    == SQL->Query(
		        inter->sql_handle, "UPDATE `%s` SET `child`='0' WHERE `char_id`='%d' OR `char_id`='%d'",
		        char_db, father_id, mother_id
		    ))
			Sql_ShowDebug(inter->sql_handle);
		if (SQL_ERROR
		    == SQL->Query(
		        inter->sql_handle, "DELETE FROM `%s` WHERE `id` = '410'AND (`char_id`='%d' OR `char_id`='%d')",
		        skill_db, father_id, mother_id
		    ))
			Sql_ShowDebug(inter->sql_handle);
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle, "DELETE FROM `%s` WHERE `char_id`='%d' AND `incubate` = '0'", pet_db, char_id
	    ))
		Sql_ShowDebug(inter->sql_handle);
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle,
	        "DELETE FROM `%s` USING `%s` JOIN `%s` ON `pet_id` = `card1`|`card2`<<16 WHERE `%s`.char_id = '%d' AND card0 = -256",
	        pet_db, pet_db, inventory_db, inventory_db, char_id
	    ))
		Sql_ShowDebug(inter->sql_handle);
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle,
	        "DELETE FROM `%s` USING `%s` JOIN `%s` ON `pet_id` = `card1`|`card2`<<16 WHERE `%s`.char_id = '%d' AND card0 = -256",
	        pet_db, pet_db, cart_db, cart_db, char_id
	    ))
		Sql_ShowDebug(inter->sql_handle);
	...
	if (SQL_ERROR
	    == SQL->Query(inter->sql_handle, "DELETE FROM `%s` WHERE `char_id`='%d'", char_reg_str_db, char_id))
		Sql_ShowDebug(inter->sql_handle);
	if (SQL_ERROR
	    == SQL->Query(inter->sql_handle, "DELETE FROM `%s` WHERE `char_id`='%d'", char_reg_num_db, char_id))
		Sql_ShowDebug(inter->sql_handle);
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle, "DELETE FROM `%s` WHERE `account_id` = '%d' AND `char_id`='%d'", scdata_db,
	        account_id, char_id
	    ))
		Sql_ShowDebug(inter->sql_handle);
#endif
	...
	if (SQL_ERROR == SQL->Query(inter->sql_handle, "DELETE FROM `%s` WHERE `char_id`='%d'", char_db, char_id)) {
		...
		if (SQL_ERROR
		    == SQL->Query(
		        inter->sql_handle,
		        "INSERT INTO `%s`(`time`, `account_id`, `char_id`, `char_num`, `char_msg`, `name`)" " VALUES (NOW(), '%d', '%d', '%d', 'Deleted character', '%s')",
	...
	if( SQL_ERROR == SQL->Query(inter->sql_handle, "DELETE FROM `%s` WHERE `char_id`='%d'", guild_member_db,
	char_id) ) Sql_ShowDebug(inter->sql_handle);
	*/

	if (SQL_ERROR
	    == SQL->Query(inter->sql_handle, "SELECT `guild_id` FROM `%s` WHERE `char_id` = '%d'", guild_db, char_id))
		Sql_ShowDebug(inter->sql_handle);
...
static int char_mmo_char_tobuf(uint8 *buffer, struct mmo_charstatus *p)
{
...
#else
	WBUFL(buf, 4) = std::min(
	    (uint32)(p->base_exp), (uint32)INT32_MAX
	); // FIXME: This should run on the larger type and be cast to uint32 after the clamping
#endif
	WBUFL(buf, 8) = p->zeny;
#if PACKETVER >= 20170830
	...
	buf             = WBUFP(uint8 *, buffer, offset);
#else
	WBUFL(buf, 12) = std::min(
	    (uint32)(p->job_exp), (uint32)INT32_MAX
	); // FIXME: This should run on the larger type and be cast to uint32 after the clamping
#endif
	...
	//  FIXME[Haru]: is OPTION_HANBOK intended to be part of this list? And if it is, should the list also include
	//  other OPTION_ costumes?
	WBUFW(buf, 56)
	    = (p->option
	       & (OPTION_RIDING | OPTION_DRAGON | OPTION_WUG | OPTION_WUGRIDER | OPTION_MADOGEAR | OPTION_HANBOK))
	        ? 0
	        : p->look.weapon;
...
static void char_send_HC_ACK_CHARINFO_PER_PAGE(int fd, struct char_session_data *sd)
{
	...
	p->packetLen
	= chr->mmo_chars_fromsql(sd, WFIFOP(uint8 *, fd, len), &count) + len;
...
static void char_mmo_char_send_ban_list(int fd, struct char_session_data *sd)
{
	...
	if (i != MAX_CHARS) {
		...
		for (i = 0, c = 0; i < MAX_CHARS; i++) {
			if (sd->unban_time[i]) {
				timestamp2string(
				    WFIFOP(char *, fd, 8 + (28 * c)), 20, sd->unban_time[i], "%Y-%m-%d %H:%M:%S"
				);
...
static int char_mmo_char_send_characters(int fd, struct char_session_data *sd)
{
	...
	WFIFOB(fd, 6) = MAX_CHARS; // Premium slots. AKA any existent chars past sd->char_slots but within MAX_CHARS
	                           // will show a 'Premium Service' in red
#endif
...
{
	if (SQL_ERROR
	    == SQL->Query(inter->sql_handle, "SELECT `partner_id` FROM `%s` WHERE `char_id` = '%d'", char_db, pl1))
		Sql_ShowDebug(inter->sql_handle);
...
static int char_char_child(int parent_id, int child_id)
{
	if (parent_id == 0 || child_id == 0) // Failsafe, avoild querys and fix EXP bug dividing with lower level chars
		return 0;
	if (SQL_ERROR
	    == SQL->Query(inter->sql_handle, "SELECT `child` FROM `%s` WHERE `char_id` = '%d'", char_db, parent_id))
		Sql_ShowDebug(inter->sql_handle);
	else if (SQL_SUCCESS == SQL->NextRow(inter->sql_handle)) {
		char *data;
...
{
	if (cid1 == 0 || cid2 == 0 || cid3 == 0) // Failsafe, and avoid querys where there is no sense to keep executing
	                                         // if any of the inputs are 0
		return 0;
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle,
	        "SELECT `char_id`,`partner_id`,`child` FROM `%s` WHERE `char_id` IN ('%d','%d','%d')", char_db, cid1,
	        cid2, cid3
	    ))
		Sql_ShowDebug(inter->sql_handle);
	else
		while (SQL_SUCCESS == SQL->NextRow(inter->sql_handle)) {
			...
			if ((cid1 == charid
			    ...
			    || (cid1 == childid
			        && ((cid2 == charid && cid3 == partnerid) || (cid2 == partnerid && cid3 == charid)))) {
				SQL->FreeResult(inter->sql_handle);
				return childid;
			}
		}
...
static void char_disconnect_player(int account_id)
{
	...
	ARR_FIND(
	    0, sockt->fd_max, i,
	    sockt->session[i]
	        && (sd = (struct char_session_data *)sockt->session[i]->session_data)
...
static void char_auth_ok(int fd, struct char_session_data *sd)
{
	...
	if ((character = (struct online_char_data *)idb_get(chr->online_char_db, sd->account_id)) != NULL) {
		...
		if (character->mapserver_connection == OCS_CONNECTED) {
			...
			mapif->disconnectplayer(
			    character->account_id, character->char_id, 2
			); // 2: Already connected to server
			if (character->waiting_disconnect == INVALID_TIMER)
				character->waiting_disconnect = timer->add(
				    timer->gettick() + 20000, chr->waiting_disconnect, character->account_id, 0
				);
			character->pincode_enable = -1;
...
static int char_parse_fromlogin_connection_state(int fd)
{
	...
	case 1: // Invalid username/password
		...
		ShowError(
		    "Also, please make sure your login db has the correct communication username/passwords, the gender of the account is S and its id is between 0 and (MAX_SERVERS - 1).\n"
		);
		ShowError(
...
static void char_parse_fromlogin_auth_state(int fd)
{
	...
	if (sockt->session_is_active(request_id)
	    && (sd = (struct char_session_data *)sockt->session[request_id]->session_data)
	    && !sd->auth
	    && sd->account_id == account_id
...
static void char_parse_fromlogin_account_data(int fd)
{
	...
	if (i < sockt->fd_max) {
		...
		if ((max_connect_user == 0 && sd->group_id != gm_allow_group)
		    || (max_connect_user > 0
		        && chr->count_users() >= max_connect_user
		        && sd->group_id != gm_allow_group)) {
...
static void char_change_sex_sub(int sex, int acc, int char_id, int class_, int guild_id)
{
	...
	const char *query_char
	    = "UPDATE `%s` SET `class`=?, `weapon`='0', `shield`='0', `head_top`='0', " "`head_mid`='0', `head_bottom`='0', `robe`='0', `sex`=? WHERE `char_id`=?";
...
static int char_parse_fromlogin_changesex_reply(int fd)
{
	...
	if (SQL_ERROR
	        == SQL->StmtPrepare(
	            stmt, "SELECT `char_id`,`class`,`guild_id` FROM `%s` WHERE `account_id` = '%d'", char_db, acc
	        )
	    || SQL_ERROR == SQL->StmtExecute(stmt)
...
static void char_parse_fromlogin_kick(int fd)
{
	...
	if (character != NULL) {
		...
		if (character->mapserver_connection == OCS_CONNECTED) {
			...
			if (character->waiting_disconnect == INVALID_TIMER)
				character->waiting_disconnect = timer->add(
				    timer->gettick() + AUTH_TIMEOUT, chr->waiting_disconnect, character->account_id, 0
				);
		} else {
			...
			ARR_FIND(
			    0, sockt->fd_max, i,
			    sockt->session[i]
			        && (tsd = (struct char_session_data *)sockt->session[i]->session_data)
			        && tsd->account_id == aid
			);
			if (i < sockt->fd_max) {
				chr->authfail_fd(i, 2);
...
{
	inter->accinfo2(
	    false, RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10), RFIFOL(fd, 14), NULL, NULL, NULL, NULL, NULL, NULL,
	    NULL, -1, 0, 0
	);
	RFIFOSKIP(fd, 18);
}
...
{
	inter->accinfo2(
	    true, RFIFOL(fd, 167), RFIFOL(fd, 171), RFIFOL(fd, 175), RFIFOL(fd, 179), RFIFOP(char *, fd, 2),
	    RFIFOP(char *, fd, 26), RFIFOP(char *, fd, 59), RFIFOP(char *, fd, 99), RFIFOP(char *, fd, 119),
...
/**
 * Completes global account reg saving that starts chr->global_accreg_to_login_start and continues with
 * chr->global_accreg_to_login_add
 **/
...
static void char_read_fame_list(void)
{
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle,
	        "SELECT `char_id`,`fame`,`name` FROM `%s` WHERE `fame`>0 AND (`class`='%d' OR `class`='%d' OR `class`='%d' OR `class`='%d' OR `class`='%d' OR `class`='%d') ORDER BY `fame` DESC LIMIT 0,%d",
	        char_db, JOB_BLACKSMITH, JOB_WHITESMITH, JOB_BABY_BLACKSMITH, JOB_MECHANIC, JOB_MECHANIC_T,
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle,
	        "SELECT `char_id`,`fame`,`name` FROM `%s` WHERE `fame`>0 AND (`class`='%d' OR `class`='%d' OR `class`='%d' OR `class`='%d' OR `class`='%d' OR `class`='%d') ORDER BY `fame` DESC LIMIT 0,%d",
	        char_db, JOB_ALCHEMIST, JOB_CREATOR, JOB_BABY_ALCHEMIST, JOB_GENETIC, JOB_GENETIC_T, JOB_BABY_GENETIC,
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle,
	        "SELECT `char_id`,`fame`,`name` FROM `%s` WHERE `fame`>0 AND (`class` in('%d', '%d')) ORDER BY `fame` DESC LIMIT 0,%d",
	        char_db, JOB_TAEKWON, JOB_BABY_TAEKWON, fame_list_size_taekwon
	    ))
		Sql_ShowDebug(inter->sql_handle);
...
static int char_loadName(int char_id, char *name)
{
	...
	if (SQL_ERROR
	    == SQL->Query(inter->sql_handle, "SELECT `name` FROM `%s` WHERE `char_id`='%d'", char_db, char_id))
		Sql_ShowDebug(inter->sql_handle);
	else if (SQL_SUCCESS == SQL->NextRow(inter->sql_handle)) {
		SQL->GetData(inter->sql_handle, 0, &data, &len);
		safestrncpy(name, data, NAME_LENGTH);
...
static void char_parse_frommap_map_names(int fd)
{
	...
	ShowStatus(
	    "Map-Server connected: %d maps, from IP %u.%u.%u.%u port %d.\n", (int)VECTOR_LENGTH(chr->map_server.maps),
	    CONVIP(chr->map_server.ip), chr->map_server.port
	);
...
static void char_send_scdata(int fd, int aid, int cid)
{
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle,
	        "SELECT `type`, `tick`, `total_tick`, `val1`, `val2`, `val3`, `val4` " "FROM `%s` WHERE `account_id` = '%d' AND `char_id`='%d'",
	        scdata_db, aid, cid
	    )) {
		Sql_ShowDebug(inter->sql_handle);
		return;
	...
	if (SQL->NumRows(inter->sql_handle) > 0) {
		...
		for (count = 0; count < 50 && SQL_SUCCESS == SQL->NextRow(inter->sql_handle); ++count) {
			...
			memcpy(
			    WFIFOP(struct status_change_data *, fd, 14 + count * sizeof(struct status_change_data)),
			    &scdata, sizeof(struct status_change_data)
			);
		}
		if (count >= 50)
		...
		if (count > 0) {
			...
			if (SQL_ERROR
			    == SQL->Query(
			        inter->sql_handle, "DELETE FROM `%s` WHERE `account_id` = '%d' AND `char_id`='%d'",
			        scdata_db, aid, cid
			    ))
				Sql_ShowDebug(inter->sql_handle);
		}
...
static void char_parse_frommap_set_users(int fd)
{
	...
	chr->online_char_db->foreach(
	    chr->online_char_db, chr->db_setoffline
	...
	for (int i = 0; i < chr->map_server.users; i++) {
		...
		struct online_char_data *character
		    = (struct online_char_data *)idb_ensure(chr->online_char_db, aid, chr->create_online_char_data);
		character->mapserver_connection = OCS_CONNECTED;
...
static void char_parse_frommap_save_character(int fd)
{
	...
	if (size - 13 != sizeof(struct mmo_charstatus)) {
		ShowError(
		    "parse_from_map (save-char): Size mismatch! %d != %" PRIuS "\n", size - 13,
	...
	if (RFIFOB(fd, 12)
		...
		ShowError(
		    "parse_from_map (save-char): Received data for non-existing/offline character (%d:%d).\n", aid, cid
		);
...
static void char_parse_frommap_remove_friend(int fd)
{
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle, "DELETE FROM `%s` WHERE `char_id`='%d' AND `friend_id`='%d' LIMIT 1", friend_db,
	        char_id, friend_id
	    )) {
		Sql_ShowDebug(inter->sql_handle);
	}
...
static void char_parse_frommap_change_email(int fd)
{
	if (chr->login_fd > 0) { // don't send request if no login-server
		...
		memcpy(
		    WFIFOP(void *, chr->login_fd, 0), RFIFOP(void *, fd, 0), 86
...
static void char_ban(
    int account_id, int char_id, time_t *unban_time, short year, short month, short day, short hour, short minute,
    short second
)
{
	time_t timestamp;
	...
	if (SQL_SUCCESS
	        != SQL->StmtPrepare(stmt, "UPDATE `%s` SET `unban_time` = ? WHERE `char_id` = ? LIMIT 1", char_db)
...
static void char_unban(int char_id, int *result)
{
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle, "UPDATE `%s` SET `unban_time` = '0' WHERE `char_id` = '%d' LIMIT 1", char_db, char_id
	    )) {
		Sql_ShowDebug(inter->sql_handle);
		if (result)
			*result = 1;
...
static void char_parse_frommap_change_account(int fd)
{
	...
	enum zh_char_ask_name_type type = (enum zh_char_ask_name_type)RFIFOW(
	    fd, 30
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle, "SELECT `account_id`,`char_id`,`unban_time` FROM `%s` WHERE `name` = '%s'", char_db,
	        esc_name
	    )) {
		Sql_ShowDebug(inter->sql_handle);
	} else if (SQL->NumRows(inter->sql_handle) == 0) {
	...
	if (acc != -1
	    && type != CHAR_ASK_NAME_CHANGESEX
	    && type != CHAR_ASK_NAME_CHANGECHARSEX) { // Don't send answer for changesex
...
static void char_parse_frommap_ragsrvinfo(int fd)
{
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle, "INSERT INTO `%s` SET `index`='%d',`name`='%s',`exp`='%u',`jexp`='%u',`drop`='%u'",
...
static void char_parse_frommap_save_status_change_data(int fd)
{
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle, "DELETE FROM `%s` WHERE `account_id` = '%d' AND `char_id`='%d'", scdata_db, aid, cid
	    ))
		Sql_ShowDebug(inter->sql_handle);
	...
	if (count > 0) {
		...
		StrBuf->Printf(
		    &buf,
		    "INSERT INTO `%s` (`account_id`, `char_id`, `type`, `tick`, `total_tick`, `val1`, `val2`, `val3`, `val4`) VALUES ",
		    scdata_db
		);
		for (i = 0; i < count; ++i) {
			memcpy(
			    &data, RFIFOP(struct status_change_data *, fd, 14 + i * sizeof(struct status_change_data)),
			    sizeof(struct status_change_data)
			);
			if (i > 0)
				StrBuf->AppendStr(&buf, ", ");
			StrBuf->Printf(
			    &buf, "('%d','%d','%hu','%d','%d','%d','%d','%d','%d')", aid, cid, data.type, data.tick,
			    data.total_tick, data.val1, data.val2, data.val3, data.val4
			);
		}
		if (SQL_ERROR == SQL->QueryStr(inter->sql_handle, StrBuf->Value(&buf)))
			Sql_ShowDebug(inter->sql_handle);
...
static void char_parse_frommap_scdata_update(int fd)
{
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle,
	        "REPLACE INTO `%s`" " (`account_id`,`char_id`,`type`,`tick`,`total_tick`,`val1`,`val2`,`val3`,`val4`)" " VALUES ('%d','%d','%d','%d','%d','%d','%d','%d','%d')",
...
static void char_parse_frommap_scdata_delete(int fd)
{
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle,
	        "DELETE FROM `%s` WHERE `account_id` = '%d' AND `char_id` = '%d' AND `type` = '%d' LIMIT 1", scdata_db,
	        account_id, char_id, type
	    )) {
		Sql_ShowDebug(inter->sql_handle);
	}
	RFIFOSKIP(fd, 12);
...
static int char_parse_frommap(int fd)
{
	...
	while (RFIFOREST(fd) >= 2) {
		...
		case 0x2B0E: // Request from map-server to change an account's or character's status (accounts will just
		             // be forwarded to login server)
			if (RFIFOREST(fd) < 44)
				return 0;
			{
...
static uint32 char_lan_subnet_check(uint32 ip)
{
	...
	if (sockt->lan_subnet_check(ip, &lan)) {
		ShowInfo(
		    ...
		    CONVIP(lan.ip & lan.mask), CONVIP(lan.mask)
		);
		return lan.ip;
	}
...
{ // CH: <0827>.W <char id>.L
	...
	if (SQL_SUCCESS
	        != SQL->Query(
	            inter->sql_handle, "SELECT `delete_date` FROM `%s` WHERE `char_id`='%d'", char_db, char_id
	        )
	...
	if (char_aegis_delete) {
		...
		if (SQL_SUCCESS
		        != SQL->Query(
		            inter->sql_handle, "SELECT `party_id`, `guild_id` FROM `%s` WHERE `char_id`='%d'", char_db,
		            char_id
		        )
		    || SQL_SUCCESS != SQL->NextRow(inter->sql_handle)) {
			Sql_ShowDebug(inter->sql_handle);
	...
	if (SQL_SUCCESS
	    != SQL->Query(
	        inter->sql_handle, "UPDATE `%s` SET `delete_date`='%lu' WHERE `char_id`='%d'", char_db,
	        (unsigned long)delete_date, char_id
	    )) {
		Sql_ShowDebug(inter->sql_handle);
		chr->delete2_ack(fd, char_id, 3, 0); // 3: A database error occurred
		return;
...
{ // CH: <0829>.W <char id>.L <birth date:YYMMDD>.6B
	...
	if (SQL_SUCCESS
	        != SQL->Query(
	            inter->sql_handle, "SELECT `base_level`,`delete_date` FROM `%s` WHERE `char_id`='%d'", char_db,
	            char_id
	        )
	    || SQL_SUCCESS != SQL->NextRow(inter->sql_handle)) { // data error
		Sql_ShowDebug(inter->sql_handle);
		chr->delete2_accept_ack(fd, char_id, 3); // 3: A database error occurred
		return;
	}
...
{ // CH: <082b>.W <char id>.L
	...
	if (SQL_SUCCESS
	    != SQL->Query(
	        inter->sql_handle, "UPDATE `%s` SET `delete_date`='0' WHERE `char_id`='%d'", char_db, char_id
	    )) {
		Sql_ShowDebug(inter->sql_handle);
		chr->delete2_cancel_ack(fd, char_id, 2); // 2: A database error occurred
		return;
...
static void char_parse_char_select(int fd, struct char_session_data *sd, uint32 ipl)
{
	...
	if (pincode->enabled) { // hack check
		struct online_char_data *character
		    = (struct online_char_data *)idb_get(chr->online_char_db, sd->account_id);
	...
	if (SQL_SUCCESS
	        != SQL->Query(
	            inter->sql_handle, "SELECT `char_id` FROM `%s` WHERE `account_id`='%d' AND `char_num`='%d'",
	            char_db, sd->account_id, slot
	        )
	    || SQL_SUCCESS != SQL->NextRow(inter->sql_handle)
	...
	/* set char as online prior to loading its data so 3rd party applications will realize the sql data is not
	 * reliable */
	...
	if (chr->enable_logs) {
		...
		if (SQL_ERROR
		    == SQL->Query(
		        inter->sql_handle,
		        "INSERT INTO `%s`(" " `time`, `char_msg`, `account_id`, `char_id`, `char_num`, `class`, `name`," " `str`, `agi`, `vit`, `int`, `dex`, `luk`," " `hair`, `hair_color`" ") VALUES (" " NOW(), 'char select', '%d', '%d', '%d', '%d', '%s'," " '%d', '%d', '%d', '%d', '%d', '%d'," " '%d', '%d')",
		        charlog_db, sd->account_id, cd->char_id, slot, char_dat.class_, esc_name, char_dat.str,
		        char_dat.agi, char_dat.vit, char_dat.int_, char_dat.dex, char_dat.luk, char_dat.hair,
		        char_dat.hair_color
		    ))
			Sql_ShowDebug(inter->sql_handle);
	}
	...
	if (!chr->mapserver_has_map(cd->last_point.map) || cd->last_point.map == 0) {
		...
		if (!chr->find_available_map_fallback(cd)) {
			ShowInfo(
			    "Connection Closed. No map server available that has a major city, and unable to find map-server for '%s'.\n",
	...
	if ((map_fd = chr->map_server.fd) < 1 || sockt->session[map_fd] == NULL) {
		ShowError(
		    "chr->parse_char: Attempting to write to invalid session %d! Map Server disconnected.\n", map_fd
		);
		chr->map_server.fd = -1;
...
static void char_parse_char_create_new_char(int fd, struct char_session_data *sd)
{
	...
	if (!enable_char_creation) {
		...
		result = chr->make_new_char_sql(
		    sd, RFIFOP(char *, fd, 2), 1, 1, 1, 1, 1, 1, RFIFOB(fd, 26), RFIFOW(fd, 27), RFIFOW(fd, 29),
		    RFIFOL(fd, 31), sex
		);
...
#else
		result = chr->make_new_char_sql(
		    sd, RFIFOP(char *, fd, 2), RFIFOB(fd, 26), RFIFOB(fd, 27), RFIFOB(fd, 28), RFIFOB(fd, 29),
		    RFIFOB(fd, 30), RFIFOB(fd, 31), RFIFOB(fd, 32), RFIFOW(fd, 33), RFIFOW(fd, 35), JOB_NOVICE, 'U'
		);
#endif
	}
...
{
	ShowError(
	    "chr->parse_char: Received unknown packet " CL_WHITE "0x%x" CL_RESET " from ip '" CL_WHITE "%s" CL_RESET
	    "'! Disconnecting!\n",
	...
	if (sockt->session[fd]->flag.eof) {
		if (sd != NULL && sd->auth) {
			...
			if (data != NULL && data->fd == fd)
				data->fd = -1;
			if (data == NULL
			    || data->mapserver_connection
			           == OCS_NOT_CONNECTED) // If it is not in any server, send it offline. [Skotlex]
...
#define FIFOSD_CHECK(rest) \
	do { \
		...
		case 0x66:
			...
			// create new char
#if PACKETVER >= 20151001
		// S 0a39 <name>.24B <slot>.B <hair color>.W <hair style>.W <starting job class ID>.W <Unknown>.(W or 2
		// B's)??? <sex>.B
		case 0xA39: {
			FIFOSD_CHECK(36);
#elif PACKETVER >= 20120307
		...
		case 0x28F:
			...
			// name. 4: Another user is using this character name, so please select another one.
			FIFOSD_CHECK(6);
			{
...
{
	struct online_char_data *character;
	if ((character = (struct online_char_data *)idb_get(chr->online_char_db, id)) != NULL
...
static bool char_sql_config_read_registry(const char *filename, const struct config_t *config, bool imported)
{
	...
	if ((setting = libconfig->lookup(config, "inter_configuration/database_names/registry")) == NULL) {
		if (imported)
			return true;
		ShowError(
		    "sql_config_read: inter_configuration/database_names/registry was not found in %s!\n", filename
		);
		return false;
	}
...
static bool char_sql_config_read_pc(const char *filename, const struct config_t *config, bool imported)
{
	...
	libconfig->setting_lookup_mutable_string(
	    setting, "achievement_db", char_achievement_db, sizeof(char_achievement_db)
	);
	...
	libconfig->setting_lookup_mutable_string(
	    setting, "skill_homunculus_db", skill_homunculus_db, sizeof(skill_homunculus_db)
	);
	libconfig->setting_lookup_mutable_string(setting, "mercenary_db", mercenary_db, sizeof(mercenary_db));
	libconfig->setting_lookup_mutable_string(
	    setting, "mercenary_owner_db", mercenary_owner_db, sizeof(mercenary_owner_db)
	);
...
static bool char_sql_config_read_guild(const char *filename, const struct config_t *config, bool imported)
{
	...
	libconfig->setting_lookup_mutable_string(
	    setting, "expulsion_db", guild_expulsion_db, sizeof(guild_expulsion_db)
	);
...
static bool char_config_read_top(const char *filename, const struct config_t *config, bool imported)
{
	...
	if (libconfig->setting_lookup_mutable_string(setting, "server_name", chr->server_name, sizeof(chr->server_name))
	    == CONFIG_TRUE) {
	...
	if (libconfig->setting_lookup_mutable_string(
	        setting, "wisp_server_name", wisp_server_name, sizeof(wisp_server_name)
	    )
	    == CONFIG_TRUE) {
		...
		if (strlen(wisp_server_name) < 4) { // TODO: This length should be a #define (i.e. MIN_NAME_LENGTH)
			ShowWarning(
			    "char_config_read: char_configuration/wisp_server_name is too small! Defaulting to: Server.\n"
			);
...
static bool char_config_read_player_name(const char *filename, const struct config_t *config, bool imported)
{
	...
	libconfig->setting_lookup_mutable_string(
	    setting, "unknown_char_name", unknown_char_name, sizeof(unknown_char_name)
	);
	...
	if ((setting2 = libconfig->setting_get_member(setting, start_point_setting))) {
		...
		if (libconfig->setting_lookup_string(setting2, "map", &str) == CONFIG_TRUE) {
			...
			if (start_point.map == 0)
				ShowError(
				    "char_config_read_player_new: Specified start_point %s not found in map-index cache.\n",
				    str
				);
...
static bool char_config_read_permission(const char *filename, const struct config_t *config, bool imported)
{
	...
	if (libconfig->setting_lookup_int(setting, "server_type", &chr->server_type) == CONFIG_TRUE) {
		if (chr->server_type < CST_NORMAL || chr->server_type >= CST_MAX) {
			ShowWarning(
			    "char_config_read: Invalid permission/server_type %d, defaulting to CST_NORMAL.\n",
			    chr->server_type
			);
...
#undef CHECK_OLD_LOCAL_CONF
	...
	if (strcmp(chr->userid, "s1") == 0 && strcmp(chr->passwd, "p1") == 0) {
		...
		ShowNotice(
		    "And then change the user/password to use in conf/char/char-server.conf (or conf/import/char-server.conf)\n"
		);
	}
#endif
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle,
	        "DELETE FROM `%s` WHERE `guild_lv` = '0' AND `max_member` = '0' AND `exp` = '0' AND `next_exp` = '0' AND `average_lv` = '0'",
	        guild_db
	    ))
		Sql_ShowDebug(inter->sql_handle);
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle, "DELETE FROM `%s` WHERE `guild_id` = '0' AND `account_id` = '0' AND `char_id` = '0'",
	        guild_member_db
	    ))
		Sql_ShowDebug(inter->sql_handle);
