...
void pincode_handle (int fd, struct char_session_data* sd)
{
	...
	if (strlen(sd->pincode) == 4) {
		if (pincode->check_blacklist && pincode->is_blacklisted(sd->pincode)) {
...
void pincode_check(int fd, struct char_session_data* sd)
{
	...
	if (pincode->check_blacklist && pincode->is_blacklisted(pin)) {
...
* @return bool
*/
bool pincode_is_blacklisted(const char *pin)
{
	int i;
...
void pincode_change(int fd, struct char_session_data* sd)
{
	...
	if (pincode->check_blacklist && pincode->is_blacklisted(newpin)) {
...
*/
void pincode_makestate(int fd, struct char_session_data *sd, uint16 state)
{
	nullpo_retv(sd);
...
* @param[in] state Pincode Edit State
*/
void pincode_editstate(int fd, struct char_session_data *sd, uint16 state)
{
	nullpo_retv(sd);
...
void pincode_loginstate(int fd, struct char_session_data* sd, uint16 state)
{
	nullpo_retv(sd);
	WFIFOHEAD(fd, 12);
	WFIFOW(fd, 0) = 0x8b9;
...
void pincode_defaults(void) {
	...
	pincode->is_blacklisted = pincode_is_blacklisted;
