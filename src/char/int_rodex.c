...
// Loads mails of this char_id/account_id
// and also retrieves the total number of mails
static int inter_rodex_fromsql(int char_id, int account_id, int8 opentype, struct rodex_maillist *mails)
{
	int i, count = 0;
	...
	nullpo_retr(false, stmt);
	nullpo_retr(false, stmt_items);

	nullpo_retr(-1, mails);

	switch (opentype) {
	case RODEX_OPENTYPE_MAIL:
		// Prepares the statement to retrieve first page of mail box
		if (SQL_ERROR == SQL->StmtPrepare(stmt,
			"SELECT `id`, `sender_name`, `sender_id`, `receiver_name`, `receiver_id`, `receiver_accountid`,"
			"`title`, `body`, `zeny`, `type`, `is_readed`, `send_date`, `expire_date`, `weight`"
			"FROM `rodex_mail` WHERE `expire_date` > '%d' AND `receiver_id` = '%d'"
			"ORDER BY `id` ASC", (int)time(NULL), char_id)
			) {
	...
	case RODEX_OPENTYPE_ACCOUNT:
		// Prepares the statement to retrieve first page of account box
		if (SQL_ERROR == SQL->StmtPrepare(stmt,
			"SELECT `id`, `sender_name`, `sender_id`, `receiver_name`, `receiver_id`, `receiver_accountid`,"
			"`title`, `body`, `zeny`, `type`, `is_readed`, `send_date`, `expire_date`, `weight`"
			"FROM `rodex_mail` WHERE "
			"`expire_date` > '%d' AND `receiver_accountid` = '%d'"
			"ORDER BY `id` ASC", (int)time(NULL), account_id)
			) {
			SqlStmt_ShowDebug(stmt);
			SQL->StmtFree(stmt);
			return -1;
	...
	case RODEX_OPENTYPE_RETURN:
		// Prepares the statement to retrieve first page of returned box
		if (SQL_ERROR == SQL->StmtPrepare(stmt,
			"SELECT `id`, `sender_name`, `sender_id`, `receiver_name`, `receiver_id`, `receiver_accountid`,"
			"`title`, `body`, `zeny`, `type`, `is_readed`, `send_date`, `expire_date`, `weight`"
			"FROM `rodex_mail` WHERE (`sender_id` = '%d' AND `expire_date` <= '%d' AND `send_date` + '%d' > '%d')"
			"ORDER BY `id` ASC", char_id, (int)time(NULL), 2 * RODEX_EXPIRE, (int)time(NULL))
			) {
			SqlStmt_ShowDebug(stmt);
			SQL->StmtFree(stmt);
			return -1;
	...
	if (SQL_ERROR == SQL->StmtExecute(stmt)
		...
		|| SQL_ERROR == SQL->StmtBindColumn(stmt, 10, SQLDT_INT8, &msg.is_readed, 0, NULL, NULL)
	...
	while (SQL_SUCCESS == SQL->StmtNextRow(stmt)) {
		...
		if (msg.type & MAIL_TYPE_ITEM) {
			if (SQL_ERROR == SQL->StmtPrepare(stmt_items, "SELECT `nameid`, `amount`, `equip`, `identify`,"
				...
				"FROM `rodex_items` WHERE mail_id = '%"PRId64"' ORDER BY `id` ASC", msg.id)
...
static int inter_rodex_refresh_fromsql(int char_id, int account_id, int8 opentype, int64 mail_id, struct rodex_maillist *mails)
{
	int i, count = 0;
	struct rodex_message msg = { 0 };
	struct SqlStmt *stmt = SQL->StmtMalloc(inter->sql_handle);
	...
	case RODEX_OPENTYPE_MAIL:
		if (SQL_ERROR == SQL->StmtPrepare(stmt,
			"SELECT `id`, `sender_name`, `sender_id`, `receiver_name`, `receiver_id`, `receiver_accountid`,"
			"`title`, `body`, `zeny`, `type`, `is_readed`, `send_date`, `expire_date`, `weight`"
			"FROM `rodex_mail` WHERE `expire_date` > '%d' AND `receiver_id` = '%d' AND `id` > '%"PRId64"'"
			"ORDER BY `id` ASC", (int)time(NULL), char_id, mail_id)
			) {
			SqlStmt_ShowDebug(stmt);
			SQL->StmtFree(stmt);
			return -1;
	...
	case RODEX_OPENTYPE_ACCOUNT:
		if (SQL_ERROR == SQL->StmtPrepare(stmt,
			...
			"`title`, `body`, `zeny`, `type`, `is_readed`, `send_date`, `expire_date`, `weight`"
			"FROM `rodex_mail` WHERE "
			"`expire_date` > '%d' AND `receiver_accountid` = '%d' AND `id` > '%"PRId64"'"
			"ORDER BY `id` ASC", (int)time(NULL), account_id, mail_id)
			) {
			SqlStmt_ShowDebug(stmt);
			SQL->StmtFree(stmt);
			return -1;
	...
	case RODEX_OPENTYPE_RETURN:
		if (SQL_ERROR == SQL->StmtPrepare(stmt,
			"SELECT `id`, `sender_name`, `sender_id`, `receiver_name`, `receiver_id`, `receiver_accountid`,"
			"`title`, `body`, `zeny`, `type`, `is_readed`, `send_date`, `expire_date`, `weight`"
			"FROM `rodex_mail` WHERE (`sender_id` = '%d' AND `expire_date` <= '%d' AND `send_date` + '%d' > '%d' AND `id` > '%"PRId64"')"
			"ORDER BY `id` ASC", char_id, (int)time(NULL), 2 * RODEX_EXPIRE, (int)time(NULL), mail_id)
			) {
			SqlStmt_ShowDebug(stmt);
			SQL->StmtFree(stmt);
			return -1;
...
{
	char sender_name[NAME_LENGTH * 2 + 1], receiver_name[NAME_LENGTH * 2 + 1];
	char body[RODEX_BODY_LENGTH * 2 + 1], title[RODEX_TITLE_LENGTH * 2 + 1];
	int i;
	...
	if (SQL_ERROR == SQL->Query(inter->sql_handle, "INSERT INTO `rodex_mail` (`sender_name`, `sender_id`, `receiver_name`, `receiver_id`, `receiver_accountid`, `title`, `body`,"
		"`zeny`, `type`, `is_readed`, `send_date`, `expire_date`, `weight`) VALUES "
		"('%s', '%d', '%s', '%d', '%d', '%s', '%s', '%d', '%d', '%d', '%d', '%d', '%d')",
		sender_name, msg->sender_id, receiver_name, msg->receiver_id, msg->receiver_accountid, title, body, (int)msg->zeny, msg->type, msg->is_readed == true ? 1 : 0, msg->send_date, msg->expire_date, msg->weight)) {
		Sql_ShowDebug(inter->sql_handle);
		return 0;
	}
	...
	for (i = 0; i < RODEX_MAX_ITEM; ++i) {
		...
		if (SQL_ERROR == SQL->Query(inter->sql_handle, "INSERT INTO `rodex_items` (`mail_id`, `nameid`, `amount`, `equip`, `identify`,"
			...
			msg->id, it->nameid, it->amount, it->equip, it->identify, it->refine, it->attribute, it->card[0], it->card[1], it->card[2], it->card[3],
...
void mapif_rodex_sendinbox(int fd, int char_id, int8 opentype, int8 flag, int count, struct rodex_maillist *mails)
{
	int i, j;
	nullpo_retv(mails);
	WFIFOHEAD(fd, count * sizeof(struct rodex_message) + 14);
	WFIFOW(fd, 0) = 0x3895;
	WFIFOW(fd, 2) = count * sizeof(struct rodex_message) + 14;
	WFIFOL(fd, 4) = char_id;
	WFIFOB(fd, 8) = opentype;
	WFIFOB(fd, 9) = flag;
	WFIFOL(fd, 10) = count;
	i = 14;
	for (j = 0; j < count; ++j, i += sizeof(struct rodex_message))
		memcpy(WFIFOP(fd, i), &VECTOR_INDEX(*mails, j), sizeof(struct rodex_message));
...
void mapif_parse_rodex_requestinbox(int fd)
{
	...
	VECTOR_INIT(mails);
	if (flag == 0)
		count = inter_rodex_fromsql(char_id, account_id, opentype, &mails);
	else
		count = inter_rodex_refresh_fromsql(char_id, account_id, opentype, mail_id, &mails);
...
void mapif_parse_rodex_updatemail(int fd)
{
	...
	switch (flag) {
	case 0: // Read
		if (SQL_ERROR == SQL->Query(inter->sql_handle, "UPDATE `%s` SET `is_readed` = 1 WHERE `id` = '%"PRId64"'", rodex_db, mail_id))
			Sql_ShowDebug(inter->sql_handle);
		break;
	
	case 1: // Get Zeny
		if (SQL_ERROR == SQL->Query(inter->sql_handle, "UPDATE `%s` SET `zeny` = 0, `type` = `type` & (~2) WHERE `id` = '%"PRId64"'", rodex_db, mail_id))
			Sql_ShowDebug(inter->sql_handle);
		break;
	...
	case 2: // Get Items
		if (SQL_ERROR == SQL->Query(inter->sql_handle, "DELETE FROM `%s` WHERE `mail_id` = '%"PRId64"'", rodex_item_db, mail_id))
			Sql_ShowDebug(inter->sql_handle);
		if (SQL_ERROR == SQL->Query(inter->sql_handle, "UPDATE `%s` SET `zeny` = 0, `type` = `type` & (~4) WHERE `id` = '%"PRId64"'", rodex_db, mail_id))
			Sql_ShowDebug(inter->sql_handle);
		break;

	case 3: // Delete Mail
		if (SQL_ERROR == SQL->Query(inter->sql_handle, "DELETE FROM `%s` WHERE `id` = '%"PRId64"'", rodex_db, mail_id))
			Sql_ShowDebug(inter->sql_handle);
		if (SQL_ERROR == SQL->Query(inter->sql_handle, "DELETE FROM `%s` WHERE `mail_id` = '%"PRId64"'", rodex_item_db, mail_id))
			Sql_ShowDebug(inter->sql_handle);
		break;
