...
static int inter_storage_tosql(int account_id, const struct storage_data *p)
{
	...
	if (VECTOR_LENGTH(cp.item) > 0) {
		...
		for (i = 0; i < VECTOR_LENGTH(cp.item); i++) {
			...
			ARR_FIND(0, VECTOR_LENGTH(p->item), j, matched_p[j] != true && (p_it = &VECTOR_INDEX(p->item, j)) != NULL && cp_it->nameid == p_it->nameid && cp_it->unique_id == p_it->unique_id && memcmp(p_it->card, cp_it->card, sizeof(int) * MAX_SLOTS) == 0 && memcmp(p_it->option, cp_it->option, 5 * MAX_ITEM_OPTIONS) == 0);
			...
			if (j < VECTOR_LENGTH(p->item)) {
				...
				if (memcmp(cp_it, p_it, sizeof(struct item)) != 0) {
					...
					StrBuf->Printf(&buf, "%s('%d', '%d', '%d', '%d', '%u', '%d', '%d', '%d', '%d'", total_updates > 0 ? ", " : "", cp_it->id, account_id, p_it->nameid, p_it->amount, p_it->equip, p_it->identify, p_it->refine, p_it->grade, p_it->attribute);
					for (k = 0; k < MAX_SLOTS; k++)
	...
	for (i = 0; i < VECTOR_LENGTH(p->item); i++) {
		...
		StrBuf->Printf(&buf, "%s('%d', '%d', '%d', '%u', '%d',  '%d', '%d', '%d', '%u', '%d', '%" PRIu64 "'", total_inserts > 0 ? ", " : "", account_id, p_it->nameid, p_it->amount, p_it->equip, p_it->identify, p_it->refine, p_it->grade, p_it->attribute, p_it->expire_time, p_it->bound, p_it->unique_id);
		for (j = 0; j < MAX_SLOTS; ++j)
...
static bool inter_storage_retrieve_bound_items(int char_id, int account_id, int guild_id)
{
	...
	for (j = 0; j < i; j++) {
		...
		if (items[j].bound && items[j].equip) {
			...
			if (items[j].equip & EQP_HAND_R || items[j].equip & EQP_HAND_L || items[j].equip & EQP_HEAD_TOP || items[j].equip & EQP_HEAD_MID || items[j].equip & EQP_HEAD_LOW || items[j].equip & EQP_GARMENT) {
	...
	StrBuf->Clear(&buf);
	StrBuf->Printf(&buf,
	               "INSERT INTO `%s` (`guild_id`,`nameid`,`amount`,`equip`,`identify`,`refine`,"
	...
	for (j = 0; j < i; ++j) {
		...
		StrBuf->Printf(&buf, "('%d', '%d', '%d', '%u', '%d', '%d', '%d', '%d', '%u', '%d', '%" PRIu64 "'", guild_id, items[j].nameid, items[j].amount, items[j].equip, items[j].identify, items[j].refine, items[j].grade, items[j].attribute, items[j].expire_time, items[j].bound, items[j].unique_id);
		for (s = 0; s < MAX_SLOTS; ++s)
