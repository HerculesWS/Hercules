...
static int inter_mercenary_parse_frommap(int fd)
{
	...
	switch (cmd) {
		case 0x3070: mapif->parse_mercenary_create(fd, RFIFOP(struct s_mercenary *, fd, 4)); break;
		...
		case 0x3073: mapif->parse_mercenary_save(fd, RFIFOP(struct s_mercenary *, fd, 4)); break;
		default:
			return 0;
	}
	return 1;
