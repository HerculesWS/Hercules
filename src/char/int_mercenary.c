...
static bool inter_mercenary_create(struct s_mercenary *merc)
{
	...
	if (SQL_ERROR == SQL->Query(inter->sql_handle, "INSERT INTO `%s` (`char_id`,`class`,`hp`,`sp`,`kill_counter`,`life_time`) VALUES ('%d','%d','%d','%d','%u','%u')", mercenary_db, merc->char_id, merc->class_, merc->hp, merc->sp, merc->kill_count, merc->life_time)) {
		Sql_ShowDebug(inter->sql_handle);
		return false;
	}
...
static bool inter_mercenary_save(const struct s_mercenary *merc)
{
	...
	if (SQL_ERROR == SQL->Query(inter->sql_handle, "UPDATE `%s` SET `char_id` = '%d', `class` = '%d', `hp` = '%d', `sp` = '%d', `kill_counter` = '%u', `life_time` = '%u' WHERE `mer_id` = '%d'", mercenary_db, merc->char_id, merc->class_, merc->hp, merc->sp, merc->kill_count, merc->life_time, merc->mercenary_id)) {
		Sql_ShowDebug(inter->sql_handle);
		return false;
	}
...
static int inter_mercenary_parse_frommap(int fd)
{
	...
	switch (cmd) {
		case 0x3070:
			mapif->parse_mercenary_create(fd, RFIFOP(fd, 4));
			break;
		case 0x3071:
			mapif->parse_mercenary_load(fd, RFIFOL(fd, 2), RFIFOL(fd, 6));
			break;
		case 0x3072:
			mapif->parse_mercenary_delete(fd, RFIFOL(fd, 2));
			break;
		case 0x3073:
			mapif->parse_mercenary_save(fd, RFIFOP(fd, 4));
			break;
		default:
			return 0;
	}
