...
static struct party_data *inter_party_fromsql(int party_id)
{
	...
	SQL->GetData(inter->sql_handle, 1, &data, &len); memcpy(p->party.name, data, HMIN(len, (size_t)NAME_LENGTH));
	...
	for( i = 0; i < MAX_PARTY && SQL_SUCCESS == SQL->NextRow(inter->sql_handle); ++i )
	{
		...
		SQL->GetData(inter->sql_handle, 2, &data, &len); memcpy(m->name, data, HMIN(len, (size_t)NAME_LENGTH));
