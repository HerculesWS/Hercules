...
static int inter_mail_fromsql(int char_id, struct mail_data *md)
{
	...
	StrBuf->AppendStr(
	    &buf,
	    "SELECT `id`,`send_name`,`send_id`,`dest_name`,`dest_id`,`title`,`message`,`time`,`status`," "`zeny`,`amount`,`nameid`,`refine`,`grade`,`attribute`,`identify`,`unique_id`"
	);
	...
	StrBuf->Printf(
	    &buf, " FROM `%s` WHERE `dest_id`='%d' AND `status` < 3 ORDER BY `id` LIMIT %d", mail_db, char_id,
	    MAIL_MAX_INBOX + 1
	);
	...
	for (i = 0; i < md->amount; i++) {
		...
		if (msg->status == MAIL_NEW) {
			if (SQL_ERROR
			    == SQL->Query(
			        inter->sql_handle, "UPDATE `%s` SET `status` = '%d' WHERE `id` = '%d'", mail_db,
			        MAIL_UNREAD, msg->id
			    ))
				Sql_ShowDebug(inter->sql_handle);
...
static int inter_mail_savemessage(struct mail_message *msg)
{
	...
	StrBuf->Printf(
	    &buf,
	    "INSERT INTO `%s` (`send_name`, `send_id`, `dest_name`, `dest_id`, `title`, `message`, `time`, `status`, `zeny`, `amount`, `nameid`, `refine`, `grade`, `attribute`, `identify`, `unique_id`",
	    mail_db
	);
	for (j = 0; j < MAX_SLOTS; j++)
	...
	StrBuf->Printf(
	    &buf,
	    ") VALUES (?, '%d', ?, '%d', ?, ?, '%lu', '%u', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%" PRIu64 "'",
	    msg->send_id, msg->dest_id, (unsigned long)msg->timestamp, (unsigned int)msg->status, msg->zeny,
	    msg->item.amount, msg->item.nameid, msg->item.refine, msg->item.grade, msg->item.attribute,
	    msg->item.identify, msg->item.unique_id
	);
	for (j = 0; j < MAX_SLOTS; j++)
	...
	if (SQL_SUCCESS != SQL->StmtPrepareStr(stmt, StrBuf->Value(&buf))
	    || SQL_SUCCESS
	           != SQL->StmtBindParam(stmt, 0, SQLDT_STRING, msg->send_name, strnlen(msg->send_name, NAME_LENGTH))
	    || SQL_SUCCESS
	           != SQL->StmtBindParam(stmt, 1, SQLDT_STRING, msg->dest_name, strnlen(msg->dest_name, NAME_LENGTH))
	    || SQL_SUCCESS
	           != SQL->StmtBindParam(stmt, 2, SQLDT_STRING, msg->title, strnlen(msg->title, MAIL_TITLE_LENGTH))
...
static bool inter_mail_loadmessage(int mail_id, struct mail_message *msg)
{
	...
	if (SQL_ERROR == SQL->QueryStr(inter->sql_handle, StrBuf->Value(&buf))
		...
		SQL->GetData(inter->sql_handle, 13, &data, NULL);
		msg->item.grade = atoi(data);
		SQL->GetData(inter->sql_handle, 14, &data, NULL);
		msg->item.attribute = atoi(data);
		SQL->GetData(inter->sql_handle, 15, &data, NULL);
		msg->item.identify = atoi(data);
		SQL->GetData(inter->sql_handle, 16, &data, NULL);
		msg->item.unique_id   = strtoull(data, NULL, 10);
...
static bool inter_mail_mark_read(int mail_id)
{
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle, "UPDATE `%s` SET `status` = '%d' WHERE `id` = '%d'", mail_db, MAIL_READ, mail_id
	    )) {
		Sql_ShowDebug(inter->sql_handle);
		return false;
	}
...
static bool inter_mail_DeleteAttach(int mail_id)
{
	...
	StrBuf->Printf(
	    &buf,
	    "UPDATE `%s` SET `zeny` = '0', `nameid` = '0', `amount` = '0', `refine` = '0', `grade` = '0', `attribute` = '0', `identify` = '0'",
	    mail_db
	);
	for (i = 0; i < MAX_SLOTS; i++)
...
static bool inter_mail_send(int account_id, struct mail_message *msg)
{
	...
	if (SQL_ERROR
	    == SQL->Query(
	        inter->sql_handle, "SELECT `account_id`, `char_id` FROM `%s` WHERE `name` = '%s'", char_db, esc_name
	    )) {
		Sql_ShowDebug(inter->sql_handle);
...
static void inter_mail_sendmail(
...
static int inter_mail_parse_frommap(int fd)
{
	switch (RFIFOW(fd, 0)) {
	case 0x3048:
		mapif->parse_mail_requestinbox(fd);
		break;
	case 0x3049:
		mapif->parse_mail_read(fd);
		break;
	case 0x304A:
		mapif->parse_mail_getattach(fd);
		break;
	case 0x304B:
		mapif->parse_mail_delete(fd);
		break;
	case 0x304C:
		mapif->parse_mail_return(fd);
		break;
	case 0x304D:
		mapif->parse_mail_send(fd);
		break;
	default:
		return 0;
	}
