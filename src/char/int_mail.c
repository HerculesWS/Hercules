...
static int inter_mail_fromsql(int char_id, struct mail_data *md)
{
	...
	StrBuf->Printf(&buf, " FROM `%s` WHERE `dest_id`='%d' AND `status` < 3 ORDER BY `id` LIMIT %d", mail_db, char_id, MAIL_MAX_INBOX + 1);
...
static int inter_mail_savemessage(struct mail_message *msg)
{
	...
	StrBuf->Printf(&buf, ") VALUES (?, '%d', ?, '%d', ?, ?, '%lu', '%u', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%" PRIu64 "'", msg->send_id, msg->dest_id, (unsigned long)msg->timestamp, msg->status, msg->zeny, msg->item.amount, msg->item.nameid, msg->item.refine, msg->item.grade, msg->item.attribute, msg->item.identify, msg->item.unique_id);
	for (j = 0; j < MAX_SLOTS; j++)
...
static bool inter_mail_loadmessage(int mail_id, struct mail_message *msg)
{
	...
	if (SQL_ERROR == SQL->QueryStr(inter->sql_handle, StrBuf->Value(&buf)) || SQL_SUCCESS != SQL->NextRow(inter->sql_handle)) {
		Sql_ShowDebug(inter->sql_handle);
		SQL->FreeResult(inter->sql_handle);
		StrBuf->Destroy(&buf);
		return false;
