...
{
	struct online_login_data *p
	    = (struct online_login_data *)idb_ensure(login->online_db, account_id, login->create_online_user);
...
{
	login->online_db->foreach(
	    login->online_db, login->online_db_setoffline, id
...
static bool login_check_password(const char *md5key, int passwdenc, const char *passwd, const char *refpass)
{
	...
	if (passwdenc == PWENC_NONE) {
		...
		// password mode set to PWENC_ENCRYPT  -> md5(md5key, refpass) enable with
		// <passwordencrypt></passwordencrypt> password mode set to PWENC_ENCRYPT2 -> md5(refpass, md5key)
		// enable with <passwordencrypt2></passwordencrypt2>
...
static void login_fromchar_auth_ack(
    int fd, int account_id, uint32 login_id1, uint32 login_id2, uint8 sex, int request_id, struct login_auth_node *node
)
{
	WFIFOHEAD(fd, 33);
	WFIFOW(fd, 0)  = 0x2713;
...
static void login_fromchar_parse_auth(int fd, int id, const char *const ip)
{
	...
	if (
		...
		nullpo_retv(ip);
		ShowStatus(
		    "Char-server '%s': authentication of the account %d REFUSED (ip: %s).\n",
		    login->dbs->server[id].name, account_id, ip
		);
...
static void login_fromchar_parse_request_change_email(int fd, int id, const char *const ip)
{
	...
	int account_id = RFIFOL(fd, 2);
	safestrncpy(email, RFIFOP(char *, fd, 6), 40);
	...
	if (e_mail_check(email) == 0)
	    ...
	    !accounts->load_num(accounts, &acc, account_id) || strcmp(acc.email, "a@a.com") == 0 || acc.email[0] == '\0'
	)
		ShowNotice(
...
static void login_fromchar_parse_account_update(int fd, int id, const char *const ip)
{
	...
	if (!accounts->load_num(accounts, &acc, account_id))
		ShowNotice(
		    "Char-server '%s': Error of Status change (account: %d not found, suggested status %u, ip: %s).\n",
		    login->dbs->server[id].name, account_id, state, ip
		);
	else if (acc.state == state)
		ShowNotice(
		    "Char-server '%s':  Error of Status change - actual status is already the good status (account: %d, status %u, ip: %s).\n",
		    login->dbs->server[id].name, account_id, state, ip
		);
	else {
		ShowNotice(
		    "Char-server '%s': Status change (account: %d, new status %u, ip: %s).\n",
		    login->dbs->server[id].name, account_id, state, ip
		);
...
static void login_fromchar_parse_change_sex(int fd, int id, const char *const ip)
{
	...
	if (!accounts->load_num(accounts, &acc, account_id))
		ShowNotice(
		    "Char-server '%s': Error of sex change (account: %d not found, ip: %s).\n",
		    login->dbs->server[id].name, account_id, ip
		);
	else if (acc.sex == 'S')
		ShowNotice(
		    "Char-server '%s': Error of sex change - account to change is a Server account (account: %d, ip: %s).\n",
		    login->dbs->server[id].name, account_id, ip
		);
	else {
...
static void login_fromchar_parse_account_online(int fd, int id)
{
	...
	// "standalone" players (such as autotraders) does not exists in API server, so we should not send data about
	// them or we will get errors from API server
	if (p->standalone == 0)
...
static void login_fromchar_parse_online_accounts(int fd, int id)
{
	...
	for (uint32 i = 0; i < p->list_length; i++) {
		...
		struct online_login_data *login_data
		    = (struct online_login_data *)idb_ensure(login->online_db, aid, login->create_online_user);
...
static bool login_fromchar_parse_wrong_pincode(int fd)
{
	...
	if (accounts->load_num(accounts, &acc, RFIFOL(fd, 2))) {
		...
		loginlog->log(
		    sockt->host2ip(acc.last_ip), acc.userid, 100, "PIN Code check failed"
...
static void login_fromchar_accinfo(
    int fd, int account_id, int u_fd, int u_aid, int u_group, int map_fd, struct mmo_account *acc
)
{
	if (acc) {
		WFIFOHEAD(fd, 183);
...
{
	struct mmo_account acc;
	int account_id = RFIFOL(fd, 2), u_fd = RFIFOL(fd, 6), u_aid = RFIFOL(fd, 10), u_group = RFIFOL(fd, 14),
	    map_fd = RFIFOL(fd, 18);
	if (accounts->load_num(accounts, &acc, account_id)) {
...
static int login_parse_fromchar(int fd)
{
	...
	if (id == ARRAYLENGTH(login->dbs->server)) { // not a char server
...
static int login_mmo_auth_new(const char *userid, const char *pass, const char sex, const char *last_ip)
{
	...
	if (accounts->load_str(accounts, &acc, userid)) {
		ShowNotice(
		    "Attempt of creation of an already existing account (account: %s_%c, pass: %s, received pass: %s)\n",
	...
	acc.expiration_time
	    = (login->config->start_limited_time != -1) ? time(NULL) + login->config->start_limited_time : 0;
	...
	ShowNotice(
	    "Account creation (account %s, id: %d, pass: %s, sex: %c)\n", acc.userid, acc.account_id, acc.pass, acc.sex
	);
	...
	if (login->config->check_client_flags && (sd->version & 0x80000000) != 0) {
		...
		if (emulatorFlags != sd->version) {
			if (login->config->report_client_flags_error)
				ShowNotice(
				    "Wrong client flags detected (account: %s, received flags: 0x%x)\n", sd->userid,
				    sd->version
				);
			return 5;
		}
...
static int login_mmo_auth(struct login_session_data *sd, bool isServer)
{
	...
	if (acc.state != 0) {
		ShowNotice(
		    "Connection refused (account: %s, pass: %s, state: %u, ip: %s)\n", sd->userid, sd->passwd,
		    acc.state, ip
		);
		return acc.state - 1;
	}
	...
	if (login->config->client_hash_check && !isServer) {
		...
		if (!match) {
			...
			if (!sd->has_client_hash) {
				ShowNotice(
				    "Client didn't send client hash (account: %s, pass: %s, ip: %s)\n", sd->userid,
				    sd->passwd, ip
				);
				return 5;
			}
			...
			ShowNotice(
			    "Invalid client hash (account: %s, pass: %s, sent md5: %s, ip: %s)\n", sd->userid,
			    sd->passwd, smd5, ip
			);
			return 5;
		}
	...
	if (sd->sex != 'S' && sd->account_id < START_ACCOUNT_NUM)
		ShowWarning(
		    ...
		    sd->account_id, START_ACCOUNT_NUM
		);
...
static void login_auth_ok(struct login_session_data *sd)
{
	...
	if (login->config->group_id_to_connect >= 0 && sd->group_id != login->config->group_id_to_connect) {
	    ...
	    login->config->min_group_id_to_connect >= 0
		...
		if (data) {                           // account is already marked as online!
			if (data->char_server > -1) { // Request char servers to kick this account out. [Skotlex]
				...
				if (data->waiting_disconnect == INVALID_TIMER)
					data->waiting_disconnect = timer->add(
					    timer->gettick() + AUTH_TIMEOUT, login->waiting_disconnect_timer,
					    sd->account_id, 0
					);
...

	{
		...
		data->waiting_disconnect
		    = timer->add(timer->gettick() + AUTH_TIMEOUT, login->waiting_disconnect_timer, sd->account_id, 0);
	}
}
...
static void login_auth_failed(struct login_session_data *sd, int result)
{
	...
	if (login->config->log_login) {
		...
		case 101:
			...
			break; // 101 = Account has been locked for a hacking investigation. Please contact the GM Team
			       // for more information
		case 102:
			error = "Bug investigation.";
			break; // 102 = This account has been temporarily prohibited from login due to a bug-related
			       // investigation
		case 103:
			error = "Deleting char.";
			break; // 103 = This character is being deleted. Login is temporarily unavailable for the time
			       // being
		case 104:
			error = "Deleting spouse char.";
			break; // 104 = This character is being deleted. Login is temporarily unavailable for the time
			       // being
		default:
			error = "Unknown Error.";
			break;
...
static bool login_client_login(int fd, struct login_session_data *sd)
{
	...
	ShowStatus(
	    "Request for connection %sof %s (ip: %s).\n", sd->passwdenc == PASSWORDENC ? " (passwdenc mode)" : "",
	    sd->userid, ip
	);
...
static void login_parse_request_connection(int fd, struct login_session_data *sd, const char *const ip, uint32 ipl)
{
	...
	ShowInfo(
	    "Connection request of the char-server '%s' @ %u.%u.%u.%u:%u (account: '%s', pass: '%s', ip: '%s')\n",
	...
	if (!sockt->allowed_ip_check(ipl)) {
		...
		ShowNotice(
		    "Connection of the char-server '%s' REFUSED (Account ID must be between 0 and %d).\n", server_name,
		    ARRAYLENGTH(login->dbs->server) - 1
		);
		login->char_server_connection_status(fd, sd, 1);
	} else if (
	    core->runflag == LOGINSERVER_ST_RUNNING
		...
		safestrncpy(
		    login->dbs->server[sd->account_id].name, server_name,
		    sizeof(login->dbs->server[sd->account_id].name)
		);
...
static void login_parse_request_api_connection(int fd, struct login_session_data *sd, const char *const ip, uint32 ipl)
{
	...
	ShowInfo(
	    "Connection request of the api-server %u.%u.%u.%u (account: '%s', pass: '%s', ip: '%s')\n",
...
static bool login_config_read_inter(const char *filename, struct config_t *config, bool imported)
{
	...
	if (libconfig->setting_lookup_uint32(setting, "ip_sync_interval", &login->config->ip_sync_interval)
	    == CONFIG_TRUE)
...
static bool login_config_read_log(const char *filename, struct config_t *config, bool imported)
{
	...
	libconfig->setting_lookup_mutable_string(
	    setting, "date_format", login->config->date_format, sizeof(login->config->date_format)
	);
	return true;
}
...
static void login_config_set_md5hash(struct config_setting_t *setting)
{
	...
	for (i = 0; i < count; i++) {
		...
		nnode->next     = login->config->client_hash_nodes; // login->config->client_hash_nodes is initialized
...
static bool login_config_read_permission_blacklist(const char *filename, struct config_t *config, bool imported)
{
	...
	if ((setting = libconfig->lookup(config, "login_configuration/permission/DNS_blacklist")) == NULL) {
		if (imported)
			return true;
		ShowError(
		    "login_config_read: login_configuration/permission/DNS_blacklist was not found in %s!\n", filename
		);
		return false;
	}
...
static bool login_config_read_permission(const char *filename, struct config_t *config, bool imported)
{
	...
	libconfig->setting_lookup_bool_real(
	    setting, "report_client_flags_error", &login->config->report_client_flags_error
	);
	libconfig->setting_lookup_uint32(
	    setting, "client_version_to_connect", &login->config->client_version_to_connect
	);
...
static bool login_config_read_users(const char *filename, struct config_t *config, bool imported)
{
	...
	libconfig->setting_lookup_bool_real(
	    setting, "send_user_count_description", &login->config->send_user_count_description
	);
...
int do_final(void)
{
	...
	if (login->dbs->account_engine->db) { // destroy account engine
...
#define CHECK_OLD_LOCAL_CONF(oldname, newname) \
	do { \
		if (stat((oldname), &fileinfo) == 0 && fileinfo.st_size > 0) { \
			...
			ShowWarning( \
			    "If it contains settings you wish to keep, please merge them into \"%s\".\n", (newname) \
			); \
...
#undef CHECK_OLD_LOCAL_CONF
	...
	if (login->config->ip_sync_interval) {
		...
		timer->add_interval(
		    timer->gettick() + login->config->ip_sync_interval, login->sync_ip_addresses, 0, 0,
		    login->config->ip_sync_interval
		);
	}
	...
	ShowStatus(
	    "The login-server is " CL_GREEN "ready" CL_RESET " (Server is listening on the port %u).\n\n",
	    login->config->login_port
	);
