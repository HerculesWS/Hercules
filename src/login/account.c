...
static bool account_db_sql_iter_next(AccountDBIterator *self, struct mmo_account *acc)
{
	...
	if (SQL_SUCCESS == SQL->NextRow(sql_handle) && SQL_SUCCESS == SQL->GetData(sql_handle, 0, &data, NULL) && data != NULL) { // get account data
		int account_id;
		account_id = atoi(data);
