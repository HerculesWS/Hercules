 ...
 * @return :0
 */
int account_disable_webtoken_timer(int tid, int64 tick, int id, intptr_t data) {
	...
	if (p == NULL) {
		...
		if (SQL_ERROR == SQL->Query(db->accounts, "UPDATE `%s` SET `web_auth_token_enabled` = '0' WHERE `account_id` = '%u'", db->account_db, id)) {
			Sql_ShowDebug(db->accounts);
			return 0;
		}
	}
...
bool account_db_sql_disable_webtoken(AccountDB* self, const uint32 account_id) {
	AccountDB_SQL* db = (AccountDB_SQL*)self;
	timer->add(timer->gettick() + 10000, account_disable_webtoken_timer, account_id, (intptr_t)db);
