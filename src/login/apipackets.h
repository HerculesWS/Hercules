...
#define WFIFO_APILOGIN_PACKET_REPLY_EMPTY(wfd) \
	...
	struct PACKET_API_PROXY *packet = WFIFOP(struct PACKET_API_PROXY *, (wfd), 0); \
	memcpy(packet, RFIFOP(struct PACKET_API_PROXY *, fd, 0), WFIFO_APICHAR_SIZE); \
...
#define WFIFO_APILOGIN_PACKET_REPLY(wfd, type) \
	...
	struct PACKET_API_REPLY_ ## type *data = WFIFOP(struct PACKET_API_REPLY_ ## type *, (wfd), sizeof(struct PACKET_API_PROXY))
