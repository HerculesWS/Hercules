...
$constants = (issetarg("--use-constants") || issetarg("-c"));
 ...
 */
if($UseItemConstantsNotIds)  {
  $itemdb[] = array();
  //get_sql_items(DATABASE_HOST, DATABASE_USER, DATABASE_PASSWORD, DATABASE_NAME, $itemdb);
  ...
  if(file_exists(DIRPATH.$file)) {
    ...
    echo "Unable to open '".DIRPATH.$file."'... defaulting to using Item ID's instead of Constants.\n";
    $UseItemConstantsNotIds = false;
  }
}
...
fclose($skmain);
echo $linecount." entries found in skill_db.txt.\n";
					...
					for ($index=0; $index<sizeof($skreqit['ItemId'][$key]); $index++) {
						...
						if (strpos($itemID, ':') == true) {
							...
							while (isset($items) && isset($items[$it])) {
								if ($UseItemConstantsNotIds && $itemID) {
									$itkey = array_search($items[$it], $itemdb['ID']);
									$itemname = $itemdb['name'][$itkey];
									$putsk .=  "\t\t\t".$itemname.": ".leveled($itemamt, $max, $id, 2)."\n";
								} else if ($itemID) {
	...
	if(strpos($str, ':') == true) {
		...
		for($i=0; $i<$max && isset($lvs[$i]); $i++) {
			if (isset($lvs[$i])) {
				...
				print "\rWarning - Invalid Level ".($i+1)." provided in Element Level for Skill Id ".$skill_id."\n";
				continue;
			}
		}
		$retval .= "\t}";
	...
	if(strpos($str, ':') == true) {
		...
		for($i=0; $i<$max && isset($lvs[$i]); $i++) {
			if(isset($lvs[$i])) {
				...
				print "\rWarning - Invalid Level index ".($i+1)." provided in skill Id ".$id."\n";
			}
		}
		$retval .= $endtab."}";
	} else $retval = intval($str);
...
function gethelp()
{
	...
	print "\t-c      [--use-constants]    to use item constants instead of IDs.\n";
	print "\t-h      [--help]             to display this help text.\n\n";  
	print "----------------------- Additional Notes ----------------------\n";
	print "Please be advised that either and only one of -re or -pre-re arguments\n";
	print "must be specified on execution.\n";
	print "This tool should only be used from the 'tools/' folder, which can be found\n";
	print "root of your Hercules installation. This tool also will not delete any files\n";
...
function printcredits()
{
	...
	print "\033[0;36mHercules Skill Database TXT to Libconfig Converter\033[0m\n";
...
//= along with this program.  If not, see <http://www.gnu.org/licenses/>.
//==============================================================================
//= ".($re?"Renewal":"Pre-Renewal")." Skill Database [Smokexyz/Hercules]
//==============================================================================
//= @Format Notes:
...

{
	------------------------------ Mandatory Fields ----------------------------
	Id: ID                                        (int)     (Required)
	Name: \"Skill_Name\"                          (string)  (Required)
