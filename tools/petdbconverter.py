...
def printIntimacy(arr):
    if int(arr[9]) == 0 or int(arr[10]) == 0 or int(arr[11]) == 0 or int(arr[12]) == 0:
        return
    print('\tIntimacy: {')
    printIntField2('Start', arr[11])
    printIntField2('Feed', arr[9])
    printIntField2('OverFeed', arr[10])
    printIntField2('OwnerDie', arr[12])
    print('\t}')
 ...
 **************************************************************************
{
	...
	// ================ Optional fields ===============================
	TamingItemID: Taming Item            (string, defaults to 0)
	EggItemID: Egg Id                    (string, defaults to 0)
	AccessoryItemID: Equipment Id        (string, defaults to 0)
	FoodItemID: Food Id                  (string, defaults to 0)
	Fullness: hunger points              (int, defaults to 0)
	HungryDelay: hunger time             (int, defaults to 0)
	Intimacy: {
		Start: start intimacy            (int, defaults to 0)
		Feed: feeding intimacy           (int, defaults to 0)
		OverFeed: overfeeding intimacy   (int, defaults to 0)
		OwnerDie: owner die intimacy     (int, defaults to 0)
	}
	...
	TalkConvertClass: convert talk       (int, defaults to 0)
	AttackRate: attack rate              (int, defaults to 0)
	DefenceAttackRate: Defence attack    (int, defaults to 0)
...
def convertFile(inFile, itemDb):
    ...
    for line in r:
        if isValidEntry(line) == True:
            ...
            printItemName('TamingItemID', secondsplit[3], itemDb)
            printItemName('EggItemID', secondsplit[4], itemDb)
            printItemName('AccessoryItemID', secondsplit[5], itemDb)
            printItemName('FoodItemID', secondsplit[6], itemDb)
            printIntField('Fullness', secondsplit[7])
            printIntField('HungryDelay', secondsplit[8])
            ...
            printIntField('TalkConvertClass', secondsplit[16])
            printIntField('AttackRate', secondsplit[17])
            printIntField('DefenceAttackRate', secondsplit[18])
